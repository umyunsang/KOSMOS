...
Wires a validated :class:`~kosax.plugins.manifest_schema.PluginManifest`
into the existing Spec 022 :class:`~kosax.tools.registry.ToolRegistry`
+ Spec 1634 :class:`~kosax.tools.executor.ToolExecutor`. The plugin
...
``kosax.plugin.install`` carrying the ``kosax.plugin.id`` attribute
(Spec 021 KOSAX extension; Migration tree § L1-A A7). A failure
...
Auto-discovery walks ``KOSAX_PLUGIN_INSTALL_ROOT`` (Spec 035
...
from kosax.plugins.manifest_schema import PluginManifest
...
def _import_adapter_module(module_path: str, *, plugin_root: Path | None) -> ModuleType:
    ...
    1. ``plugin_root`` provided → load from
       ...
       installed under ``~/.kosax/memdir/user/plugins/<id>/`` work
    ...
    if plugin_root is not None:
        ...
        spec_name = f"_kosax_plugin_{plugin_root.name}_{module_path.replace('.', '_')}"
...
def register_plugin_adapter(
    ...
    *,
    ...
    with _tracer.start_as_current_span("kosax.plugin.install") as span:
        span.set_attribute("kosax.plugin.id", plugin_id)
        span.set_attribute("kosax.plugin.version", manifest.version)
        span.set_attribute("kosax.plugin.tier", manifest.tier)
        span.set_attribute("kosax.plugin.tool_id", expected_tool_id)
        span.set_attribute("kosax.plugin.permission_layer", manifest.permission_layer)
        ...
        span.set_attribute("kosax.plugin.slsa_verification", slsa_verification)
    ...
    *,
    ...
    Args:
        ...
        strict: When True, raise on the first registration failure.
            ...
            still surfaced via OTEL ``kosax.plugin.install`` span with
            ``status=ERROR``.
