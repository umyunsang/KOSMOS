...
from ummaya.ipc.frame_schema import BackpressureSignalFrame, IPCFrame
...
# ---------------------------------------------------------------------------
# Configuration defaults (overridable via UMMAYA_IPC_* env vars)
# ---------------------------------------------------------------------------

_DEFAULT_HWM: int = int(os.environ.get("UMMAYA_IPC_HWM", "64"))
...
class BackpressureController:
    ...
    def tick(
        ...
        if depth >= self._hwm and not self._paused[source]:
            ...
            logger.info(
                "backpressure.pause",
                extra={
                    UMMAYA_IPC_BACKPRESSURE_KIND: "pause",
                    UMMAYA_IPC_BACKPRESSURE_SOURCE: source,
                    "depth": depth,
                    "hwm": self._hwm,
        ...
        if depth <= self._resume_threshold and self._paused[source]:
            ...
            logger.info(
                "backpressure.resume",
                extra={
                    UMMAYA_IPC_BACKPRESSURE_KIND: "resume",
                    UMMAYA_IPC_BACKPRESSURE_SOURCE: source,
                    "depth": depth,
                    "hwm": self._hwm,
                    "session_id": self._session_id,
    ...
    def emit_upstream_429(
        ...
        logger.info(
            ...
            extra={
                UMMAYA_IPC_BACKPRESSURE_KIND: "throttle",
                UMMAYA_IPC_BACKPRESSURE_SOURCE: "upstream_429",
                "retry_after_ms": retry_after_ms,
                "session_id": self._session_id,
            },
        )
