...
**Given** a clean working tree with η commit `1321f77` on `main` and `UMMAYA_FRIENDLI_TOKEN` set.

**Then** `smoke-citizen-taxreturn.expect` MUST:
1. `set timeout 90` (90s total budget).
2. `spawn bash -c "cd $env(HOME)/UMMAYA-w-2297/tui && bun run tui"`.
3. `expect "UMMAYA"` (boot+branding render).
4. `send "종합소득세 신고해줘\r"`.
...
The marker is implemented in the TUI side (e.g. in `dispatchPrimitive.ts` when `UMMAYA_SMOKE_CHECKPOINTS=true` env is set) — fire-and-forget `process.stderr.write` so it lands in PTY capture but does not pollute production output.
