...
"""Python entry-point fallback for ``kosax plugin init``.
...
This module exists so non-TUI users can scaffold a plugin via:
    ...
    uvx kosax-plugin-init <name> --tier live --layer 1 --no-pii
...
def _manifest_dict(opts: InitOptions) -> dict[str, object]:
    ...
    # kosax.tools.policy_derivation. Layer 1 (green) maps to "read-only" gate → AAL1.
    ...
    return {
        ...
        "dpa_reference": None,
        "slsa_provenance_url": (
            f"https://github.com/kosax-plugin-store/kosax-plugin-{opts.name}/"
...
version = "0.1.0"
description = "KOSAX plugin scaffolded by 'uvx kosax-plugin-init'"
...
def _build_tool() -> Any:
    ...
    # policy.citizen_facing_gate via kosax.tools.policy_derivation.
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="kosax-plugin-init",
        description=(
            "Scaffold a KOSAX plugin (Python entry-point fallback for the TUI command). "
