...
**Created**: 2026-05-03
**Status**: Draft
**Input**: Initiative #2636 / Epic #2638 — S2 Tool System slice의 R2/R3/R4 잔존 잔재를 CORE THESIS (UMMAYA = CC + 2 swap, byte-identical default) 기준으로 재판단·박제·분류한다. 산출물은 audit 재실행 시 R2/R3/R4 가 자동으로 "INTENTIONAL_PRESERVE" 또는 "CLASSIFIED" 마크로 통과하는 코드 주석 + 분류 문서.

## User Scenarios & Testing *(mandatory)*

> **Stakeholder**: 본 spec의 "사용자"는 (a) Initiative #2636 회귀 감사를 재실행하는 미래 Lead Opus, (b) Epic 머지 후 새 PR을 검토하는 Codex 리뷰어, (c) `tui/src/tools.ts` 를 수정하려는 다음 Sonnet teammate. 셋 모두 "왜 UMMAYA 가 CC와 다르게 보이지만 실은 byte-identical 인가" 또는 "왜 18개 파일 중 9개만 byte-identical 인가" 를 즉시 알 수 있어야 한다.
...
새 PR 에서 `tools.ts` 를 보는 리뷰어 (Codex 또는 사람) 가 14개 dev tool import (TodoWriteTool, TaskCreateTool, ConfigTool 등) 와 mock-only WebFetchTool/WebSearchTool 등 UMMAYA 13-tool surface 외 import 를 발견했을 때, **"왜 import 되지만 등록 안 되는가" 의 답이 같은 파일 위쪽 박제 주석에서 즉시 보인다**. 현재는 docstring (line 192-217) 이 일부 설명을 포함하지만 14개 import 블록 자체에는 주석이 없어 "dead import 같다 → 제거하자" 라는 회귀 충동이 발생한다.
...
- **EC-4**: UMMAYA-only 파일 (`built-in/exploreAgent.ts` 같은 것) 이 BYTE-IDENTICAL 로 잡혔다 → UMMAYA-only 면 SHA-256 비교 자체가 N/A. 본 측정에서 UMMAYA-only 파일 0 (모두 CC 에 동일 경로 존재) 이라 EC 무효
...
- **FR-001**: 시스템 (UMMAYA 코드베이스) 은 `specs/2638-tool-system-residue/agent-tool-classification.md` 문서를 보유해야 하며, 이 문서는 `tui/src/tools/AgentTool/` 의 모든 파일 (현재 18 개) 을 BYTE-IDENTICAL / PRESERVE-IDENTICAL-WITH-SHIM / MIGRATE-FOR-SWAP / 회귀 의심 4 버킷 중 정확히 한 곳에 매핑해야 한다.
...
- **SC-002**: `bun typecheck` 가 본 worktree 에서 main 과 동일하게 통과 (UMMAYA narrows to `src/stubs/**`). 박제 주석 + 신규 markdown 으로 인한 회귀 0.
...
- **Spec 2522 머지 우선**: 같은 `tui/src/tools.ts` 를 수정하는 Epic #2579 (UMMAYA-w-2522) 가 본 Epic 보다 먼저 머지되며, 본 Epic 은 그 위에 rebase 한다. 박제 주석 라인 번호는 rebase 후 결정.
...
- **REPLTool / dev tool 등록 표 변경 금지**: UMMAYA 13-tool surface (Spec 1634 P3 contracts/primitive-envelope.md § 1) 는 본 Epic 에서 수정 0. `getAllBaseTools()` 등록 도구 목록 변경은 swap-2 표면 결정이라 별도 Epic.
...
- **CC restored-src 수정 금지**: source-of-truth read-only. 박제 주석은 UMMAYA 측에만 추가.
...
| UMMAYA-only AgentTool 파일 (현재 0건이지만 future) 분류 정책 | 현재 AgentTool 18 파일 모두 CC 에 동일 경로 존재. 미래 UMMAYA-original AgentTool 확장 (가령 `built-in/koreanGovAgent.ts`) 추가 시 분류 정책 (PRESERVE-AS-SWAP-IMPL) 적용 방법은 별도 Epic. | TBD (when needed) | #2677 |
