# Feature Specification: KOSAX-original UI Residue Cleanup (Epic β)

**Feature Branch**: `2293-ui-residue-cleanup`
**Created**: 2026-04-29
**Revised**: 2026-04-29 v2 (caller-graph 박제 후 전면 재작성 — v1 의 30 cleanup-needed 일괄 deletion 결정이 KOSAX-needed file 까지 잘못 포함하여 광범위 회귀 위험 발견)
**Status**: Draft v2
**Input**: User description: "Epic β — Epic α (#2292, merged in `bc523b7`) audit 결과 30 Cleanup-needed 파일 + 6 KOSAX-only Tool deletion candidates 를 정리해 Spec 1633 closure 와 Constitution II 준수 완료. v2: caller-graph 검증 후 진짜 Anthropic dispatcher 잔재만 cleanup."
**Authority** (cite in every downstream artefact):
- `AGENTS.md § CORE THESIS` — KOSAX = AX-infrastructure callable-channel client (3rd thesis canonical)
- `.specify/memory/constitution.md § II Fail-Closed Security (NON-NEGOTIABLE)` — KOSAX-invented permission classifications removed in Spec 1979 MUST NOT be reintroduced
...
- `utils/permissions/permissionSetup.ts` — **11 importer** (main.tsx, REPL, Config, plan.tsx 등 핵심) → KOSAX-needed → **KEEP**
- `utils/permissions/permissions.ts` — **14 importer** (tools.ts registry, AgentTool, BashTool, REPL) → KOSAX-needed → **KEEP**
- `schemas/ui-l2/permission.ts` — 5 importer (PermissionReceiptContext, ExportPdfDialog, export/consent commands, i18n/uiL2; Spec 035 receipt UX) → KOSAX-needed → **KEEP** (또한 30 list 에 처음부터 포함되지 않았음)
...
KOSAX Lead 가 caller-graph.json + disposition.json 을 입력으로, 28 Anthropic dispatcher 잔재 file 을 deletion 하고 그 caller 를 cleanup 한다. 결과: KOSAX TUI 에 Anthropic 1P API 잔재 + claude.ai SaaS 호출 + AWS Bedrock 의존 + growthbook A/B testing + queryHaiku/queryWithModel/verifyApiKey + @anthropic-ai/ import 0 잔존.

**Why this priority**: Spec 1633 (Anthropic→FriendliAI) closure 가 미완 상태로 4 callsite + 3,419 LOC dispatcher 가 alive 라는 점이 MEMORY.md `project_tui_anthropic_residue` 에 기록되어 있다. 본 cleanup 이 미완이면 K-EXAONE 단일 LLM provider 라는 KOSAX thesis 가 실 코드에서 verify 되지 않는다.
...
3. **Given** 16 file 의 `@anthropic-ai/` import, **When** 모든 import 라인 제거 (또는 KOSAX 등가 type 으로 교체), **Then** `grep -rE '@anthropic-ai/' tui/src/` 0 행.
...
### User Story 2 — KOSAX-needed permissions infrastructure 보존 박제 (Priority: P2)

v1 spec 가 잘못 deletion target 으로 잡았던 `utils/permissions/{permissionSetup, permissions}.ts` 두 file 이 KOSAX-needed 임을 caller-graph + disposition.json + decision-log.md 에 박제한다. 보존 결정의 사유 + 11+14 importer 박제 + Constitution II 와의 충돌 부재 박제.

**Why this priority**: v1 spec 의 가설 ("Spec 033 잔재 3 file deletion") 이 caller-graph 미검토로 KOSAX-needed CC permissions 인프라까지 잘못 포함. 본 US 가 그 회귀 가능성을 차단하고 Constitution II 가 KOSAX-invented 권한 분류 (5-mode spectrum / pipa_class / auth_level / permission_tier / is_personal_data / is_irreversible / requires_auth / dpa_reference) 를 NON-NEGOTIABLE 하게 금지한다는 원칙은 별도 audit 영역으로 분리.
...
2. **Given** Constitution II 의 KOSAX-invented 권한 분류 금지 토큰, **When** 두 file 의 본문 token-hit 검사 (caller-graph.json `internal_anthropic_tokens` + dependency_hits 모두 0), **Then** Constitution II 비충돌 박제 — KEEP 결정 무위험.
...
**Independent Test**: schemas/ui-l2/permission.ts 의 5 importer 모두 KOSAX-needed 박제 + decision-log.md 에 KEEP + Constitution II 비충돌 사유 박제.
...
- **FR-005**: 6 KOSAX-only Tool DELETE 결정은 sonnet 1차 commit (`2f9663d`) 에서 이미 완료. decision-log.md § KOSAX-only Tool Decisions 표에 6 entry 박제 (이미 존재). 본 Epic 에서 추가 작업 없음.
- **FR-006**: `cd tui && bun typecheck` 가 `0 errors` 를 출력. (FR-001~FR-003 완료 후)
- **FR-007**: `cd tui && bun test` 의 NEW failure 가 0 (baseline 비교).
- **FR-008**: Spec 1633 closure grep gate (call-site invariant) — `grep -rE '(verifyApiKey|queryHaiku|queryWithModel)\(' tui/src/` 결과 0 행 (호출 패턴 0, comment / stub function 정의는 허용). `grep -rE '@anthropic-ai/' tui/src/` 매치는 (a) `tui/src/sdk-compat.ts` (Anthropic SDK type-only shim), (b) `tui/src/mcpb-compat.ts` (KOSAX Spec #2293 신규 shim — `@anthropic-ai/mcpb` lazy-import + type-only), (c) `tui/src/sandbox-runtime-compat.ts` (KOSAX Spec #2293 신규 shim — `@anthropic-ai/sandbox-runtime` re-export) 3 KOSAX-shim 만 허용. (Constitution II 의 KOSAX-invented 토큰 검사는 별도 audit 영역으로 분리; v1 의 광범위 token list 가 plugin-init.ts/VerifyPrimitive.ts 의 합법 사용까지 잘못 잡았던 결함 보정)
...
- **FR-010**: `tui/src/` 하위 모든 file 의 `@anthropic-ai/` import 가 제거되거나 KOSAX shim 으로 교체. 14 file (caller-graph.json 박제 + grep 검증). 잔존 매치는 KOSAX shim 3 file (`sdk-compat.ts`, `mcpb-compat.ts`, `sandbox-runtime-compat.ts`) 만 — 이들은 KOSAX 가 의도적으로 도입한 shim 으로 caller 가 `@anthropic-ai/*` 를 직접 import 하지 않고 shim 만 import 하도록 통일.

### Key Entities

- **CleanupTarget**: 단위 cleanup 대상 — `kosax_path`, `disposition` (`DELETE` / `KEEP`), `importer_count`, `importers`, `internal_anthropic_tokens`, `dependency_hits`, `caller_cleanup_required`, `risk` (`LOW` / `MEDIUM` / `HIGH` / `N/A`), `rationale`. caller-graph.json + disposition.json 에 박제.
- **CallerGraph**: 30 Cleanup-needed file × importer 매트릭스 — `kosax_path` → `importers[]` + `dependency_hits` (sdk-compat / @aws-sdk/client-bedrock-runtime / claude.ai / growthbook / feature() 등).
...
- **CallsiteMigration**: 8 queryHaiku/etc callsite 의 변환 기록 — `kosax_path`, `original_call`, `replacement` (DELETE caller block / DELETE import-only / `feature_deleted`).
...
- **SC-004**: 6 KOSAX-only Tool deletion 완료 박제 (sonnet 1차 commit `2f9663d` 에 박제).
...
- Epic α #2292 (merged in `bc523b7`) 의 Cleanup-needed 분류는 caller graph 미검토라 30 file 중 일부 KOSAX-needed 가 잘못 포함됨 (permissionSetup / permissions). v2 spec 가 caller-graph.json 박제로 진짜 dead 만 deletion target.
...
- 6 KOSAX-only Tool 모두 DELETE 결정 (research.md § R-4) — sonnet 1차에서 이미 완료, 별도 검증 불필요.
- Spec 035 receipt ledger 는 백엔드 Python (`src/kosax/permissions/ledger.py`) + TUI side 의 `schemas/ui-l2/permission.ts` (PermissionLayerT/PermissionDecisionT) — TUI side 의 receipt 출력 컴포넌트는 KOSAX-needed.
...
- 본 Epic 작업은 `/Users/um-yunsang/KOSAX-w-2293/` worktree 에서 진행되며 main worktree 와 file conflict 없음 (각자 독립 working dir).
...
- **백엔드 Python (`src/kosax/permissions/`) 정리**: Epic δ #2295 territory — 본 Epic 은 TUI 만 다룸.
...
- **KOSAX-only ADDITIONS 274 파일의 추가 audit**: Epic α 가 KEEP/Modified 만 다뤘듯, 본 Epic 도 Cleanup-needed 만 처리.
...
| Constitution II 의 KOSAX-invented 권한 분류 잔재 audit (`tui/src/` 전체) | 본 Epic 은 30 Cleanup-needed 만 다룸; 274 KOSAX-only ADDITIONS 의 Constitution II 정합성 audit 은 별도 spec | Spec 1979 의 후속 또는 Epic ζ | TBD |
