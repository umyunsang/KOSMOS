...
"""T012 — RED test for `kosax.prompt.hash` OTEL span attribute (FR-C07, SC-007).
...
exporter (monkeypatches the module-level _tracer in kosax.llm.client following
...
T012-A  test_prompt_hash_attribute_emitted_on_llm_call
        ...
        ``kosax.prompt.hash``.  This test FAILS RED now because T032 has not
...
T012-B  test_prompt_hash_equals_sha256_of_sent_bytes
        ...
        ``kosax.prompt.hash`` attribute value.  Also FAILs RED now.

Expected failure modes (RED phase)
-----------------------------------
* ``PromptLoader`` import fails (T025 not yet implemented) — test collection
  error surfaces as ImportError in the ``from kosax.context.prompt_loader …``
  import inside the test body.
* Alternatively if import is skipped, the span attribute is simply absent →
  ``AssertionError: kosax.prompt.hash not found in span attributes``.

Both are intentional.  T032 is the implementation task that makes this green.

Architecture
------------
``SystemPromptAssembler.assemble()`` (src/kosax/context/system_prompt.py)
    produces the system prompt string.
``PromptLoader`` (src/kosax/context/prompt_loader.py — T025, not yet present)
    will expose ``.get_hash(prompt_id) -> str`` and ``.all_hashes()``.
``LLMClient.complete()`` (src/kosax/llm/client.py)
...
The span emitting ``kosax.prompt.hash`` is expected on the ``chat`` span
...
# ---------------------------------------------------------------------------
try:
    from kosax.context.prompt_loader import PromptLoader  # type: ignore[import]
...
# OTEL attribute name defined in the KOSAX extension namespace (FR-C07, Spec 021).
_KOSAX_PROMPT_HASH_ATTR = "kosax.prompt.hash"

# Minimal valid env so LLMClientConfig loads without touching the network.
_FAKE_ENV = {"KOSAX_FRIENDLI_TOKEN": "test-token-for-t012"}
...
def mem_exporter(monkeypatch: pytest.MonkeyPatch) -> InMemorySpanExporter:
    """Patch _tracer in kosax.llm.client with a dedicated test TracerProvider."""
...
# ---------------------------------------------------------------------------
# T012-A: kosax.prompt.hash attribute is present on the chat span
# ---------------------------------------------------------------------------
...
async def test_prompt_hash_attribute_emitted_on_llm_call(
    ...
    """Assert that a span from the Context Assembly layer carries kosax.prompt.hash.
    ...
    assert _PROMPT_LOADER_AVAILABLE, (
        "PromptLoader not importable — kosax.context.prompt_loader does not exist yet. "
    ...
    # Find all spans from the Context Assembly layer (identified by kosax.prompt.hash).
    # T032 will stamp this attribute on the 'chat' span produced by LLMClient.stream().
    hash_attrs = [
        dict(s.attributes or {}).get(_KOSAX_PROMPT_HASH_ATTR)
        for s in spans
        if _KOSAX_PROMPT_HASH_ATTR in (s.attributes or {})
    ]

    # PRIMARY ASSERTION — fails RED until T032 is implemented.
    assert hash_attrs, (
        f"{_KOSAX_PROMPT_HASH_ATTR!r} not found in any span attribute. "
...
async def test_prompt_hash_equals_sha256_of_sent_bytes(
    ...
    """Assert kosax.prompt.hash == sha256(messages[0].content.encode('utf-8')).
    ...
    # --- Find the kosax.prompt.hash attribute on any exported span ---
    spans = mem_exporter.get_finished_spans()
    ...
    for span in spans:
        ...
        if _KOSAX_PROMPT_HASH_ATTR in attrs:
            actual_hash = str(attrs[_KOSAX_PROMPT_HASH_ATTR])
            break

    # PRIMARY ASSERTION — fails RED until T032 is implemented.
    assert actual_hash is not None, (
        f"{_KOSAX_PROMPT_HASH_ATTR!r} not found on any span. "
        f"Exported spans: {[(s.name, dict(s.attributes or {})) for s in spans]}. "
