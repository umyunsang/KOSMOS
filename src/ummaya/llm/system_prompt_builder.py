...
"""System prompt augmentation helper for the UMMAYA LLM harness.
...
``ummaya.prompt.hash`` OTEL span attribute (which hashes only the base prompt),
...
from ummaya.llm.models import ToolDefinition as LLMToolDefinition
...
def build_system_prompt_with_tools(
    ...
    logger.debug(
        "ummaya.system_prompt.augmented_chars=%d ummaya.system_prompt.tool_count=%d",
