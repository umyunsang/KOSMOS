...
//
// Walks every active registered KOSAX primitive (lookup / resolve_location /
...
/**
 * Verify that every KOSAX primitive in the registered tool array exposes the
...
export function verifyBootRegistry(registry: readonly Tool[]): BootResult {
  ...
  if (primitives.length !== PRIMITIVE_NAMES.length) {
    ...
    return {
      ...
      diagnostic:
        `[KOSAX][bootGuard] 활성 primitive 중 일부가 ToolRegistry에 등록되지 않았습니다. ` +
        `누락: ${missingNames.join(', ')}.\n` +
        `KOSAX는 활성 primitive(lookup/resolve_location/submit/verify) 모두 등록되어야 부팅을 허용합니다.\n` +
  ...
  for (const tool of primitives) {
    ...
    if (missing.length > 0) {
      return {
        ...
        diagnostic:
          `[KOSAX][bootGuard] 도구 '${tool.name}' 등록 검증 실패. ` +
          `누락 필드: ${missing.join(', ')}.\n` +
