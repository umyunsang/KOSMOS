...
def test_gov24_registration_accepts_simple_auth_tier() -> None:
    ...
    from kosax.primitives.submit import check_tier_gate
    from kosax.tools.mock.submit_module_gov24_minwon import REGISTRATION
...
async def test_gov24_happy_path_returns_succeeded() -> None:
    ...
    with (
        mock.patch(
            "kosax.tools.mock.submit_module_gov24_minwon.validate_delegation",
            return_value=DelegationValidationOutcome.OK,
        ),
        mock.patch(
            "kosax.tools.mock.submit_module_gov24_minwon.append_delegation_used"
        ) as mock_append,
    ):
...
async def test_gov24_transparency_fields_present() -> None:
    ...
    with (
        ...
        mock.patch("kosax.tools.mock.submit_module_gov24_minwon.append_delegation_used"),
    ):
        result = await invoke(params)
