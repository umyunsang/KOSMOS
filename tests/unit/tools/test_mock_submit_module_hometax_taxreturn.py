...
async def test_happy_path_returns_succeeded() -> None:
    ...
    with (
        mock.patch(
            "kosax.tools.mock.submit_module_hometax_taxreturn.validate_delegation",
...
async def test_transparency_fields_present_on_success() -> None:
    ...
    with (
        ...
        mock.patch("kosax.tools.mock.submit_module_hometax_taxreturn.append_delegation_used"),
    ):
        result = await invoke(params)
