...
| `base_time` | `str` | yes | Forecast base time in valid `HHMM` format. The official guide examples use HH30 and say to call after HH:45 KST. Live evidence on 2026-05-18 showed KMA accepts non-`HH30` values and canonicalizes returned rows to the published `baseTime`, but callers should prefer the latest published HH30 slot. |
...
- **KMA API Hub endpoint**: `https://apihub.kma.go.kr/api/typ02/openApi/VilageFcstInfoService_2.0/getUltraSrtFcst`
...
- **Freshness window**: `cache_ttl_seconds=600`. The official guide says ultra-short-term forecast data should be called after HH:45 for the HH30 base time. KMA may return rows whose `base_time` differs from the requested `base_time`; the response row's `base_time` is authoritative.
