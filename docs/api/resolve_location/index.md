...
**Model**: `ResolveLocationOutput` — `src/kosax/tools/models.py` (T039 section).
...
JUSO (`KOSAX_JUSO_CONFM_KEY`) and SGIS (`KOSAX_SGIS_KEY`) were not set during evidence capture; Kakao alone is sufficient for all 4 fields.

## Envelope (Legacy — v1–v3)

**Input model**: `ResolveLocationInput` defined at `src/kosax/tools/models.py`.
...
- **Authentication**: REST API key via `Authorization: KakaoAK {key}` header. Key source: `KOSAX_KAKAO_API_KEY` environment variable.
...
- **Pydantic models**: `KakaoSearchResult`, `KakaoAddressDocument`, `KakaoRoadAddressResult`, `KakaoAddressResult`, `KakaoSearchMeta` defined at `src/kosax/tools/geocoding/kakao_client.py:39–168`.
...
- **Authentication**: API confirm key via `confmKey` query parameter. Key source: `KOSAX_JUSO_CONFM_KEY` environment variable.
...
- **Timeout**: 10 seconds.
- **Backend function**: `lookup_adm_cd` defined at `src/kosax/tools/geocoding/juso.py:24–103`.
...
- **Authentication**: Two-step — consumer key + secret (`KOSAX_SGIS_KEY`, `KOSAX_SGIS_SECRET`) to obtain a short-lived `accessToken`, then `accessToken` on the rgeocode call.
...
- **Timeout**: 10 seconds.
- **Backend function**: `lookup_adm_cd_by_coords` defined at `src/kosax/tools/geocoding/sgis.py:28–127`.
...
- **data.go.kr endpoint**: N/A — this is a KOSAX harness-internal meta-tool that calls external geocoding APIs directly (not via data.go.kr).
...
KOSAX: 해당 주소의 좌표는 위도 37.50032, 경도 127.03665이며, 행정동 코드는 서울특별시 강남구(1168010500)입니다.
```
...
- **Error envelope examples**:
  ...
  - Tier-2 / Tier-3 (auth) fail — Kakao 401: `httpx.HTTPStatusError` with `response.status_code == 401`; recovery classifier maps to `{"kind": "error", "reason": "upstream_unavailable", "message": "Kakao API authentication expired — check KOSAX_KAKAO_API_KEY"}`.
  ...
  - JUSO key not configured: `resolve_location` silently skips JUSO (`KOSAX_JUSO_CONFM_KEY` not set → `_juso_adm_cd` returns `None`) and falls through to SGIS; if all backends miss, surfaces `not_found`.
