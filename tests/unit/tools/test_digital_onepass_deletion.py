...
(b) ``import kosax.tools.mock.verify_digital_onepass`` raises ``ModuleNotFoundError``
    (the file is deleted on disk).
(c) ``kosax.primitives.verify._VERIFY_ADAPTERS`` contains no key containing
...
def test_verify_digital_onepass_module_raises_module_not_found_error() -> None:
    """Importing kosax.tools.mock.verify_digital_onepass raises ModuleNotFoundError.

    FR-004: The file src/kosax/tools/mock/verify_digital_onepass.py is deleted.
    If this test fails, the file was accidentally re-created or the import
    was re-added to __init__.py.
    """
    with pytest.raises(ModuleNotFoundError) as exc_info:
        importlib.import_module("kosax.tools.mock.verify_digital_onepass")
...
def test_verify_digital_onepass_not_in_mock_package_namespace() -> None:
    """kosax.tools.mock does not expose verify_digital_onepass as an attribute."""
    import kosax.tools.mock as mock_pkg  # noqa: F401 — trigger side-effect registration

    assert not hasattr(mock_pkg, "verify_digital_onepass"), (
        "FR-004 violation: kosax.tools.mock still exposes verify_digital_onepass "
...
def test_verify_adapter_registry_has_no_digital_onepass_key() -> None:
    """kosax.primitives.verify._VERIFY_ADAPTERS contains no digital_onepass key.
