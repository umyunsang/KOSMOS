...
tests/observability/
└── test_prompt_hash_attribute.py        # SC-007 — kosax.prompt.hash on every LLM span
tests/shadow_eval/
├── __init__.py
...
src/kosax/context/system_prompt.py      # Refactor — read sections from PromptLoader (FR-C05)
src/kosax/context/session_compact.py    # Refactor — read header + labels from PromptLoader (FR-C06)
src/kosax/engine/*.py                   # Add kosax.prompt.hash span attribute emission (FR-C07, SC-007)
...
**Structure Decision**: Single-project Python library + workflow tooling. No `backend/` vs `frontend/` split; KOSAX has no web frontend in Phase 1. Dockerfile lives under `docker/` to keep the build context surface separate from repo documentation. Workflows live under `.github/workflows/`. Prompt assets live under `prompts/` at repo root so they are visible from every sub-tree and are covered by a single path filter (`prompts/**`) for shadow-eval.
