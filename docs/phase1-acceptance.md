...
| 레이어 | 역할 | 구현 파일 | 상태 |
|--------|------|-----------|------|
| **L1 — Query Engine** | `while(True)` 도구 루프, 상태 기계, 비동기 이벤트 스트림 | `src/kosax/engine/` (`engine.py`, `query.py`, `preprocessing.py`, `events.py`, `tokens.py`) | ✅ 완료 (PR #117) |
| **L2 — Tool System** | 도구 레지스트리, 스키마 검증, 어댑터 팩토리 | `src/kosax/tools/` (`registry.py`, `executor.py`, `models.py`, `search.py`, `rate_limiter.py`) | ✅ 완료 (PR #82, #221) |
| **L3 — Permission Pipeline** | 7단계 권한 gauntlet, fail-closed 기본값, 감사 로그 | `src/kosax/permissions/` (`pipeline.py`, `steps/`, `models.py`, `bypass.py`, `credentials.py`) | ✅ 완료 (PR #221, #402) |
| **L4 — Agent Swarms** | 코디네이터-워커 패턴, 메일박스 IPC | 해당 없음 — Phase 2 대상 | ⚠️ Phase 2 이월 |
| **L5 — Context Assembly** | 3-tier 컨텍스트, 캐시 파티셔닝, 예산 가드 | `src/kosax/context/` (`builder.py`, `system_prompt.py`, `attachments.py`, `budget.py`, 컴팩션 모듈) | ✅ 완료 (PR #221) |
| **L6 — Error Recovery** | 재시도 매트릭스, 서킷 브레이커, 오류 분류기, 캐시 폴백 | `src/kosax/recovery/` (`executor.py`, `classifier.py`, `circuit_breaker.py`, `retry.py`, `cache.py`, `policies.py`) | ✅ 완료 (PR #284) |
...
| ToolExecutor → RecoveryExecutor 배선 | PR #402, `src/kosax/tools/executor.py` | ✅ |
| RecoveryExecutor → 재시도 / 서킷 브레이커 | `tests/recovery/test_retry.py`, `test_classifier.py` | ✅ |
| 최종 assistant 메시지 → CLI 스트리밍 렌더링 | `src/kosax/cli/repl.py`, `renderer.py` | ✅ |
...
|---------|---------|-----------|--------|------|------|
| `koroad_accident_search` | KOROAD `getRestFrequentzoneLg` | `src/kosax/tools/koroad/koroad_accident_search.py` | `tests/tools/koroad/test_koroad_accident_search.py` | `docs/tools/koroad.md` | ✅ |
| `kma_weather_alert_status` | KMA `getPwnStatus` | `src/kosax/tools/kma/kma_weather_alert_status.py` | `tests/tools/kma/test_kma_weather_alert_status.py` | `docs/tools/kma.md` | ✅ |
| `kma_current_observation` | KMA `getUltraSrtNcst` | `src/kosax/tools/kma/kma_current_observation.py` | `tests/tools/kma/test_kma_current_observation.py` | `docs/tools/kma.md` | ✅ |
| `kma_short_term_forecast` | KMA `getVilageFcst` | `src/kosax/tools/kma/kma_short_term_forecast.py` | `tests/tools/kma/test_kma_short_term_forecast.py` | `docs/tools/kma.md` | ✅ |
| `kma_ultra_short_term_forecast` | KMA `getUltraSrtFcst` | `src/kosax/tools/kma/kma_ultra_short_term_forecast.py` | `tests/tools/kma/test_kma_ultra_short_term_forecast.py` | `docs/tools/kma.md` | ✅ |
| `kma_pre_warning` | KMA `getWthrPwnList` | `src/kosax/tools/kma/kma_pre_warning.py` | `tests/tools/kma/test_kma_pre_warning.py` | `docs/api/kma/pre_warning.md` | ✅ |
| `address_to_region` | Kakao Local API | `src/kosax/tools/geocoding/address_to_region.py` | `tests/tools/geocoding/test_address_to_region.py` | — | ✅ |
| `address_to_grid` | Kakao Local API | `src/kosax/tools/geocoding/address_to_grid.py` | `tests/tools/geocoding/test_grid_conversion.py` | — | ✅ |
| `search_address` (Kakao client) | Kakao Local API | `src/kosax/tools/geocoding/kakao_client.py` | `tests/tools/geocoding/test_kakao_client.py` | — | ✅ |
...
- 모든 secrets: `KOSAX_*` 접두사 환경 변수에서만 로드
...
| 컴포넌트 | 구현 파일 | 기능 | 상태 |
|---------|-----------|------|------|
| `MetricsCollector` | `src/kosax/observability/metrics.py` | 카운터, 게이지, 히스토그램 (p50/p95/p99) | ✅ |
| `EventLogger` | `src/kosax/observability/event_logger.py` | 구조화된 JSON 이벤트 로그 | ✅ |
| `ObservabilityEvent` | `src/kosax/observability/events.py` | 이벤트 스키마 정의 | ✅ |
| `/metrics` REPL 명령어 | `src/kosax/cli/repl.py` | 인프로세스 메트릭 스냅샷 렌더링 | ✅ |
| LLMClient 계측 | `src/kosax/llm/client.py` | `llm.requests.total`, `llm.tokens.prompt/completion` | ✅ |
| PermissionPipeline 계측 | `src/kosax/permissions/pipeline.py` | 단계별 deny rate, 레이턴시 | ✅ |
...
KOSAX Phase 1 목표는 `docs/vision.md` §Roadmap에서 다음과 같이 정의됨:
