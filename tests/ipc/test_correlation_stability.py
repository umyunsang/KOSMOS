...
Mirrors ``quickstart.md § 5.2``::
    ...
    uv run python -m ummaya.ipc.demo.full_turn_probe --session s-trace \\
...
def test_probe_emits_single_correlation_id(session: str) -> None:
    ...
    result = subprocess.run(  # noqa: S603
        [sys.executable, "-m", "ummaya.ipc.demo.full_turn_probe", "--session", session],
        cwd=PROJECT_ROOT,
        check=True,
        capture_output=True,
        text=True,
...
def test_probe_frames_are_ndjson() -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "ummaya.ipc.demo.full_turn_probe", "--session", "s-ndjson"],
        cwd=PROJECT_ROOT,
        check=True,
        capture_output=True,
        text=True,
...
def test_probe_tool_call_carries_transaction_id() -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "ummaya.ipc.demo.full_turn_probe", "--session", "s-tx"],
        cwd=PROJECT_ROOT,
        check=True,
        capture_output=True,
        text=True,
