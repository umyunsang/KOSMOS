...
**Epic**: [#1632 · Baseline Runnable](https://github.com/umyunsang/KOSAX/issues/1632)
**Canonical reference**: `docs/requirements/kosax-migration-tree.md § 실행 Phase 순서 → P0`

## User Scenarios & Testing *(mandatory)*

<!--
  Audience: KOSAX 기여자(내부 팀 + 외부 플러그인 기여자). 본 Epic은 하네스 인프라
...
KOSAX 기여자가 fresh clone 후 문서 지시(`bun install` → `bun run src/main.tsx`)만으로
CC 베이스라인 스플래시를 터미널에 띄운다. 스플래시는 최소 3초 동안 크래시 없이 유지되며,
이 시점에서 KOSAX 하네스가 "실행 가능"한 상태임이 시각적으로 입증된다.
...
- **Anthropic API 실제 호출 복구**: KOSAX 미션상 Anthropic API 는 FriendliAI +
  K-EXAONE 으로 전면 교체됨 (`AGENTS.md § What KOSAX is`). 이 Epic 은 Anthropic
  type import 만 해결하고, 실제 호출은 영구적으로 제거된다.
- **Developer-only Tools 복구**: `Read`, `Write`, `Edit`, `Bash`, `Glob`, `Grep`,
  `NotebookEdit` 는 KOSAX 시민용 하네스 범위에서 영구 제외 (`CLAUDE.md §
  Architecture pillars` → C6).
- **Anthropic OAuth / MDM / keychain 실흐름**: KOSAX 는 Anthropic 계정 시스템에
...
| UI L2 컴포넌트 구현 (B REPL · C Permission Gauntlet · D Ministry Agent · E 보조 surface · A Onboarding) | CC 베이스라인 렌더가 선행돼야 KOSAX UI 변형을 얹을 수 있음 | Phase 4 · UI L2 Implementation | #1635 |
...
| GrowthBook analytics · refreshGrowthBookAfterAuthChange · src/services/analytics/* | Anthropic-only 실험 플랫폼 · KOSAX 에서 영구 삭제 또는 교체 결정은 P1 | Phase 1 · Dead code elimination | #1633 |
