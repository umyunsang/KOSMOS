...
Span attributes follow the Spec 021 ``kosax.*`` namespace convention:
    - ``kosax.permission.killswitch.reason`` — the trigger reason
    - ``kosax.permission.tool_id``           — the adapter that triggered the killswitch
    - ``kosax.permission.mode``              — session mode at trigger time
    - ``kosax.permission.correlation_id``   — Spec 032 IPC correlation id
...
def emit_killswitch_span(
    ...
    with _tracer.start_as_current_span(_KILLSWITCH_SPAN_NAME) as span:
        span.set_attribute("kosax.permission.killswitch.reason", reason)
        span.set_attribute("kosax.permission.tool_id", tool_id)
        span.set_attribute("kosax.permission.mode", mode)
        span.set_attribute("kosax.permission.correlation_id", correlation_id)
