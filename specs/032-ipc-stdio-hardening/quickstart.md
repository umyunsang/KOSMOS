...
**Audience**: UMMAYA developers validating Spec 032 end-to-end before merge.
...
from ummaya.ipc.frame_schema import UserInputFrame
...
```bash
uv run python -m ummaya.ipc.demo.session_backend --session-id s-demo &
BACKEND=$!
```

(The synthetic backend is a Spec 032 test harness under `src/ummaya/ipc/demo/` — it drives a single session through a scripted frame sequence.)
...
```bash
uv run python -m ummaya.ipc.demo.upstream_429_probe --retry-after 15
```
...
```bash
uv run python -m ummaya.ipc.demo.register_irreversible_fixture  # seeds AdapterRegistration
```
...
Expected: span for second call has attribute `ummaya.ipc.tx.cache_state = "hit"`.
...
```bash
uv run python -m ummaya.ipc.demo.full_turn_probe --session s-trace > /tmp/trace.ndjson
```
...
Expected: every span in the turn has `ummaya.ipc.correlation_id` set to the same UUIDv7 value.
...
| `resume_rejected(reason="ring_evicted")` in Scenario 2                   | `--after-frames > 256`; ring overflowed during idle                                 | Use smaller value or increase `UMMAYA_IPC_RING_SIZE`.               |
