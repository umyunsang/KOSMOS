...
def test_as02_friendli_token_is_forbidden(tmp_path: Path) -> None:
    ...
    _write_workflow(
        tmp_path,
        "env:\n  KOSAX_FRIENDLI_TOKEN: ${{ secrets.FRIENDLI_TOKEN }}\n",
    )
    result = _run(tmp_path)
...
# ---------------------------------------------------------------------------
# T-AS03: ${{ secrets.KOSAX_KAKAO_API_KEY }} → exit 1
# ---------------------------------------------------------------------------
def test_as03_kosax_api_key_is_forbidden(tmp_path: Path) -> None:
    """T-AS03: Any KOSAX_* secret reference triggers DENY-KOSAX; exit 1."""
    _write_workflow(
        tmp_path,
        "env:\n  KEY: ${{ secrets.KOSAX_KAKAO_API_KEY }}\n",
    )
    result = _run(tmp_path)
    assert result.returncode == 1, f"stderr: {result.stderr}"
    assert "FORBIDDEN" in result.stderr
...
def test_as04_comment_line_suppressed(tmp_path: Path) -> None:
    ...
    _write_workflow(
        tmp_path,
        "# rotate this token: ${{ secrets.KOSAX_FRIENDLI_TOKEN }} quarterly\n",
    )
    result = _run(tmp_path)
    assert result.returncode == 0, f"stderr: {result.stderr}"
