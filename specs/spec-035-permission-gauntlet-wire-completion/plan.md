...
- **CC byte-identical 우선** — 신규 모달 컴포넌트 작성 금지. CC `PermissionRequest.tsx` + `PermissionDialog.tsx` 패턴을 KOSAX dispatcher 한 층 (`KosaxPermissionRequest.tsx`) 로 thin-wrap. (`AGENTS.md § CORE THESIS`)
...
- **권한 정책 발명 금지** — KOSAX 는 어댑터 metadata 의 `auth_level` + `is_irreversible` 를 backend 결정과 함께 표시할 뿐, 새 정책 분류 추가 X. (`AGENTS.md § Tool wrapping is the work`)
...
- `src/kosax/plugins/consent_bridge.py:176` — backend emit path 확인: `kind="permission_request"` 가 plugin consent bridge 에서 emit.
...
### R-3: CC `PermissionRequest.tsx` dispatcher 패턴 + KOSAX 적응 표면

**Question**: KOSAX 는 어떤 component 를 추가해야 CC reference 와 ≥ 90% structural fidelity 를 유지할 수 있는가?
...
- KOSAX 에는 도구 = 4 primitive (lookup/verify/submit/subscribe) — switch 가 4-arm 으로 축소.
- CC `PermissionDialog.tsx` — 모달 골격 (Y/A/N + 메시지), CC import 가능하지만 KOSAX-specific 본문 (한국어 + 어댑터 metadata) 를 prop drilling 으로 전달.

**Verdict**: 추가 컴포넌트 = 1개 (`KosaxPermissionRequest.tsx`, ≤ 80 LOC switch dispatcher). CC `PermissionDialog.tsx` 는 직접 import 하여 child 로 사용 (CC byte-identical). 신규 LOC 총 약 200줄 (dispatcher + AAL 매핑 + i18n).
...
**Investigation**:
- `.references/claude-code-sourcemap/restored-src/src/components/permissions/PermissionExplanation.tsx` — CC reinforcement 패턴 alive, KOSAX 측에서는 dead.
- 부활 비용: ≤ 50 LOC, dependency 0 (이미 React + Ink 사용).

**Verdict**: 부활 (FR-014). `tui/src/components/permissions/KosaxBypassReinforcement.tsx` 추가 (CC `PermissionExplanation.tsx` 패턴 thin-port + 한국어 본문).
...
- `tui/src/screens/REPL.tsx:5534-5536` 주석 — KosaxActivePermissionGate / PermissionGauntletModal × 2 / BypassReinforcementModal 만 제거됨, queue state 자체는 alive.
...
│  │    ├─ <KosaxPermissionRequest>              (NEW - thin)    │
│  │    │    ├─ aalToLayer(primitive, auth_level, is_irreversible)│
│  │    │    ├─ <KosaxBypassReinforcement>       (NEW if bypass) │
...
2. **`tui/src/components/permissions/KosaxPermissionRequest.tsx`** (~ 80 LOC, FR-004)
...
3. **`tui/src/components/permissions/KosaxBypassReinforcement.tsx`** (~ 50 LOC, FR-014)
...
6. **`tui/src/screens/REPL.tsx`** (FR-003/008)
   ...
   - `<KosaxPermissionRequest>` mount logic 추가 (FullscreenLayout `bottom` slot, CC `toolPermissionOverlay` 패턴 따라).
   - `permission_receipt` IPC frame 수신 핸들러 추가 → `addReceipt` 호출.
   - bypassMode state 를 `<KosaxPermissionRequest bypassMode={...}>` 로 prop drilling.
...
[T+45.6s] React reconcile → <KosaxPermissionRequest> mounts → aalToLayer('verify', 'AAL2', false) → 1 → <PermissionDialog> with Layer 1 green ⓵ glyph
...
| bypass mode + Y | `<KosaxBypassReinforcement>` dual-confirm → 후 approve | FR-014 |
...
Phase 2 Hook + Components (T003-T005): sonnet-hook-components
  ...
  ├─ T004 KosaxPermissionRequest.tsx (NEW, ~80 LOC) + unit test
  └─ T005 KosaxBypassReinforcement.tsx (NEW, ~50 LOC) + unit test

Phase 3 REPL wire + addReceipt (T006-T008): sonnet-repl-wire
  ├─ T006 REPL.tsx delete dead modal placeholder + mount KosaxPermissionRequest
...
- Total file changes:
  - NEW: 6 files (aalToLayer.ts, KosaxPermissionRequest.tsx, KosaxBypassReinforcement.tsx, permission.ko.ts, permission.en.ts, smoke scripts)
...
| R3 BypassReinforcementModal 부활 | ✅ R-4 에서 design 결정 | `KosaxBypassReinforcement.tsx` 신규 (CC pattern thin-port) |
