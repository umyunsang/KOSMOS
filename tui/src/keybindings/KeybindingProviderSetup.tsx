...
// UMMAYA migration alias: legacy test naming retained for compatibility.
// Refs: Spec 1637 P6 T032 — keybindings test contract preserved.
// ---------------------------------------------------------------------------
// KeybindingProviderSetup — Tier 1 integration shim
//
// Accepts `handlerOverrides` (a bag of { context: ActionHandlers } entries)
// and registers them with the module-level `registerHandlers` map so that
// `dispatchAction(context, action)` can find and invoke them. Unmounts clean
// up all registrations. Used by `tier1-wiring.test.ts` to assert the wiring
// without mounting the full `KeybindingSetup` with file I/O.
// ---------------------------------------------------------------------------

import { registerHandlers, type ActionHandlers } from './useKeybinding.js'

type KeybindingProviderSetupProps = {
  handlerOverrides: Record<string, ActionHandlers>
  announcer?: unknown
...
export function KeybindingProviderSetup({
  ...
  // Register handler overrides when the component mounts; unregister on unmount.
  useEffect(() => {
    const cleanups: Array<() => void> = []
    for (const [context, handlers] of Object.entries(handlerOverrides)) {
      cleanups.push(registerHandlers(context as KeybindingContextName, handlers))
    }
    return () => {
      for (const cleanup of cleanups) cleanup()
    }
  // We intentionally re-register only when handlerOverrides identity changes.
