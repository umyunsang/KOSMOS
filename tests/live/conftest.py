...
def friendli_token() -> str:
    ...
    return _require_env("KOSAX_FRIENDLI_TOKEN")


@pytest.fixture(scope="session")
def data_go_kr_api_key() -> str:
    """Return the data.go.kr API key from the environment."""
    return _require_env("KOSAX_DATA_GO_KR_API_KEY")
...
def kakao_api_key() -> str:
    ...
    Reads ``KOSAX_KAKAO_API_KEY``.  Hard-fails with an exact message required
    ...
    """
    value = os.environ.get("KOSAX_KAKAO_API_KEY", "").strip()
    if not value:
        pytest.fail("set KOSAX_KAKAO_API_KEY to run live geocoding tests")
    return value
