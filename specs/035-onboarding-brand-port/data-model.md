...
| `primaryHex` | `string` matching `/^#[0-9a-f]{6}$/` | yes | Primary value. Lowercase hex. Must appear in `assets/kosax-logo.svg` palette. |
...
- **I-5**: `primaryHex` MUST be drawn from the authoritative 16-hex palette in `assets/kosax-logo.svg` OR inherit from a preserved semantic slot in the pre-edit `dark.ts`.
...
**Representation**: Pydantic v2 `frozen=True` model on Python side; Zod discriminated union on TypeScript side. Serialised as JSON at `~/.kosax/memdir/user/consent/<timestamp>-<session_id>.json`.
...
**Representation**: Pydantic v2 `frozen=True` model on Python side; Zod schema on TypeScript side. Serialised as JSON at `~/.kosax/memdir/user/ministry-scope/<timestamp>-<session_id>.json`.
...
## Entity 5 — `KosaxThemeToken`

**Purpose**: TypeScript type alias exported by `tui/src/theme/tokens.ts`; closed union of KOSAX metaphor tokens + retained semantic slots.
...
- **I-20**: Header comment in `tokens.ts` AND `dark.ts` includes the two lines required by FR-008 (upstream source + "KOSAX 브랜드 토큰으로 리네이밍됨 (ADR-006 A-9)").

**Relationships**:

- Concretised by `darkTheme: ThemeToken` in `dark.ts` (the type's canonical value instance).
- Referenced by `useTheme()` in every KOSAX TUI component.
...
| `kosaxTargetPath` | `str` | yes | Path under `tui/src/components/`. |
...
| # | ccSourcePath | kosaxTargetPath | verdict | anchor | primary tokens |
|---|---|---|---|---|---|
| 1 | `LogoV2/AnimatedAsterisk.tsx` | `onboarding/LogoV2/AnimatedAsterisk.tsx` | REWRITE | `[ag-logov2]` | `kosaxCore`, `kosaxCoreShimmer` |
...
| 7 | `LogoV2/WelcomeV2.tsx` | `onboarding/LogoV2/WelcomeV2.tsx` | REWRITE | `[ag-logov2]` | `wordmark`, `subtitle`, `kosaxCore` |
| 8 | `FastIcon.tsx` | `chrome/KosaxCoreIcon.tsx` | REWRITE | `[ag-logo-wordmark]` | `kosaxCore` |
| 9 | `Onboarding.tsx` | `onboarding/Onboarding.tsx` | REWRITE | `[ag-onboarding]` | `wordmark`, `subtitle`, `kosaxCore`, `orbitalRing` |
...
- **X-4 — Palette → component closure**: every `LogoV2RewriteComponent.tokenReferences` entry is a member of the DELETE-stripped / ADD-augmented `KosaxThemeToken` surface. No component references a token that does not exist.
...
| `PIPAConsentRecord` | Pydantic v2 + Zod mirror | `src/kosax/memdir/user_consent.py` | immutable (frozen=True) |
| `MinistryScopeAcknowledgment` | Pydantic v2 + Zod mirror | `src/kosax/memdir/ministry_scope.py` | immutable (frozen=True) |
| `KosaxThemeToken` | TS type alias | `tui/src/theme/tokens.ts` | compile-time contract |
