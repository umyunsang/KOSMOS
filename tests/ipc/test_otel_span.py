...
from ummaya.ipc.frame_schema import AssistantChunkFrame, UserInputFrame
...
def mem_exporter(monkeypatch: pytest.MonkeyPatch) -> InMemorySpanExporter:
    """Patch _tracer in ummaya.ipc.stdio with a dedicated test TracerProvider.
    ...
    monkeypatch.setattr(
        stdio_mod,
        "_tracer",
        provider.get_tracer("ummaya.ipc"),
    )
...
async def test_inbound_user_input_span(
    ...
    """Inbound user_input frame → one ummaya.ipc.frame span with direction=inbound."""
    from ummaya.ipc.stdio import _reader_loop  # noqa: PLC0415
    ...
    ipc_spans = [s for s in spans if s.name == "ummaya.ipc.frame"]
    assert len(ipc_spans) == 1, (
        f"Expected exactly 1 ummaya.ipc.frame span, got {len(ipc_spans)}. "
    ...
    assert attrs.get("ummaya.frame.kind") == "user_input", f"ummaya.frame.kind mismatch: {attrs}"
    assert attrs.get("ummaya.frame.direction") == "inbound", (
        f"ummaya.frame.direction mismatch: {attrs}"
    )
    assert isinstance(attrs.get("ummaya.ipc.latency_ms"), float), (
        f"ummaya.ipc.latency_ms must be a float: {attrs}"
    )
...
async def test_outbound_assistant_chunk_span(
    ...
    """Outbound assistant_chunk via write_frame → ummaya.ipc.frame span (direction=outbound)."""
