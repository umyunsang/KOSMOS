...
from kosax.recovery.circuit_breaker import CircuitBreakerConfig
from kosax.recovery.classifier import ErrorClass
...
async def test_attempt_auth_refresh_returns_true_when_specific_var_set(
    ...
    monkeypatch.setenv("KOSAX_MY_TOOL_API_KEY", "secret_key_value")
...
async def test_attempt_auth_refresh_returns_true_when_global_var_set(
    ...
    """Returns True when only the global KOSAX_API_KEY is set."""
    monkeypatch.delenv("KOSAX_MY_TOOL_API_KEY", raising=False)
    monkeypatch.delenv("KOSAX_DATA_GO_KR_API_KEY", raising=False)
    monkeypatch.setenv("KOSAX_API_KEY", "global_key")
    result = await attempt_auth_refresh("my_tool")
    assert result is True
...
async def test_attempt_auth_refresh_returns_false_when_no_vars(
    ...
    monkeypatch.delenv("KOSAX_MY_TOOL_API_KEY", raising=False)
    monkeypatch.delenv("KOSAX_DATA_GO_KR_API_KEY", raising=False)
    monkeypatch.delenv("KOSAX_API_KEY", raising=False)
    result = await attempt_auth_refresh("my_tool")
...
async def test_attempt_auth_refresh_returns_false_for_empty_var(
    ...
    monkeypatch.setenv("KOSAX_MY_TOOL_API_KEY", "")
    monkeypatch.delenv("KOSAX_DATA_GO_KR_API_KEY", raising=False)
    monkeypatch.delenv("KOSAX_API_KEY", raising=False)
    result = await attempt_auth_refresh("my_tool")
    assert result is False
...
async def test_attempt_auth_refresh_prefers_specific_over_global(
    ...
    monkeypatch.setenv("KOSAX_SPECIFIC_TOOL_API_KEY", "specific_val")
    monkeypatch.setenv("KOSAX_API_KEY", "global_val")
...
async def test_401_triggers_auth_refresh_and_succeeds_on_retry(
    ...
    monkeypatch.setenv("KOSAX_AUTH_TEST_TOOL_API_KEY", "refreshed_key")
...
async def test_401_without_credentials_returns_auth_expired(
    ...
    monkeypatch.delenv("KOSAX_AUTH_TEST_TOOL_API_KEY", raising=False)
    monkeypatch.delenv("KOSAX_DATA_GO_KR_API_KEY", raising=False)
    monkeypatch.delenv("KOSAX_API_KEY", raising=False)
...
async def test_401_with_refresh_but_still_fails_returns_error(
    ...
    monkeypatch.setenv("KOSAX_AUTH_TEST_TOOL_API_KEY", "some_key")
...
async def test_401_classification_in_classifier() -> None:
    ...
    from kosax.recovery.classifier import DataGoKrErrorClassifier
...
async def test_kakao_tool_refresh_uses_kakao_key(
    ...
    """Regression: Kakao-backed tools must discover KOSAX_KAKAO_API_KEY."""
    monkeypatch.delenv("KOSAX_ADDRESS_TO_REGION_API_KEY", raising=False)
    monkeypatch.delenv("KOSAX_DATA_GO_KR_API_KEY", raising=False)
    monkeypatch.delenv("KOSAX_API_KEY", raising=False)
    monkeypatch.setenv("KOSAX_KAKAO_API_KEY", "kakao-key")
...
async def test_data_go_kr_tool_refresh_uses_data_go_kr_key(
    ...
    """Regression: KOROAD/KMA tools discover KOSAX_DATA_GO_KR_API_KEY."""
    monkeypatch.delenv("KOSAX_KOROAD_ACCIDENT_SEARCH_API_KEY", raising=False)
    monkeypatch.delenv("KOSAX_KAKAO_API_KEY", raising=False)
    monkeypatch.delenv("KOSAX_API_KEY", raising=False)
    monkeypatch.setenv("KOSAX_DATA_GO_KR_API_KEY", "data-key")
...
async def test_kakao_tool_rejects_data_go_kr_only_environment(
    ...
    monkeypatch.delenv("KOSAX_KAKAO_API_KEY", raising=False)
    monkeypatch.delenv("KOSAX_API_KEY", raising=False)
    monkeypatch.setenv("KOSAX_DATA_GO_KR_API_KEY", "data-only")
...
async def test_per_tool_override_wins_over_provider_var(
    ...
    monkeypatch.setenv("KOSAX_ADDRESS_TO_REGION_API_KEY", "override")
    monkeypatch.setenv("KOSAX_KAKAO_API_KEY", "provider")
    monkeypatch.delenv("KOSAX_API_KEY", raising=False)
