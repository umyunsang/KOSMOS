...
"""Append-only JSONL session store for UMMAYA session persistence.

Session files live at ``~/.ummaya/memdir/user/sessions/{session_id}.jsonl``.
Each line is a JSON-serialised :class:`~ummaya.session.models.SessionEntry`.
...
def _get_default_session_dir() -> Path:
    ...
    Respects ``UMMAYA_MEMDIR_USER`` (canonical Spec 027 env var) first, then
    falls back to ``~/.ummaya/memdir/user``.  The sessions sub-directory is
    always appended.

    Legacy ``UMMAYA_SESSION_DIR`` is retained as a lower-priority override for
    test isolation when the caller cannot set ``UMMAYA_MEMDIR_USER``.
    """
    memdir_user = os.environ.get("UMMAYA_MEMDIR_USER", "").strip()
    if memdir_user:
        return Path(memdir_user).expanduser() / "sessions"
    return Path.home() / ".ummaya" / "memdir" / "user" / "sessions"
...
def _get_session_dir() -> Path:
    ...
    1. ``UMMAYA_MEMDIR_USER`` env var — Spec 027 canonical.  Points to the
       memdir user-tier root; ``sessions/`` sub-dir is appended automatically.
    2. ``UMMAYA_SESSION_DIR`` env var — legacy test-isolation override.
       Treated as a full path (no sub-dir appended).
    3. Hard-coded default: ``~/.ummaya/memdir/user/sessions/``.
    """
    memdir_user = os.environ.get("UMMAYA_MEMDIR_USER", "").strip()
    if memdir_user:
        session_dir = Path(memdir_user).expanduser() / "sessions"
    else:
        legacy_override = os.environ.get("UMMAYA_SESSION_DIR", "").strip()
        if legacy_override:
