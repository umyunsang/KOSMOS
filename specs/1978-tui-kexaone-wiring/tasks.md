...
**Branch**: `feat/1978-tui-kexaone-wiring` (worktree at `/Users/um-yunsang/UMMAYA-wiring`)
**Initiative**: [#1631](https://github.com/umyunsang/UMMAYA/issues/1631) (reopened — premature closure correction). Sibling Epics: [#1979 Plugin DX TUI integration](https://github.com/umyunsang/UMMAYA/issues/1979), [#1980 Agent Swarm TUI integration](https://github.com/umyunsang/UMMAYA/issues/1980).
...
For every code-touching task:
  ...
  2. Copy → adapt — services/api/* over stdio JSONL, tools/* over 5-primitive surface,
     net-new domain (PIPA, Korean tier, swarm) → UMMAYA-original with header
  3. Cite upstream path in task body
  4. Per-file `NOTICE` retains Anthropic attribution where lifted
```

Each task body below includes a `Ref:` or `UMMAYA-original (closest CC pattern: ...)` line. Tasks without one are constitution violations.
...
- Every task cites concrete UMMAYA file paths AND the CC reference baseline.
...
- Backend Python: `src/ummaya/...` and `tests/...` at repo root
...
- [ ] T001 [P] Create PTY scenario harness skeleton at `scripts/pty-scenario.py` using stdlib `pty.fork`. Supports subcommands `greeting | lookup-emergency-room | submit-fine-pay | verify-gongdong | subscribe-cbs`, captures stdout (frames) and stderr (DEBUG log) separately. (Subcommand bodies land in Phase 7.)
       Ref: UMMAYA-original (closest CC pattern: `.references/claude-code-sourcemap/restored-src/src/utils/expect/` PTY testing patterns; CC has no scenario harness equivalent — UMMAYA needs one for `feedback_runtime_verification`)
       Acceptance: harness can spawn `bun run tui`, capture two streams, exit cleanly on SIGTERM. FR: testing infra for SC-001/002/003.
- [ ] T002 Patch `tui/src/ipc/bridge.ts:184-194 _log` helper so `UMMAYA_TUI_LOG_LEVEL=DEBUG` output goes ONLY to stderr, never to stdout (frame stream stays pure NDJSON).
       Ref: `.references/claude-code-sourcemap/restored-src/src/utils/log.ts` (CC `logForDebugging` pattern — stderr-only by convention)
       Action: copy CC stream-routing convention; UMMAYA deviation: UMMAYA uses NDJSON stdout for frames, so DEBUG MUST stay on stderr (CC has no IPC frame stream, both go to stdout). Deps: T001.
- [ ] T003 Run T001 harness against current main, capture exact `PromptInput.tsx` guard line that swallows Enter. Document findings in `docs/spec-1978/B1-root-cause-trace.md` with file:line evidence.
       Ref: `.references/claude-code-sourcemap/restored-src/src/components/PromptInput/PromptInput.tsx` (compare against UMMAYA port)
...
- [ ] T004 Patch the identified guard in `tui/src/components/PromptInput/PromptInput.tsx` so Enter passes through to `onSubmit` end-to-end. Add a regression-guard comment citing the trace doc.
       ...
       Action: align UMMAYA guard with CC original — fidelity ≥ 99%. Deps: T003.
...
- [ ] T005 [P] [US1] Replace `verifyApiKey` runtime call in `tui/src/hooks/useApiKeyVerification.ts:3` with UMMAYA-aware stub returning `'valid'` when `UMMAYA_FRIENDLI_TOKEN` set.
       ...
       Action: keep the `VerificationStatus` enum + state machine identical; UMMAYA deviation: replace `verifyApiKey()` HTTPS call with env-var presence check, `getAnthropicApiKeyWithSource` → `getUmmayaFriendliTokenWithSource`. file: `tui/src/hooks/useApiKeyVerification.ts`. Acceptance: FR-004.
- [ ] T006 [P] [US1] Replace `queryHaiku` call in `tui/src/utils/teleport.tsx:15` with `LLMClient.complete` adapter (same input/output shape).
       ...
       Action: copy file as-is; UMMAYA deviation: replace `queryHaiku(...)` import with `import { LLMClient } from '../ipc/llmClient.js'` + `new LLMClient({...}).complete({...})` adapter wrapping the same prompt+output expectation. file: `tui/src/utils/teleport.tsx`. Acceptance: FR-004.
- [ ] T007 [P] [US1] Replace `queryHaiku` call in `tui/src/commands/rename/generateSessionName.ts:1` with `LLMClient.complete` adapter.
       ...
       Action: copy file as-is; UMMAYA deviation: same as T006 (Haiku call → LLMClient.complete). file: `tui/src/commands/rename/generateSessionName.ts`. Acceptance: FR-004.
- [ ] T008 [P] [US1] Replace `queryWithModel` call in `tui/src/commands/insights.ts:17` with `LLMClient.complete` adapter.
       ...
       Action: copy file as-is; UMMAYA deviation: `queryWithModel(...)` call swap; UMMAYA uses single fixed model so model-arg parameter dropped. file: `tui/src/commands/insights.ts`. Acceptance: FR-004.
- [ ] T009 [US1] Convert `tui/src/services/api/claude.ts` to type-only stub — preserve TypeScript type/interface re-exports referenced elsewhere.
       Ref: `.references/claude-code-sourcemap/restored-src/src/services/api/claude.ts` (canonical 3,419-line CC source — UMMAYA file is byte-identical today; this task carves out the type-only subset)
       Action: identify all named imports `from '.../services/api/claude'` across `tui/src/`, retain types only. Deps: T005, T006, T007, T008.
- [ ] T010 [US1] Collapse `tui/src/services/api/claude.ts` runtime body to ≤ 200 lines — every removed function reduced to `throw new Error('Anthropic SDK removed by Spec 1633/1978; use ummaya.ipc LLMClient')`.
...
- [ ] T011 [P] [US1] Add `tui/tests/services/api/claude-stub.test.ts` asserting runtime calls to deleted exports throw the canonical error and type-only imports compile.
       Ref: UMMAYA-original (closest CC pattern: existing `tui/tests/services/api/claude.test.ts` patterns — replicate test fixture conventions)
       Acceptance: bun test green. file: `tui/tests/services/api/claude-stub.test.ts`.
- [ ] T012 [P] [US1] Add `tests/ipc/test_anthropic_residue_zero.py` — pytest grep ratchet asserting zero alive `from .*services/api/claude` imports outside the stub file or `// UMMAYA:` deletion comments. **Enforces invariant D5** (Anthropic SDK byte path unreachable in normal operation).
       Ref: UMMAYA-original (closest CC pattern: none — defence-in-depth regression guard for Constitution §I rewrite boundary)
...
- [ ] T013 [US1] Update existing `tui/tests/services/api/claude*.test.ts` to test the stub or delete tests no longer applicable.
       Ref: existing tests in `.references/.../tui/tests/services/api/` (where present); UMMAYA port preserves test naming
...
- [ ] T014 [US1] Add `CHANGELOG.md` Unreleased entry: `fix(1978): T1 anthropic residue eliminated — claude.ts collapsed 3419→<200 lines, 4 callsites adapted to LLMClient`.
       Ref: UMMAYA-original (release-note convention from existing `CHANGELOG.md`)
...
- [ ] T015 [P] [US1] Document residue elimination in `docs/spec-1978/anthropic-residue-elim-trace.md` — before/after file:line table, line-count delta.
       Ref: UMMAYA-original (closest CC pattern: none — UMMAYA deviation diagnostic)
...
- [ ] T016 [US1] Run `cd tui && bun test && cd ..` and `uv run pytest -q tests/ipc/test_anthropic_residue_zero.py`; capture results to `docs/spec-1978/phase-B-test-run.log`.
       Ref: UMMAYA-original (CI gate; no CC analog)
...
- [ ] T017 [US2] Add `ChatRequestFrame` Pydantic model to `src/ummaya/ipc/frame_schema.py` per `contracts/chat-request-frame.md`. **Enforces invariant D4** (tool message integrity — `name` and `tool_call_id` populated together when `role="tool"` via `model_validator`).
       Ref: `.references/claude-code-sourcemap/restored-src/src/entrypoints/agentSdkTypes.ts` (CC `SDKMessage` shape: messages/tools/system); `src/ummaya/ipc/frame_schema.py:194 UserInputFrame` (sibling arm in same union)
       Action: UMMAYA-original IPC arm; mirror CC SDKMessage field names (`messages`, `tools`, `system`) for impedance match with `tui/src/query/deps.ts:queryModelWithStreaming` adapter. file: `src/ummaya/ipc/frame_schema.py`.
- [ ] T018 [US2] Add `chat_request → {"tui"}` to `_ROLE_KIND_ALLOW_LIST`; do NOT add to `_TERMINAL_KINDS`.
       Ref: `src/ummaya/ipc/frame_schema.py:_ROLE_KIND_ALLOW_LIST` (UMMAYA-original — Spec 032 invariant E3)
...
- [ ] T019 [US2] Add `ChatRequestFrame` to `IPCFrame` discriminated union; update exhaustiveness checks.
       Ref: `src/ummaya/ipc/frame_schema.py` (FrameUnion type alias — UMMAYA-original)
...
- [ ] T020 [US2] Update `tui/src/ipc/schema/frame.schema.json` — add `chat_request` arm definition (lockstep with Pydantic).
       Ref: `tui/src/ipc/schema/frame.schema.json` (UMMAYA-original — Spec 032 published contract)
...
- [ ] T022 [P] [US2] Add `tests/ipc/test_chat_request_frame.py` — pydantic round-trip + each validation rule.
       Ref: existing `tests/ipc/test_*.py` patterns (Spec 032 UMMAYA-original)
...
- [ ] T024 [US2] Update `ummaya.ipc.schema.hash` OTEL attribute computation in `src/ummaya/ipc/stdio.py` boot sequence.
       Ref: `src/ummaya/ipc/stdio.py` (Spec 032 FR-037 — UMMAYA-original observability hook)
       Action: re-hash schema bytes. Deps: T020.

### Backend tool wiring (Phase D — ADR-0005)

- [ ] T025 [US2] Replace `_handle_user_input_llm` with `_handle_chat_request` in `src/ummaya/ipc/stdio.py` — accepts `ChatRequestFrame`, forwards `messages, tools, system` to `LLMClient.stream`.
       Ref: `.references/claude-code-sourcemap/restored-src/src/services/api/claude.ts:752 queryModelWithStreaming` (signature + content_block_* event handling); `tui/src/query/deps.ts:queryModelWithStreaming` (UMMAYA adapter that this task mirrors on backend side)
       Action: copy CC streaming-event handling logic; UMMAYA deviation: yields IPC frames instead of CC StreamEvent objects. file: `src/ummaya/ipc/stdio.py`. Deps: T019, T024.
- [ ] T026 [US2] Add `pending_calls: dict[str, asyncio.Future[ToolResultFrame]]` registry per data-model.md D1 invariant.
       ...
       Action: copy pattern; UMMAYA deviation: futures resolved by inbound IPC frame instead of in-process callback. file: `src/ummaya/ipc/stdio.py`. Deps: T025.
- [ ] T027 [US2] Implement K-EXAONE `function_call` → `ToolCallFrame` emit — accumulate `input_json_delta`, parse on `content_block_stop`.
       Ref: `tui/src/query/deps.ts` lines 96-145 (UMMAYA already has this exact accumulation logic on TUI side; backend mirrors it)
       Action: port the accumulation loop from TS to Python; emit IPC frame. file: `src/ummaya/ipc/stdio.py`. Deps: T026.
- [ ] T028 [US2] Implement `tool_result` frame consumer in `_dispatch_inbound_frame` — looks up `pending_calls[call_id]`, calls `future.set_result`.
       ...
       Action: UMMAYA deviation: future resolution by inbound frame consumer hook. file: `src/ummaya/ipc/stdio.py`. Deps: T026.
- [ ] T029 [US2] Implement message-history injection — append `{role:"tool", content, name, tool_call_id}` to local messages list, re-invoke `LLMClient.stream` (ReAct loop continuation per ADR-0005).
       ...
       Action: copy ReAct iteration logic; UMMAYA deviation: messages array stays backend-local for one chat_request span. file: `src/ummaya/ipc/stdio.py`. Deps: T028.
- [ ] T030 [US2] Add `UMMAYA_TOOL_RESULT_TIMEOUT_SECONDS` env (default 120); gate outer `asyncio.gather` per `contracts/tool-bridge-protocol.md`.
       Ref: `.references/claude-code-sourcemap/restored-src/src/services/api/withRetry.ts` (CC timeout patterns)
       Action: copy timeout/retry semantics; UMMAYA deviation: synthetic `tool_result` injected on timeout. files: `src/ummaya/ipc/stdio.py` + `src/ummaya/llm/config.py`. Deps: T029.
- [ ] T031 [P] [US2] Update `tui/src/ipc/llmClient.ts` `LLMClient.stream` to send `ChatRequestFrame` instead of `UserInputFrame` when payload includes messages/tools/system.
       Ref: `.references/claude-code-sourcemap/restored-src/src/services/api/claude.ts` request body shape (mirrored on TUI side via `UmmayaToolDefinition`)
...
- [ ] T032 [P] [US2] Update `tui/src/ipc/codec.ts` to add `ChatRequestFrame` writer hook + type guard.
       Ref: `tui/src/ipc/codec.ts` (UMMAYA-original — Spec 032)
...
- [ ] T033 [P] [US2] Backward compat: keep `_handle_user_input_llm` legacy path active treating `UserInputFrame` as `ChatRequestFrame{messages=[{role:"user",content:text}]}` for echo / smoke tests.
       Ref: existing legacy handler in `src/ummaya/ipc/stdio.py:461`
       Action: UMMAYA-original (defence-in-depth — no CC analog). file: `src/ummaya/ipc/stdio.py`. Deps: T025.
...
- [ ] T036 [P] [US2] Add `tests/ipc/test_tool_result_consume.py` — tool_result resolution + history injection + restream.
       ...
       Ref: UMMAYA-original (Spec 032 IPC test pattern)
- [ ] T037 [US2] Add OTEL `ummaya.turn` span emission in `_handle_chat_request` per ADR-0004.
       Ref: `.references/claude-code-sourcemap/restored-src/src/services/api/claude.ts` (CC's Anthropic-side span pattern); UMMAYA uses `opentelemetry-sdk` per Spec 021
       Action: open at frame receipt, close at terminal. file: `src/ummaya/ipc/stdio.py`. Deps: T025.
- [ ] T038 [US2] Add `tests/ipc/test_react_loop_invariants.py` — D1/D3/D6 invariants on synthetic two-tool turn.
       Ref: UMMAYA-original (data-model.md D-series invariants)
...
- [ ] T039 [US2] Expose `submit` primitive to LLM core surface — register Pydantic input/output schema in `src/ummaya/tools/mvp_surface.py` (or sibling) so it appears in `registry.export_core_tools_openai()`.
       Ref: `.references/claude-code-sourcemap/restored-src/src/Tool.ts` (Tool registration pattern); existing `src/ummaya/tools/mvp_surface.py:RESOLVE_LOCATION_TOOL`
       Action: copy mvp_surface.py registration template; UMMAYA deviation: 5 primitives are domain-agnostic envelopes (Spec 031). file: `src/ummaya/tools/mvp_surface.py`. Deps: T029.
- [ ] T040 [US2] Expose `verify` primitive to LLM core surface (same pattern as T039).
       Ref: `src/ummaya/primitives/verify.py:VerifyInput` (existing Spec 031 model)
       Action: extend `mvp_surface.py` with VERIFY_TOOL registration. file: `src/ummaya/tools/mvp_surface.py`. Deps: T039.
- [ ] T041 [US2] Expose `subscribe` primitive to LLM core surface.
       Ref: `src/ummaya/primitives/subscribe.py:SubscribeInput`
       Action: extend `mvp_surface.py` with SUBSCRIBE_TOOL. file: `src/ummaya/tools/mvp_surface.py`. Deps: T039.
...
- [ ] T043 [US2] Import `PermissionPipeline` + `SessionContext` into `src/ummaya/ipc/stdio.py`; instantiate one pipeline + per-session context inside `_handle_chat_request` scope.
       Ref: `src/ummaya/permissions/pipeline.py:PermissionPipeline.evaluate` (existing Spec 033 — UMMAYA-original)
       Action: pipeline integration; mirror existing `ummaya` Rich REPL pattern in `src/ummaya/cli/app.py:236`. file: `src/ummaya/ipc/stdio.py`. Deps: T025.
- [ ] T044 [US2] Implement `_check_permission_via_bridge(tool, ctx)` per `contracts/permission-bridge-protocol.md`.
       Ref: `.references/claude-code-sourcemap/restored-src/src/hooks/useCanUseTool.ts` (CC sync-await permission gate); `src/ummaya/permissions/pipeline.py` (UMMAYA gauntlet)
       Action: UMMAYA-original — combine CC sync-wait with Spec 033 ASK branch. file: `src/ummaya/ipc/stdio.py`. Deps: T043.
- [ ] T045 [US2] Implement `permission_request` frame emit + `pending_perms` dict + `asyncio.wait_for(timeout=60)` per ADR-0002.
       ...
       Action: copy pending-future pattern; UMMAYA deviation: permission keyed by transaction_id. file: `src/ummaya/ipc/stdio.py`. Deps: T044.
- [ ] T046 [US2] Implement `permission_response` frame consumer in `_dispatch_inbound_frame`.
       ...
       Action: future resolution by transaction_id. file: `src/ummaya/ipc/stdio.py`. Deps: T045.
- [ ] T047 [US2] Implement default-deny on timeout per ADR-0002 (Constitution §II fail-closed). **Enforces invariant D2** (every `permission_request` resolved within 60 s OR synthetic deny generated).
       ...
       Action: UMMAYA deviation — default deny + audit synthetic frame. file: `src/ummaya/ipc/stdio.py`. Deps: T045.
- [ ] T048 [US2] Implement consent receipt write to `~/.ummaya/memdir/user/consent/<receipt_id>.json`.
       Ref: existing `src/ummaya/permissions/` consent store patterns (Spec 035 — UMMAYA-original)
       Action: append-only JSON write per data-model.md schema. files: `src/ummaya/ipc/stdio.py` + `src/ummaya/permissions/consent_store.py` (new if absent). Deps: T046.
- [ ] T049 [US2] Implement session-scoped grant memory — `allow_session` adds tool_id to `session_grants: set[str]`.
       Ref: `src/ummaya/permissions/models.py:SessionContext` (extend if needed)
       Action: UMMAYA deviation: session-bounded grant cache. files: `src/ummaya/ipc/stdio.py` + `src/ummaya/permissions/models.py`. Deps: T048.
...
- [ ] T051 [P] [US2] Add `tests/ipc/test_permission_bridge.py` — full handshake (ASK / ALLOW_ONCE / ALLOW_SESSION suppress / DENY / TIMEOUT).
       ...
       Ref: UMMAYA-original (Spec 033 contract test)
- [ ] T052 [US2] Add OTEL `ummaya.permission.*` and `ummaya.consent.receipt_id` attributes per ADR-0004 + Spec 033.
       Ref: `src/ummaya/observability/semconv.py` (extend existing constants)
       Action: UMMAYA-original semconv extension. files: `src/ummaya/ipc/stdio.py` + `src/ummaya/observability/semconv.py`. Deps: T037, T045.

### Mock adapter registration (Phase F — submit branch)

- [ ] T053 [US2] Create `src/ummaya/tools/mock/__init__.py:register_mock_adapters(registry, executor)` that wires every mock under `src/ummaya/tools/mock/` into the appropriate primitive dispatcher.
       Ref: `src/ummaya/tools/register_all.py` (existing Live adapter registration pattern)
       Action: UMMAYA deviation — Mock adapters bypass the GovAPITool registry, plug directly into primitive dispatchers (`submit._ADAPTER_REGISTRY`, etc.). file: `src/ummaya/tools/mock/__init__.py`. Deps: T039+T040+T041.
- [ ] T054 [US2] Register `mock_traffic_fine_pay_v1` from `src/ummaya/tools/mock/data_go_kr/fines_pay.py` into the `submit` primitive dispatcher.
       Ref: `src/ummaya/tools/mock/data_go_kr/fines_pay.py` (existing Mock — Spec 031 ship)
       Action: invoke its `REGISTRATION` object via `submit._ADAPTER_REGISTRY[tool_id] = ...` from T053's loader. file: `src/ummaya/tools/mock/data_go_kr/__init__.py`. Deps: T053.
- [ ] T055 [US2] Update `src/ummaya/tools/register_all.py` to call `register_mock_adapters(...)` after Live registrations.
       Ref: existing register_all.py flow
       Action: append Mock registration step. file: `src/ummaya/tools/register_all.py`. Deps: T053.
...
- [ ] T057 [US2] Implement eager spawn of `ummaya.ipc.mcp_server` at TUI startup per ADR-0003 in `tui/src/ipc/bridgeSingleton.ts`.
       Ref: `.references/claude-code-sourcemap/restored-src/src/services/mcp/mcpClient.ts` (CC eager-spawn pattern)
       Action: copy CC spawn flow; UMMAYA deviation: spawns Python `mcp_server.py` instead of npm MCP servers. file: `tui/src/ipc/bridgeSingleton.ts`. Deps: T056.
...
- [ ] T059 [P] [US2] Add `tui/tests/ipc/mcp-eager-spawn.test.ts`.
       Ref: UMMAYA-original (closest CC pattern: existing `tui/tests/services/mcp/` if present)
       Acceptance: spawn happens before mcpClients consumed. file: `tui/tests/ipc/mcp-eager-spawn.test.ts`.
- [ ] T060 [US2] Verify `tools/list` JSON-RPC reply on `ummaya.ipc.mcp_server` returns the registered primitive surface non-empty after T053-T055 register.
       Ref: `src/ummaya/ipc/mcp_server.py` (existing Spec 1634 — UMMAYA-original)
       Acceptance: N9 closed; tools/list non-empty. file: `src/ummaya/ipc/mcp_server.py`. Deps: T057+T055.
...
- [ ] T061 [US3] Register `verify_gongdong_injeungseo` Mock from `src/ummaya/tools/mock/verify_gongdong_injeungseo.py` into `verify` primitive dispatcher (via T053 loader).
       Ref: `src/ummaya/tools/mock/verify_gongdong_injeungseo.py` (existing Spec 031 ship)
       Action: extend T053 loader to walk `verify_*.py` mocks. file: `src/ummaya/tools/mock/__init__.py`. Deps: T053+T040.
- [ ] T062 [US3] Register `verify_digital_onepass` Mock from `src/ummaya/tools/mock/verify_digital_onepass.py`.
       Ref: `src/ummaya/tools/mock/verify_digital_onepass.py`
       Action: same as T061. file: `src/ummaya/tools/mock/__init__.py`. Deps: T061.
...
- [ ] T065 [US3] Verify `verify` primitive's `family_hint` mismatch returns `VerifyMismatchError` per Spec 031 FR-010.
       Ref: `src/ummaya/primitives/verify.py:VerifyMismatchError` (existing)
...
- [ ] T066 [US3] Add `tui/src/components/verify/AuthContextDisplay.tsx` (or extend existing) to render the discriminated-union output in citizen-readable form.
       Ref: `.references/claude-code-sourcemap/restored-src/src/components/` for any AuthContext-shaped component (likely none — UMMAYA-original); existing message renderer patterns
       Action: UMMAYA-original UI; closest CC pattern: `Message` renderer for tool_use blocks. file: `tui/src/components/verify/AuthContextDisplay.tsx`.
...
- [ ] T068 [US4] Register `mock_cbs_disaster_msg` from `src/ummaya/tools/mock/cbs/` into `subscribe` primitive dispatcher.
       Ref: `src/ummaya/tools/mock/cbs/` (existing Spec 031 ship); `src/ummaya/primitives/subscribe.py:SubscriptionHandle`
       Action: extend T053 loader. file: `src/ummaya/tools/mock/__init__.py`. Deps: T053+T041.
- [ ] T069 [US4] Wire `SubscriptionHandle` lifetime — backend keeps the handle alive across multiple chat_request frames within the same session, streams CBS events as `assistant_chunk` frames or via a new lightweight notification path.
       Ref: `src/ummaya/primitives/subscribe.py:SubscriptionHandle` lifetime model (existing Spec 031)
       Action: UMMAYA-original — bridge primitive lifetime to IPC stream. file: `src/ummaya/ipc/stdio.py`. Deps: T068+T029.
...
- [ ] T072 [US4] Verify `/quit` cleanly closes subscription — no orphan process.
       ...
       Acceptance: clean shutdown. files: `src/ummaya/ipc/stdio.py` + `tui/src/main.tsx`. Deps: T069.
...
- [ ] T073 Implement `scripts/pty-scenario.py greeting` subcommand body — sends `안녕하세요`, captures latencies.
       Ref: UMMAYA-original (T001 harness skeleton extension)
...
- [ ] T079 [P] Add `scripts/probe-bridge.py` — hand-rolled `ChatRequestFrame` direct send.
       Ref: UMMAYA-original (quickstart.md "On failure" tooling)
...
- [ ] T081 [P] Implement `ummaya.session` root span in `src/ummaya/ipc/stdio.py` per ADR-0004; close at `session_event{event=exit}`.
       Ref: existing OTEL setup in `src/ummaya/observability/` (Spec 021)
       Action: span hierarchy completion. file: `src/ummaya/ipc/stdio.py`. Deps: T037.
...
- [ ] T083 [P] Update `docs/ipc-protocol.md` + `docs/requirements/ummaya-migration-tree.md` to mark P3 closure superseded by Epic #1978.
       Ref: existing docs
       file: `docs/ipc-protocol.md`, `docs/requirements/ummaya-migration-tree.md`.
...
- [ ] T085 Verify SC-004 measurable evidence — run `nettop -P -t wifi` (macOS) or `tcpdump -i any -n host anthropic.com` (Linux) during a 20-minute mixed conversation session; assert zero packets to `*.anthropic.com`; capture to `docs/spec-1978/anthropic-egress-zero-trace.md`. Closes analyze finding C1.
       Ref: UMMAYA-original (closest CC pattern: none — Constitution §I rewrite-boundary runtime evidence; complements static grep guard at T012)
...
- [ ] T086 Record `asciinema rec` of the three demo flows (lookup / submit / verify) for KSC 2026 backup; commit casts to `docs/spec-1978/asciinema/{greeting,lookup,submit,verify}.cast`. Closes analyze finding C2.
       Ref: UMMAYA-original (closest CC pattern: none — KSC stage backup asset)
...
Convergence: `src/ummaya/ipc/stdio.py` (heaviest shared file). Tasks T024-T030, T033, T037, T043-T049, T052, T069, T081 — sequential.
...
- Plugin DX TUI integration → tracked separately under [Epic #1979](https://github.com/umyunsang/UMMAYA/issues/1979).
- Agent Swarm TUI integration → tracked separately under [Epic #1980](https://github.com/umyunsang/UMMAYA/issues/1980).
- 7 Deferred Items in spec.md table — 2 already linked to issues (#1979 / #1980), 5 remain `NEEDS TRACKING` for `/speckit-taskstoissues`.
- All paths are absolute relative to `/Users/um-yunsang/UMMAYA-wiring/`.
- Every task body includes a `Ref:` line citing CC source path or a `UMMAYA-original (closest CC pattern: ...)` justification per `feedback_cc_source_migration_pattern`.
