...
# Files in the CC baseline that are intentionally NOT ported to KOSAX
# (1P-Anthropic business surfaces — see tui/src/components/.never-port.md).
# These paths are accepted as "missing in KOSAX" without triggering the
...
def main() -> int:  # noqa: C901
    ...
    parser.add_argument("--slice-root", required=True, type=Path,
                        help="Root of the KOSAX slice (typically tui/src)")
    ...
    # removed from KOSAX without an explicit NEVER-PORT carve-out. This is
    # the inverse of the divergence walk below: if we only walked KOSAX
    # files, a `git rm tui/src/components/App.tsx` would silently pass.
    kosax_rels = {p.relative_to(slice_root).as_posix() for p in slice_files}
    for cc_rel in baseline:
        if cc_rel in kosax_rels:
            continue
    ...
    for kosax_path in slice_files:
        stats["total"] += 1
        rel_to_slice = kosax_path.relative_to(slice_root).as_posix()
        actual_sha = sha256_of(kosax_path)

        cc_sha = baseline.get(rel_to_slice)
        if cc_sha is None:
            # KOSAX-only file — no CC baseline. PASS (these are KOSAX-original
        ...
        try:
            repo_rel = kosax_path.resolve().relative_to(repo_root).as_posix()
        except ValueError:
    ...
    if stats["failed"]:
        # `failed` aggregates two failure modes: (a) divergent KOSAX file
        ...
        sys.stderr.write(
            ...
            f"unjustified divergence(s) (missing-from-kosax OR sha-mismatch-without-whitelist). "
