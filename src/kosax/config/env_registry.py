...
"""TUI operational configuration loaded from KOSAX_TUI_* environment variables.
...
script (`scripts/audit-env-registry.py`) treats every ``KOSAX_TUI_*`` reference
found in ``src/`` as registered and does not flag them as unregistered leaked values.

Usage::

    from kosax.config.env_registry import tui_settings
...
class TUISettings(BaseSettings):
    ...
    """Active theme for the Ink terminal UI (KOSAX_TUI_THEME)."""
    ...
    """Log verbosity for IPC bridge frames (KOSAX_TUI_LOG_LEVEL)."""
    ...
    """Korean IME input strategy (KOSAX_TUI_IME_STRATEGY)."""
    ...
    """Soak-test IPC event rate in events/second (KOSAX_TUI_SOAK_EVENTS_PER_SEC)."""
