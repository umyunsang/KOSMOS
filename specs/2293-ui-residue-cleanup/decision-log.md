...
|------|-----------|----------|-----------|
| services/api/adminRequests.ts | — | DELETE | Dead — no UMMAYA citizen use case |
...
| services/api/filesApi.ts | — | DELETE | Claude Files API — no UMMAYA equivalent |
| services/api/firstTokenDate.ts | — | DELETE | Dead analytics |
| services/api/grove.ts | — | DELETE | Anthropic internal service — dead |
| services/api/logging.ts | — | DELETE | Anthropic telemetry — UMMAYA uses OTEL |
...
| services/api/withRetry.ts | — | DELETE | Anthropic retry logic — UMMAYA has its own |
...
|----------|------|----------|-----------|
| cli/print.ts | verifyApiKey + queryHaiku | DELETE | UMMAYA = FriendliAI single provider; API key verification is different |
...
| utils/mcp/dateTimeParser.ts | queryHaiku | DELETE | UMMAYA DateParser (MVP7) replaces |
...
## § 3 UMMAYA-only Tool Decisions

| Tool | Decision | Rationale | Reference |
|------|----------|-----------|-----------|
| MonitorTool | DELETE | No background tasks for citizens in UMMAYA synchronous lookup/submit flow | research.md § R-4 |
...
| WorkflowTool | DELETE | UMMAYA multi-step handled by system prompt + 4 primitive chain (Spec 022) | research.md § R-4 |
...
|------|---------|----------|-----------|
| schemas/ui-l2/permission.ts | — | DELETE (pending caller check) | Constitution II NON-NEGOTIABLE — UMMAYA-invented permission enums must not remain in codebase |
