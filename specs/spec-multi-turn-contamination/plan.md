...
- Python — `pydantic >= 2.13` (frame schema, `ChatRequestFrame.messages`), `pydantic-settings >= 2.0` (env catalog: `KOSAX_CHAT_REQUEST_DUMP`, `KOSAX_QUERY_TRACE`), `httpx >= 0.27` (FriendliAI client, untouched), `opentelemetry-sdk` + `opentelemetry-semantic-conventions` (Spec 021 spans, NEW span attribute `kosax.chat.turn_index` on diagnostic emit), `pytest` + `pytest-asyncio` (existing test stack).
...
- Layer 5 tmux smoke (`scripts/tui-tmux-capture.sh`) — `specs/spec-multi-turn-contamination/scripts/{repro-two-turn.sh,regress-multi-turn.sh}` against deterministic fake LLM (`KOSAX_LLM_PROVIDER=fake-multi-turn`).
- `@pytest.mark.live` manual — real K-EXAONE on FriendliAI two-turn smoke. AGENTS.md hard rule: NEVER in CI.

**Target Platform**: macOS / Linux developer terminals running Bun v1.2.x + Python 3.12+. Identical to all prior KOSAX deliverables.
...
**Constraints**:
- AGENTS.md hard rules (frozen) — no new deps, KOSAX_-prefixed env vars, stdlib `logging`, no `print()`, no `--force` push to main.
...
**Scale/Scope**: Single feature affecting two files (frontend `tui/src/query/deps.ts`, backend `src/kosax/ipc/stdio.py`) for diagnostic emit + ≤2 files for the fix (layer determined by US1 verdict) + 2 test files + 1 ADR + 1 spec directory. Total LOC delta projected ≤300.
...
| Env vars prefixed `KOSAX_` | `KOSAX_CHAT_REQUEST_DUMP`, `KOSAX_QUERY_TRACE` (extends existing), `KOSAX_LLM_PROVIDER=fake-multi-turn` (test-only) | PASS |
...
| Decision | CC Reference | KOSAX Adaptation |
|----------|--------------|-------------------|
...
| Two-turn scenario harness (US1 / US3) | `.references/claude-code-sourcemap/restored-src/src/screens/REPL.tsx` multi-turn paint cycle — CC has no canonical multi-turn smoke (developer harness assumes single dev session) | KOSAX layer 5 tmux scenario adds `wait_for_pane <regex> <deadline>` between turns to handle K-EXAONE reasoning latency (per memory `feedback_debug_infra_rebuild`) |
| Hypothesis classification → fix layer (US2 / FR-008) | N/A (CC byte-copy methodology presumes no contamination class — KOSAX-novel) | Spec 2521 procedure: if fix touches LLM client (H2), MUST cite CC line range + use byte-copy + swap commit pattern |
| Regression test pattern (US3 / FR-012) | CC test stack uses `bun:test` + `ink-testing-library` v4 across many TUI tests | KOSAX extends with stub-bridge intercept of `ChatRequestFrame` payload; existing pattern, not new infrastructure |
...
**File**: `src/kosax/ipc/frame_schema.py:222-310`. `ChatRequestFrame.messages: list[ChatMessage]` carries the full conversation history per turn. `ChatMessage` is a pydantic v2 frozen model with `role: Literal["system", "user", "assistant", "tool"]` + `content: str` + optional `name` / `tool_call_id` for tool-role messages. Validator on the frame asserts `role="tool"` messages MUST set both `name` AND `tool_call_id` (data-model invariant D4 — Spec 1978 ADR-0001).
...
**FriendliAI cache key collision** (H2 sub-hypothesis): FriendliAI's serverless tier may cache prompt prefixes by `session_id`. If KOSAX reuses the same `session_id` across turns and FriendliAI's cache returns a stale completion, the turn-2 response would be a verbatim turn-1 re-emission. The diagnostic test (edge case "session_id rotation isolation test") rotates `session_id` per turn and observes whether contamination persists; persistence rules out FriendliAI cache, disappearance confirms it.
...
### R-4: Backend agentic loop — `src/kosax/ipc/stdio.py:_handle_chat_request`

**File**: `src/kosax/ipc/stdio.py:1786-2900` (covers handler entry through max-turns termination). Backend takes `frame.messages`, augments the system prompt with `<available_adapters>` BM25 suffix (Spec 2521), then enters an agentic loop where each iteration:
...
3. Bounded by `KOSAX_AGENTIC_LOOP_MAX_TURNS=8`.
...
`KOSAX_QUERY_TRACE=1` is the existing TUI-side trace gate (`tui/src/query/deps.ts:113`). The backend has no equivalent gate; `logger.info` calls fire unconditionally. Per AGENTS.md "stdlib `logging` only", the backend gate MUST be a `logger.isEnabledFor(logging.DEBUG)` check OR a `KOSAX_*` env-var check at handler entry. The latter is more discoverable (matches the existing TUI pattern) — choose `KOSAX_CHAT_REQUEST_DUMP=1`.
...
```
`turnIndex` source: a session-scoped monotonic counter on the `KosaxBridge` singleton (existing — `tui/src/ipc/bridgeSingleton.ts`). Increment on every `bridge.send(ChatRequestFrame)`.
...
For `[REASONING_PREVIEW]`, instrument the LLMClient stream consumer (existing path that accumulates `reasoning_content` into `accumulatedThinking` — KOSAX already has this buffer per Spec 2521). Emit when the buffer first reaches 1024 bytes OR on stream completion (whichever first).

**Span attribute** (additive, OTEL): `kosax.chat.turn_index` set on every `kosax.chat.request` span (existing span from Spec 021). Lets Langfuse trace dashboards group by turn.
...
1. `tmux new-session -d -s kosax-multi 'KOSAX_CHAT_REQUEST_DUMP=1 KOSAX_QUERY_TRACE=1 bun run tui 2>backend.log'`
2. `wait_for_pane "KOSAX" 10` (boot)
3. `tmux send-keys -t kosax-multi "강남역 근처 내과 찾아줘" Enter`
...
5. `tmux capture-pane -t kosax-multi -p > snap-001-turn1-done.txt`
6. `tmux send-keys -t kosax-multi "재난 알림 구독해줘" Enter`
7. `wait_for_pane "subscribe\|disaster\|재난" 90` — OR — fallback `wait_for_pane ".+" 90` if contamination dispatches a wrong tool
8. `tmux capture-pane -t kosax-multi -p > snap-002-turn2-done.txt`
9. `tmux send-keys -t kosax-multi C-c C-c` (graceful exit)
...
- **H2 isolation 2**: Set `KOSAX_K_EXAONE_THINKING=false`. If reasoning blob disappears AND tool dispatch ALSO becomes correct, H2 confirmed (the reasoning was driving the wrong tool selection). If only the blob disappears, H3 (tool-result residue or BM25 suffix contamination) is still active.
...
Per Spec 2521 + AGENTS.md `feedback_cc_source_migration_pattern`: any change to `src/kosax/llm/client.py` MUST first verify the file's CC analog (`.references/claude-code-sourcemap/restored-src/src/services/...`) and any deviation MUST be a labeled `swap/<category>` commit (categories per Spec 2521).

For H2 in particular, the canonical CC pattern `yieldMissingToolResultBlocks` (`services/api/claude.ts:957`) is the well-known fix for "assistant has tool_use without matching tool_result" multi-turn drift. KOSAX may need to port this directly.
...
└── diagnostic-runs/
    └── <UTC-timestamp>/          # FR-005 committed reproduction artefacts
        ├── backend.log           # Stderr from KOSAX_CHAT_REQUEST_DUMP=1
        ├── frontend.log          # Stderr from KOSAX_QUERY_TRACE=1 (extracted from PTY)
...
docs/testing.md                   # MODIFY: Document KOSAX_CHAT_REQUEST_DUMP env var (FR-014 / SC-006)
```
