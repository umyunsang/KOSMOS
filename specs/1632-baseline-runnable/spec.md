...
- **Anthropic API 실제 호출 복구**: KOSMOS 미션상 Anthropic API 는 FriendliAI +
  EXAONE 으로 전면 교체됨 (`AGENTS.md § What KOSMOS is`). 이 Epic 은 Anthropic
...
| FriendliAI 엔드포인트 연결 · EXAONE system prompt 적용 · 세션 JSONL 저장소 연결 | P0 에는 실제 LLM 호출 없이 스플래시만 목표 | Phase 2 · Anthropic → FriendliAI | #1633 |
