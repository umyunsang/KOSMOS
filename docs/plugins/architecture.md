...
| 심볼 | 타입 | 책임 |
|---|---|---|
| `TOOL` | `kosax.tools.models.GovAPITool` | 레지스트리 메타데이터. `id` 가 manifest 의 `adapter.tool_id` 와 byte-equal. |
...
manifest.yaml ──parse─▶ PluginManifest (Pydantic v2 frozen)
                       ...
                       register_plugin_adapter(manifest)
                              ...
                              └─ OTEL span "kosax.plugin.install"
                                  └─ kosax.plugin.id = manifest.plugin_id
```

이 흐름은 다음 파일에 구현되어 있습니다:
- `src/kosax/plugins/manifest_schema.py` — PluginManifest + PIPATrusteeAcknowledgment.
- `src/kosax/plugins/registry.py` — `register_plugin_adapter` + `auto_discover` + `_rebuild_bm25_index_for`.
- `src/kosax/plugins/installer.py` — 8-phase install (catalog → bundle → SLSA → manifest → consent → register → receipt).
- `src/kosax/tools/registry.py` — Spec 022 ToolRegistry + Spec 1636 T010 shim.

---

## OTEL 발산 계약

플러그인 install/invoke 시 다음 span 이 자동 발산됩니다 (Spec 021 KOSAX extension):

| Span | Trigger | 핵심 attribute |
|---|---|---|
| `kosax.plugin.install` | `register_plugin_adapter` 진입 | `kosax.plugin.id`, `kosax.plugin.version`, `kosax.plugin.tier`, `kosax.plugin.tool_id`, `kosax.plugin.permission_layer` |
| `execute_tool <tool_id>` | 어댑터 호출 | `gen_ai.tool.name`, `gen_ai.tool.type`, `kosax.tool.adapter` |

manifest.yaml 의 `otel_attributes["kosax.plugin.id"]` 는 **반드시 `plugin_id` 와 일치** 해야 합니다 (`_v_otel_attribute` validator 가 enforce). 이는 Langfuse / OTLP collector 가 plugin-별 trace 를 정확히 묶도록 보장합니다.

---

## 디렉토리 레이아웃

설치 후 host 는 플러그인을 다음 경로에 보관합니다 (`KOSAX_PLUGIN_INSTALL_ROOT` 기본값):

```
~/.kosax/memdir/user/plugins/
...
번들 다운로드 캐시 (`KOSAX_PLUGIN_BUNDLE_CACHE` 기본 `~/.kosax/cache/plugin-bundles/`) 와 vendored slsa-verifier (`KOSAX_PLUGIN_VENDOR_ROOT` 기본 `~/.kosax/vendor/`) 는 별도 경로 — install 실패 시에도 forensic 분석을 위해 번들이 보존됩니다.
...
| Repo | 역할 | 비고 |
|---|---|---|
| `kosax-plugin-store/kosax-plugin-template` | scaffold 템플릿 | "Use this template" 진입점 |
| `kosax-plugin-store/index` | 카탈로그 인덱스 | `KOSAX_PLUGIN_CATALOG_URL` 기본값 |
| `kosax-plugin-store/kosax-plugin-seoul-subway` | Live 예제 1 | 서울 지하철 도착 정보 |
| `kosax-plugin-store/kosax-plugin-post-office` | Live 예제 2 | 우정사업본부 택배 추적 |
| `kosax-plugin-store/kosax-plugin-nts-homtax` | Mock 예제 1 | 국세청 홈택스 (mock — 정부 partnership 부재) |
| `kosax-plugin-store/kosax-plugin-nhis-check` | Mock 예제 2 | 국민건강보험공단 건강검진 (mock) |

`kosax-plugin-store` org 는 SLSA-provenance attribution 을 위한 canonical source URI prefix 입니다 (R-3). 외부 fork 도 가능하나 그 경우 `slsa_provenance_url` 이 fork 의 GitHub Releases 를 가리켜야 합니다.
...
| 카탈로그 | catalog | `kosax-plugin-store/index/index.json`. |
