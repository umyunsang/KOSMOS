...
from ummaya.agents.errors import MailboxOverflowError, MailboxWriteError
...
class FileMailbox(Mailbox):
    ...
    Args:
        ...
        root: Base directory for all mailbox files. Defaults to
              ``UMMAYA_AGENT_MAILBOX_ROOT`` from settings if not provided.
        max_messages: Per-session message cap. Defaults to
                      ``UMMAYA_AGENT_MAILBOX_MAX_MESSAGES`` from settings.
    """
    ...
    def __init__(
        ...
        *,
        ...
        from ummaya.settings import UmmayaSettings  # local to avoid circular import

        _settings = UmmayaSettings()
    ...
    async def send(self, message: AgentMessage) -> None:
        ...
        with _tracer.start_as_current_span("gen_ai.agent.mailbox.message") as span:
            span.set_attribute(UMMAYA_AGENT_MAILBOX_MSG_TYPE, message.msg_type.value)
            span.set_attribute(
                UMMAYA_AGENT_MAILBOX_CORRELATION_ID,
                str(message.correlation_id) if message.correlation_id else "",
            )
            span.set_attribute(UMMAYA_AGENT_MAILBOX_SENDER, message.sender)
            span.set_attribute(UMMAYA_AGENT_MAILBOX_RECIPIENT, message.recipient)
