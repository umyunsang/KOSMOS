...
v1 의 R-3 (Constitution II 평가) 가 caller graph 미검토로 utils/permissions 3 file 모두 DELETE 결론. v2 의 caller-graph 박제로 permissionSetup (11 imp) + permissions (14 imp) UMMAYA-needed 발견 → KEEP 으로 결론 정정.
...
| Constitution II audit (`tui/src/` 전체 274 UMMAYA-only ADDITIONS) | NEEDS TRACKING | Spec 1979 후속 또는 Epic ζ |
...
**Alternatives**:
- (rejected) v1 의 수동 grep + 추측 결정: UMMAYA-needed file 까지 deletion 결정에 포함된 결함 발생 (이번 v2 가 보정).
...
**Q**: 각 callsite 가 UMMAYA 시민 use case 에서 살아있는가?

**Decision matrix** (v1 그대로):
| Callsite | 기능 | UMMAYA 결정 |
|---|---|---|
| `cli/print.ts` (verifyApiKey + queryHaiku) | Anthropic API key 검증 + 한 줄 응답 출력 | DELETE — UMMAYA는 FriendliAI 단일 provider, API key 검증 흐름 다름 |
...
| `utils/mcp/dateTimeParser.ts` (queryHaiku) | 자연어 → datetime 파싱 | DELETE — UMMAYA DateParser 보조 도구 (MVP7) 와 별개; 0 importer |
...
**Rationale**: UMMAYA는 시민 use case 만 보존. claude-code 의 개발자-중심 자동 요약 / 분석 도구는 대부분 dead. K-EXAONE migration 은 별도 spec.
...
1. **`utils/permissions/permissionSetup.ts`** (11 importer):
   ...
   - 모두 UMMAYA TUI 의 정상 사용 (Plan mode / Settings / REPL / Permission setup UX)
   - Anthropic-token = 0 / sdk-compat = 0 / growthbook = 0 / `feature()` 만 12회 (in-file dead-flag, 별도 정리 영역)
   - **결론: KEEP** (Constitution II 비충돌 — UMMAYA-invented 권한 분류 토큰 0)
...
2. **`utils/permissions/permissions.ts`** (14 importer):
   ...
   - UMMAYA Tool registry + 모든 Tool 의 permission 검사 인프라
...
3. **`utils/permissions/yoloClassifier.ts`** (2 importer):
   ...
   - UMMAYA 는 growthbook A/B testing 안 씀 + Anthropic 의존
...
4. **`schemas/ui-l2/permission.ts`** (5 importer; 30 list 외):
   ...
   - PermissionLayerT/PermissionDecisionT 는 Spec 033 5-mode spectrum (mode_state / aal_level / pipa_class 등 UMMAYA-invented 권한 분류) 와 별개 — receipt 출력용
   - **결론: KEEP** + decision-log.md 박제

**Rationale**: Constitution II 가 NON-NEGOTIABLE 하게 금지하는 것은 "UMMAYA-invented 권한 분류 (5-mode spectrum / pipa_class / auth_level / permission_tier / is_personal_data / is_irreversible / requires_auth / dpa_reference)". CC-restored 의 정상 permissions 인프라 (PermissionMode / PermissionRule / classifierShared / yoloClassifier 등) 와 Spec 035 의 receipt UX schema 는 별개. v1 가 그 구별 부재로 UMMAYA-needed 를 포함했고, v2 가 caller-graph 박제로 정정.

**Alternatives**:
- (rejected) v1 의 보수적 일괄 deletion: UMMAYA TUI 의 11+14 importer 광범위 회귀.
...
## R-4 — 6 UMMAYA-only Tool 시민 use case 평가 (v1 유지)
...
**Decision matrix** (v1 그대로):
| Tool | claude-code original purpose | UMMAYA 시민 use case 평가 |
|---|---|---|
| MonitorTool | 백그라운드 작업 모니터링 | DELETE — 시민이 모니터링할 백그라운드 작업 없음 (UMMAYA는 동기 lookup/submit 흐름) |
...
| WorkflowTool | 다단계 워크플로 도구 | DELETE — UMMAYA multi-step submit 시나리오는 system prompt + primitive chain (Spec 022) 으로 처리, 별도 도구 불필요 |
...
**Rationale**: UMMAYA bun test 는 약 983 pass / 1 fail (snapshot) 이 baseline. cleanup 으로 테스트 코드도 같이 삭제되면 일부 failure 가 자연 사라질 수도 있고 (good), pass 수가 줄어들 수도 있음 (also acceptable, "no NEW failure" gate 는 통과).
...
caller-graph + disposition 박제로 v1 의 결함 (UMMAYA-needed file 까지 deletion 포함) 차단. Constitution II 의 "UMMAYA-invented 권한 분류" 금지는 Spec 1633 closure 토큰 list 와 분리되어 별도 audit 영역으로 명시.
