...
Called from ``ummaya.tools.executor`` immediately after the adapter returns its
raw output and immediately before ``normalize()`` (FR-006, FR-013).

Ordering (FR-013):
  1. Run :func:`ummaya.safety._injection.run_detector` on the serialised
  ...
  2. Else, if ``SafetySettings.redact_tool_output`` is true, walk the dict
     leaves and pass each string through :func:`ummaya.safety._redactor.run_redactor`.
...
Defense-in-depth note (T037, commit 50e2c17):
  The per-file log redactions in ``ummaya.llm.client`` (``reasoning_content``
  length-only logging; ``tool_call_delta.arguments`` metadata-only logging)
  and ``ummaya.tools.executor`` (``raw_args_len``-only logging on validation
