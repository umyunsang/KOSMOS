...
Every issuance + use + revocation appends exactly one JSONL line to `~/.kosax/memdir/user/consent/<YYYY-MM-DD>.jsonl`. The append path is unchanged from Spec 035 (`open(path, "a")` + `json.dumps()` + fsync); only the new event-kind union members are added.
