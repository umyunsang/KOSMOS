...
- Worktree: `/Users/um-yunsang/UMMAYA-w-2293/` on branch `2293-ui-residue-cleanup`
- 시작점: `2f9663d` (Sonnet 1차 안전 commit — 6 UMMAYA-only Tool deletion + 5 callsite cleanup + claude.ts modify 일부)
- `cd tui && bun install` 완료
- 신규 dependency 0 (FR-008)

```bash
cd /Users/um-yunsang/UMMAYA-w-2293
...
검증:
- 30 row × { ummaya_path, importer_count, importers, internal_anthropic_tokens, dependency_hits }
...
```bash
cd /Users/um-yunsang/UMMAYA-w-2293/tui
...
작업 없음. caller-graph.json 의 11 + 14 importer 가 UMMAYA-needed 박제. decision-log.md § Cleanup Targets 표에 KEEP 행 추가:

```markdown
| utils/permissions/permissionSetup.ts | 11 (main.tsx, REPL, Config, plan.tsx, etc.) | KEEP | UMMAYA-needed CC permissions setup; Anthropic-token = 0; in-file `feature()` cleanup deferred to follow-up | caller-graph.json #25, disposition.json |
| utils/permissions/permissions.ts | 14 (tools.ts, AgentTool, BashTool, REPL, etc.) | KEEP | UMMAYA-needed CC permissions core; only sdk-compat APIUserAbortError import + 7 feature() removable | caller-graph.json #26, disposition.json |
```
...
## 5. R5 단계 — 6 UMMAYA-only Tool 결정 박제 (US3 보조)

이미 sonnet 1차 commit `2f9663d` 에서 6 디렉토리 deletion 완료. decision-log.md § UMMAYA-only Tool Decisions 섹션은 v1 의 6 entry 그대로 유지 (research.md § R-4 인용).
...
feat(2293): ummaya-original ui residue cleanup (v2 caller-graph based)
...
- 3 UMMAYA-needed files KEPT (caller-graph 검증 박제)
...
- 6 UMMAYA-only Tool deletion = sonnet 1차 commit 2f9663d 에서 이미 완료
- v2 spec / quickstart / decision-log / data-model / tasks 전면 재작성
  (v1 의 30 file 일괄 deletion 가설이 caller-graph 미검토라 UMMAYA-needed file 까지
   광범위 회귀 위험이었던 결함 보정)
...
```bash
gh pr create --title "feat(2293): ummaya-original ui residue cleanup (v2 caller-graph based)" --body "$(cat <<'EOF'
## Summary
- 28 Anthropic dispatcher residue files deleted (caller-graph + disposition.json 기반)
- 3 UMMAYA-needed files kept (permissionSetup / permissions / ui-l2/permission) — caller graph 검증 박제
- 6 UMMAYA-only Tool deletion 은 sonnet 1차 commit `2f9663d` 에서 완료
...
```bash
gh api repos/umyunsang/UMMAYA/pulls/<PR#>/comments \
