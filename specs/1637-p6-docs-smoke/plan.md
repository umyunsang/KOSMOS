...
**Target Platform**: KOSAX developer terminal (Bun + uv) and KOSAX citizen terminal (Bun-built TUI). PDF inline-render path conditional on Kitty / iTerm2 graphics protocol.
**Project Type**: monorepo — Python backend (`src/kosax/**`), TypeScript TUI (`tui/**`), shared docs (`docs/**`), shared scripts (`scripts/**`). Single repo, no microservices.
...
| V. Policy Alignment | PASS | Adapter spec seven-field template explicitly carries permission-tier rationale citing Spec 033 (PIPA permission gauntlet). KOSAX v0.1-alpha CHANGELOG references AI Action Plan Principles 8/9/5 alignment as the migration outcome. |
...
src/kosax/                      # Read-only at this Epic — schema source of truth
└── tools/
