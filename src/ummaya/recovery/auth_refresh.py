...
- Credential resolution is centralised in :mod:`ummaya.permissions.credentials`
  ...
  ``address_to_grid``) correctly discover ``UMMAYA_KAKAO_API_KEY`` here,
  and data.go.kr-backed tools discover ``UMMAYA_DATA_GO_KR_API_KEY``.
"""
...
from ummaya.permissions.credentials import candidate_env_vars, resolve_credential
...
async def attempt_auth_refresh(tool_id: str) -> bool:
    ...
    :func:`ummaya.permissions.credentials.resolve_credential`, which tries
