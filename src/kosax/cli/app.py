...
"""KOSAX CLI entry point — initialises the backend stack and launches the REPL."""
...
from kosax.config.guard import verify_startup
from kosax.observability import setup_tracing

logger = logging.getLogger(__name__)

# KOSAX version string
__version__ = "0.1.0"

# Typer application
_app = typer.Typer(
    name="kosax",
    help="KOSAX — Korean Public API Conversational Platform",
    add_completion=False,
)

# Sub-application: `kosax session …`
...
def _version_callback(value: bool) -> None:
    if value:
        typer.echo(f"kosax {__version__}")
        raise typer.Exit()
...
def _cli_command(
    ...
    """Launch the KOSAX interactive CLI."""
    if debug:
...
def _run_ipc_stdio() -> None:
    ...
    ``Bun.spawn(['uv', 'run', 'kosax', '--ipc', 'stdio'])``.
    ...
    """
    from kosax.ipc.stdio import run as _ipc_run  # noqa: PLC0415
...
def _run_list_sessions() -> None:
    ...
    from kosax.session.store import list_sessions  # noqa: PLC0415
...
def _run_repl(resume_session_id: str | None = None) -> None:
    ...
    from kosax.context.builder import ContextBuilder  # noqa: PLC0415
    from kosax.engine.engine import QueryEngine  # noqa: PLC0415
    from kosax.llm.client import LLMClient  # noqa: PLC0415
    from kosax.llm.errors import ConfigurationError  # noqa: PLC0415
    from kosax.observability import MetricsCollector, ObservabilityEventLogger  # noqa: PLC0415
    from kosax.permissions.models import SessionContext  # noqa: PLC0415
    from kosax.recovery.executor import RecoveryExecutor  # noqa: PLC0415
    ...
    try:
        ...
        _stderr_console.print(
            ...
            "[dim]KOSAX_FRIENDLI_TOKEN 환경 변수가 설정되어 있는지 확인하세요.[/dim]"
        )
        sys.exit(1)
    except Exception as exc:  # noqa: BLE001
...
def _gc_stubs_command(
    ...
    session_dir: Annotated[
        ...
        typer.Option(
            ...
            help=(
                ...
                "Defaults to KOSAX_MEMDIR_USER/sessions or ~/.kosax/memdir/user/sessions."
            ),
            hidden=True,
        ),
    ] = None,
    ...
    version of KOSAX at IPC boot before lazy session creation was introduced.
    ...
    Example::
        ...
        kosax session gc-stubs --dry-run
        kosax session gc-stubs --no-dry-run --older-than 7
    """
    import asyncio as _asyncio  # noqa: PLC0415
    from pathlib import Path  # noqa: PLC0415

    from kosax.session.store import gc_empty_stubs  # noqa: PLC0415
...
def main() -> None:
    ...
    load_repo_dotenv()
    verify_startup()  # fail-fast guard — exits 78 if required KOSAX_* vars missing
    ...
    from kosax.permissions.hmac_key import bootstrap_hmac_key  # noqa: PLC0415
