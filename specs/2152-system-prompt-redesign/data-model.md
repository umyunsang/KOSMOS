...
All types are Pydantic v2 frozen models (constitution Principle III; `Any` forbidden in I/O schemas). Types live in `src/ummaya/llm/prompt_assembler.py` unless noted.
...
| `prefix_hash` | `str` | SHA-256 of `static_prefix.encode("utf-8")` rendered as 64 lowercase hex chars. Emitted as the OTEL `ummaya.prompt.hash` attribute. |
...
Additive Pydantic field on the existing tool-adapter schema. Lives in `src/ummaya/tools/_base.py` (existing module, additive change).
...
A pure formatting helper (not a Pydantic model — it returns `str` directly). Lives in `src/ummaya/ipc/citizen_request.py` (new file, single function).
...
PromptAssembler(static_prefix, registry) ──build(ctx)──▶ SystemPromptManifest
                                                              ...
                                                              ├── prefix_hash (ummaya.prompt.hash)
