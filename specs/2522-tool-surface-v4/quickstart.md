...
- KOSMOS worktree: `/Users/um-yunsang/KOSMOS-w-2522/` (이미 생성됨)
- `KOSMOS_DATA_GO_KR_API_KEY` (KMA / HIRA / NMC / NFA / MOHW / KOROAD 통합 키, `.env`)
- `KOSMOS_KAKAO_API_KEY` (resolve_location 의 Kakao 백엔드)
- `KOSMOS_FRIENDLI_TOKEN` (K-EXAONE on FriendliAI tier 1)
- `uv sync --frozen` 실행됨
...
```bash
# pytest live
cd /Users/um-yunsang/KOSMOS-w-2522
...
- KOSMOS 가 cross-domain chain 강제 안 함 (LLM 자율 결정)
