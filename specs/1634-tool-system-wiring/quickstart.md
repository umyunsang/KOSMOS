...
> Audience: contributor adding a new Korean public-service API adapter to KOSAX *after* Epic #1634 (P3) ships. This walkthrough is the SC-006 measurement target — a fresh contributor must reach a working adapter without editing any central registry, TUI tool wrapper, or routing module.

## 1. Prerequisites

- KOSAX checked out, `uv sync` complete, `uv run pytest` passes from a clean main.
- Decision made: which Korean ministry/agency owns the API. Map to the `Ministry` enum in `src/kosax/tools/models.py`. If the institution is not in the enum, open a small PR to add it before continuing (do NOT use `OTHER` for new permanent adapters).
...
```bash
mkdir -p src/kosax/tools/<ministry_lower>/
touch src/kosax/tools/<ministry_lower>/<verb>_<resource>.py
```
...
Append one line to `src/kosax/tools/register_all.py`:

```python
from kosax.tools.<ministry_lower>.<verb>_<resource> import TOOL as <verb_resource>_tool
...
uv run pytest src/kosax/tools/<ministry_lower>/test_<verb>_<resource>.py -v
...
If your adapter does NOT appear in the search results: BM25 indexing happens at registration time — confirm `register_all.py` was modified and KOSAX was restarted.
...
| File | Reason |
|---|---|
| `src/kosax/tools/models.py` | GovAPITool schema. Edit only when adding a ministry to the enum (small dedicated PR). |
| `src/kosax/tools/registry.py` | Registry mechanics. Edit only via spec-driven epic. |
| `src/kosax/tools/routing_index.py` | Routing validation. Edit only via spec-driven epic. |
| `src/kosax/tools/permissions.py` | Permission tier helper. Edit only via spec-driven epic. |
| `src/kosax/ipc/mcp_server.py` | MCP server stub. Edit only via spec-driven epic. |
...
| Anything under `src/kosax/primitives/*.py` | Spec 031 primitive logic. Edit only via spec-driven epic. |
...
After P5 (Epic #1636) ships, contributors who are NOT KOSAX maintainers add adapters via the plugin system using the `plugin.<id>.<verb>` namespace. The internal walkthrough above remains valid for KOSAX-internal adapters; the plugin walkthrough will live in `docs/plugins/quickstart.md` after P5.
...
1. File lives under `src/kosax/tools/mock/<verb>_<resource>.py`.
