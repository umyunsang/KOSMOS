...
from ummaya.tools.nmc.freshness import FreshnessResult, check_freshness
...
class TestThresholdConfig:
    ...
    def test_default_threshold_from_settings(self, mock_dt: object) -> None:
        ...
        # ``from ummaya.settings import settings``, so we patch the singleton
        # on its home module (ummaya.settings.settings).
        with patch("ummaya.settings.settings") as mock_settings:
    ...
    def test_pydantic_validation_rejects_zero(self) -> None:
        """UmmayaSettings(nmc_freshness_minutes=0) must raise ValidationError (ge=1)."""
        from ummaya.settings import UmmayaSettings

        with pytest.raises(ValidationError):
            UmmayaSettings(nmc_freshness_minutes=0)

    def test_pydantic_validation_rejects_1441(self) -> None:
        """UmmayaSettings(nmc_freshness_minutes=1441) must raise ValidationError (le=1440)."""
        from ummaya.settings import UmmayaSettings

        with pytest.raises(ValidationError):
            UmmayaSettings(nmc_freshness_minutes=1441)
