...
CLI entry point::
    ...
    python -m kosax.eval.retrieval eval/retrieval_queries.yaml
...
def _backend_env_overlay(backend: str) -> Iterator[None]:
    """Context manager that overlays KOSAX_RETRIEVAL_BACKEND for the duration.
...
def _eager_cold_start_overlay() -> Iterator[None]:
    """Force ``KOSAX_RETRIEVAL_COLD_START=eager`` for the duration of a block.
    ...
    """
    key = "KOSAX_RETRIEVAL_COLD_START"
    previous = os.environ.get(key)
...
def _build_registry() -> tuple[object, object]:
    ...
    with _eager_cold_start_overlay():
        ...
        _try_register_adapter(
            "kosax.tools.koroad.accident_hazard_search",
            "register",
            registry,
            executor,
        ...
        _try_register_adapter(
            "kosax.tools.hira.hospital_search",
            "register",
            registry,
            executor,
            requires_executor=True,
        ...
        _try_register_adapter(
            "kosax.tools.nmc.emergency_search",
            "register",
            registry,
            executor,
            requires_executor=True,
...
def run_extended_gate(
    *,
    ...
    - Pluggable backend selection via ``KOSAX_RETRIEVAL_BACKEND`` env overlay.
    ...
    Args:
        backend: Retrieval backend to activate (``bm25``, ``dense``, ``hybrid``).
            When ``None`` (default), the ambient ``KOSAX_RETRIEVAL_BACKEND``
    ...
    # ``KOSAX_RETRIEVAL_BACKEND`` to bind a retriever). Scope it narrowly so
...
def main(argv: list[str] | None = None) -> None:
    ...
    if raw_args and raw_args[0].startswith("--"):
        ...
        parser.add_argument(
            ...
            help=(
                ...
                "Defaults to honouring KOSAX_RETRIEVAL_BACKEND "
        ...
        effective_backend = parsed.backend or os.environ.get("KOSAX_RETRIEVAL_BACKEND", "bm25")
        ...
        # ``KOSAX_RETRIEVAL_BACKEND`` env var rather than force-overlaying a
    ...
    if not raw_args:
        logger.error("Usage: python -m kosax.eval.retrieval <queries.yaml>")
        sys.exit(2)
