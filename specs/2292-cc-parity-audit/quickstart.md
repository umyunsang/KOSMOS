...
- repo root = `/Users/um-yunsang/KOSAX` 또는 동등.
- `tui/src/` (KOSAX 입력) 와 `.references/claude-code-sourcemap/restored-src/src/` (CC 입력) 양쪽 존재 확인:
  ```bash
...
**목표**: 1,531 byte-identical / 73 SDK-import-only-diff / 212 modified / 274 KOSAX-only / 68 CC-only 5 카테고리 list 를 deterministic 산출.
...
1. `find tui/src -type f \( -name '*.ts' -o -name '*.tsx' \)` → KOSAX 파일 set.
...
5. 5 카테고리 결과를 `data/enumerated-{keep-byte-identical|import-candidate|modified|kosax-only|cc-only}.txt` 에 박제.
...
- `data/spot-check-results.json` — 50 entry, 각 `{kosax_path, cc_source_path, kosax_sha256, cc_sha256, hash_match, sampling_seed: 2292, sampling_index: 0..49}`.
...
[research.md § R-4](./research.md#r-4--212-modified-파일-분류-휴리스틱-legitimate--cleanup-needed--suspicious) 의 휴리스틱 표 그대로 구현. KOSAX-known 잔재 path / 토큰 list (Spec 1633 영향 영역, `claude.ts`, `verifyApiKey`, `@anthropic-ai/`) 는 스크립트 상단 상수.
...
```bash
git -C /Users/um-yunsang/KOSAX status --short -- ':!specs/2292-cc-parity-audit'
