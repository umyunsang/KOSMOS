...
// SSE responses from the KOSAX fixture JSON files.
//
...
async function handleCompletions(req: Request): Promise<Response> {
  ...
  // Non-streaming path (rare for KOSAX but handle it)
  if (!stream) {
    if (!fixture) {
...
console.log("[aimock-bun] set KOSAX_FRIENDLI_BASE_URL=http://localhost:4010/v1 to use this server");
