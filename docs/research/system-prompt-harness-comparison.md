...
> **Authored**: 2026-04-28 · **Trigger**: User direction during Epic #2112 — investigate why K-EXAONE answered citizen queries directly without invoking `lookup` / KMA / HIRA tools, and whether KOSAX's system-prompt design matches CC's. Research insights drive the next Epic on system-prompt redesign.
>
...
**Insight for KOSAX**: The "official" Anthropic Python SDK pattern is just `system_prompt: str | None`. CC's section-based composition is the *implementation* not the *interface*.
...
**Insight for KOSAX**: Pattern of `str | Callable[..., str]` is the de facto standard. KOSAX's current `prompts/system_v1.md` is the static half; the dynamic half (lazy callable) is missing.
...
**Insight for KOSAX**: This is the cleanest *composable* pattern. KOSAX could expose:
```python
@kosax_agent.system_prompt
def inject_ministry_scope(ctx: KosaxContext) -> str:
...
**Insight for KOSAX**: Multi-agent coordination needs the `description` field — what one agent advertises to another. Spec 027 swarm should formalise this.
...
**Insight for KOSAX**: LangGraph's "callable receives full state" model is the most powerful. KOSAX's IPC layer already transmits full ChatRequestFrame state — a Python-side prompt assembler can take it and emit the full LLM message stack.
...
**Insight for KOSAX**: Simplest TS pattern — matches KOSAX's current `prompts/system_v1.md` baseline.
...
## 9. KOSAX current state (`prompts/system_v1.md`)
...
1. **CC dev-context bleed**: The TUI's `getSystemContext()` (cwd + git + claude.md) is still being injected somewhere — K-EXAONE's reply mentioned "현재 `/Users/um-yunsang/KOSAX/tui` 디렉토리에서 작업 중이며…", treating the user as a developer.
2. **Generic tool-use prompt**: KOSAX prompt says "Use available tools when… requires live data lookup" — *non-specific*. K-EXAONE Opus-class models (4.6+) are documented to require *clear, concrete* tool-use examples to fire (Anthropic best-practices §8.4).
...
```markdown
<role>
You are KOSAX — a citizen-facing AI assistant for Korean public services.
...
Introduce `kosax.llm.prompt_assembler.assemble(ctx)` that builds:
...
Audit the TUI's `getSystemContext()` injection path. CC's git/cwd context is **dead under KOSAX** (citizen ≠ developer) — should be removed entirely from the chat_request flow, not just the prompt. This belongs to the same Epic.
...
| 11 | `prompts/system_v1.md` (KOSAX current) | KOSAX canonical |
| 12 | `src/kosax/llm/system_prompt_builder.py` | KOSAX canonical |
...
```
- [System prompt design references](docs/research/system-prompt-harness-comparison.md) — CC has 7 static + 12 dynamic sections, boundary marker for prompt cache; KOSAX uses single 5-paragraph file. R1-R6 actions queued for next Epic.
```
