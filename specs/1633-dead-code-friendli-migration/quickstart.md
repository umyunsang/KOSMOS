# Quickstart — Citizen's first EXAONE response
...
# export KOSMOS_FRIENDLI_MODEL="LGAI-EXAONE/EXAONE-236B-A23B"
...
# expected: LGAI-EXAONE/EXAONE-236B-A23B
```
...
5. **First EXAONE token appears within 5 seconds** (SC-001).
...
- Python backend emits OTEL `gen_ai.client.invoke` span with `gen_ai.system=friendli_exaone`, `gen_ai.request.model=LGAI-EXAONE/EXAONE-236B-A23B`, `kosmos.prompt.hash=<64-char hex>`.
...
grep -n "LGAI-EXAONE/EXAONE-236B-A23B" tui/src/utils/model/model.ts
...
| `ErrorFrame(class=llm, code=not_found)` | model ID wrong | ensure `KOSMOS_FRIENDLI_MODEL` is `LGAI-EXAONE/EXAONE-236B-A23B` or unset (use default) |
