...
**Status**: Accepted
**Date**: 2026-04-14
**Epic**: #288 (KOSAX Geocoding Adapter)

---

## Context

KOSAX tools (KOROAD accident search, KMA weather APIs) require precise Korean
...
2. **Single API key**: Kakao Local API is authenticated via a single REST API
   key (`KOSAX_KAKAO_API_KEY`), fitting the KOSAX `KOSAX_` env-var convention.
   This is independent of the `KOSAX_DATA_GO_KR_API_KEY` used by KOROAD/KMA.
...
**Negative / Trade-offs**:
- Adds a new dependency on a commercial API (`KOSAX_KAKAO_API_KEY`).  This key
  must be managed separately from `KOSAX_DATA_GO_KR_API_KEY`.
...
- **data.go.kr road address API**: Shares the `KOSAX_DATA_GO_KR_API_KEY` quota
...
- `src/kosax/tools/koroad/code_tables.py` — SidoCode, GugunCode enums
- `src/kosax/tools/kma/grid_coords.py` — REGION_TO_GRID, latlon_to_grid
