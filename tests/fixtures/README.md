...
This directory contains recorded HTTP response fixtures used by the KOSAX tool
...
async def test_record_koroad_fixture():
    ...
    import os
    api_key = os.environ["KOSAX_KOROAD_API_KEY"]
...
```bash
KOSAX_KOROAD_API_KEY=... KOSAX_DATA_GO_KR_API_KEY=... uv run pytest -m live -v
```
