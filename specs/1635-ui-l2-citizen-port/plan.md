...
**Storage**: User-tier memdir under `~/.kosax/memdir/user/` (existing infrastructure from Spec 027 and Spec 035). New paths added by this epic: `~/.kosax/memdir/user/onboarding/state.json` (resumable step state, FR-002), `~/.kosax/memdir/user/preferences/a11y.json` (accessibility toggles, FR-005). Permission receipts continue to live in `~/.kosax/memdir/user/consent/` (Spec 035), append-only.
