...
describe('ConfigOverlay — UMMAYA_CONFIG_CATALOG (FR-030)', () => {
  it('catalog contains no secret entries', () => {
    const secrets = UMMAYA_CONFIG_CATALOG.filter((e) => e.isSecret);
    expect(secrets).toHaveLength(0);
  });

  it('catalog contains at least one non-secret entry', () => {
    const nonSecrets = UMMAYA_CONFIG_CATALOG.filter((e) => !e.isSecret);
    expect(nonSecrets.length).toBeGreaterThan(0);
  });

  it('all entries have both ko and en labels', () => {
    for (const entry of UMMAYA_CONFIG_CATALOG) {
  ...
  it('does not expose FriendliAI API keys through /config', () => {
    const entry = UMMAYA_CONFIG_CATALOG.find((e) => e.key.includes('FRIENDLI'));
