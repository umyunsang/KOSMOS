...
No UMMAYA_NMC_* env vars are required because the gate fires before any network IO.
"""
...
from ummaya.tools.models import LookupError, LookupFetchInput  # noqa: A004
from ummaya.tools.nmc.emergency_search import register
...
def nmc_executor(nmc_registry: ToolRegistry) -> ToolExecutor:
    ...
    from ummaya.tools.nmc.emergency_search import NmcEmergencySearchInput, handle
...
class TestNmcAuthGate:
    ...
    async def test_fetch_returns_auth_required_error(self, nmc_reg_exec):
        ...
        inp = LookupFetchInput(
            ...
            params={"mode": "coordinate", "lat": 37.5, "lon": 127.0, "limit": 5},
        )
    ...
    async def test_respx_call_count_zero_overall(self, nmc_reg_exec):
        ...
        inp = LookupFetchInput(
            mode="fetch",
            tool_id="nmc_emergency_search",
            params={"mode": "coordinate", "lat": 35.1, "lon": 129.0, "limit": 10},
        )
        await lookup(inp, executor=executor)
    ...
    async def test_auth_gate_fires_regardless_of_params(self, nmc_reg_exec):
        ...
        test_cases = [
            {"mode": "coordinate", "lat": 0.0, "lon": 0.0, "limit": 1},
            {"mode": "coordinate", "lat": -90.0, "lon": -180.0, "limit": 100},
            {"mode": "coordinate", "lat": 90.0, "lon": 180.0, "limit": 50},
            {"mode": "coordinate", "lat": 37.5665, "lon": 126.9780, "limit": 3},
        ]
