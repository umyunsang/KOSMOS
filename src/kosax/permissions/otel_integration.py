...
Attribute namespace (Spec 021 kosax.* convention):
    ``kosax.permission.mode``           — the PermissionMode string
    ``kosax.permission.decision``       — "granted" or "denied"
    ``kosax.consent.receipt_id``        — consent_receipt_id (when available)
...
Reference:
    ...
    src/kosax/observability/semconv.py (KOSAX attribute namespace)
    src/kosax/permissions/models.py ConsentDecision, ToolPermissionContext
"""
...
from kosax.permissions.models import ConsentDecision, ToolPermissionContext

__all__ = [
    "KOSAX_PERMISSION_MODE",
    "KOSAX_PERMISSION_DECISION",
    "KOSAX_CONSENT_RECEIPT_ID",
...
# ---------------------------------------------------------------------------
# Attribute-name constants (kosax.permission.* namespace — Spec 033 FR-F03)
# ---------------------------------------------------------------------------

KOSAX_PERMISSION_MODE: str = "kosax.permission.mode"
...
KOSAX_PERMISSION_DECISION: str = "kosax.permission.decision"
...
KOSAX_CONSENT_RECEIPT_ID: str = "kosax.consent.receipt_id"
...
def enrich_tool_call_span(
    ...
    Attributes set:
    - ``kosax.permission.mode``         — always set from ``ctx.mode``
    - ``kosax.permission.decision``     — "granted" / "denied" / "not_required"
    - ``kosax.consent.receipt_id``      — set only when consent is granted
    ...
    # FR-F03 — kosax.permission.mode (always emitted)
    span.set_attribute(KOSAX_PERMISSION_MODE, ctx.mode)

    # FR-F03 — kosax.permission.decision
    if consent is None:
    ...
    span.set_attribute(KOSAX_PERMISSION_DECISION, decision_value)

    # FR-F03 — kosax.consent.receipt_id (only when consent was actively granted)
    if consent is not None and consent.granted:
        receipt_id = consent.action_digest
        if receipt_id:
            span.set_attribute(KOSAX_CONSENT_RECEIPT_ID, receipt_id)
        else:
            _logger.warning(
                "otel_integration.missing_receipt_id: tool_id=%s "
                "consent.granted=True but action_digest is empty; "
                "kosax.consent.receipt_id not set on span.",
                ctx.tool_id,
            )
