...
/**
 * KOSAX-original ThemeProvider.
 *
 * Reads KOSAX_TUI_THEME at startup and exposes the resolved ThemeToken
 ...
 * FR-041 (KOSAX_TUI_THEME env var).
 */
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  const name = process.env['KOSAX_TUI_THEME'] ?? 'default'
  const theme = THEMES[name] ?? defaultTheme
  if (THEMES[name] === undefined && name !== 'default') {
    process.stderr.write(
      `[KOSAX-TUI] Unknown theme "${name}", falling back to default\n`,
    )
  }
