...
"""Theme system for the UMMAYA CLI.
...
and a :func:`load_theme` loader that respects the ``UMMAYA_THEME`` environment
variable with a ``UMMAYA_CLI_`` prefix for consistency.

Usage::

    from ummaya.cli.themes import load_theme
...
class Theme(BaseModel):
    """Colour/style definitions for the UMMAYA CLI renderer.
...
def load_theme() -> Theme:
    ...
    Reads ``UMMAYA_THEME`` (or its alias ``UMMAYA_CLI_THEME``) from the
    ...
    """
    raw = os.environ.get("UMMAYA_THEME") or os.environ.get("UMMAYA_CLI_THEME", "")
    name = raw.strip().lower() or _DEFAULT_THEME_NAME
    if name not in _THEMES:
        logger.warning(
            "Unknown UMMAYA_THEME %r — falling back to %r. Valid choices: %s",
            name,
            _DEFAULT_THEME_NAME,
            ", ".join(_THEMES),
        )
