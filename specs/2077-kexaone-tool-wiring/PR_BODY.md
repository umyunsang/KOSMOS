...
Restores the Claude Code "tool inventory + agentic loop" pattern so K-EXAONE invokes only UMMAYA-registered tools (`lookup` / `resolve_location` / `submit` / `subscribe` / `verify` + MVP-7 auxiliary), eliminating training-data hallucinations of CC tools (`Read` / `Glob` / `Bash` / etc.).
...
2. **Backend** (`src/ummaya/llm/system_prompt_builder.py` + `src/ummaya/ipc/stdio.py`) — appends a `## Available tools` section to the system prompt, falls back to `ToolRegistry.export_core_tools_openai()` when the frame omits tools, and migrates the hardcoded primitive whitelist to a `ummaya.primitives.PRIMITIVE_REGISTRY` single source of truth.
...
Every change is migrated from `src/ummaya/llm/_cc_reference/` (CC 2.1.88 research-use mirror, Constitution §I) — no new abstractions, **zero new runtime dependencies** (SC-006 verified, see `specs/2077-kexaone-tool-wiring/sc006-evidence.txt`).

## What changed

### Backend Python

- `src/ummaya/llm/_cc_reference/` — 9 new cp files (api.ts, tools.ts, prompts.ts, query.ts, toolOrchestration.ts, toolExecution.ts, messages.ts, permissions.ts, toolResultStorage.ts) + index `README.md`. All 13 cp files carry a research-use header per Constitution §I (4 retro-fitted from `fdfd3e9`).
- `src/ummaya/llm/system_prompt_builder.py` — new module exporting `build_system_prompt_with_tools(base, tools)`. Byte-stable composition (sort_keys, ensure_ascii=False) for Spec 026 prompt-hash invariant.
- `src/ummaya/ipc/stdio.py` — module-level `_TOOL_REGISTRY` cache + `_ensure_tool_registry()`, registry fallback when `frame.tools=[]`, `build_system_prompt_with_tools` wired into `_handle_chat_request`, `_PERMISSION_GATED_PRIMITIVES` migrated to `from ummaya.primitives import GATED_PRIMITIVES`. Pre-existing schema bug fixed: `ErrorFrame(role="llm")` → `role="backend"` (rejected by E3 role-kind allow-list).
- `src/ummaya/primitives/__init__.py` — adds `PRIMITIVE_REGISTRY` + `GATED_PRIMITIVES` constants (kept out of `__all__` to preserve Spec 031 SC-001 5-primitive surface invariant).
...
- `tui/src/store/pendingPermissionSlot.ts` (new) + `tui/src/store/session-store.ts` — Promise-based pending permission slot with FIFO queue + 5-minute timeout (`UMMAYA_PERMISSION_TIMEOUT_SEC`). `_resetPermissionSlotForTest` exported for test isolation.
- `tui/src/screens/REPL.tsx` — `UmmayaActivePermissionGate` component subscribes to `getActivePermission()` selector and bridges `onGrant`/`onDeny` to `resolvePermissionDecision`.
...
- `tests/ipc/test_stdio.py` (new, 4 pass + 1 xfail) — fallback / system-prompt inject / max-turns / OTEL preservation. Includes module-scoped `_restore_llmclient_pydantic_validators_after_module` fixture that reloads the engine chain (`ummaya.engine.{models,engine,query}`) to discard the Pydantic validator capture left over from `LLMClient` monkeypatch.
...
| FR-003 — Single-source registry | ✓ | `ummaya.primitives.PRIMITIVE_REGISTRY` is the only enumeration; `_PERMISSION_GATED_PRIMITIVES` reads from `GATED_PRIMITIVES` |
...
- 9 new CC reference cp files + README index in `src/ummaya/llm/_cc_reference/`
- 4 retro-fitted research-use headers on existing `_cc_reference/` files
- 1 new Python module: `src/ummaya/llm/system_prompt_builder.py`
- 3 modified Python modules: `src/ummaya/ipc/stdio.py`, `src/ummaya/primitives/__init__.py`, plus 1 ErrorFrame role fix
