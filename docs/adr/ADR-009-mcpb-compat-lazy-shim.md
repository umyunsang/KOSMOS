# ADR-009: `mcpb-compat.ts` lazy-load shim (KOSAX-original)
...
Most KOSAX sessions never process a `.dxt` (MCP-bundle) file — citizen
...
typical session shape. KOSAX's citizen workflow shape is different —
...
1. **CC parity discipline**: every TS file that diverges from CC must
   ...
   — it is a KOSAX-original performance optimisation. AGENTS.md
...
> `mcpb-compat.ts` — KOSAX-original 혁신, ADR 등록 권고
> Epic #2293 FR-010 의 mcpb v3 lazy-load shim 은 CC 대응 없음. swap 1/2
> 가 아닌 성능 최적화(700KB heap 회피) → KOSAX-original 혁신으로 분류.
...
`@anthropic-ai/mcpb` is published by Anthropic; KOSAX cannot patch the
zod-v3 import behaviour without forking the package. A 26-line shim
inside KOSAX is the smallest possible carrier of the optimisation.
...
- Future agents need to read this ADR to understand why the shim exists.
  Mitigated by the in-file `// KOSAX-original — Epic #2293 FR-010`
...
(Spec 1636); reimplementing it inside KOSAX would regress security.
