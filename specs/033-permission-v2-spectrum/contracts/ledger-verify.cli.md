# Contract — `kosax permissions verify` CLI

**Feature**: 033-permission-v2-spectrum
**Artifact**: Behavioral contract for `src/kosax/permissions/cli.py::verify` command
**Date**: 2026-04-20

> Verifies the hash chain + HMAC seal of `~/.kosax/consent_ledger.jsonl`. Fail-closed on any integrity error. Designed so an external auditor can run it without KOSAX runtime dependencies beyond the Python stdlib + HMAC key file.

## 1. Invocation

```
kosax permissions verify [--path PATH] [--hash-only] [--acknowledge-key-loss] [--json]
```

| Flag | Default | Purpose |
|------|---------|---------|
| `--path PATH` | `~/.kosax/consent_ledger.jsonl` | Override ledger location (test / audit). |
...
| `3` | HMAC key unknown. | Record `hmac_key_id` not in key registry at `~/.kosax/keys/`. |
| `4` | Schema violation. | Record fails Pydantic validation against `ConsentLedgerRecord`. |
| `5` | File not found or empty. | `~/.kosax/consent_ledger.jsonl` missing or 0 bytes. |
| `6` | HMAC key file mode mismatch. | `~/.kosax/keys/ledger.key` mode != `0400`. Fail-closed load refusal (Invariant C3). |
...
✓ Ledger verification PASSED
  Path: /Users/alice/.kosax/consent_ledger.jsonl
  Records: 42
  Chain: OK
...
| Insert a forged record with valid HMAC (attacker has key). | Chain mismatch → exit 1. HMAC key compromise requires `kosax permissions rotate-key` + forensic review (out-of-band). |
...
- Portability: Runnable via `uv run python -m kosax.permissions.cli verify` with ONLY the `kosax/permissions/` package vendored.
...
- [ ] CLI wired via entry point `kosax permissions verify` (no new dependency).
