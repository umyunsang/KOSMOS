...
"""Live E2E validation tests for the full KOSAX Scenario 1 pipeline.
...
Required environment variables (validated by conftest fixtures):
    ...
    KOSAX_DATA_GO_KR_API_KEY — data.go.kr public data portal key
    KOSAX_DATA_GO_KR_API_KEY — data.go.kr public data portal key (shared by KMA + KOROAD)
"""
...
def _build_live_engine() -> tuple[QueryEngine, LLMClient]:
    ...
    Reads KOSAX_FRIENDLI_TOKEN from the environment (already validated by the
...
# Source: src/kosax/tools/koroad/code_tables.py — SidoCode.SEOUL / GugunCode.SEOUL_GANGNAM.
...
async def test_live_scenario1_from_natural_address(
    ...
    """
    event_logger = ObservabilityEventLogger()
    log_target = logging.getLogger("kosax.events")
