...
**Epic**: [#1632](https://github.com/umyunsang/UMMAYA/issues/1632)
...
- [ ] T004 [P] Flatten `tui/src/constants/constants/` into `tui/src/constants/` via `git mv` (FR-007):
  ...
  - **충돌 3 개** (`figures.ts` · `messages.ts` · `xml.ts`) → 기존 상위 파일(7 줄 UMMAYA stub) **유지**, 하위 CC 원본 **삭제**
...
- [ ] T011 [US2] Run `bun test` and verify `Passed ≥ 540` (SC-003, FR-006):
  ...
  - 명령: `cd tui && bun test 2>&1 | tee /tmp/ummaya-p0-bun-test.log`
