...
|------|-----------|----------|-----------|
| services/api/adminRequests.ts | — | DELETE | Dead — no KOSAX citizen use case |
...
| services/api/filesApi.ts | — | DELETE | Claude Files API — no KOSAX equivalent |
| services/api/firstTokenDate.ts | — | DELETE | Dead analytics |
| services/api/grove.ts | — | DELETE | Anthropic internal service — dead |
| services/api/logging.ts | — | DELETE | Anthropic telemetry — KOSAX uses OTEL |
...
| services/api/withRetry.ts | — | DELETE | Anthropic retry logic — KOSAX has its own |
...
|----------|------|----------|-----------|
| cli/print.ts | verifyApiKey + queryHaiku | DELETE | KOSAX = FriendliAI single provider; API key verification is different |
...
| utils/mcp/dateTimeParser.ts | queryHaiku | DELETE | KOSAX DateParser (MVP7) replaces |
...
## § 3 KOSAX-only Tool Decisions

| Tool | Decision | Rationale | Reference |
|------|----------|-----------|-----------|
| MonitorTool | DELETE | No background tasks for citizens in KOSAX synchronous lookup/submit flow | research.md § R-4 |
...
| WorkflowTool | DELETE | KOSAX multi-step handled by system prompt + 4 primitive chain (Spec 022) | research.md § R-4 |
...
|------|---------|----------|-----------|
| schemas/ui-l2/permission.ts | — | DELETE (pending caller check) | Constitution II NON-NEGOTIABLE — KOSAX-invented permission enums must not remain in codebase |
