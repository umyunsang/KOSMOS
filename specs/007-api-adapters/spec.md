...
Scenario 1 is the primary acceptance test for Phase 1 of UMMAYA:

> Citizen: "오늘 서울 가는 길 안전해?" (Is the road to Seoul safe today?)
> UMMAYA: fuses KOROAD accident data + KMA weather alerts + road risk index
...
A citizen asks "서울 강남구 사고 많은 곳 알려줘" (Tell me the accident-prone areas in Gangnam-gu, Seoul). UMMAYA calls `koroad_accident_search` with `sido=11` (Seoul) and `gugun=680` (Gangnam-gu) and returns a list of accident hotspot locations with coordinates, accident counts, casualties, and road geometry.
...
A citizen asks "오늘 경부고속도로 가는데 기상 경보 있어?" (Any weather warnings on the Gyeongbu Expressway today?). UMMAYA calls `kma_weather_alert_status` to retrieve all currently active weather warnings. The system then filters by relevant `areaCode` values for the route.
...
A citizen provides a location (e.g., 서울 서초구). UMMAYA resolves it to KMA grid coordinates (nx, ny) and calls `kma_current_observation` to get current precipitation, temperature, and wind speed.
...
1. **Given** no `UMMAYA_KOROAD_API_KEY` or `UMMAYA_DATA_GO_KR_KEY` env vars, **When** `uv run pytest tests/tools/` is run, **Then** all non-`@pytest.mark.live` tests pass using recorded JSON fixtures.
...
- **Module path**: `src/ummaya/tools/koroad/koroad_accident_search.py`
- **Endpoint**: `http://apis.data.go.kr/B552061/frequentzoneLg/getRestFrequentzoneLg`
- **HTTP method**: GET
- **Auth**: `serviceKey` query parameter (URL-encoded); sourced from `UMMAYA_KOROAD_API_KEY`
...
- **Module path**: `src/ummaya/tools/kma/kma_weather_alert_status.py`
- **Endpoint**: `http://apis.data.go.kr/1360000/WthrWrnInfoService/getPwnStatus`
- **HTTP method**: GET
- **Auth**: `serviceKey` query parameter; sourced from `UMMAYA_DATA_GO_KR_KEY`
...
- **Module path**: `src/ummaya/tools/kma/kma_current_observation.py`
...
- **Module path**: `src/ummaya/tools/composite/road_risk_score.py`
...
- **Module path**: `src/ummaya/tools/koroad/code_tables.py`
...
- **Module path**: `src/ummaya/tools/kma/grid_coords.py`
...
- `UMMAYA_KOROAD_API_KEY` — for KOROAD `B552061/frequentzoneLg/` endpoints
- `UMMAYA_DATA_GO_KR_KEY` — for KMA `1360000/WthrWrnInfoService/` and `1360000/VilageFcstInfoService_2.0/` endpoints
...
| SC-009 | `uv run pytest tests/` passes with zero live API calls (no `UMMAYA_*_KEY` env vars set) | CI green badge |
...
| `UMMAYA_KOROAD_API_KEY` env var | Not in repo | Developer must export before recording fixtures; never committed |
| `UMMAYA_DATA_GO_KR_KEY` env var | Not in repo | Developer must export before recording fixtures; never committed |
