...
lookup, verify, submit. Subscribe is deferred until KOSAX has a real
...
class _LookupInputForLLM(BaseModel):
    ...
    3. ``LookupSearchInput`` / ``LookupSearchResults`` survive as
       ...
       (``kosax.eval.retrieval``); they are NOT exposed to the LLM.
    """
...
RESOLVE_LOCATION_TOOL = GovAPITool(
    ...
    policy=AdapterRealDomainPolicy(
        ...
        real_classification_text=(
            "공공데이터포털 개인정보처리방침 (KOSAX 내부 geocoding 표면 — 시민 PII 미포함)"
        ),
...
LOOKUP_SEARCH_TOOL = GovAPITool(
    ...
    policy=AdapterRealDomainPolicy(
        real_classification_url="https://www.data.go.kr/policy/privacyPolicy.do",
        real_classification_text=(
            "공공데이터포털 개인정보처리방침 (KOSAX 내부 lookup 메타-표면 — 시민 PII 미포함)"
        ),
        citizen_facing_gate="read-only",
        last_verified=datetime(2026, 4, 29, tzinfo=UTC),
    ),
...
class _SubmitInputForLLM(BaseModel):
    ...
    Mirrors :class:`kosax.primitives.submit.SubmitInput` but lives here so the
...
VERIFY_TOOL = GovAPITool(
    ...
    policy=AdapterRealDomainPolicy(
        real_classification_url="https://www.data.go.kr/policy/privacyPolicy.do",
        real_classification_text=(
            "공공데이터포털 개인정보처리방침 (KOSAX 내부 verify primitive — "
            "각 family adapter 가 기관 자체 정책 citation; KOSAX 권한 발명 X)"
        ),
...
SUBMIT_TOOL = GovAPITool(
    ...
    policy=AdapterRealDomainPolicy(
        real_classification_url="https://www.data.go.kr/policy/privacyPolicy.do",
        real_classification_text=(
            "공공데이터포털 개인정보처리방침 (KOSAX 내부 submit primitive — "
            "각 adapter 가 기관 자체 정책 citation; KOSAX 권한 발명 X)"
        ),
...
def register_mvp_surface(registry: object) -> None:
    ...
    handled directly by the KOSAX orchestrator loop (or its primitive sub-
