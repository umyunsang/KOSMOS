...
- [x] **T001** — Worktree set up at `/Users/um-yunsang/KOSMOS-w-2642`, branch `feat/2642-s7-ipc-bridge`. (Done before /speckit-implement.)
...
- [x] **T005** [P] — Edit `tui/src/screens/REPL.tsx`:
  ...
  - Add a header comment at the top of the diff range: `// KOSMOS-2642 / Epic F · S7 — directConnect/server/ DROPPED (claude.ai sync swap-out, Spec 2642 § US1).`
...
- [x] **T008** [P] — Edit `src/kosmos/ipc/frame_schema.py:NotificationPushFrame.__doc__` to insert the CC-parity verification literal `"CC parity: NO equivalent"` and a 4-line explanation of CC's terminal-OSC notification path (per plan § 1.2).
...
- [x] **T011** [P] — Create `tests/ipc/test_codec_envelope_parity.py`. Implementation:
  ...
  5. `test_drift_negative_fixture_triggers_failure(monkeypatch)` — sets `KOSMOS_IPC_PARITY_DRIFT_FIXTURE=1`, expects `pytest.raises(AssertionError)` matching `"correlation_id"`.
...
- [x] **T013** [P] — Edit `tests/ipc/conftest.py` (or create if absent in that path; check first) to add a guard fixture that ensures `KOSMOS_IPC_PARITY_DRIFT_FIXTURE` is unset by default in every test that does not explicitly opt in. (If a separate `tests/conftest.py` already exists, add the guard there.)
