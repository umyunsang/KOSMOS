...
under src/ummaya/tools/mock/ imports or references the remaining truly-OPAQUE
system identifiers.

OPAQUE-forever systems (must stay in docs/scenarios/, never in src/ummaya/tools/mock/):
...
Scope reframing (Spec 2296 / Initiative #2290):
  - gov24, kec (and hometax, modid, simple_auth, geumyung, public_mydata) are
    ...
    delegation-flow-design.md § 12. UMMAYA = client-side reference
...
The check scans all .py files under src/ummaya/tools/mock/ for the remaining
...
MOCK_ADAPTER_DIR = REPO_ROOT / "src" / "ummaya" / "tools" / "mock"
...
def _mock_python_files() -> list[pathlib.Path]:
    """Return all .py files under src/ummaya/tools/mock/."""
...
def test_mock_adapter_dir_exists_or_is_absent() -> None:
    """
    src/ummaya/tools/mock/ may not exist yet (no mock adapters implemented).
...
def test_no_opaque_identifier_in_mock_adapter(mock_file: pathlib.Path) -> None:
    """
    No .py file under src/ummaya/tools/mock/ may import or reference
