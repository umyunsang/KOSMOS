...
**Mitigation:** Manual rehearsal — citizen runs `bun run tui` in their terminal, types `안녕하세요` and `강남구 응급실 알려줘` interactively. Frame capture is achieved by setting `KOSAX_IPC_DEBUG_LOG=/tmp/ipc.log` (if implemented) or running with `KOSAX_TUI_LOG_LEVEL=DEBUG` and tailing stderr. The CC-fidelity TUI surface is preserved (per memory `feedback_cc_tui_90_fidelity`).
...
The Spec 1978 backend + IPC + agentic-loop architecture is **validated end-to-end**. The CC query-engine (per memory `feedback_kosax_uses_cc_query_engine` — KOSAX adopts CC's native function calling + streaming + parallel tool dispatch architecture, not academic ReAct) handles K-EXAONE conversational responses correctly with 21-arm frame schema parity.
