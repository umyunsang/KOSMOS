...
# Boundary marker — kept identical to ``ummaya.ipc.stdio._DYNAMIC_BOUNDARY_MARKER``
# and ``ummaya.llm.client``'s slicing literal so all three sites agree.
    ...
    def build(self, ctx: PromptAssemblyContext) -> SystemPromptManifest:
        ...
        for name, fn in self._registry.items():
            try:
                ...
                logger.warning(
                    "ummaya.prompt_assembler injector %r raised %s; skipping",
                    name,
