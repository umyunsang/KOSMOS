...
When ``UMMAYA_RETRIEVAL_BACKEND=dense`` but the SentenceTransformer model
...
def _patch_sentence_transformer(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    if importlib.util.find_spec("ummaya.tools.retrieval.dense_backend") is not None:
        # Ensure the module is imported so setattr can target it.
        import ummaya.tools.retrieval.dense_backend  # noqa: F401

        monkeypatch.setattr(
            "ummaya.tools.retrieval.dense_backend.SentenceTransformer",
            _raise_on_construct,
            raising=False,
        )
...
def test_dense_load_failure_degrades_to_bm25(
    ...
    """
    monkeypatch.setenv("UMMAYA_RETRIEVAL_BACKEND", "dense")
    _patch_sentence_transformer(monkeypatch)

    with caplog.at_level(logging.WARNING, logger="ummaya"):
