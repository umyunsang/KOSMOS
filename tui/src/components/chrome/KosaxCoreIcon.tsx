...
// KOSAX REWRITE + file rename per Epic H #1302 (035-onboarding-brand-port) — ADR-006 A-9
...
/**
 * KosaxCoreIcon renders a single `*` (U+002A) glyph in the `kosaxCore`
 * theme token.  When `shimmering === true` AND reduced motion is not active,
 * the glyph cycles between `kosaxCore` and `kosaxCoreShimmer` at 6 fps
 ...
 */
export function KosaxCoreIcon({ shimmering = false }: Props): React.ReactElement {
