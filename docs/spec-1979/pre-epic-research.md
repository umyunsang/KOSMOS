...
| Backend registry | `src/kosax/plugins/registry.py` | ACTIVE · `auto_discover()` + OTEL emit |
| Backend installer | `src/kosax/plugins/installer.py` | ACTIVE · 8-phase flow · **IPC emit 0** |
| Manifest schema | `src/kosax/plugins/manifest_schema.py` | ACTIVE · Pydantic v2 |
| Validation checks | `src/kosax/plugins/checks/q1..q10.py` | ACTIVE · 50-item matrix |
| Memdir path | `~/.kosax/memdir/user/plugins/<id>/` | USED BY auto_discover |
| **Backend `plugin_op` dispatcher** | `src/kosax/` | **MISSING** (H7 deferred) |

**핵심 갭**: TUI는 `plugin_op` request를 보낼 줄 알지만 backend는 받아서 `installer.py` 8-phase에 라우팅 + progress/complete emit하는 코드가 없다. **단일 모듈(`src/kosax/plugins/dispatcher.py` 신설)** 가 핵심 작업.

### 1.3 Agent Swarm 코드 인벤토리 (#1980 대상)

| 항목 | 경로 | 상태 |
|---|---|---|
| Coordinator | `src/kosax/agents/coordinator.py:78+` | ACTIVE · 4-phase (Research→Synthesis→Implementation→Verification) |
| Worker | `src/kosax/agents/worker.py` | ACTIVE · Spec 027 mailbox 통합 |
| Mailbox base | `src/kosax/agents/mailbox/base.py` | ACTIVE |
| File mailbox | `src/kosax/agents/mailbox/file_mailbox.py` | ACTIVE · POSIX `~/.kosax/mailbox/` |
| Messages | `src/kosax/agents/mailbox/messages.py` | ACTIVE · discriminated union |
| Consent gateway | `src/kosax/agents/consent.py` | ACTIVE · `AlwaysGrantConsentGateway` |
...
| Primitive | 경로 | 상태 |
|---|---|---|
| `lookup` | `src/kosax/tools/lookup.py` | ACTIVE · BM25 search + typed fetch |
| `resolve_location` | `src/kosax/tools/resolve_location.py` | ACTIVE · geocoding |
| `submit` | `src/kosax/primitives/submit.py` | ACTIVE · Spec 031 US1 |
| `subscribe` | — | DEFERRED · app/push runtime required |
| `verify` | `src/kosax/primitives/verify.py` | ACTIVE · Spec 031 US2 |

활성 어댑터 14종 (`src/kosax/tools/register_all.py:51-65`):
...
| Soft dependency | `kosax-plugin-store/<repo>` 4종 publish 상태 | Mock 어댑터 인벤토리 (현재 6 family ship) |
...
| **KOSAX Spec 1636** | `kosax plugin install` + 5-tier registry | `manifest.yaml` (Pydantic) — `tool_id` `plugin.<id>.<verb>` + verb + ko/en hint + permission level + auth_type + pipa_class | host 공유 (미구현) | 3-layer (Spec 033) + PIPA §26 SHA-256 ack | `kosax-plugin-store/index` GitHub + SLSA verifier | semver + SLSA provenance |

#### 적용 표준 (2026-04 시점)

| 표준 | 상태 | KOSAX 적합 지점 | 비용 |
|---|---|---|---|
...
| RFC 9700 OAuth BCP | Published 2025-01 | KOSAX auth 패턴에 PKCE 강제 upgrade | 낮음 |
...
#### KOSAX 갭 인사이트 (#1979에서 도입 가치)

1. **`/.well-known/mcp/server-card.json` dual-publish** — KOSAX plugin manifest를 MCP Server Card 호환으로 dual-publish 시 lock-in 회피 + 표준 영향력. CC `Tool.ts` 마이그레이션 정신과 일치.
...
4. **`engines.kosax` 호환성 필드** — 현 manifest는 plugin semver만, host 호환성 미선언 (VSCode 패턴 차용).
...
**KOSAX 매핑**: `coordinator.py` 4-phase ≈ Anthropic Lead-Sub 패턴, spawn은 POSIX mailbox = AutoGen actor 모델에 사상적으로 가까움.

#### Spec 027 mailbox 패턴 vs 외부

| KOSAX 027 요소 | 가장 가까운 외부 패턴 | 평가 |
|---|---|---|
| `~/.kosax/mailbox/<sid>/<sender>/<mid>.json` | qmail/Postfix Maildir + Akka.NET `AtLeastOnceDeliveryActor` | 잘 알려진 crash-safe 패턴 |
| `<mid>.json.consumed` sibling marker | Maildir `cur`/`new` + Postfix queue D flag | 동일 사상; rename atomic하므로 KOSAX marker가 더 안전 |
| crash replay (no `.consumed` scan) | Akka.NET persistence journal · Ray actor `max_task_retries=-1` | KOSAX는 marker로 idempotency 보장 → stateful 워커에도 OK |
...
|---|---|---|---|---|---|
| 이사 (행안부 전입 + 국토부 부동산 + 건강보험 자격) | ★★★ | △ (행안부 submit OPAQUE — receipt mock 가능) | ★★ 폼/receipt 정적 | 없음 | ★★★★☆ — KOSAX 미션 직설 |
...
| Agentic JWT (IETF I-D goswami, 2025) | 워커 버전+워크플로+위임체인 토큰 claim | OTEL `kosax.permission.*` span으로 절반 커버 |
| Anthropic "scopes-not-prompts" (2025) | `flight.booking:create` capability grant | KOSAX 4 primitive × adapter scope에 그대로 매핑 |
...
| MIRROR (IJCAI 2025) | Planner → Tool agent → Executor → Answer agent | ◎ KOSAX 4-phase 1:1 대응 · "임의가 아닌 IJCAI 검증된 분할" 주장 가능 |
| Plan-and-Execute (LangChain) | 큰 plan 한 번 + step + replan | △ ReWOO 약화판 |

> 시연/논문 narration: "ReAct 가 아니라 ReWOO + MIRROR 계열 plan-then-decompose" — `feedback_kosax_uses_cc_query_engine` 메모리와 정합.
...
- **architectural question 5종** (Epic #1980 body 명시) — `/speckit-clarify`로 처리:
  ...
  2. Worker process model — separate `kosax.ipc.worker_server` subprocess vs in-process asyncio task?
