...
"""Safety-pipeline runtime configuration for UMMAYA.

All fields are loaded from ``UMMAYA_SAFETY_*`` environment variables,
except ``openai_moderation_api_key`` which uses its own namespaced variable
``UMMAYA_OPENAI_MODERATION_API_KEY`` (no ``SAFETY_`` infix — deliberate).
...
class SafetySettings(BaseSettings):
    """UMMAYA safety-pipeline runtime configuration (UMMAYA_SAFETY_* env vars).
    ...
    openai_moderation_api_key: SecretStr | None = Field(
        default=None,
        validation_alias="UMMAYA_OPENAI_MODERATION_API_KEY",
    )
    ...
    def _fail_closed_on_missing_moderation_key(self) -> SafetySettings:
        if self.moderation_enabled:
            ...
            if key is None or not key.get_secret_value().strip():
                raise ConfigurationError("UMMAYA_OPENAI_MODERATION_API_KEY")
        return self
