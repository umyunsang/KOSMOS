...
2. **Given** `UMMAYA_LOOKUP_TOPK=3` is set, **When** `lookup(mode="search", top_k=10)` is called, **Then** the effective `top_k` is clamped to the env-configured default (3) or the explicit per-call `top_k` (whichever applies per §5.2 of the design doc), and the clamp bounds `[1, 20]` are enforced.
...
- **NMC freshness SLO (post-gate)**: Documented now, enforced when the Layer 3 gate is lifted — `hvidate` older than `UMMAYA_NMC_FRESHNESS_MINUTES` minutes returns `LookupError(reason="stale_data")`.
...
- **FR-009**: `top_k` MUST default to `min(UMMAYA_LOOKUP_TOPK, len(registry))`, with `UMMAYA_LOOKUP_TOPK` defaulting to 5 and clamped to `[1, 20]`.
...
- **FR-032**: All external keys MUST be read from `UMMAYA_`-prefixed env vars: `UMMAYA_KAKAO_REST_KEY`, `UMMAYA_JUSO_CONFM_KEY`, `UMMAYA_SGIS_KEY`, `UMMAYA_SGIS_SECRET`, `UMMAYA_DATA_GO_KR_API_KEY`.
- **FR-033**: `UMMAYA_LOOKUP_TOPK` MUST default to 5 and be clamped to `[1, 20]`.
- **FR-034**: `UMMAYA_NMC_FRESHNESS_MINUTES` MUST default to 30 and be clamped to `[1, 1440]` (documented but only enforced once the Layer 3 gate is lifted — see deferred item).
...
- The `data.go.kr` shared key (`UMMAYA_DATA_GO_KR_API_KEY`) remains valid across KOROAD, KMA, and HIRA — this matches the KOROAD Portal reference memory.
...
- The Kakao REST key naming follows the spec `UMMAYA_KAKAO_REST_KEY` (not a prior shorter variant); any existing older naming is migrated in this epic.
