...
// KOSAX: legacy oauth/client surface deleted by Spec 1633; KOSAX uses FriendliAI.
...
import { getAPIProvider, isFirstPartyKosaxBaseUrl } from './providers.js'
...
function isModelCapabilitiesEligible(): boolean {
  // KOSAX: legacy capability-cache eligibility gate is dead under single-fixed
  // FriendliAI provider; refresh path always returns false.
  if (getAPIProvider() !== 'firstParty') return false
  if (!isFirstPartyKosaxBaseUrl()) return false
  return false
}
...
export async function refreshModelCapabilities(): Promise<void> {
  ...
  // KOSAX: capability refresh is a no-op (FriendliAI backend manages the model list).
  logForDebugging('[modelCapabilities] refresh skipped — KOSAX uses FriendliAI single-fixed model')
}
