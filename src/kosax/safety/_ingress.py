...
Called from ``kosax.tools.executor`` immediately after the adapter returns its
raw output and immediately before ``normalize()`` (FR-006, FR-013).

Ordering (FR-013):
  1. Run :func:`kosax.safety._injection.run_detector` on the serialised
  ...
  2. Else, if ``SafetySettings.redact_tool_output`` is true, walk the dict
     leaves and pass each string through :func:`kosax.safety._redactor.run_redactor`.
...
Defense-in-depth note (T037, commit 50e2c17):
  The per-file log redactions in ``kosax.llm.client`` (``reasoning_content``
  length-only logging; ``tool_call_delta.arguments`` metadata-only logging)
  and ``kosax.tools.executor`` (``raw_args_len``-only logging on validation
...
from kosax.safety._injection import run_detector
from kosax.safety._models import (
    InjectionBlockedEvent,
    RedactedEvent,
)
from kosax.safety._redactor import run_redactor
