...
FR-005: search → BM25 retrieval gate via ``kosax.tools.search.search()``.
FR-006: fetch → typed adapter invocation via ``executor.invoke()``.
FR-009: top_k adaptive clamp [1, 20], default from KOSAX_LOOKUP_TOPK.
"""
...
async def _lookup_search(
    ...
    *,
    ...
    from kosax.tools.search import search
    ...
    default_k = settings.lookup_topk  # from KOSAX_LOOKUP_TOPK
...
async def _lookup_fetch(
    ...
    *,
    ...
    # the KOSAX orchestrator loop". Citizen-visible symptom without this
    ...
    if inp.tool_id == "resolve_location":
        ...
        from kosax.tools.resolve_location import resolve_location as _resolve_fn
    ...
    # FR-018: annotate the current execute_tool span with kosax.tool.adapter
    ...
    current_span.set_attribute("kosax.tool.adapter", inp.tool_id)
