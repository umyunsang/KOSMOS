...
- [ ] T018 Lift Ink reconciler files from `.references/claude-code-sourcemap/restored-src/src/ink/` into `tui/src/ink/` (reconciler.ts, renderer.ts, root.ts, dom.ts, layout/, events/, node-cache.ts, measure-text.ts, render-*.ts, parse-keypress.ts) — every file prepended with the FR-011 attribution header (~29 files)
...
- [ ] T020 [P] Add CI check that runs `tui/scripts/diff-upstream.sh` and asserts every lifted file retains its attribution header via grep (FR-011, SC-9)
