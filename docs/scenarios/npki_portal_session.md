...
A citizen uses UMMAYA to initiate a certificate-based authentication or signature that requires a valid, active NPKI certificate. If the citizen's certificate is expired or not yet issued, the journey includes a portal session step:

1. The citizen requests an operation requiring NPKI authentication (e.g., signing a form for a government submission).
2. UMMAYA attempts to load the citizen's PKCS#12 bundle from the device using the `npki_crypto` mock layer.
3. If the certificate is expired or absent, UMMAYA presents a human-readable message explaining the situation and, if the citizen consents, invokes the `delegate` primitive to hand off to the NPKI CA portal session.
...
6. UMMAYA is notified of the completion via a callback registered at handoff time (`delegate` return value includes a `session_complete` polling token).
7. UMMAYA polls the polling token (CA-specific endpoint) until `status: "complete"` or timeout (5 minutes), then re-attempts the PKCS#12 load using the `npki_crypto` mock layer.

## UMMAYA ↔ real system handoff point

The handoff occurs at step 3: when UMMAYA calls `delegate(tool_id="npki_portal_session", params={"ca_code": ..., "cert_type": ..., "citizen_id_hash": ...})`.

At this point:
- UMMAYA has determined that the citizen's certificate is expired, absent, or rejected by the target system.
- UMMAYA has the citizen's explicit consent to initiate a portal session (this is a consent-gated action).
- UMMAYA emits a `ToolCallAuditRecord` with `is_irreversible=False` (the session can be abandoned) before the handoff.
- The `delegate` call opens the CA portal URL in the citizen's default browser — UMMAYA does not render the portal UI.
...
- UMMAYA receives a `{ session_token, poll_url, expires_at }` response from the `delegate` call; it uses `poll_url` to check session status.
- On successful certificate issuance: UMMAYA records `outcome: "portal_session_complete"` in the audit record and proceeds with the original operation.
- On timeout or failure: UMMAYA records the structured error (`{ ca_code, error_message, session_token }`) in the audit record and presents a human-readable failure message to the citizen.
...
- UMMAYA does not implement the CA portal session protocol — the browser plugin handshake and OTP/biometric challenge are entirely opaque.
- UMMAYA does not access the CA portal's internal session state or inject into the browser session.
- UMMAYA does not store the citizen's PKCS#12 password or private key material — these are managed by the citizen's device and CA.
- UMMAYA does not support the legacy ActiveX plugin path — only the native messaging extension path is supported (modern browsers only).
- UMMAYA does not implement multi-CA session orchestration — the citizen must select a single CA before the handoff.
