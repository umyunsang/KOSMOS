...
The UMMAYA process crashes mid-session (e.g., OOM kill) after a worker has posted a `result` message but before the coordinator has read it. On process restart with the same `session_id`, the coordinator can replay unread messages from the file-based mailbox in per-sender FIFO order and resume synthesis.
...
1. **Given** a worker that has written a `result` message to `~/.ummaya/mailbox/<session_id>/` before the process terminates, **When** the coordinator restarts with the same `session_id`, **Then** it reads and processes the unread `result` message before dispatching new workers.
...
2. **Given** a worker that executes two tool-loop iterations, **When** the trace is exported, **Then** exactly two `gen_ai.agent.worker.iteration` spans appear tagged with `ummaya.agent.role` set to the worker's specialist role value (from the `AgentContext`).
...
- **Coordinator mailbox full**: If the per-session mailbox directory contains more messages than `UMMAYA_AGENT_MAILBOX_MAX_MESSAGES`, new writes MUST fail with a structured `MailboxOverflowError` rather than silently dropping messages or growing unbounded.
...
- **FR-001**: The system MUST provide a `Coordinator` class at `src/ummaya/agents/coordinator.py` that orchestrates the 4-phase workflow: `Research → Synthesis → Implementation → Verification` as defined in `docs/vision.md § Layer 4`.
...
- **FR-008**: The system MUST provide a `Worker` class at `src/ummaya/agents/worker.py` that wraps one `QueryEngine` instance and drives it through the 2-tool surface (`lookup` + `resolve_location`).
- **FR-009**: Workers MUST reuse `src/ummaya/engine/query.py`'s `_query_inner()` async generator as their inner tool loop verbatim. The worker MUST NOT reimplement the tool loop.
...
- **FR-014**: The system MUST provide a `Mailbox` class and related infrastructure at `src/ummaya/agents/mailbox/`. The file-based implementation MUST store messages under `UMMAYA_AGENT_MAILBOX_ROOT/<session_id>/<sender_id>/<message_id>.json`.
- **FR-015**: `UMMAYA_AGENT_MAILBOX_ROOT` MUST default to `~/.ummaya/mailbox` and MUST be configurable as a `UMMAYA_AGENT_MAILBOX_ROOT` environment variable.
...
- **FR-021**: The mailbox MUST enforce a per-session message cap of `UMMAYA_AGENT_MAILBOX_MAX_MESSAGES` (default: 1000, clamped to [100, 10000]). Writes beyond the cap MUST raise `MailboxOverflowError` immediately.
...
- **FR-028**: The coordinator MUST emit one `gen_ai.agent.coordinator.phase` OTel span per phase transition. The span MUST carry the attribute `ummaya.agent.coordinator.phase: Literal["research", "synthesis", "implementation", "verification"]`.
- **FR-029**: Each worker MUST emit one `gen_ai.agent.worker.iteration` span per tool-loop iteration. The span MUST carry `ummaya.agent.role` (the worker's specialist role string from `AgentContext`) and `ummaya.agent.session_id` (the session UUID, not PII).
- **FR-030**: The mailbox `send()` operation MUST emit one `gen_ai.agent.mailbox.message` span per delivery. The span MUST carry `ummaya.agent.mailbox.msg_type` and `ummaya.agent.mailbox.correlation_id`.
- **FR-031**: All new `ummaya.agent.*` attribute names MUST be declared as string constants in `src/ummaya/observability/semconv.py` (extending the existing module). The attribute names MUST be submitted to Epic #501's boundary table before any collector deployment.

#### Configuration and environment

- **FR-032**: `UMMAYA_AGENT_MAILBOX_ROOT` MUST default to `~/.ummaya/mailbox` and MUST accept any writable absolute path.
- **FR-033**: `UMMAYA_AGENT_MAILBOX_MAX_MESSAGES` MUST default to 1000 and be clamped to [100, 10000].
- **FR-034**: `UMMAYA_AGENT_MAX_WORKERS` MUST default to 4 and be clamped to [1, 16]. The coordinator MUST NOT spawn more concurrent workers than this cap.
- **FR-035**: `UMMAYA_AGENT_WORKER_TIMEOUT_SECONDS` MUST default to 120 and be clamped to [10, 600]. A worker that does not post a `result` or `error` message within this timeout MUST be cancelled by the coordinator and treated as an error.
- **FR-036**: All four `UMMAYA_AGENT_*` env vars MUST be documented in `docs/configuration.md` (the catalog owned by #468).
...
- **`FileMailbox`**: Concrete implementation. Stores messages as JSON files under `UMMAYA_AGENT_MAILBOX_ROOT/<session_id>/<sender_id>/`. At-least-once delivery via fsync.
...
- **SC-006**: `gen_ai.agent.coordinator.phase`, `gen_ai.agent.worker.iteration`, and `gen_ai.agent.mailbox.message` spans appear in the recording exporter output with all required attributes present (no missing `ummaya.agent.*` attributes).
...
- **SC-009**: All four `UMMAYA_AGENT_*` env vars are listed in `docs/configuration.md` before the PR is merged.
...
- Epic #501 (observability boundary table) is either merged or its attribute boundary table is available in draft form so that the `ummaya.agent.*` attribute names declared here can be submitted before any collector deploys.
...
- The file system where `UMMAYA_AGENT_MAILBOX_ROOT` resides supports POSIX fsync semantics (macOS and Linux CI environments both satisfy this).
- The `AgentContext` model uses `arbitrary_types_allowed=True` to accommodate `LLMClient` and `ToolRegistry` which are not Pydantic models. This follows the precedent set by `QueryContext` in `src/ummaya/engine/models.py`.
...
| Formal observability attribute registration with OTel upstream | `ummaya.agent.*` attributes are UMMAYA-internal extensions; upstream registration is a post-Phase-2 contribution | Phase 3 / OSS contribution | NEEDS TRACKING |
