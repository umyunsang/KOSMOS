...
**Resolution** — top-level manifest-list digest and the two arches UMMAYA cares about:
...
| CI `UMMAYA_LANGFUSE_OTLP_AUTH_HEADER` via Infisical OIDC | `[NEEDS TRACKING]`, dep on #468 | RESOLVED BY `/speckit-taskstoissues`; #468 is in-flight (Infisical IDs captured). |
...
| `batch` processor tuning (`timeout: 5s`, `send_batch_size: 512`) | OpenTelemetry Collector Contrib — `processor/batchprocessor` defaults | UMMAYA uses the recommended dev preset; production Epic may tune upward |
| Health extension on port `13133` | OpenTelemetry Collector — `extension/healthcheckextension` | Default port; no change needed |
| PII whitelist single source of truth | UMMAYA spec 021 `_ALLOWED_METADATA_KEYS` frozenset | `src/ummaya/observability/event_logger.py` — Python-layer first gate; collector is second gate |
| Warn-and-skip when endpoint unset | UMMAYA spec 021 FR-010 | Inherited behavior — observability failure never breaks app |
| `OTEL_SDK_DISABLED=true` CI path | UMMAYA spec 021 FR-014 | Extended here — SC-004 |
| Docker Hub multi-arch digest pinning | Docker Buildx docs — "imagetools inspect" | Used via v2 registry API (equivalent output) |
| Environment variable `UMMAYA_` prefix | AGENTS.md hard rule | Applied to all three new vars |
...
5. **Env-var-sourced auth header** — `UMMAYA_LANGFUSE_OTLP_AUTH_HEADER` sourced from `.env`. Empty default means local dev works without an API key against an anonymous Langfuse project.
