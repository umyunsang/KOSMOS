...
- `tui/src/components/consent/ConsentListView.tsx` — KOSAX 측 consent UI 패턴 (i18n + theme 사용 + Esc fallback)
...
- box-bordered + theme 토큰 (`theme.kosaxCore`, `theme.error`, `theme.text`, `theme.subtle`).
...
**Files**:
- `src/kosax/ipc/frame_schema.py` (수정)
- `tui/src/ipc/frames.generated.ts` (재생성)

**Reference baseline**:
- `src/kosax/ipc/frame_schema.py:462-509` — `PermissionRequestFrame` / `PermissionResponseFrame` 패턴
...
- `pytest src/kosax/ipc/tests/test_frame_schema.py` PASS (기존 테스트 + 신규 arm 검증).
- `pytest src/kosax/ipc/tests/test_frame_schema_role_allowlist.py` PASS (Spec 032 invariant).
...
**Files**:
- `src/kosax/ipc/stdio.py` (수정 — 새 dispatcher 추가)

**Reference baseline**:
- `src/kosax/ipc/stdio.py:1320-1490` — 기존 permission_response 핸들러 (consent receipt write 패턴)
- `src/kosax/permissions/ledger.py:append()` — Spec 033 ledger append 함수
- `src/kosax/permissions/hmac_key.py:load_or_generate_key` — fail-closed key check

**Implementation**: plan.md § 1.3 의 6단계 sketch 구현:
1. HMAC 키 검증 → 부재 시 `ledger_key_missing` 응답.
2. Receipt 파일 (`~/.kosax/memdir/user/consent/<receipt_id>.json`) 존재 검증 → 부재 시 `not_found`.
3. `revoked_at` 필드 idempotent 검증 → 이미 set 시 `already_revoked` (FR-021).
4. `kosax.permissions.ledger.append()` 호출 — `action="withdraw"`, `scope=<receipt_id>`, `decision="denied"`, `withdrawn_at=<ISO8601 UTC>`, `consent_receipt_id=<UUIDv7>`.
...
- OTEL span: `kosax.consent.revoke` with attributes `kosax.consent.receipt_id`, `kosax.permission.decision="revoked"`.
...
**Files**:
- `src/kosax/ipc/tests/test_consent_revoke_dispatch.py` (신규)
...
- Test 5 `test_otel_span_emitted`: span exporter mock 으로 `kosax.consent.revoke` span 1건 capture.
...
**Reference baseline**:
- `tui/src/screens/REPL.tsx:3664-3693` — 기존 `_kosaxCmd === 'consent'` 분기
...
- `parseConsentArgs(_kosaxArgs)` 호출 + 4 분기 (`unknown` / `not found` / `already revoked` / 모달 mount).
- `consentBridge.requestRevoke()` 호출 + 성공 시 `permissionReceiptsCtx.revokeReceipt()` + 토스트.
- 실패 시 error code 별 토스트 (FR-E02).
- 기존 placeholder `addNotification({key: 'kosax-consent-revoke', text: '... (P5 연동 예정)', ...})` 라인 완전 삭제 (FR-E03).
...
**Reference baseline**:
- `scripts/tui-tmux-capture.sh` — KOSAX tmux 시나리오 harness (Spec debug-infra-rebuild)
