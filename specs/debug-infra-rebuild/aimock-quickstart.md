# aimock Quickstart — UMMAYA Operator Guide

> **Spec**: `specs/debug-infra-rebuild/RFC.md § P0`
> **Status**: Phase 2 deliverable — OPT-IN only.
> Default UMMAYA path (real FriendliAI) is unchanged.
...
UMMAYA uses it to replace the live FriendliAI K-EXAONE endpoint in:
...
aimock listens on **`http://localhost:4010`** (override with `UMMAYA_AIMOCK_PORT` env var).

---

## Point UMMAYA at aimock
...
`UMMAYA_FRIENDLI_TOKEN` can be any non-empty string — aimock does not validate auth.

Both variables map to `LLMClientConfig` in `src/ummaya/llm/config.py` and are
...
1. **aimock is OPT-IN**. UMMAYA runs against real FriendliAI by default. Never set
   `UMMAYA_FRIENDLI_BASE_URL=http://localhost:4010` in `.env` committed to the repo.
...
4. **`UMMAYA_FRIENDLI_TOKEN` must remain non-empty** even for aimock — the pydantic
...
```bash
# Start aimock-bun on port 4010 (same port, transparent to UMMAYA code)
bun scripts/aimock-bun.ts --port 4010

# Optional: different port
bun scripts/aimock-bun.ts --port 4011
# Then: export UMMAYA_FRIENDLI_BASE_URL=http://localhost:4011/v1
```
...
A fixture that always returns `toolCalls` causes the UMMAYA agentic loop to
...
| `UMMAYA_FRIENDLI_TOKEN must not be empty` | Token env var not set | `export UMMAYA_FRIENDLI_TOKEN=aimock-test` |
| Container exits immediately | Image pull failed | `docker pull ghcr.io/copilotkit/aimock:latest`; or use aimock-bun fallback |
| Port 4010 already in use | Another service on that port | `UMMAYA_AIMOCK_PORT=4011 docker compose -f docker-compose.aimock.yml up -d` or `bun scripts/aimock-bun.ts --port 4011`; set `UMMAYA_FRIENDLI_BASE_URL=http://localhost:4011/v1` |
