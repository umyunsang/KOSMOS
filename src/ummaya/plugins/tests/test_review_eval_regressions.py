...
* **CLI subprocess smoke**: ``ummaya-plugin-validate`` must work when
...
from ummaya.plugins.checks.framework import CheckContext, run_all_checks
from ummaya.plugins.checks.q10_tests import check_no_live_in_ci
from ummaya.plugins.installer import _allocate_consent_position, _safe_extract
...
class TestRegistryModulePathResolution:
    ...
    which broke the canonical scaffold layout shipped by `ummaya plugin
    init` (`plugin_<id>/adapter.py`).
    """

    def test_dotted_module_path_resolves_to_nested_file(self, tmp_path: Path) -> None:
        from ummaya.plugins.registry import _import_adapter_module
...
# ---------------------------------------------------------------------------
# CLI subprocess smoke — ummaya-plugin-validate works as installed script.
# ---------------------------------------------------------------------------
...
class TestCliSubprocessSmoke:
    def test_help_returns_zero(self) -> None:
        ...
        """
        from ummaya.plugins.checks.framework import _cli_main
...
class TestInstallPluginHttpxErrorEnvelope:
    ...
    def test_catalog_fetcher_raising_arbitrary_exc_returns_exit_one(
        ...
        from ummaya.plugins.installer import install_plugin
        from ummaya.tools.executor import ToolExecutor
        from ummaya.tools.registry import ToolRegistry

        # Isolate filesystem state.
        monkeypatch.setattr(
            "ummaya.plugins.installer.settings.plugin_install_root",
            tmp_path / "install",
        )
        monkeypatch.setattr(
            "ummaya.plugins.installer.settings.plugin_bundle_cache",
            tmp_path / "cache",
        )
        monkeypatch.setattr(
