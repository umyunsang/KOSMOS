# 권한 위임 흐름 (Delegation Flow) — KOSAX One-Stop AX 설계
...
>
> - **1차 잘못**: KOSAX-original 권한 시스템 발명 (Spec 033 5-mode 등) → ✅ 삭제 완료 (Wave 1-3)
> - **2차 잘못 (§5-§9)**: KOSAX가 protocol pioneer로 기관에 새 protocol 제안 → §11에서 정정
> - **2차 정정 (§11)도 부정확**: "기관 시스템 변경 X, browser substitute" 가정이 **국가인공지능전략위원회 정책 timing 무시** → §12에서 final 정정
> - **3차 정정 final (§12)**: KOSAX = **한국 국가AX 인프라가 만들 LLM-accessible 보안 wrapping 통로의 client-side reference implementation**. 정부 자체 시스템 개편 동력 = 국가AI전략위원회 (2025-09 출범) + 행동계획 2026-2028 (2026-02 확정, 99 과제) + 공공AX 분과 + 범정부 AI 공통기반.
>
...
KOSAX의 권한 위임 비전은 **기술적으로 conventional**. OAuth 2.0 + RFC 8693 Token Exchange + OpenID for Verifiable Presentations (OID4VP) + W3C Verifiable Credentials를 정부 도메인에 적용한 것 — Singapore Myinfo, Japan JPKI + マイナポータル API, EU EUDI Wallet (2026-09 출시 예정), UK HMRC MTD가 이미 운영 중. **한국의 blocker는 protocol이 아니라 statutory + agency roadmap**: KOMSCO 모바일 신분증은 wallet을 가지고 있고, 금결원/한국신용정보원은 동의 인프라를 운영하며, 표준은 RFC + W3C로 존재. 누락된 것은 (a) 표준동의서의 action-scope claim, (b) 행정 도메인의 write API, (c) 전자정부법 / 새 행정마이데이터법의 행위위임권 명시화. **KOSAX의 역할은 이 누락된 protocol surface를 mock으로 byte/shape mirror하여, 정책 입안자와 기관 아키텍트가 만져볼 수 있는 reference artifact를 제공하는 것**. AI Action Plan 2026-2028 §원칙 8 (공공AX) + 디지털플랫폼정부 2025-2027 마스터플랜이 요구하는 정확히 그것.
...
```
[시민]                  [활용기관 = KOSAX-analog]      [통합인증 = 금결원]      [정보제공자 = 은행]
   │ (1) 가입             │                                 │                      │
   │ ───────────────────►│                                  │                      │
   │                     │ (2) 표준동의서 redirect          │                      │
   │ ◄───────────────────────────────────────────────────►│                       │
...
**Verdict**: KOSAX는 이 envelope를 verbatim mock하면 됨. write extension이 입법되는 순간 KOSAX의 mock surface는 그대로 적용 가능.
...
- KOSAX가 필요한 token endpoint (`scope=submit:hometax.tax-return`) — **부재**
...
```
[KOSAX RP]                [시민 wallet = 모바일신분증 앱]       [Issuer = 행안부/KOMSCO]
...
**Critical extensibility**: VP는 임의의 signed claim을 carry 가능. 만약 국세청이 `TaxReturnAuthorizationCredential` VC type을 issue하면, 시민은 KOSAX에게 그것을 present → KOSAX가 그 VP의 audit anchor로 신고 — **이게 정확히 OID4VP (draft 21) 패턴**.
...
- KOSAX Confidential Client가 assertion replay 불가

**Verdict**: 간편인증은 KOSAX delegation primitive로 사용 불가. provider가 OAuth2 client-credentials + token-exchange endpoint를 expose 해야 함 (현재 X).
...
### 3.1 Singapore Myinfo — **Gold standard, KOSAX가 가장 가까이 학습할 것**
...
### 3.3 EU EUDI Wallet — **2026-09 launch, KOSAX와 architecturally 동일**
...
- **EU는 정확히 KOSAX 비전을 union scale로 진행 중**
...
- **이게 production에서 가장 가까운 KOSAX-vision 시스템**
...
**KOSAX의 frame**: "시민 본인이 작성한 신고서를 KOSAX가 단순 전송" — §2 ② carve-out 적용 가능. 단 judicial test 안 됨 → 법무 검토 필수.
...
**해석**: KOSAX가 시민-side 서명 ceremony를 trigger하면 (KOMSCO 모바일 신분증 통해), 서명된 payload는 **legally 시민-originated**. KOSAX 자체는 서명 X, ceremony trigger만.
...
**시나리오**: 시민이 "내 종합소득세 신고해줘" 요청 → KOSAX 한 turn으로 처리

```
[시민]──"종합소득세 신고해줘"──►[KOSAX LLM]
                                 │
...
2. 📜 **Three-way reconcilable audit**: KOSAX memdir + KOMSCO ledger + 행정 도메인 ledger (모두 mock에서도 logging)
...
```python
# src/kosax/primitives/delegation.py
...
위임 token의 issuance + use + revocation은 모두 `~/.kosax/memdir/user/consent/` JSONL ledger에 append. Format:
...
| Standard | KOSAX 채택 | 한국 채택 |
|---|---|---|
| OAuth 2.0 (RFC 6749) | ✅ verify/submit base | ✅ 마이데이터 |
| **OAuth 2.0 Token Exchange (RFC 8693)** | ✅ verify→submit token swap | ❌ 미채택 (KOSAX pilot의 핵심 추가) |
...
**Critical 누락 (KOSAX pilot이 demonstrate)**:
...
- 세무대리인 ERP 통합 케이스 = KOSAX pattern과 가장 유사

**제안 angle**:
> "마이데이터 표준 extension pilot — 종합소득세 신고를 KOSAX가 활용기관으로, KOMSCO 모바일신분증 VP를 delegation grant로 사용하는 reference implementation"

### 7.2 Secondary target: 행정안전부 디지털정부국 (공공마이데이터 운영주관)

**이유**:
- 공공마이데이터 framework이 statutory home for 행위위임 extension
- 디지털플랫폼정부 마스터플랜 2025-2027 Phase 2 (2026 H2) "AI 기반 선제적 행정서비스" — KOSAX가 정확히 그 reference

**제안 angle**:
> "공공마이데이터 표준동의서 확장 — action-scope claim 추가, KOSAX가 reference open-source 활용기관 client로 LLM-mediated 위임의 안전성 demonstrate"
...
= KOSAX가 그 missing piece (학부생 student project + Apache-2.0 + 즉시 audit 가능)
```
...
- ❌ KOSAX를 "세무대리 서비스"로 pitch — frame은 "citizen-authored return transmission" (세무사법 §2 ② carve-out)
...
| Task | 산출물 | 의존 |
|---|---|---|
| ε.1-1 | `src/kosax/primitives/delegation.py` (DelegationToken + DelegationContext Pydantic 모델) | — |
| ε.1-2 | `src/kosax/tools/mock/verify_modid_vp.py` (KOMSCO OID4VP shape-mirror) | ε.1-1 |
| ε.1-3 | `src/kosax/tools/mock/verify_mydata_consent.py` (마이데이터 OAuth2 byte-mirror) | ε.1-1 |
| ε.1-4 | `src/kosax/tools/mock/submit_hometax_taxreturn.py` (가상 홈택스 endpoint) | ε.1-1 |
| ε.1-5 | `src/kosax/tools/mock/submit_gov24_minwon.py` (가상 정부24 endpoint) | ε.1-1 |
| ε.1-6 | `src/kosax/tools/mock/lookup_hometax_simplified.py` (가상 간소화 조회) | ε.1-1 |
...
| ε.2-3 | Vision demo gif (kosax-migration-tree § L1-A 4-tier OTEL trace 포함) |
...
| KOMSCO가 OID4VP RP-facing spec 비공개 유지 | KOSAX pilot이 정확히 그 압력 (open-source RP가 spec 요구) — 정책적으로 의미 있는 leverage. |
| `delegation_token`이 LLM context에 leak | system prompt rule: token을 시민에게 echo 금지. memdir에만 저장, conversation 표면에 show 안 함. |
| 5년 horizon — 우리가 그 때까지 KOSAX 유지? | KSC 2026 + 졸업 후에도 student maintainer 풀 유지 가능 (Apache-2.0 + plugin DX = 외부 기여 받기 쉬움). |
...
|---|---|
| KOSAX가 새 protocol 제안 (RFC 8693, OID4VP RP spec 등) | KOSAX는 기존 도메인 시스템 변경 요청 X |
| 기관에 "이렇게 API 공개하세요" 공문 발송 | 기관 시스템 변경 요청 X |
| Bearer token으로 행정 API 직접 호출 | LLM이 시민 대신 기존 web/mobile UX navigate |
| OPAQUE 도메인이 KOSAX에 protocol 풀어줘야 작동 | OPAQUE 도메인은 그대로 OPAQUE — LLM이 시민처럼 navigate |
| AX = "기관이 변하면 가능" | AX = "오늘 바로 가능, LLM이 시민의 hands and eyes 역할" |

### 11.2 정정된 architecture

KOSAX의 진짜 모델은 **agentic browser substitute**:
...
KOSAX = 위 패턴들이지만 한국 정부 도메인 + 한국어 시민 + 5-primitive 표면.
...
- KOSAX Python 백엔드에 `playwright` 의존성 추가 (단, AGENTS.md hard rule "신규 runtime 의존성 0" 검토 필요)
...
- KOSAX가 시민의 모바일 인증 결과를 통해 세션 발급받음
- 행정 도메인은 KOSAX의 모바일 SDK 또는 deep link로 호출
- 이 경우 Browser automation 없이 가능
- 위험: 도메인이 mobile SDK 공개 안 함 (대부분 hometax/gov24가 자체 앱만)

**Option C: 끝까지 mock 유지 (가장 안전)**
- KOSAX는 demo + reference implementation으로 가동
...
> **이런 KOSAX 방향성은 기관에 변경 요청 X**. 시민이 hometax 직접 접속하던 것을 LLM 대화로 대체하는 것. 만약 기관과 협력할 수 있다면 단지 "테스트 계정 / API 키 / dev sandbox 접근"만 요청 — 그 외 기관 시스템 변경 X.

기관 제안의 본질은:
1. 학부생 KOSAX demo 보여주기 (browser automation으로 작동하는 것)
2. "정부24 dev sandbox 접근권 주세요" — 기존 sandbox에 KOSAX 연결
3. 기존 시스템 변경 요청 0건

### 11.7 결론 — 정정된 KOSAX 정체성

KOSAX = **시민의 web browser를 대신 운영하는 LLM agent + 한국어 + 한국 정부 도메인 specialized**.

- 시민 입장: hometax/gov24 안 들어가도 LLM 대화로 일 처리
- 기관 입장: 기존 시스템 변경 0건 — KOSAX는 그냥 시민이 평소 하던 일을 자동화
...
- 핵심 — KOSAX는 protocol pioneer가 아니라 **LLM agent productization** project.
...
§11 ("Browser substitute") 가정 = "기관 시스템은 절대 변하지 않는다 → KOSAX는 시민의 web browser를 대신 운영". 이건 정책 timing 무시. **국가인공지능전략위원회가 2025-09-08 출범** + **인공지능 행동계획 2026-2028이 2026-02-24 확정 (99 과제, 326 권고)** + **공공AX 분과 + 범정부 AI 공통기반 (2025-11~)** = 한국 정부가 **이미 시스템 개편 중**. 따라서 KOSAX는 browser-side scraper가 아니라 **국가 AX 인프라가 만들 LLM-accessible 보안 wrapping 통로의 client-side reference implementation**.

### 12.3 정정 매트릭스 — 1차 → 2차 → 3차

| 차원 | 1차 (이전 잘못) | 2차 ("browser substitute") | **3차 (FINAL CANONICAL)** |
|---|---|---|---|
| 기관 변경 | KOSAX가 새 protocol 제안 → 기관에 공문 | 기관 시스템 변경 X | **기관이 자체적으로 시스템 개편 (국가AX 인프라 정책 동력). KOSAX는 caller** |
| KOSAX 역할 | Protocol pioneer | Browser/Mobile use agent | **AX 인프라 client-side reference implementation** |
...
| 인증 도메인 | OID4VP RP spec 제안 | 기존 시스템 push 알림 trigger | **인증 서비스도 LLM-callable 모듈로 시스템 개편 — KOSAX는 그것 호출** |
| 정책 hook | 학부생이 정책 입안자에 제안 | 정책 무관 demo | **국가인공지능전략위원회·DPG·공공AX의 reference impl** |

### 12.4 진짜 KOSAX architecture (FINAL)
...
[국가 AX 인프라 (정책)]
  ...
  → LLM 전용 endpoint 노출 (전체 공개 X, 부분 공개)
                │
                │ ※ KOSAX가 demonstrate하려는 것 = 이 통로의 reference shape
                ▼
[시민] ──"종합소득세 신고해줘"──► [KOSAX LLM]
                                   │
                                   │ (1) verify primitive
...
**핵심**:
- KOSAX는 **5-primitive (lookup/submit/verify/subscribe + resolve_location) → boundaries → 보안 wrapping 통로 → 기관 내부 API**의 client-side caller
...
- 정책 timing — 국가AX 인프라 발표가 KOSAX-style architecture를 합법화
...
- KOSAX의 `permissions live at adapter layer only` (`docs/requirements/kosax-migration-tree.md § L1-C C5`)와 1:1 mapping

다음 닮은 시스템:
- **Estonia X-Road** (★★★★) — 각 기관 앞 Security Server, PKI signed assertion + non-repudiation. KOSAX Spec 024 audit ledger와 정합
- **EU EUDI Wallet** (★★★) — VC presentation. KOSAX는 wallet 계층 미보유 (학부생 단계 OOS)
- **Japan マイナポータル API** (★★★) — Digital Agency 단일 게이트웨이. third-party app 통합 절차가 KOSAX plugin DX와 유사

### 12.6 한국 현재 상황 — KOSAX의 정확한 빈자리

리서치 결과:

| 인프라 | 운영 | AI agent 확장성 |
|---|---|---|
| **공공데이터포털 data.go.kr** | 행안부 + NIA | ★★★★ — 이미 OpenAPI 키 + REST 게이트웨이 backbone. KOSAX Phase 1 어댑터 운영 중. **AI agent 전용 확장 (rate limit, scope, agent-id audit) 미정** |
| **디지털플랫폼정부 (DPG)** | DPG위원회 | ★★★ — 가이드라인 2.0 (2025-04-16) wrapping·격리·개인정보 분리 권고. **단일 통합 게이트웨이 spec 미공개** |
| **공공마이데이터** | 행안부 + KLID | ★★★ — read 167종 (2025-12 확대). **write extension 미규정** ← KOSAX submit primitive가 정확히 채울 수 있는 빈자리 |
...
**KOSAX의 차별 포인트 (리서치 결론)**: "한국에는 AI agent 단독을 위한 정부 API 게이트웨이가 아직 정형화되지 않음". KOSAX는 client-side harness로 정확히 그 빈자리를 메움.
...
```json
{
  ...
  "_actual_endpoint_when_live": "https://api.gateway.kosax.gov.kr/v1/...",
...
### 12.8 KOSAX positioning — "client-side reference implementation"

리서치 권고 (Phase A R4 결론): KOSAX는 다음 4개 OSS 가시화 채널로 정책 reference 진입 가능:
1. DPG 「공공 AI 서비스 실증 사례집」 등재
2. 공공데이터 활용 경진대회
3. NIA·KISA 보안가이드 reference 부록
4. 행동계획 공공AX 분과 시민의견 channel

학부생 단독 정책 인용 사례는 [research-blocked]이지만 Estonia X-Road · EU EUDI ARF 처럼 **표준 spec과 OSS impl이 동반 진화**하는 국제 패턴이 존재. KOSAX = "client-side reference impl"로 자리잡으면 정부 gateway spec이 정형화될 때 adapter 갈아끼우는 전략이 안전.
...
| **Mock + Live data.go.kr** | 공개된 data.go.kr 어댑터는 Live, 비공개 통로는 Mock | 낮음 — KOSAX Phase 1 이미 운영 중 |
| **Full Live (졸업 후 / partnership)** | 정부 gateway spec 정형화 후 KOSAX adapter를 live 통로에 연결 | 높음 — 학부생 단계 OOS |

**Verdict**: 학부생 단계는 **Mock-only or Mock + Live data.go.kr**. Full Live는 정부 gateway spec 정형화 + KOSAX partnership이 전제.
...
**KOSAX = 한국 국가AX 인프라가 만들 LLM-accessible 보안 wrapping 통로의 client-side reference implementation**.

- 시민 입장: LLM 대화 한 번으로 행정일 처리 (변함없음)
- 기관 입장: 자체적으로 시스템 개편 (국가AI전략위원회 행동계획 동력) — KOSAX는 변경 요구 X, 단지 reference shape 제공
- KOSAX 입장: 5-primitive client + Mock reference + 정부 gateway spec 정형화 시 즉시 adapter swap
...
- ✅ KOSAX는 권한 시스템 발명 X (1차)
- ✅ KOSAX는 새 protocol 발명 X (2차 정정)
- ✅ KOSAX는 **국가AX 인프라가 정형화할 보안 wrapping 통로의 client-side caller** (3차 final)
