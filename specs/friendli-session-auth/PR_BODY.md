...
- Remove KOSAX-only `/agents` swarm UI and SubscribePrimitive from the TUI, LLM, IPC, backend, schema, docs, and test surfaces; active primitives are now `lookup`, `resolve_location`, `submit`, and `verify`.
- Defer `subscribe` until KOSAX has a real app/push-notification runtime instead of fabricating CLI-only notification UI.
...
- `cd tui && NODE_ENV=test KOSAX_FRIENDLI_TOKEN=test-token KOSAX_FRIENDLI_SESSION_ACTIVE=1 bun run src/entrypoints/cli.tsx auth status --json`
