...
## 1. Shared module — SSIS code tables (`src/ummaya/tools/ssis/codes.py`)
...
## 2. NFA 119 — Input schema (`src/ummaya/tools/nfa119/emergency_info_service.py`)
...
## 5. MOHW — Input schema (`src/ummaya/tools/ssis/welfare_eligibility_search.py`)
...
## 8. `TOOL_MIN_AAL` diff (`src/ummaya/security/audit.py`)
...
**Scope (when authored)**: UMMAYA's §26 수탁자 (PIPA) relationship with SSIS
...
for SSIS responses in UMMAYA session logs, and the synthesis-consent gate for
...
```
src/ummaya/security/audit.py            # TOOL_MIN_AAL entries
src/ummaya/tools/register_all.py        # register() calls for both new adapters
```

No other file requires modification. No new runtime dependency. No new
environment variable (the existing `UMMAYA_DATA_GO_KR_API_KEY` is reused).
