...
| **KOSAX files in slice (total)** | **452** | 446 components + 3 screens + 3 top-level + 1 outputStyles + 1 moreright |
...
| **MIGRATE-FOR-SWAP** (CC ≠ KOSAX) | **60** | 57 components + REPL.tsx + interactiveHelpers + replLauncher |
| **PORT** (CC has, KOSAX missing) | **6** | All P2 / DROP-CANDIDATE in spirit — see § 4 |
| **DROP-CANDIDATE** (KOSAX-only) | **64** | All KOSAX-original UI L2 / brand / legitimate features — see § 6 |

**Headline**: This slice is in the **best shape of the entire audit**.
**85%** of files where CC and KOSAX overlap are **byte-identical**.
...
hypothesis assumed. KOSAX components/ has **more** files than CC's
...
(KOSAX = CC + 2 swaps; payments / 1P telemetry are the third thing
correctly removed per `feedback_kosax_scope_cc_plus_two_swaps`).
...
find tui/src/components -type f \( -name '*.ts' -o -name '*.tsx' \) \
  | sed 's|.*/components/||' | sort > kosax.txt
comm -23 cc.txt kosax.txt   # CC-only → PORT candidates
comm -13 cc.txt kosax.txt   # KOSAX-only → DROP candidates
comm -12 cc.txt kosax.txt   # in both → bucket via SHA-256
```
...
in is "every file in `comm -12 cc.txt kosax.txt` whose SHA-256 matches
...
## 4. Bucket B — PORT (6 files in CC, missing in KOSAX)
...
|---|---|---:|---|---|
| 1 | `Feedback.tsx` | — | CC `/feedback` slash-command UX (telemetry submission to anthropic.com) | **DROP-CONFIRMED**. KOSAX has no Anthropic feedback endpoint; we collect feedback via GitHub Issues. |
| 2 | `grove/Grove.tsx` | — | CC's "Grove" growth-experiment shelf | **DROP-CONFIRMED**. Pure 1P growth surface. |
| 3 | `LogoV2/GuestPassesUpsell.tsx` | — | "Guest passes" sales pitch overlay (claude.ai paid plan upsell) | **DROP-CONFIRMED**. KOSAX is open-source / no paid tier. |
...
The CC files import from `@anthropic-ai/sdk/resources/...`; the KOSAX
...
| File | Change |
|---|---|
| `LogoV2/Opus1mMergeNotice.tsx` | "Opus now defaults to 1M context" promo string removed/changed (KOSAX has no Opus). |
| `IdeOnboardingDialog.tsx` | `Welcome to Claude Code for {ideName}` → `Welcome to KOSAX for {ideName}`. |
| `HelpV2/HelpV2.tsx` | `Claude Code v${VERSION}` → `KOSAX v${VERSION}`. |
...
KOSAX-equivalent or feature-flagged off), Korean string injections,
or `sdk-compat` imports. **Justified** but each file should carry a
short `// SWAP:` or `// KOSAX:` comment marker for traceability. Some
...
These are the swap surfaces where KOSAX legitimately diverges, each
...
| `LogoV2/feedConfigs.tsx` | 23 | Brand-feed config (feed slots removed for KOSAX). |
...
| `permissions/ExitPlanModePermissionRequest/ExitPlanModePermissionRequest.tsx` | 51 | Plan-mode permission dialog adapted for KOSAX permission gauntlet (3-tier color, Korean labels). Verify in-file comment. **Action**: confirm. |
...
| `Messages.tsx` | 56 | **Spec 2521 multi-tool layout (`createStreamingThinkingLayoutMessage` + `getStreamingThinkingInsertIndex` + `isSameAssistantToolStack`)** + **Phase 4 `useFrameCommitTracker` OTEL hook** for `kosax.tui.frame_commit` event. Both in-file comments. Justified. |
...
character per `docs/requirements/kosax-migration-tree.md § Mascot`).
Each file carries an SPDX header + KOSAX comment block citing the
...
## 6. Bucket D — DROP-CANDIDATE / KOSAX-only legitimate features (64 files)

These are KOSAX-original UI L2 / brand / spec-driven features added on
...
### 6.6 Prompt-input extensions (2 files) — **KEEP — Spec 287 / kosax-migration-tree UI-B.2 + B.6**

`PromptInput/CtrlOToExpand.tsx` (CC has expand/collapse but as inline
helper; KOSAX extracted to component),
...
`chrome/KosaxCoreIcon.tsx`, `ReplHeader.tsx`, `Spinner/types.ts`.

### 6.8 KOSAX-original infrastructure (11 files) — **KEEP**
...
- `FeedbackSurvey/utils.ts` (KOSAX feedback collection scaffold).
- `mcp/types.ts` (KOSAX MCP type module).
...
| W2 | Brand string substitution | `Claude Code` → `KOSAX` (visible UI text only) | `IdeOnboardingDialog.tsx`, `HelpV2/HelpV2.tsx`, `Welcome*.tsx` |
...
| W4 | Brand glyph | `✻` preserved (per `kosax-migration-tree § Mascot`) | All files using `Asterisk` / `AnimatedAsterisk` |
...
| W6 | Color palette | CC primary blue → KOSAX violet (`#a78bfa` body, `#4c1d95` background) | Token definitions in `tui/src/theme/tokens.ts`; component-level usage threaded via theme |
| W7 | Anthropic 1P-business surfaces removed | `Settings/Usage`, `Passes`, `Feedback`, `LogoV2/*Upsell`, `grove/Grove` (no KOSAX replacement) | 6 PORT-DROPPED files in § 4 |
...
| W12 | OTEL frame_commit instrumentation | `useFrameCommitTracker(conversationId)` hook added (Phase 4) | `Messages.tsx` (and any future `kosax.tui.frame_commit` consumer) |
...
**Verified**: all 8 above exist in KOSAX and either match SHA-256
...
any of them would violate CORE THESIS ("KOSAX = CC + 2 swaps;
...
Set-difference output (reproducible):
  ...
  /tmp/kosax-components.txt      (sorted KOSAX enumeration, 446 entries)
  /tmp/missing-in-kosax.txt      (CC-only, 7 entries)
  /tmp/extra-in-kosax.txt        (KOSAX-only, 64 entries)
