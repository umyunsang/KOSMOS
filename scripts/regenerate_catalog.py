...
"""Regenerate the ummaya-plugin-store/index/index.json catalog.

Walks every `ummaya-plugin-store/ummaya-plugin-<name>` repo via the
...
ORG = "ummaya-plugin-store"
META_REPO = "index"
PLUGIN_PREFIX = "ummaya-plugin-"
...
def _list_plugin_repos() -> list[str]:
    """List every `ummaya-plugin-<name>` repo under the org (excludes index + template)."""
...
def _bundle_sha(repo_name: str) -> str | None:
    """Compute SHA-256 of the main-branch tarball — same shape `ummaya plugin install` will see."""
    import hashlib
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "CI drift gate: read PATH (or ummaya-plugin-store/index/index.json) "
