...
- [ ] T004 Fix environment variable naming in .env.example (UMMAYA_DATA_GO_KR_KEY → UMMAYA_DATA_GO_KR_API_KEY) and add base URL documentation comment
- [ ] T005 [P] Wire PermissionPipeline into QueryEngine.__init__ and QueryContext creation in src/ummaya/engine/engine.py
...
- [ ] T023 Run type checking via `uv run mypy src/ummaya` and linting via `uv run ruff check src/ tests/` — fix any regressions
...
Task: "Wire PermissionPipeline in src/ummaya/engine/engine.py"
```
