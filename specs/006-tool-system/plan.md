...
**Project Type**: Library module within KOSAX (`src/kosax/tools/`)
...
**Post-Phase 1 re-check**: PASS. `GovAPITool.input_schema` and `output_schema` fields hold `type[BaseModel]` references (not `Any`). The `to_openai_tool()` method generates JSON Schema via Pydantic's `model_json_schema()`, which internally uses `Any` in the generated schema dict — this is acceptable because it's an export format, not KOSAX I/O.
...
**Structure Decision**: Single project layout. The tool system is a library module (`kosax.tools`) within the KOSAX package, parallel to `kosax.llm` (Epic #4).
