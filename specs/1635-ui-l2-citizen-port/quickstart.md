...
rm -rf ~/.ummaya/memdir/user/onboarding ~/.ummaya/memdir/user/preferences
...
1. **Preflight** — Bun ≥ 1.2 ✓, terminal graphics protocol detected (Kitty / iTerm2 / none) ✓, `UMMAYA_*` env vars present ✓ (FR-001).
...
5. **Terminal setup** — Citizen toggles, e.g., `large_font = true` (FR-005). The change reflects in the next render frame (≤ 500 ms, SC-011) and `~/.ummaya/memdir/user/preferences/a11y.json` is written.
...
- Tool call proceeds.
- `ummaya.ui.surface=permission_gauntlet` OTEL span attribute is emitted (FR-037).
...
- A PDF is written to `~/Downloads/ummaya-export-<timestamp>.pdf` (or platform default).
