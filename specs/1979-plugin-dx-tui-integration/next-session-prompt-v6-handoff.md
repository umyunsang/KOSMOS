...
- **잔재 deletion**: `src/kosax/permissions/` 14 source file + `steps/` + 4 test file 삭제. Spec 035 receipt set 8 + `models.py` (trimmed) + `credentials.py` 만 잔존 (11 entry).
- **AdapterRealDomainPolicy**: Pydantic v2 frozen 모델 신설 + 19 adapter metadata 마이그레이션.
- **Path B**: Spec 024/025/1636 V1-V6 invariants 를 `policy.citizen_facing_gate` derivation 기반으로 재작성 (신규 `src/kosax/tools/policy_derivation.py` + `AdapterRegistration.@computed_field` backward-compat).
...
- **영구 룰 박제**: 메모리 `feedback_path_b_policy_derivation.md` (★★ KOSAX-invented 권한 cleanup 시 derivation table + computed_field 패턴).
...
5. **KOSAX-invented 권한 cleanup 시 Path B**: 단순 삭제 X / derivation table + computed_field backward-compat 패턴 (메모리 `feedback_path_b_policy_derivation`).
...
- **ζ**: 19 adapter URL 실 검증 + LLM agentic loop end-to-end smoke + `docs/scenarios/` OPAQUE 시나리오 수정 + KOSAX v0.1-beta tag.
