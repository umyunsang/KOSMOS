...
"""Tests for ummaya.cli.themes — theme loading and default fallback."""
...
from ummaya.cli.themes import Theme, get_theme, load_theme
...
class TestLoadTheme:
    def test_returns_default_when_no_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.delenv("UMMAYA_THEME", raising=False)
        monkeypatch.delenv("UMMAYA_CLI_THEME", raising=False)
        theme = load_theme()
        assert theme == get_theme("default")

    def test_respects_ummaya_theme_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("UMMAYA_THEME", "dark")
        monkeypatch.delenv("UMMAYA_CLI_THEME", raising=False)
        theme = load_theme()
        assert theme == get_theme("dark")

    def test_respects_ummaya_cli_theme_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.delenv("UMMAYA_THEME", raising=False)
        monkeypatch.setenv("UMMAYA_CLI_THEME", "light")
        theme = load_theme()
        assert theme == get_theme("light")

    def test_ummaya_theme_takes_precedence(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("UMMAYA_THEME", "dark")
        monkeypatch.setenv("UMMAYA_CLI_THEME", "light")
        theme = load_theme()
        assert theme == get_theme("dark")
    ...
    def test_unknown_env_falls_back_to_default_with_warning(
        ...
        monkeypatch.setenv("UMMAYA_THEME", "ultraviolet")
        monkeypatch.delenv("UMMAYA_CLI_THEME", raising=False)
    ...
    def test_whitespace_stripped(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("UMMAYA_THEME", "  light  ")
        monkeypatch.delenv("UMMAYA_CLI_THEME", raising=False)
        theme = load_theme()
        assert theme == get_theme("light")
