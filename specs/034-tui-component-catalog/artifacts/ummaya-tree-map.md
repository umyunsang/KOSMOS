# UMMAYA Current TUI Tree Map

**Source**: `find tui/src/components -type f \( -name "*.tsx" -o -name "*.ts" \)` on 2026-04-20 at UMMAYA HEAD `34c48f4`.
**Consumer**: T010–T014 (`UMMAYA target` column for PORT/REWRITE rows).
...
## 2 · CC → UMMAYA target-directory mapping (PORT/REWRITE column hint)

| CC family | UMMAYA target parent | Rationale |
|---|---|---|
...
- When a new top-level subdir is needed (e.g., `chrome/`, `dialogs/`, `onboarding/`, `shortcuts/`, `mcp/`), spell the path in the `UMMAYA target` column and note in rationale: `creates new subdir tui/src/components/<X>/ (first PORT into family)`.
...
- **PORT, implementation complete**: the one UMMAYA file that already exists covers CC's core dialog surface. Mark `—` Task sub-issue.
...
IPC hardening lives in `src/ummaya/ipc/` (backend) and TUI-side `src/ipc/`. CC has no `components/ipc/*` family; A is not a likely Owning Epic for any `src/components/*` file. If a row nonetheless cites A (e.g., a component rendering connection status), apply §R3 re-parent.

## 4 · Part D-3 carve-out reminder (FR-033)

`tui/src/components/primitive/*` (18 files listed in §1) are UMMAYA-original citizen-domain primitives from Spec 287. They have **no CC analog** and do **not** appear in the catalog. Reverse-check only: no CC component should have `UMMAYA target = tui/src/components/primitive/<X>.tsx`.
