...
Span attributes follow the Spec 021 ``ummaya.*`` namespace convention:
    - ``ummaya.permission.killswitch.reason`` — the trigger reason
    - ``ummaya.permission.tool_id``           — the adapter that triggered the killswitch
    - ``ummaya.permission.mode``              — session mode at trigger time
    - ``ummaya.permission.correlation_id``   — Spec 032 IPC correlation id
...
def emit_killswitch_span(
    ...
    with _tracer.start_as_current_span(_KILLSWITCH_SPAN_NAME) as span:
        span.set_attribute("ummaya.permission.killswitch.reason", reason)
        span.set_attribute("ummaya.permission.tool_id", tool_id)
        span.set_attribute("ummaya.permission.mode", mode)
        span.set_attribute("ummaya.permission.correlation_id", correlation_id)
