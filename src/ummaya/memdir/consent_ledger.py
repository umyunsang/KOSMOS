...
Storage layout (per Spec 035 + data-model.md § 6):
    ``~/.ummaya/memdir/user/consent/<YYYY-MM-DD>.jsonl``
...
HMAC ledger at ``~/.ummaya/consent_ledger.jsonl`` handles that).
...
_DEFAULT_LEDGER_ROOT: Path = Path.home() / ".ummaya" / "memdir" / "user" / "consent"
...
def append_delegation_issued(
    ...
    *,
    ...
    Args:
        ...
        ledger_root: Override ledger directory (default: ``~/.ummaya/memdir/user/consent``).
...
class FileLedgerReader:
    ...
    Used by ``validate_delegation`` in ``ummaya.primitives.delegation``.
    """
