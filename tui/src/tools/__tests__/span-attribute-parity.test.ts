...
// `ummayaCitations` extension slot on ToolUseContext — that slot is what the
...
//    context, leaves a ummayaCitations slot whose shape matches what an OTEL
...
//   - ummaya.tool.id              ← input.tool_id (fetch mode)
//   - ummaya.tool.mode            ← input.mode ('fetch' or 'search')
//   - ummaya.adapter.real_classification_url  ← citation.real_classification_url
//
...
// real_classification_url from the ummayaCitations slot, which would break both
// the permission UI and any future tool-call span emitter.
//
// Spec 021 reference: ummaya.tool.id + ummaya.tool.mode in Spec 021 attribute schema.
// Epic δ reference: ummaya.adapter.real_classification_url added in c6747dd.
...
// These are the canonical UMMAYA OTEL attribute names (Spec 021 + Epic δ).
...
const OTEL_ATTRIBUTE_PROJECTION = {
  real_classification_url: 'ummaya.adapter.real_classification_url',
  policy_authority: 'ummaya.adapter.policy_authority',
...
describe('OTEL attribute projection — key mapping is byte-stable', () => {
  test('real_classification_url projects to ummaya.adapter.real_classification_url', () => {
    expect(OTEL_ATTRIBUTE_PROJECTION['real_classification_url']).toBe(
      'ummaya.adapter.real_classification_url',
    )
  })

  test('policy_authority projects to ummaya.adapter.policy_authority', () => {
    expect(OTEL_ATTRIBUTE_PROJECTION['policy_authority']).toBe(
      'ummaya.adapter.policy_authority',
    )
  })
...
// ---------------------------------------------------------------------------
// Group 3: validateInput populates ummayaCitations slot (OTEL emitter intent)
...
describe('LookupPrimitive.validateInput — ummayaCitations slot shape', () => {
  test('mode=fetch with valid adapter: result=true and ummayaCitations populated', async () => {
    ...
    const ctxWithCitations = ctx as unknown as { ummayaCitations?: AdapterCitation[] }
    expect(ctxWithCitations.ummayaCitations).toBeDefined()
    expect(Array.isArray(ctxWithCitations.ummayaCitations)).toBe(true)
    expect(ctxWithCitations.ummayaCitations!.length).toBeGreaterThan(0)

    const citation = ctxWithCitations.ummayaCitations![0]!
    // These are the exact keys a OTEL span emitter would read to set
    // ummaya.adapter.real_classification_url and ummaya.adapter.policy_authority.
    expect(typeof citation.real_classification_url).toBe('string')
    expect(citation.real_classification_url.length).toBeGreaterThan(0)
    expect(typeof citation.policy_authority).toBe('string')
    expect(citation.policy_authority.length).toBeGreaterThan(0)
  ...
  test('mode=fetch with unknown tool_id: result=false, errorCode=AdapterNotFound', async () => {
    ...
    const ctxWithCitations = ctx as unknown as { ummayaCitations?: AdapterCitation[] }
    expect(ctxWithCitations.ummayaCitations).toBeUndefined()
  })
...
// ---------------------------------------------------------------------------
// Group 4: ummaya.tool.id and ummaya.tool.mode span attribute intent
...
describe('span attribute source fields — Spec 2521 fetch-only input schema', () => {
  test('input carries tool_id (source of ummaya.tool.id span attribute)', () => {
