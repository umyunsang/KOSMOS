...
**Decision**: `find` + `diff -rq` 조합으로 두 디렉토리 (`tui/src/`, `.references/claude-code-sourcemap/restored-src/src/`) 의 매칭/비매칭 파일을 산출하고 `sort` 로 deterministic 순서 강제. 결과 list 를 `data/enumerated-{keep-1531|import-73|modified-212|ummaya-only-274|cc-only-68}.txt` 로 박제. 이 산출 단계가 FR-010 의 "drift 발견 시 정정" 요구를 자동 충족.

**Rationale**:
- `diff -rq` 는 텍스트/바이너리 무관 byte 비교만 보고 (not line-level). 이 단계에서 `Files differ` 행이 modified, `Only in restored-src` 가 cc-only DELETE, `Only in tui/src/` 가 UMMAYA-only ADDITIONS 후보.
...
**Rationale**:
- TypeScript `import` / `re-export` 문법 한정 패턴. SDK swap (`@anthropic-ai/...` → `@ummaya/...`) 은 import line 1-2 줄에 국한된다는 cc-source-scope-audit § 1.1 가설을 직접 검증.
...
|---|---|---|---|
| **(a) 디렉토리 패턴** | `i18n/`, `ipc/`, `theme/`, `observability/`, `ssh/` (UMMAYA 인프라) — KEEP 후보가 modified 로 잘못 분류된 경우 reclassify | `services/api/claude.ts`, `services/api/sonnet.ts`, `services/api/anthropic.ts` (Spec 1633 잔재) | 기타 — 추가 검토 필요 |
| **(b) Git history** | `git log --follow` 첫 커밋이 UMMAYA spec id (`feat/NNNN-...`) 매핑 가능 | `Spec 1633` 기간 (2026-04-23 ~ 2026-04-28) 내 마지막 변경 + 후속 cleanup 미완 | author/메시지에서 추적 불가 (예: 단순 `chore` / `refactor` 만) |
| **(c) Import scan** | `@ummaya/...` import / 한국어 i18n key / EXAONE 모델 ID 등 UMMAYA-only 토큰 존재 | `claude.ts` / `Anthropic` / `claude-code-style` 잔재 import | UMMAYA 도 CC 도 아닌 외부 의존 또는 dead-code 감지 |

자동 분류 알고리즘 (`scripts/classify-modified.py`):
1. (a) 디렉토리 매칭 — UMMAYA 인프라 디렉토리 매칭이면 Legitimate, Spec 1633 알려진 잔재 path 매칭이면 Cleanup-needed.
...
**Rationale**:
- UMMAYA 의 known cleanup 잔재 path 와 토큰 list (`docs/spec-1633-status` + `MEMORY.md § project_tui_anthropic_residue` + `project_frame_schema_dead_arms`) 가 이미 grounded-truth 제공 → 휴리스틱 base 무난.
- spec history 매핑은 UMMAYA 의 `feat/NNNN-...` branch 컨벤션 덕에 grep 으로 충분.
...
R-1~R-6 모두 신규 의존성 0 건, source 변경 0 라인, UMMAYA-invented permission classification 도입 0 건. Constitution 전 항목 PASS 유지. Phase 1 진입 가능.
