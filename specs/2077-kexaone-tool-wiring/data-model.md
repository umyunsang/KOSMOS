...
type ToolResultBlockParam = {
  ...
  is_error?: boolean              // optional flag; backend signals via envelope.kind === 'error' for UMMAYA
}
```
...
| `envelope.kind === 'error'` | `is_error: true` | UMMAYA convention |
...
- `timeoutMs` reads `UMMAYA_PERMISSION_TIMEOUT_SEC * 1000` (Spec 033 default 300).
...
**Source of truth**: Python helper `ummaya.llm.system_prompt_builder.build_system_prompt_with_tools()`.
...
- The Spec 026 `ummaya.prompt.hash` OTEL span attribute hashes the **base** prompt (`prompts/system_v1.md`) — the augmentation is excluded from the hash so changing the inventory does not invalidate prompt-cache reuse.
