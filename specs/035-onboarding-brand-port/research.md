...
- **Decision**:
  - Consent records at `~/.kosax/memdir/user/consent/<timestamp>-<session_id>.json`.
  ...
  - Ministry-scope records at `~/.kosax/memdir/user/ministry-scope/<timestamp>-<session_id>.json`.
...
- **Rationale**: Spec 027 § 4 established the `~/.kosax/mailbox/` precedent for Python-backend POSIX filesystem state; extending under `~/.kosax/memdir/user/` keeps the state-root coherent. Append-only is required by Constitution Principle II (audit log) + vision.md § Permission pipeline Step 7 ("every call is logged"). UUIDv7 is already used by Spec 032 for `correlation_id` generation — `uuid.uuid7()` is a 3.12 stdlib function, no new dep. The `consent/` and `ministry-scope/` subdirectories are new but fall under the existing memdir umbrella.
- **Alternatives considered**:
  - Single overwriting file (`~/.kosax/memdir/user/consent.json`) — rejected: violates audit-log invariant; a mistaken re-consent could erase evidence of the prior decision.
  - SQLite database (`~/.kosax/memdir/user.db`) — rejected: new runtime dep (`sqlite3` is stdlib, but a schema migration story is required; violates the "files only" convention of Spec 027).
...
- **Rationale**: Spec 033 merged 2026-04 (PR #1441, Epic M precursor) and is the normative source for permission + authentication assurance levels in KOSAX. Duplicating an enum here would violate DRY and create drift. The `aalGate` field in `PIPAConsentRecord` is a snapshot of the citizen's AAL at consent time; subsequent AAL upgrades (e.g., identity-verification completion) trigger new consent records per R-4.
- **Alternatives considered**:
...
- **Decision**: Partial PORT. From `.references/claude-code-sourcemap/restored-src/src/utils/logoV2Utils.ts`:
  ...
  - **REWRITE** (CC-specific content binding): `getRecentActivitySync` → `getKosaxSessionHistorySync` (reads `~/.kosax/memdir/session/` via Spec 027 infrastructure); `getRecentReleaseNotesSync` → `getMinistryAvailabilitySync` (reads Spec 022 adapter registry snapshot).
  - **DISCARD** (CC brand identifier): `getLogoDisplayData` — embeds Anthropic Clawd branding; LogoV2 KOSAX-REWRITE inlines its own display data.
- **Rationale**: Component-catalog PORT-first principle — lift verbatim when the logic is content-agnostic; REWRITE when content binds to CC-specific data sources. The 5 PORT-candidates are pure geometry / string utilities; the 2 REWRITE targets are thin adapters over KOSAX data sources. `getLogoDisplayData` is the only DISCARD because it hardcodes CC wordmark + Clawd poses.
- **Alternatives considered**:
...
- **Decision**: Create `tui/src/hooks/useReducedMotion.ts` (new hook) that reads `process.env.NO_COLOR` and `process.env.KOSAX_REDUCED_MOTION` at hook-init time and returns a boolean `prefersReducedMotion`. The hook returns a stable value for the session lifetime (env flags are read once at mount, per Ink convention). Consumed by:
...
- **Decision**: Single-line edit to `docs/design/brand-system.md § 1 Permanent cross-references` — replace the bullet `../../assets/kosax-logo-dark.svg — logo optimised for the KOSAX navy dark background; cited directly in ADR-006 A-9 as the onboarding splash source` with `../../assets/kosax-banner-dark.svg — wide wordmark + subtitle on dark background; canonical palette extraction source per ADR-006 A-9` (the existing next-line entry already names this file; the replacement collapses the two entries into one accurate line). Bundle in Epic H PR (FR-027 § 3 authoring already touches § 1 ministry roster — low-conflict edit).
- **Rationale**: Filesystem verification (`ls /Users/um-yunsang/KOSAX/assets/`) confirms only `kosax-logo.svg`, `kosax-banner-dark.svg`, `kosax-banner-light.svg`, `kosax-logo.png`, `kosax-banner-dark.png`, `kosax-banner-light.png`, `kosax-org-avatar.svg`, `kosax-org-avatar.png` exist. ADR-006 A-9 line "`assets/kosax-logo-dark.svg` / icon component equivalent" indicates the SVG was planned but the "icon component equivalent" (KosaxCoreIcon per FR-023) was accepted as the delivered form. TUI does not render SVG — the hex palette is the only consumed output.
- **Alternatives considered**:
  - Author `kosax-logo-dark.svg` from scratch — rejected: redundant (TUI does not render SVG); would fork the `kosax-logo.svg` file with an identical palette.
...
3. Audio / voice feedback — no audio subsystem in KOSAX.
...
| 11 | `brand-system.md § 1` doc-fix (kosax-logo-dark.svg ref) | Epic H PR (this spec) | In-scope (bundled) | Resolved in R-10 above |
