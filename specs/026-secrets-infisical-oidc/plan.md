# Implementation Plan: Secrets & Config — Infisical OIDC + 12-Factor + UMMAYA_* Registry
...
Close Epic #468 by (a) **replacing long-lived GitHub Encrypted Secrets** with Infisical Cloud Free via `Infisical/secrets-action@v1` + GitHub Actions OIDC federation in `.github/workflows/ci.yml`, (b) **publishing the canonical `UMMAYA_*` env-var registry** at `docs/configuration.md` covering all 17 variables currently consumed by `src/`, (c) **adding a fail-fast startup guard** at `src/ummaya/config/guard.py` wired into the CLI entry point that exits <100 ms with a single-line remediation message, and (d) **eliminating drift** via `scripts/audit-secrets.sh` (workflow-grep gate) + `scripts/audit-env-registry.py` (registry ↔ code cross-check). Regression-guards #458. No new runtime dependencies (AGENTS.md hard rule) — guard uses stdlib only; registry/audit scripts use stdlib + pydantic-settings that are already in the dep graph.

Reference anchor: Claude Code's **permission gauntlet** — `src/ummaya/config/guard.py` mirrors Claude Code's discipline of running a fail-fast boundary validator **before** any network-bound tool loop begins. Constitution Principle I maps "Permission Pipeline → `src/ummaya/permissions/` (Claude Code)"; this Epic extends that doctrine to the *startup* phase.
...
**Project Type**: Library + CLI (`ummaya.cli`). Guard is a library function invoked by CLI entry point.
...
- "Shell wins over `.env`" invariant preserved from `src/ummaya/_dotenv.py:40` (FR-041).
- Forbidden file surface: `.env`, `docker/`, `.devcontainer/`, `prompts/`, `src/ummaya/safety/`, `.github/workflows/docker.yml`, `.github/workflows/shadow-eval.yml`.
- Korean domain data only; all spec prose and source code in English (AGENTS.md hard rule).
**Scale/Scope**: 17 UMMAYA_* variables + 2 LANGFUSE_* (conditional-required, #501-owned) + 6 future LiteLLM variables (catalogued schema-extensibly for #465). Registry grows by ≤ 2 vars/quarter at current velocity.
...
| **III. Stdlib logging / no print()** | ✅ PASS | Guard emits to `sys.stderr` via the CLI boundary (FR-003); library code in `ummaya.config.guard` returns structured diagnostics, never `print()`s. |
| **IV. Pydantic v2, no `Any`** | ✅ PASS | Existing `UmmayaSettings`/`LLMClientConfig`/`CLIConfig` preserved unchanged. Guard consumes typed `list[RequiredVar]` dataclasses — no `Any` anywhere. |
...
- ✅ No `.env` write path.
- ✅ `UMMAYA_` prefix enforced; `LANGFUSE_*` sole documented exception.
...
```text
src/ummaya/config/               # NEW — fail-fast startup guard package
├── __init__.py
└── guard.py                     # verify_startup() + required-names registry

src/ummaya/cli/app.py            # EDIT — wire guard into main() between
...
docs/configuration.md            # NEW — human-readable UMMAYA_* registry
...
1. **Guard invocation site**: `src/ummaya/cli/app.py:main()`, *after* `load_repo_dotenv()` and *before* `setup_tracing()`. Guard must see the loaded env; tracing must not spin up before the guard validates config. Cite: `_dotenv.py` docstring + Claude Code permission-pipeline doctrine.
2. **Required-name source of truth**: In-code Python list (`_REQUIRED_VARS` in `ummaya.config.guard`). Registry markdown is the human-facing mirror; the code is machine truth. `scripts/audit-env-registry.py` reconciles them.
3. **Activation flag**: `UMMAYA_ENV ∈ {dev, ci, prod}`; unknown values fall through to `dev`. Cite: 12-Factor App §III "strict separation".
4. **Infisical OIDC trust pinning**: `repository=umyunsang/UMMAYA` + `workflow=.github/workflows/ci.yml`; `ref` **not** pinned (required for PR-CI). Cite: Infisical GitHub Actions + OIDC docs.
5. **Audit parsing**: Regex `UMMAYA_[A-Z_]+` over `*.py` / `*.yml` / `*.md`; markdown table parse for the registry section. Stdlib-only (`re`, `argparse`, `pathlib`).
6. **`.env.example` format**: Dotenv (`UMMAYA_X=<redacted>`), matches `_dotenv.py` parser semantics. No `export` prefix.
...
1. **T001–T002**: Skeleton package — `src/ummaya/config/__init__.py`, `tests/config/__init__.py`.
2. **T003**: `tests/config/test_guard.py` — write failing tests first for FR-001..008 + NFR-001 (100 ms budget).
3. **T004**: Implement `src/ummaya/config/guard.py` to satisfy T003.
4. **T005**: Wire guard into `src/ummaya/cli/app.py:main()` + dedicated test verifying invocation order.
...
9. **T010**: `.github/workflows/ci.yml` — Infisical action migration + typo fix (`UMMAYA_DATA_GO_KR_KEY` → `_API_KEY`).
10. **T011**: `docs/design/mvp-tools.md:642` — one-line typo fix (`UMMAYA_KAKAO_REST_KEY` → `UMMAYA_KAKAO_API_KEY`).
