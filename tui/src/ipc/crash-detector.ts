...
// UMMAYA-original — no upstream analog (Claude Code uses HTTP; no child-process crash surface).
//
...
//   Any UMMAYA_* env var whose name ends in _KEY, _SECRET, _TOKEN, or _PASSWORD
...
/** Regex matching UMMAYA_* env var keys that look like secrets. */
const SECRET_KEY_RE = /^UMMAYA_[A-Z0-9_]+(?:_KEY|_SECRET|_TOKEN|_PASSWORD)$/

/**
 * Return a copy of *text* with the values of any UMMAYA_*_KEY/SECRET/TOKEN/PASSWORD
 ...
 */
export function redactUmmayaSecrets(text: string): string {
  let result = text
 ...
 * 3. Redacts UMMAYA_*_KEY/SECRET/TOKEN/PASSWORD values from the tail.
...
export function startCrashDetector(
  ...
  if (stderrStream instanceof ReadableStream) {
    // UMMAYA Epic #2077 diagnostics — when UMMAYA_BACKEND_STDERR_FILE is
    ...
    const teePath = process.env.UMMAYA_BACKEND_STDERR_FILE
  ...
  ;(async () => {
    ...
    const redacted = redactUmmayaSecrets(raw)
