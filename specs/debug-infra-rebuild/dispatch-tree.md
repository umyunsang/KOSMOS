...
4. Document the env switch: `KOSAX_FRIENDLI_BASE_URL=http://localhost:4010`
   + `KOSAX_FRIENDLI_TOKEN=aimock-test`.
...
1. Trace the call path from the new `<available_adapters>` suffix
   (in `src/kosax/ipc/stdio.py:_build_available_adapters_suffix`)
...
2. Reproduce the K-EXAONE call with a httpx probe that mirrors KOSAX's
   exact payload (use the prior probe at `/tmp/kosax-payload-probe.py`
   as the seed). Capture the actual `tool_calls[0].function.arguments`
   K-EXAONE returns and compare to the schema KOSAX sent.
