...
1. **Given** the agent has just started a session, **When** the citizen asks "강남구 근처 24시간 응급실 알려주세요", **Then** the agent invokes one of the registered UMMAYA tools (e.g., a hospital-search lookup), receives a tool result envelope, and emits a final assistant message that incorporates that result.
...
- The five primitive surface (`lookup`, `resolve_location`, `submit`, `subscribe`, `verify`) plus the MVP-7 auxiliary tools are stable and continue to be the only LLM-visible roots, per `docs/requirements/ummaya-migration-tree.md § L1-C.C6/C7`.
- The platform's existing reference implementation of Claude Code's streaming + agentic loop (`src/ummaya/llm/_cc_reference/claude.ts`) is the canonical migration source for any unclear behavioral question, per `feedback_cc_source_migration_pattern`.
...
- Korean is the primary language for citizen-facing copy; English is the fallback per `docs/requirements/ummaya-migration-tree.md § UI-A.A.3`.

## Scope Boundaries & Deferred Items *(mandatory)*

### Out of Scope (Permanent)

- **Composite / macro tool combinations** — explicitly removed by `docs/requirements/ummaya-migration-tree.md § L1-B.B6`. The model chains primitives instead of invoking platform-side macros.
