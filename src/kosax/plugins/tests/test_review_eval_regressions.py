...
* **CLI subprocess smoke**: ``kosax-plugin-validate`` must work when
...
from kosax.plugins.checks.framework import CheckContext, run_all_checks
from kosax.plugins.checks.q10_tests import check_no_live_in_ci
from kosax.plugins.installer import _allocate_consent_position, _safe_extract
...
class TestRegistryModulePathResolution:
    ...
    which broke the canonical scaffold layout shipped by `kosax plugin
    init` (`plugin_<id>/adapter.py`).
    """

    def test_dotted_module_path_resolves_to_nested_file(self, tmp_path: Path) -> None:
        from kosax.plugins.registry import _import_adapter_module
...
# ---------------------------------------------------------------------------
# CLI subprocess smoke — kosax-plugin-validate works as installed script.
# ---------------------------------------------------------------------------
...
class TestCliSubprocessSmoke:
    def test_help_returns_zero(self) -> None:
        ...
        """
        from kosax.plugins.checks.framework import _cli_main
...
class TestInstallPluginHttpxErrorEnvelope:
    ...
    def test_catalog_fetcher_raising_arbitrary_exc_returns_exit_one(
        ...
        from kosax.plugins.installer import install_plugin
        from kosax.tools.executor import ToolExecutor
        from kosax.tools.registry import ToolRegistry

        # Isolate filesystem state.
        monkeypatch.setattr(
            "kosax.plugins.installer.settings.plugin_install_root",
            tmp_path / "install",
        )
        monkeypatch.setattr(
            "kosax.plugins.installer.settings.plugin_bundle_cache",
            tmp_path / "cache",
        )
        monkeypatch.setattr(
