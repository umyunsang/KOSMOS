...
8. Q9-OTEL-ATTR missing (otel_attributes['ummaya.plugin.id'] mismatch)
...
def scaffold(tmp_path: Path) -> Path:
    ...
    padded = (
        ...
        + "https://github.com/umyunsang/UMMAYA/blob/main/docs/plugins/permission-tier.md "
        + "참고. 추가 검증은 Spec 033 permission gauntlet 와 연계되며, "
        + "Layer 1 어댑터는 OTEL span 의 ummaya.permission.layer=1 attribute "
        + "와 함께 emit 됩니다.\n"
    )
    if len(padded) < 500:
...
class TestValidScaffoldScores50of50:
    ...
    def test_mock_tier_passes_q7_mock_invariants(self, scaffold: Path, tmp_path: Path) -> None:
        ...
        """
        from ummaya.plugins import cli_init
...
class TestNegativeCases:
    ...
    def test_q9_otel_attr_missing(self, scaffold: Path) -> None:
        manifest = _load_manifest(scaffold)
        manifest["otel_attributes"] = {"ummaya.plugin.id": "wrong_id"}
        _save_manifest(scaffold, manifest)
...
class TestFrameworkSanity:
    def test_yaml_has_50_rows(self) -> None:
        from ummaya.plugins.checks.framework import load_checklist_rows

        rows = load_checklist_rows(_YAML_PATH)
        assert len(rows) == 50

    def test_every_check_implementation_resolves(self) -> None:
        from ummaya.plugins.checks.framework import (
            load_checklist_rows,
            resolve_check,
        )
