...
// `kosaxCitations` extension slot on ToolUseContext — that slot is what the
...
//    context, leaves a kosaxCitations slot whose shape matches what an OTEL
...
//   - kosax.tool.id              ← input.tool_id (fetch mode)
//   - kosax.tool.mode            ← input.mode ('fetch' or 'search')
//   - kosax.adapter.real_classification_url  ← citation.real_classification_url
//
...
// real_classification_url from the kosaxCitations slot, which would break both
// the permission UI and any future tool-call span emitter.
//
// Spec 021 reference: kosax.tool.id + kosax.tool.mode in Spec 021 attribute schema.
// Epic δ reference: kosax.adapter.real_classification_url added in c6747dd.
...
// These are the canonical KOSAX OTEL attribute names (Spec 021 + Epic δ).
...
const OTEL_ATTRIBUTE_PROJECTION = {
  real_classification_url: 'kosax.adapter.real_classification_url',
  policy_authority: 'kosax.adapter.policy_authority',
...
describe('OTEL attribute projection — key mapping is byte-stable', () => {
  test('real_classification_url projects to kosax.adapter.real_classification_url', () => {
    expect(OTEL_ATTRIBUTE_PROJECTION['real_classification_url']).toBe(
      'kosax.adapter.real_classification_url',
    )
  })

  test('policy_authority projects to kosax.adapter.policy_authority', () => {
    expect(OTEL_ATTRIBUTE_PROJECTION['policy_authority']).toBe(
      'kosax.adapter.policy_authority',
    )
  })
...
// ---------------------------------------------------------------------------
// Group 3: validateInput populates kosaxCitations slot (OTEL emitter intent)
...
describe('LookupPrimitive.validateInput — kosaxCitations slot shape', () => {
  test('mode=fetch with valid adapter: result=true and kosaxCitations populated', async () => {
    ...
    const ctxWithCitations = ctx as unknown as { kosaxCitations?: AdapterCitation[] }
    expect(ctxWithCitations.kosaxCitations).toBeDefined()
    expect(Array.isArray(ctxWithCitations.kosaxCitations)).toBe(true)
    expect(ctxWithCitations.kosaxCitations!.length).toBeGreaterThan(0)

    const citation = ctxWithCitations.kosaxCitations![0]!
    // These are the exact keys a OTEL span emitter would read to set
    // kosax.adapter.real_classification_url and kosax.adapter.policy_authority.
    expect(typeof citation.real_classification_url).toBe('string')
    expect(citation.real_classification_url.length).toBeGreaterThan(0)
    expect(typeof citation.policy_authority).toBe('string')
    expect(citation.policy_authority.length).toBeGreaterThan(0)
  ...
  test('mode=fetch with unknown tool_id: result=false, errorCode=AdapterNotFound', async () => {
    ...
    const ctxWithCitations = ctx as unknown as { kosaxCitations?: AdapterCitation[] }
    expect(ctxWithCitations.kosaxCitations).toBeUndefined()
  })
...
// ---------------------------------------------------------------------------
// Group 4: kosax.tool.id and kosax.tool.mode span attribute intent
...
describe('span attribute source fields — Spec 2521 fetch-only input schema', () => {
  test('input carries tool_id (source of kosax.tool.id span attribute)', () => {
