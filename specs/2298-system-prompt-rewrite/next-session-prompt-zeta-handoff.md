...
✅ **Layer 2** — `src/kosax/tools/mvp_surface.py` active GovAPITool surface (resolve_location + lookup + verify + submit) `is_core=True`. `_VerifyInputForLLM` / `_SubmitInputForLLM` envelope.
...
- `src/kosax/ipc/stdio.py:_handle_user_input_llm` — K-EXAONE function_call → IPC `tool_call` frame → primitive sub-dispatcher → `tool_result` frame 흐름 구축
...
- **policy-mapping.md**: KOSAX adapter ↔ Singapore APEX / Estonia X-Road / EU EUDI / Japan マイナポータル.
...
git worktree add ../KOSAX-w-2297 -b 2297-zeta-e2e-smoke
cd ../KOSAX-w-2297
