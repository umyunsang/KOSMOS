...
Attribute namespace (Spec 021 ummaya.* convention):
    ``ummaya.permission.mode``           — the PermissionMode string
    ``ummaya.permission.decision``       — "granted" or "denied"
    ``ummaya.consent.receipt_id``        — consent_receipt_id (when available)
...
Reference:
    ...
    src/ummaya/observability/semconv.py (UMMAYA attribute namespace)
    src/ummaya/permissions/models.py ConsentDecision, ToolPermissionContext
"""
...
from ummaya.permissions.models import ConsentDecision, ToolPermissionContext

__all__ = [
    "UMMAYA_PERMISSION_MODE",
    "UMMAYA_PERMISSION_DECISION",
    "UMMAYA_CONSENT_RECEIPT_ID",
...
# ---------------------------------------------------------------------------
# Attribute-name constants (ummaya.permission.* namespace — Spec 033 FR-F03)
# ---------------------------------------------------------------------------

UMMAYA_PERMISSION_MODE: str = "ummaya.permission.mode"
...
UMMAYA_PERMISSION_DECISION: str = "ummaya.permission.decision"
...
UMMAYA_CONSENT_RECEIPT_ID: str = "ummaya.consent.receipt_id"
...
def enrich_tool_call_span(
    ...
    Attributes set:
    - ``ummaya.permission.mode``         — always set from ``ctx.mode``
    - ``ummaya.permission.decision``     — "granted" / "denied" / "not_required"
    - ``ummaya.consent.receipt_id``      — set only when consent is granted
    ...
    # FR-F03 — ummaya.permission.mode (always emitted)
    span.set_attribute(UMMAYA_PERMISSION_MODE, ctx.mode)

    # FR-F03 — ummaya.permission.decision
    if consent is None:
    ...
    span.set_attribute(UMMAYA_PERMISSION_DECISION, decision_value)

    # FR-F03 — ummaya.consent.receipt_id (only when consent was actively granted)
    if consent is not None and consent.granted:
        receipt_id = consent.action_digest
        if receipt_id:
            span.set_attribute(UMMAYA_CONSENT_RECEIPT_ID, receipt_id)
        else:
            _logger.warning(
                "otel_integration.missing_receipt_id: tool_id=%s "
                "consent.granted=True but action_digest is empty; "
                "ummaya.consent.receipt_id not set on span.",
                ctx.tool_id,
            )
