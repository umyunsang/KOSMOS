...
**Why this priority**: The Epic #2112 smoke run showed the assistant saying "현재 `/Users/um-yunsang/KOSAX/tui` 디렉토리에서 작업 중이며…" in reply to a citizen weather query. That single failure mode breaks every other improvement: the citizen frame collapses the moment the assistant talks about cwd or git. This story is the gating cleanup for Stories 1 and 2 to work in production.
...
**Why this priority**: The harness already emits a `kosax.prompt.hash` OTEL attribute (Spec 026) precisely so the team can observe cache health. Two consecutive turns with different hashes means the prompt is being recomputed per turn — wasted tokens and broken cache economics.

**Independent Test**: Run a TUI session, send two messages, capture the OTEL spans for both turns, and assert that `kosax.prompt.hash` from turn 1 equals turn 2.

**Acceptance Scenarios**:

1. **Given** the same session and the same set of registered tools, **When** the citizen sends a second message, **Then** the `kosax.prompt.hash` for turn 2 equals turn 1.
...
- **FR-007**: System MUST hash only the static prefix into the OTEL `kosax.prompt.hash` attribute so observability of cache prefix stability is meaningful.
...
- **FR-012**: System MUST be expressed entirely in source-tree files that live under `prompts/` and `src/kosax/llm/` for the backend portion and under `tui/src/` for the citizen-input wrapping portion, with no new runtime dependencies in `pyproject.toml` or `package.json`.
...
- **SC-3**: The OTEL `kosax.prompt.hash` attribute is byte-stable across two consecutive turns of the same session with an unchanged registered tool set. Verifiable by capturing two spans and asserting equality.
...
- **Plugin developer experience** — Owned by Epic #1636 (P5 plugin DX). This Epic does not change `kosax plugin init`, `kosax plugin install`, the SLSA verification path, or the plugin manifest schema.
