...
* SC-007 — every plugin invocation emits a span with ummaya.plugin.id.
...
from ummaya.plugins.installer import (
    CatalogEntry,
    CatalogIndex,
    CatalogVersion,
    install_plugin,
...
def _manifest_dict() -> dict[str, Any]:
    ...
    return {
        ...
        "dpa_reference": None,
        "slsa_provenance_url": (
            "https://github.com/ummaya-plugin-store/ummaya-plugin-timing-demo/"
            "releases/download/v1.0.0/timing.intoto.jsonl"
        ),
        "otel_attributes": {"ummaya.plugin.id": "timing_demo"},
...
def isolated_settings(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr("ummaya.plugins.installer.settings.plugin_install_root", install_root)
    monkeypatch.setattr("ummaya.plugins.installer.settings.plugin_bundle_cache", bundle_cache)
    monkeypatch.setattr("ummaya.plugins.installer.settings.plugin_slsa_skip", True)
    monkeypatch.setattr(
        "ummaya.plugins.installer.settings.user_memdir_root",
        user_memdir,
    )
    return tmp_path
...
# ---------------------------------------------------------------------------
# SC-007 — ummaya.plugin.id span attribute.
# ---------------------------------------------------------------------------
...
class TestOtelEmissionSC007:
    def test_install_span_carries_plugin_id(
        ...
        # The contract under test (ummaya.plugin.install span carries
        # ummaya.plugin.id) requires real emission, so we re-enable the
        ...
        local_tracer = provider.get_tracer("ummaya.plugins.registry")
        monkeypatch.setattr("ummaya.plugins.registry._tracer", local_tracer)
        try:
            result = install_plugin(
                "timing-demo",
                registry=registry,
        ...
        assert attrs.get("ummaya.plugin.tier") == "live"
...
class TestAutoDiscoverBootCostSC010:
    def test_auto_discover_under_200ms_per_plugin(
        ...
        from ummaya.plugins.registry import auto_discover
