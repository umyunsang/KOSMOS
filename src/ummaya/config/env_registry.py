...
"""TUI operational configuration loaded from UMMAYA_TUI_* environment variables.
...
script (`scripts/audit-env-registry.py`) treats every ``UMMAYA_TUI_*`` reference
found in ``src/`` as registered and does not flag them as unregistered leaked values.

Usage::

    from ummaya.config.env_registry import tui_settings
...
class TUISettings(BaseSettings):
    ...
    """Active theme for the Ink terminal UI (UMMAYA_TUI_THEME)."""
    ...
    """Log verbosity for IPC bridge frames (UMMAYA_TUI_LOG_LEVEL)."""
    ...
    """Korean IME input strategy (UMMAYA_TUI_IME_STRATEGY)."""
    ...
    """Soak-test IPC event rate in events/second (UMMAYA_TUI_SOAK_EVENTS_PER_SEC)."""
