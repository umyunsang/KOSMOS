...
- `PluginOpFrame` (Spec 1636 + Spec 032) — already at `src/kosax/ipc/frame_schema.py:780-936`
- `PluginManifest` (Spec 1636) — already at `src/kosax/plugins/manifest_schema.py`
- `PluginConsentReceipt` (Spec 1636) — already at `src/kosax/plugins/installer.py:153-181`
- `CatalogIndex` / `CatalogEntry` / `CatalogVersion` (Spec 1636) — already at `src/kosax/plugins/installer.py:91-134`
...
**Module**: `src/kosax/ipc/plugin_op_dispatcher.py` (NEW)
...
**OTEL**: Each handler opens a child span `kosax.plugin.<install|uninstall|list>` under the existing `kosax.session` root. `kosax.plugin.id` attribute populated from `frame.name` (install/uninstall) or absent (list).

---

## E2 — `IPCConsentBridge` (backend-side adapter)

**Module**: `src/kosax/plugins/consent_bridge.py` (NEW)
...
**OTEL**: Inherits the dispatcher's `kosax.plugin.install` span. Adds attribute `kosax.permission.decision` ∈ {`allow_once`, `allow_session`, `deny`, `timeout`} on completion.

---

## E3 — `uninstall_plugin` function

**Module**: `src/kosax/plugins/uninstall.py` (NEW)
...
**State**: Reads `~/.kosax/memdir/user/plugins/<plugin_id>/` (Spec 1636 install_root). Writes `~/.kosax/memdir/user/consent/<receipt_id>.json` (Spec 1636 + Spec 035 ledger).

**OTEL**: Span `kosax.plugin.uninstall` carrying `kosax.plugin.id` attribute.

---

## E4 — `ToolRegistry._inactive` shadow set

**Module**: `src/kosax/tools/registry.py` (MODIFIED)
...
export async function executePlugins(
  ...
  // Phase E.1: emit kosax.ui.surface=plugins (FR-037 — preserved)
...
**OTEL**: TUI does not emit OTEL directly; the round-trip's `kosax.plugin.list` span lives on the backend (E1.handle_list).
...
- **I-1** (FR-005, ✓): A failed install leaves zero state under `~/.kosax/memdir/user/plugins/` and `~/.kosax/memdir/user/consent/`.
