...
"""Tool-specific credential resolution for KOSAX government API adapters.
...
- :mod:`kosax.permissions.steps.step1_config` to check that the credential
  required by the tool's :class:`~kosax.permissions.models.AccessTier` is
  actually configured *for that tool's provider* (rather than accepting any
  ``KOSAX_*_API_KEY``).
- :mod:`kosax.recovery.auth_refresh` to resolve the correct credential when
  refreshing after an HTTP 401.

Lookup order for a given tool id:

1. ``KOSAX_<TOOL_ID_UPPER>_API_KEY`` — tool-specific override.
2. ``KOSAX_KAKAO_API_KEY`` for Kakao-backed tools, or
   ``KOSAX_DATA_GO_KR_API_KEY`` for data.go.kr-backed tools.
3. ``KOSAX_API_KEY`` — legacy global fallback.
...
class CredentialProvider(StrEnum):
    ...
    kakao = "kakao"
    """Kakao Local API; credential is ``KOSAX_KAKAO_API_KEY``."""
...
_PROVIDER_ENV_VAR: dict[CredentialProvider, str] = {
    CredentialProvider.kakao: "KOSAX_KAKAO_API_KEY",
    CredentialProvider.data_go_kr: "KOSAX_DATA_GO_KR_API_KEY",
}
...
_GLOBAL_KEY_VAR: str = "KOSAX_API_KEY"


def _tool_specific_var(tool_id: str) -> str:
    """Return the canonical per-tool env var name.

    ``koroad_accident_search`` → ``KOSAX_KOROAD_ACCIDENT_SEARCH_API_KEY``.
    """
    return f"KOSAX_{tool_id.upper()}_API_KEY"
...
def resolve_credential(tool_id: str) -> str | None:
    ...
    1. ``KOSAX_<TOOL_ID_UPPER>_API_KEY`` (per-tool override).
    2. Provider-specific env var (e.g. ``KOSAX_KAKAO_API_KEY``).
    3. ``KOSAX_API_KEY`` (legacy global fallback).
    """
