...
class QueryState:
    ...
    usage: UsageTracker
    """Token budget tracker (reused from ummaya.llm.usage)."""

    resolved_tasks: list[str] = []
    """Completed civil-affairs sub-goals tracked for session continuity."""
```

**Relationships**: Owned by `QueryEngine`. Passed by reference to `QueryContext` each turn. References `ChatMessage` (from `ummaya.llm.models`) and `UsageTracker` (from `ummaya.llm.usage`).
...
class QueryEvent(BaseModel):
    ...
    """Structured result from tool execution (reused from ummaya.tools.models)."""

    # --- usage_update fields ---
    usage: TokenUsage | None = None
    """Token usage snapshot after an LLM call (reused from ummaya.llm.models)."""
...
**Relationships**: References `ToolResult` (from `ummaya.tools.models`), `TokenUsage` (from `ummaya.llm.models`), `StopReason`.
...
QueryEngine (per-session orchestrator)
  ...
  │     ├── messages: list[ChatMessage]  (from ummaya.llm.models)
  │     ├── usage: UsageTracker          (from ummaya.llm.usage)
  │     └── resolved_tasks: list[str]
  ├── owns QueryEngineConfig (immutable)
  ├── references LLMClient              (from ummaya.llm.client)
  ├── references ToolExecutor            (from ummaya.tools.executor)
  └── references ToolRegistry            (from ummaya.tools.registry)
        │
        ▼
  ...
  │     ├── tool_result → ToolResult (from ummaya.tools.models)
