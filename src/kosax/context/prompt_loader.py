...
"""PromptLoader — Prompt Registry for the KOSAX platform.
...
CLI::
    ...
    python -m kosax.context.prompt_loader --regenerate-manifest
    python -m kosax.context.prompt_loader --emit-hashes
"""
...
from kosax.context.prompt_models import PromptManifest
...
def default_manifest_path() -> Path:
    ...
    Resolution order:
      1. Package-bundled resource at ``kosax/prompts/manifest.yaml``
    ...
    try:
        bundled = Path(str(files("kosax").joinpath("prompts", "manifest.yaml")))
        if bundled.is_file():
            return bundled
    ...
    # (src/kosax/context/prompt_loader.py → repo root).
...
class PromptLoader:
    ...
    langfuse_enabled:
        ...
        ``KOSAX_PROMPT_REGISTRY_LANGFUSE``.  Passing an explicit bool is
    ...
    def __init__(
        ...
        if langfuse_enabled is None:
            langfuse_enabled = (
                os.environ.get("KOSAX_PROMPT_REGISTRY_LANGFUSE", "").strip().lower() == "true"
            )
    ...
    def get_hash(self, prompt_id: str) -> str:
        ...
        Used by the Context Assembly layer to stamp ``kosax.prompt.hash`` on
    ...
    def _verify_langfuse(self) -> None:
        """When KOSAX_PROMPT_REGISTRY_LANGFUSE=true, lazily import langfuse and
        ...
        try:
            ...
            raise PromptRegistryError(
                "KOSAX_PROMPT_REGISTRY_LANGFUSE=true but the langfuse package is "
                "not installed. Install it with: uv sync --extra langfuse"
            ) from exc

        host = os.environ.get("KOSAX_LANGFUSE_HOST", "")
        public_key = os.environ.get("KOSAX_LANGFUSE_PUBLIC_KEY", "")
        secret_key = os.environ.get("KOSAX_LANGFUSE_SECRET_KEY", "")
...
def main(argv: list[str] | None = None) -> None:
    """CLI entry point: python -m kosax.context.prompt_loader [options]."""
    parser = argparse.ArgumentParser(
        prog="python -m kosax.context.prompt_loader",
        description="KOSAX Prompt Registry CLI utilities.",
    )
    parser.add_argument(
        "--regenerate-manifest",
        action="store_true",
