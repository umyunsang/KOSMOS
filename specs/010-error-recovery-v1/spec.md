...
(`src/ummaya/llm/retry.py`). Tool adapters have no retry, no circuit
...
V1 introduces a new `src/ummaya/recovery/` package that provides:
...
The existing `RetryPolicy` in `src/ummaya/llm/retry.py` is NOT reused. It is
...
- **FR-002**: `retry_tool_call()` MUST implement exponential backoff with full jitter: `delay = random.uniform(0, min(max_delay, base_delay * multiplier^attempt))`. This matches the formula already used in `src/ummaya/llm/retry.py`.
...
- **NFR-005**: The module MUST be located at `src/ummaya/recovery/` (new sub-package).
...
| `ToolRetryPolicy` as frozen Pydantic v2 model | Constitution Section III; `RetryPolicy` in `src/ummaya/llm/retry.py` as structural precedent |
...
| `src/ummaya/recovery/` new sub-package | Layer separation principle — `docs/vision.md` six-layer architecture |
