...
(1) Request arrives at ToolExecutor with (session_id, transaction_id, tool_id, params)
    ...
    └─ if tool.is_irreversible == true:
         ...
         ├─ if hit != None:
         │    ├─ span attribute: kosax.ipc.tx.cache_state = "hit"
         │    ├─ return hit.cached_response immediately
         │    └─ audit: ToolCallAuditRecord(status="dedup_hit", ...)
         └─ if hit == None:
              ├─ span attribute: kosax.ipc.tx.cache_state = "miss"
              ├─ execute tool
...
- Capacity: 512 entries (`KOSAX_IPC_TX_CACHE_CAPACITY`, `ge=1`).
...
- `kosax.ipc.tx.cache_state` = `"hit" | "miss" | "bypass"` (for `is_irreversible=false`)
