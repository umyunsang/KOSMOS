...
from kosax.tools.retrieval.dense_backend import DenseBackend, DenseBackendLoadError
from kosax.tools.retrieval.hybrid import HybridBackend
...
class Retriever(Protocol):
    ...
    applied downstream by ``kosax.tools.search``.
    """
...
class _DenseFailOpenWrapper:
    ...
    When ``KOSAX_RETRIEVAL_BACKEND=dense`` and cold-start is ``lazy``
...
def _resolve_cold_start() -> Literal["lazy", "eager"]:
    """Parse and validate ``KOSAX_RETRIEVAL_COLD_START``.
    ...
    """
    raw = os.getenv("KOSAX_RETRIEVAL_COLD_START", "lazy").strip().lower()
    if raw not in {"lazy", "eager"}:
        raise ValueError(
            f"KOSAX_RETRIEVAL_COLD_START={raw!r} is not recognised "
...
def _resolve_hybrid_fusion_k() -> int:
    """Parse and validate KOSAX_RETRIEVAL_FUSION / KOSAX_RETRIEVAL_FUSION_K.

    Raises:
        ValueError: On unrecognised fusion algorithm or invalid k value.
    """
    fusion = os.getenv("KOSAX_RETRIEVAL_FUSION", "rrf").strip().lower()
    if fusion != "rrf":
        raise ValueError(
            f"KOSAX_RETRIEVAL_FUSION={fusion!r} is not recognised "
            "(only 'rrf' is supported). Fail-closed per FR-001."
        )
    fusion_k_raw = os.getenv("KOSAX_RETRIEVAL_FUSION_K", "60").strip()
    try:
        fusion_k = int(fusion_k_raw)
    except ValueError as exc:
        raise ValueError(
            f"KOSAX_RETRIEVAL_FUSION_K={fusion_k_raw!r} is not a valid integer."
        ) from exc
    if fusion_k < 1:
        raise ValueError(f"KOSAX_RETRIEVAL_FUSION_K={fusion_k} is invalid; must be >= 1.")
    return fusion_k
...
def build_retriever_from_env(
    *,
    ...
    """Construct a ``Retriever`` from ``KOSAX_RETRIEVAL_*`` env vars.

    Env vars (spec 026, registered via #468):
        KOSAX_RETRIEVAL_BACKEND: ``bm25`` | ``dense`` | ``hybrid``
    ...
    """
    backend = os.getenv("KOSAX_RETRIEVAL_BACKEND", "bm25").strip().lower()
    if backend not in {"bm25", "dense", "hybrid"}:
        raise ValueError(
            f"KOSAX_RETRIEVAL_BACKEND={backend!r} is not recognised "
    ...
    model_id = os.getenv("KOSAX_RETRIEVAL_MODEL_ID", "intfloat/multilingual-e5-small").strip()
