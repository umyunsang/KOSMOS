...
How to add a new government API adapter to KOSAX. This guide covers everything from directory layout to fixture capture. See `docs/design/mvp-tools.md` for the canonical architectural decisions.
...
Canonical examples:
- KOROAD: `src/kosax/tools/koroad/accident_hazard_search.py`
- HIRA: `src/kosax/tools/hira/hospital_search.py`
...
To capture a live fixture (only when `KOSAX_DATA_GO_KR_API_KEY` is set):

```bash
KOSAX_DATA_GO_KR_API_KEY=<key> uv run pytest -m live tests/tools/<provider>/
```
...
from kosax.tools.hira.hospital_search import register as register_hira_hospital_search
