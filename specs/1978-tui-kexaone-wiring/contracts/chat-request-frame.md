...
**Pydantic location**: `src/ummaya/ipc/frame_schema.py` (new arm under the existing 19-arm union)
**JSON Schema location**: `tui/src/ipc/schema/frame.schema.json` (lockstep with Pydantic; SHA-256 hash will change — re-published at backend boot via `ummaya.ipc.schema.hash` OTEL attribute per Spec 032 FR-037)
...
- Total approximate token estimate (sum of message lengths × 4) must be ≤ `UMMAYA_LLM_SESSION_BUDGET` env (default 100,000); over-budget rejected with `error{code="budget_exceeded"}` frame.
...
When `_handle_chat_request` receives this frame, a new OTEL span `ummaya.turn` is opened with:

```
ummaya.session_id      = <envelope.session_id>
ummaya.correlation_id  = <envelope.correlation_id>
ummaya.frame.kind      = "chat_request"
ummaya.frame.seq       = <envelope.frame_seq>
...
Every `ChatRequestFrame` is logged to the structured observability event log (`ummaya.observability.event_logger.ObservabilityEventLogger`) with `event_type="chat_request_received"`. Tool dispatches inside the turn append `chat_request.{call_id}.invoked` events. This stitches into Spec 024 `ToolCallAuditRecord` chain via `correlation_id`.
