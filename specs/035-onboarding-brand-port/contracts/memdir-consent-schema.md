...
**Feature**: Epic H #1302
**Phase**: 1
**Owner of authoritative source**: `src/kosax/memdir/user_consent.py` (Pydantic v2) + `tui/src/memdir/consent.ts` (Zod mirror)
**Storage root**: `~/.kosax/memdir/user/consent/`
**PIPA reference**: 개인정보 보호법 § 22 (수집·이용 동의) + § 24 (고유식별정보 처리); KOSAX as PIPA § 26 수탁자 per project memory
...
```json
{
  ...
  "$id": "kosax.memdir.user.consent/1",
...
```python
# src/kosax/memdir/user_consent.py
...
from kosax.permissions import AuthenticatorAssuranceLevel  # Spec 033
