...
def test_info_log_emitted_per_resolved_prompt(valid_prompt_tree, caplog):
    ...
    on the ummaya.context.prompt_loader logger during PromptLoader construction."""
    with caplog.at_level(logging.INFO, logger="ummaya.context.prompt_loader"):
        _loader = PromptLoader(manifest_path=valid_prompt_tree)

    loader_records = [r for r in caplog.records if r.name == "ummaya.context.prompt_loader"]
    assert len(loader_records) >= 3, (
        f"Expected >= 3 INFO records from 'ummaya.context.prompt_loader', "
