...
"""OpenTelemetry TracerProvider bootstrap for KOSAX.
...
Usage::
    ...
    from kosax.observability.tracing import setup_tracing
...
def setup_tracing(settings: TracingSettings | None = None) -> TracerProvider | NoOpTracerProvider:
    ...
    if settings.endpoint is None:
        if _WARN_MISSING_ENDPOINT_ONCE:
            logger.warning(
                "KOSAX tracing: OTEL_EXPORTER_OTLP_ENDPOINT is not set and"
