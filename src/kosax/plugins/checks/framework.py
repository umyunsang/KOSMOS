...
class CheckContext:
    ...
    ``kosax plugin init`` and the install-time bundle extractor.
    """
...
class ChecklistRow(BaseModel):
    ...
    check_implementation: str = Field(
        min_length=1,
        description="Dotted path 'kosax.plugins.checks.q1_schema:check_pyv2'.",
    )
...
def _resolve_default_yaml_path() -> Path | None:
    ...
    Resolution order:
    1. Wheel-bundled resource (``kosax/_canonical/checklist_manifest.yaml``).
    ...
    try:
        ...
        bundled = resources.files("kosax._canonical").joinpath("checklist_manifest.yaml")
...
def _cli_main(argv: list[str] | None = None) -> int:
    """Entry point for ``kosax-plugin-validate <plugin_root>``.
    ...
    Closes review eval D3: quickstart Step 8 promised ``kosax-plugin-
    ...
    parser = argparse.ArgumentParser(
        prog="kosax-plugin-validate",
        description=(
            "Run KOSAX plugin 50-item review checklist against a "
    ...
    print(f"# KOSAX plugin validation — {plugin_root}")
