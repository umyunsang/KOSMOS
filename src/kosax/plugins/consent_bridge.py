...
1. Accepts the same synchronous callable signature as
   :class:`kosax.plugins.installer.ConsentPrompt` so :func:`install_plugin`
   keeps working unchanged.
2. Internally builds a :class:`~kosax.ipc.frame_schema.PermissionRequestFrame`
   carrying the manifest's ``permission_layer`` + (when ``processes_pii``)
   the trustee org name + canonical PIPA SHA-256.
3. Awaits the citizen's :class:`~kosax.ipc.frame_schema.PermissionResponseFrame`
...
class IPCConsentBridge:
    ...
    :class:`kosax.plugins.installer.ConsentPrompt` signature exactly:
    ...
    def __call__(  # noqa: C901 — branch count reflects PIPA + layer + emit + await fan-out; splitting hurts readability
        ...
        :class:`kosax.plugins.installer.CatalogEntry` /
        :class:`CatalogVersion` and
        :class:`kosax.plugins.manifest_schema.PluginManifest` respectively.
        """
