# Contract: stdio MCP Bridge (TUI ↔ UMMAYA Python backend)
...
tui/src/ipc/mcp.ts ── MCP client ──┐
       ...
       src/ummaya/ipc/stdio.py (Spec 032 transport — UNCHANGED)
                                    ...
                                    │ JSONL frame
                                    ▼
src/ummaya/ipc/mcp_server.py ── MCP server ──┐
                                              │
                                              │ Python call: lookup(mode=…, query=…)
                                              ▼
                       src/ummaya/tools/lookup.py + registry.py + routing_index.py
```
...
|---|---|---|---|
| 1 | client → server | `initialize` | `{protocolVersion: "2025-06-18", capabilities: {tools: {}}, clientInfo: {name: "ummaya-tui", version: "<package.json>"}}` |
| 2 | server → client | `initialize` response | `{protocolVersion: "2025-06-18", capabilities: {tools: {listChanged: false}}, serverInfo: {name: "ummaya-backend", version: "<__version__>"}}` |
...
- `ummaya.tools.register_all` import + registry build: ≤ 150 ms
...
|---|---|---|
| Backend process exit during handshake | (transport-level EOF detected by `bridge.ts`) | TUI shows "tool subsystem unavailable — restart UMMAYA" — never an empty tool list (FR-023) |
...
| Handshake exceeds cold budget by 2× | (no protocol error; perf alert) | TUI logs WARN to stderr but proceeds (citizen experience prioritized over budget); OTEL span carries `ummaya.mcp.handshake_ms` attribute for monitoring |
...
- Heartbeat (UMMAYA_IPC_HEARTBEAT_MS)
...
- OTEL span attributes: `ummaya.mcp.handshake_ms`, `ummaya.mcp.tool_call_id`, `ummaya.mcp.protocol_version`

## 5. Schema versioning

The `protocolVersion` exchanged in § 2.1 follows the upstream MCP version (`2025-06-18` at time of writing). UMMAYA does NOT introduce a custom MCP dialect. If MCP releases a new version, UMMAYA bumps both `mcp.ts` and `mcp_server.py` in lockstep behind a single ADR (`docs/adr/`).

## 6. Out-of-scope for this contract

- The `tui/src/tools/MCPTool/` external-MCP passthrough (a separate concern — that one connects the LLM to *external* MCP servers; this contract is for the TUI ↔ UMMAYA internal bridge).
- The `mcp/resources` and `mcp/prompts` MCP capabilities — UMMAYA exposes only `tools/*`. Resource/prompt capabilities are listed as `{}` in the `initialize` capabilities. Extending the server to advertise these capabilities is not planned; no commitment.
