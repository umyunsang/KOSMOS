...
| Decision | Frozen §§ | Primary reference | Secondary reference | Adaptation for UMMAYA |
|---|---|---|---|---|
...
| `mode="search"` BM25 gate over cold-path adapters | §5, §5.5 | AnyTool (arXiv:2402.04253) hierarchical router | Cursor Dynamic Context Discovery (46.9% prod A/B) | `top_k` is adaptive: `min(UMMAYA_LOOKUP_TOPK, len(registry))` to handle sparse MVP registry |
...
- **License**: Apache-2.0 (compatible with UMMAYA Apache-2.0)
...
- **Post-MVP freshness SLO** (documented, not enforced in MVP): `hvidate` older than `UMMAYA_NMC_FRESHNESS_MINUTES` (default 30, clamp [1, 1440]) → `LookupError(reason="stale_data")`
