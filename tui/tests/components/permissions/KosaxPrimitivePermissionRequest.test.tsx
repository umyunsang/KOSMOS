...
// Spec 2294 — KosaxPrimitivePermissionRequest tests.
//
...
import { KosaxPrimitivePermissionRequest } from '@/components/permissions/KosaxPrimitivePermissionRequest/KosaxPrimitivePermissionRequest'
import type { PrimitiveDecision } from '@/components/permissions/KosaxPrimitivePermissionRequest/KosaxPrimitivePermissionRequest'
...
describe('KosaxPrimitivePermissionRequest — verify (Layer 1)', () => {
  test('renders layer 1 glyph ⓵', () => {
    const props = makeProps('verify')
    const { lastFrame } = render(
      <Wrap>
        <KosaxPrimitivePermissionRequest {...props} />
      </Wrap>,
    )
...
describe('KosaxPrimitivePermissionRequest — submit reversible (Layer 2)', () => {
...
describe('KosaxPrimitivePermissionRequest — submit irreversible (Layer 3)', () => {
...
describe('KosaxPrimitivePermissionRequest — lookup (bypass)', () => {
  test('renders nothing for lookup primitive', () => {
    const onDecision = mock(() => {})
    const onDismiss = mock(() => {})
    const { lastFrame } = render(
      <Wrap>
        <KosaxPrimitivePermissionRequest
          primitive="lookup"
...
describe('KosaxPrimitivePermissionRequest — selector labels', () => {
  for (const primitive of ['verify', 'submit'] as const) {
    test(`${primitive}: shows selector labels without KOSAX-only hotkey prefixes`, () => {
...
describe('KosaxPrimitivePermissionRequest — receiptId footer', () => {
