...
|---|---|---|---|
| `TracingSettings` | Pydantic v2 model | `src/kosax/observability/tracing.py` | 프로세스 부팅 시 1회 생성 |
| `InvokeAgentSpan` | OTel Span (논리적 분류) | `src/kosax/engine/query.py` | query 호출 당 1개 (parent) |
| `ChatSpan` | OTel Span | `src/kosax/llm/client.py` | LLM 호출 당 1개 |
| `ExecuteToolSpan` | OTel Span | `src/kosax/tools/executor.py` | 도구 실행 당 1개 |
| `FilteredAttributes` | `dict[str, AttributeValue]` | `src/kosax/observability/otel_bridge.py` | span.set_attributes 입력 |
| `RateLimitRetryCounter` | Counter metric (기존 `MetricsCollector` 확장) | `src/kosax/observability/metrics.py` 소비자 | 프로세스 lifetime |
...
```python
# src/kosax/observability/tracing.py
...
class TracingSettings(BaseModel):
    ...
    service_name: str = Field(default="kosax", description="OTel resource service.name.")
...
| 속성 | 필수 | 타입 | 값 | 출처 |
|---|---|---|---|---|
| `span.name` | ✅ | str | `"invoke_agent kosax-query"` | 상수 |
| `gen_ai.operation.name` | ✅ | str | `"invoke_agent"` | semconv 상수 |
| `gen_ai.agent.name` | ✅ | str | `"kosax-query"` | 상수 |
...
**Streaming usage aggregation**: 청크마다 속성을 갱신하지 않는다. `LLMClient.generate_stream` 내부 누적기(`src/kosax/llm/usage.py`)가 기존 방식대로 동작, **스트림 finalize 직후** `span.set_attributes(...)` 1회 호출로 커밋.

**Retry behavior**: 429 재시도는 span을 분기하지 않는다. 같은 `chat` span 안에서 `kosax_llm_rate_limit_retries_total` counter가 증가한다(E6 참조).
...
| `gen_ai.tool.type` | ✅ | str | `"function"` (KOSAX는 전 도구가 함수형) |
...
```python
# src/kosax/observability/otel_bridge.py
from kosax.observability.event_logger import _ALLOWED_METADATA_KEYS
...
| 항목 | 값 |
|---|---|
| 메트릭 이름 | `kosax_llm_rate_limit_retries_total` |
| 종류 | Counter (누적) |
