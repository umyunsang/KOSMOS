...
FR-005: search → BM25 retrieval gate via ``ummaya.tools.search.search()``.
FR-006: fetch → typed adapter invocation via ``executor.invoke()``.
FR-009: top_k adaptive clamp [1, 20], default from UMMAYA_LOOKUP_TOPK.
"""
...
async def _lookup_search(
    ...
    *,
    ...
    default_k = settings.lookup_topk  # from UMMAYA_LOOKUP_TOPK
