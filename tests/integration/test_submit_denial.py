...
- src/kosax/primitives/submit.py (check_tier_gate + submit dispatcher)
...
class TestSubmitDenialNoAdapterCall:
    ...
    async def test_adapter_invoke_not_called_on_denial(self) -> None:
        ...
        with (
            patch(
                "kosax.primitives.submit.check_tier_gate",
            ...
            patch.dict(
                ...
                {
                    TOOL_ID: (
                        kosax.tools.mock.data_go_kr.fines_pay.REGISTRATION,
                        spy_invoke,
                    )
                },
            ),
