# Contract — `ummaya permissions verify` CLI

**Feature**: 033-permission-v2-spectrum
**Artifact**: Behavioral contract for `src/ummaya/permissions/cli.py::verify` command
**Date**: 2026-04-20

> Verifies the hash chain + HMAC seal of `~/.ummaya/consent_ledger.jsonl`. Fail-closed on any integrity error. Designed so an external auditor can run it without UMMAYA runtime dependencies beyond the Python stdlib + HMAC key file.

## 1. Invocation

```
ummaya permissions verify [--path PATH] [--hash-only] [--acknowledge-key-loss] [--json]
```

| Flag | Default | Purpose |
|------|---------|---------|
| `--path PATH` | `~/.ummaya/consent_ledger.jsonl` | Override ledger location (test / audit). |
...
| `3` | HMAC key unknown. | Record `hmac_key_id` not in key registry at `~/.ummaya/keys/`. |
| `4` | Schema violation. | Record fails Pydantic validation against `ConsentLedgerRecord`. |
| `5` | File not found or empty. | `~/.ummaya/consent_ledger.jsonl` missing or 0 bytes. |
| `6` | HMAC key file mode mismatch. | `~/.ummaya/keys/ledger.key` mode != `0400`. Fail-closed load refusal (Invariant C3). |
...
✓ Ledger verification PASSED
  Path: /Users/alice/.ummaya/consent_ledger.jsonl
  Records: 42
  Chain: OK
...
| Insert a forged record with valid HMAC (attacker has key). | Chain mismatch → exit 1. HMAC key compromise requires `ummaya permissions rotate-key` + forensic review (out-of-band). |
...
- Portability: Runnable via `uv run python -m ummaya.permissions.cli verify` with ONLY the `ummaya/permissions/` package vendored.
...
- [ ] CLI wired via entry point `ummaya permissions verify` (no new dependency).
