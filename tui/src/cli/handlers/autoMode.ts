/**
 * UMMAYA: Auto mode CLI handlers stubbed out.
 * utils/permissions/yoloClassifier deleted (Anthropic growthbook TRANSCRIPT_CLASSIFIER).
 * UMMAYA uses K-EXAONE on FriendliAI — the Anthropic-only auto-mode classifier
 * feature does not apply.
 */

export function autoModeDefaultsHandler(): void {
  process.stdout.write(
    'Auto mode classifier is not available in UMMAYA (K-EXAONE / FriendliAI build).\n',
  )
}
