...
:mod:`ummaya.tools.policy_derivation`) rather than reading standalone
UMMAYA-invented fields that are no longer stored.
...
When ``manifest.adapter.policy is None`` (UMMAYA-internal synthetic surfaces)
...
def check_v2_dpa(ctx: CheckContext) -> CheckOutcome:
    ...
    passes silently (UMMAYA-internal surfaces have no agency policy).
    """
...
def check_v6_auth_level_map(ctx: CheckContext) -> CheckOutcome:
    ...
    ``adapter.policy.citizen_facing_gate`` rather than a stored field.
    When ``adapter.policy is None`` the check passes silently
    (UMMAYA-internal surfaces use their own auth_type governance).
    """
