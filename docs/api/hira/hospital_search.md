...
| `radius` | `int` (1–10000, default 2000) | no | Search radius in meters. Maximum 10,000 m. Preserve an explicit numeric distance from the citizen request; otherwise use the default. |
| `dgsbjt` | `str \| null` | no | HIRA medical-specialty filter. Accepts Korean names/codes such as `내과`, `이비인후과`, `가정의학과`, `소아과`, or comma-separated values. Pass it when the citizen explicitly states a specialty. |
| `clCd` | `str \| null` | no | HIRA institution-type filter. Pass it when the citizen explicitly states an institution type such as `의원`, `병원`, `종합병원`, `상급종합`, `치과병원`, `한의원`, `약국`, or `보건소`. |
...
| `telno` | `str` | Phone number. If HIRA returns a local landline without area code (for example `265-2468`), the adapter prefixes the area code inferred from `sidoCdNm` / `addr` (for example 부산 → `051-265-2468`). |
...
- **Phone formatting quirk**: Some HIRA rows return a local landline without an area code
  even though `sidoCdNm` / `addr` identify the region. The adapter does not invent phone
  numbers; it only normalizes those rows by prefixing the public Korean area code for the
  returned region so citizen-facing answers stay dialable.
- **No semantic fallback**: The adapter does not infer specialty or institution type from
  symptom text. `dgsbjt` and `clCd` are applied only when the caller provides them.
- **Specialty ranking**: HIRA's `dgsbjtCd` filter returns institutions that offer the specialty,
  even when the institution name advertises another specialty. The adapter ranks results whose
  visible `yadmNm` contains the requested specialty name before distance-only matches so
  `근처 내과 3곳` prefers `내과의원` rows.
- **Specialty candidate over-fetch**: For `dgsbjt` searches, the adapter requests up to 50 rows
  from the same HIRA endpoint and same filters before sorting, then returns only the caller's
  requested `numOfRows`. This prevents a small upstream page from cutting off better visible
  specialty matches before client-side ranking. It is not a fallback dataset or alternate source.
