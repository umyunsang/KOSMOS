...
**Target Platform**: Bun v1.2.x runtime on macOS / Linux (existing UMMAYA support matrix; no platform change).
...
The Python backend (`src/ummaya/primitives/{lookup,submit,verify,subscribe}.py`) is the IPC routing destination. **Not modified by this Epic** — that is the entire point of preserving `PrimitiveInput`/`PrimitiveOutput` (FR-011).
