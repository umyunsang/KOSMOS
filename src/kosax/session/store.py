...
"""Append-only JSONL session store for KOSAX session persistence.

Session files live at ``~/.kosax/memdir/user/sessions/{session_id}.jsonl``.
Each line is a JSON-serialised :class:`~kosax.session.models.SessionEntry`.
...
def _get_default_session_dir() -> Path:
    ...
    Respects ``KOSAX_MEMDIR_USER`` (canonical Spec 027 env var) first, then
    falls back to ``~/.kosax/memdir/user``.  The sessions sub-directory is
    always appended.

    Legacy ``KOSAX_SESSION_DIR`` is retained as a lower-priority override for
    test isolation when the caller cannot set ``KOSAX_MEMDIR_USER``.
    """
    memdir_user = os.environ.get("KOSAX_MEMDIR_USER", "").strip()
    if memdir_user:
        return Path(memdir_user).expanduser() / "sessions"
    return Path.home() / ".kosax" / "memdir" / "user" / "sessions"
...
def _get_session_dir() -> Path:
    ...
    1. ``KOSAX_MEMDIR_USER`` env var — Spec 027 canonical.  Points to the
       memdir user-tier root; ``sessions/`` sub-dir is appended automatically.
    2. ``KOSAX_SESSION_DIR`` env var — legacy test-isolation override.
       Treated as a full path (no sub-dir appended).
    3. Hard-coded default: ``~/.kosax/memdir/user/sessions/``.
    """
    memdir_user = os.environ.get("KOSAX_MEMDIR_USER", "").strip()
    if memdir_user:
        session_dir = Path(memdir_user).expanduser() / "sessions"
    else:
        legacy_override = os.environ.get("KOSAX_SESSION_DIR", "").strip()
        if legacy_override:
