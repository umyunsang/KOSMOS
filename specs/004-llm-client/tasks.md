...
- [ ] T001 Create `src/kosax/llm/` package directory with `__init__.py` and create `tests/llm/` package directory with `__init__.py`
...
- [ ] T003 [P] Implement error hierarchy (`KosaxLLMError`, `ConfigurationError`, `BudgetExceededError`, `AuthenticationError`, `LLMConnectionError`, `LLMResponseError`, `StreamInterruptedError`) in `src/kosax/llm/errors.py`
- [ ] T004 [P] Implement Pydantic v2 message models (`ChatMessage`, `ToolCall`, `FunctionCall`, `TokenUsage`, `ChatCompletionResponse`, `StreamEvent`) in `src/kosax/llm/models.py` — include role-based validation (tool role requires tool_call_id, system/user require content)
- [ ] T005 [P] Implement `LLMClientConfig` using pydantic-settings with `KOSAX_FRIENDLI_TOKEN` (required SecretStr), `KOSAX_FRIENDLI_BASE_URL` (default), `KOSAX_FRIENDLI_MODEL` (default), `KOSAX_LLM_SESSION_BUDGET` (default 100000), `timeout`, `max_retries` in `src/kosax/llm/config.py`
...
- [ ] T007 [US1] Implement `LLMClient.__init__`, `close()`, `__aenter__`/`__aexit__` context manager, and non-streaming `complete()` method using httpx async POST to `/chat/completions` in `src/kosax/llm/client.py`
- [ ] T008 [US1] Implement `LLMClient.stream()` async generator method with SSE line parsing (`data:` prefix, `[DONE]` sentinel, JSON chunk → `StreamEvent`) in `src/kosax/llm/client.py`
...
- [ ] T013 [US2] Implement `UsageTracker` with `can_afford()`, `debit()`, `remaining` property, `is_exhausted` property, and `call_count` in `src/kosax/llm/usage.py`
- [ ] T014 [US2] Integrate `UsageTracker` into `LLMClient`: initialize from config budget, pre-flight check in `complete()`/`stream()`, post-call debit from response usage in `src/kosax/llm/client.py`
...
- [ ] T016 [US3] Implement `RetryPolicy` model and `retry_with_backoff()` async function (exponential backoff, full jitter, base 1s, multiplier 2x, cap 60s, max 3 retries, retryable codes {429, 503}) in `src/kosax/llm/retry.py`
- [ ] T017 [US3] Integrate retry logic into `LLMClient.complete()` and `LLMClient.stream()` — wrap HTTP calls with retry, map non-retryable status codes to specific error types (401→AuthenticationError, 400→LLMResponseError) in `src/kosax/llm/client.py`
...
- [ ] T019 [US4] Add `ToolDefinition` and `FunctionSchema` Pydantic v2 models to `src/kosax/llm/models.py`
- [ ] T020 [US4] Implement tool_calls support in `LLMClient.complete()` and `LLMClient.stream()` — serialize `tools` parameter, parse `tool_calls` in response/stream events, handle `tool_call_delta` StreamEvent assembly in `src/kosax/llm/client.py`
...
- [ ] T022 Finalize public exports in `src/kosax/llm/__init__.py` — export `LLMClient`, all models, all errors, `UsageTracker`, `RetryPolicy`, `LLMClientConfig`
- [ ] T023 Add stdlib `logging` calls for all LLM operations (request metadata, token counts, latency, retry attempts, errors) per FR-012 in `src/kosax/llm/client.py`
