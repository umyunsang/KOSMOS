...
**Artifact**: Behavioral contract for `src/kosax/permissions/modes.py` + TUI `tui/src/permissions/ModeCycle.tsx`
**Date**: 2026-04-20

> References: Claude Code 2.1.88 `PermissionMode.ts` / `getNextPermissionMode.ts` (mirrored with the `dontAsk` KOSAX addition). Constitution §II (fail-closed, NON-NEGOTIABLE).
...
| Persistent rules | ✓ Yes | `~/.kosax/permissions.json` |
| Consent ledger | ✓ Yes | `~/.kosax/consent_ledger.jsonl` |
