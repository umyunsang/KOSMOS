...
**Audience**: A developer who has never touched KOSAX, has Python 3.12+ and uv installed, and wants to land their first KOSAX plugin.
...
- Optional: KOSAX TUI installed locally — only needed for `kosax plugin init` interactive flow. CLI scaffolding can also run via `uvx kosax-plugin-init <name>` (vendored entry-point) without the full TUI.
...
1. Go to https://github.com/kosax-plugin-store/kosax-plugin-template
2. Click "Use this template" → "Create a new repository"
3. Name it `kosax-plugin-<your-plugin-name>` (e.g., `kosax-plugin-busan-bike`)
4. Clone locally:
   ```sh
   git clone https://github.com/<your-org>/kosax-plugin-busan-bike
   cd kosax-plugin-busan-bike
   ```

### Option B — `kosax plugin init` CLI (if you have the TUI)

```sh
kosax plugin init busan-bike
cd busan-bike
```
...
Open `docs/plugins/architecture.md` (in this repo, vendored as a stub during scaffold; canonical version at https://github.com/umyunsang/KOSAX/blob/main/docs/plugins/architecture.md). Skim:
...
slsa_provenance_url: https://github.com/<your-org>/kosax-plugin-busan-bike
otel_attributes:
  kosax.plugin.id: busan_bike
search_hint_ko: "부산 자전거 따릉이 대여소 자전거 잔여대수 부산광역시"
search_hint_en: "Busan bike rental station availability"
permission_layer: 1
```

If `processes_pii: true`, add the `pipa_trustee_acknowledgment` block here (run `kosax plugin pipa-text` to print the canonical text and SHA-256, or read `docs/plugins/security-review.md`).
...
Rules:
- Read API key from `KOSAX_*` env var (never hardcode — Q-NO-HARDCODED-KEY).
...
```sh
uvx --from git+https://github.com/umyunsang/KOSAX@main \
    kosax-plugin-validate .
```
...
- Publishing to `kosax-plugin-store` — out of scope for this quickstart; happens after PR merge.
