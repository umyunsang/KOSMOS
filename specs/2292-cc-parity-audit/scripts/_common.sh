...
UMMAYA_ROOT="$(git rev-parse --show-toplevel 2>/dev/null)"
if [[ -z "$UMMAYA_ROOT" ]]; then
  echo "ERROR: not inside a git repository" >&2
  exit 1
fi

# Canonical input directories (read-only).
UMMAYA_DIR="$UMMAYA_ROOT/tui/src"
CC_DIR="$UMMAYA_ROOT/.references/claude-code-sourcemap/restored-src/src"

# Output base.
AUDIT_DIR="$UMMAYA_ROOT/specs/2292-cc-parity-audit"
...
if [[ ! -d "$UMMAYA_DIR" ]]; then
  echo "ERROR: UMMAYA_DIR not found: $UMMAYA_DIR" >&2
  exit 1
fi
if [[ ! -d "$CC_DIR" ]]; then
...
export UMMAYA_ROOT UMMAYA_DIR CC_DIR AUDIT_DIR DATA_DIR
