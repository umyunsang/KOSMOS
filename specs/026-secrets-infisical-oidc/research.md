...
| `LANGFUSE_*` + `UMMAYA_OTEL_ENDPOINT` code wiring | #501 | `#501` | ✅ open Epic (this Epic catalogues names only) |
...
| Legacy `UMMAYA_API_KEY` full removal (read-path) | `NEEDS TRACKING` | post-#468 | ✅ flagged |
...
## R1 — Guard invocation site in `src/ummaya/cli/app.py`
...
**Reference**: `src/ummaya/_dotenv.py:40`; `src/ummaya/cli/app.py:245–249`; Claude Code `permissions` module (`src/ummaya/permissions/`).

## R2 — Source of truth for required variable names

**Decision**: In-code Python list (`_REQUIRED_VARS: list[RequiredVar]`) inside `src/ummaya/config/guard.py`. Registry markdown (`docs/configuration.md`) is a human mirror; the code is machine truth. `scripts/audit-env-registry.py` reconciles the two, **failing CI** on drift.
...
**Alternatives considered**:
- **Derive from `UmmayaSettings` class introspection**: rejected — `LLMClientConfig` and `CLIConfig` live in separate modules with different prefixes; a single introspection pass is brittle. Explicit list is clearer.
...
## R3 — Activation flag semantics (`UMMAYA_ENV`)

**Decision**: `UMMAYA_ENV ∈ {dev, ci, prod}`. Unknown / unset → treat as `dev`. The flag gates only *conditional-required* promotion (e.g., `LANGFUSE_*` becomes required when `UMMAYA_ENV=prod`), never the shape of config.
...
- Dev experience: a new contributor cloning the repo with no `UMMAYA_ENV` set gets the most permissive mode (dev), so the guard only yells about truly required vars.
...
- **Boolean `UMMAYA_STRICT`**: rejected — loses the `ci` tier, which has different truths (synthetic tokens allowed) than `prod`.
...
**Decision**: Pin `repository=umyunsang/UMMAYA` + `workflow=.github/workflows/ci.yml` + `actor_type=User`. **Do NOT pin `ref`** — PR CI requires arbitrary-branch executions.
...
**Decision**: Regex-based. `audit-secrets.sh` greps YAML workflow files for denylisted patterns; `audit-env-registry.py` regex-matches `UMMAYA_[A-Z_]+` literals in `*.py`, `*.yml`, `*.md`, and parses the registry Markdown via a minimal table-line matcher. **Stdlib only** (`re`, `argparse`, `pathlib`, `sys`).

**Rationale**:
- AST parse of Python + proper YAML parse of workflows would be more precise, but (a) adds PyYAML/libcst deps (AGENTS.md violation), (b) the target strings are deliberate literals — no macro/template expansion occurs in UMMAYA to hide a name from regex, (c) the audit is a **drift alarm**, not a compiler: false positives are tolerable if they prompt a human check.
- Regex `UMMAYA_[A-Z_]+` with word-boundaries captures every prefixed env var in the codebase. The registry Markdown uses a fixed table format (Variable | Required | ... | Source), so a line-regex matches each row.
- Markdown table parse: the registry table is the only such construct in `docs/configuration.md` that starts with `| UMMAYA_` or `| LANGFUSE_`; a 5-line regex is sufficient.

**Alternatives considered**:
- **AST-based Python parse**: rejected — adds complexity without catching more; no `UMMAYA_*` name is constructed dynamically in the current codebase.
...
**Decision**: Dotenv format, one `UMMAYA_X=<redacted>` per line. No `export ` prefix. Comments via `#` prefix group vars by subsystem.

**Rationale**:
- `src/ummaya/_dotenv.py` is the parser of record at runtime (and at contributor setup time via `source .env`). It accepts `KEY=VALUE`. `.env.example` must be copy-paste-compatible with what the parser expects.
...
**Reference**: `src/ummaya/_dotenv.py:26–46`; dotenv convention per 12-Factor App tooling ecosystem.
...
|------------|-------------------|------------|
| #507 (MVP tools) | Names `UMMAYA_KAKAO_API_KEY`, `UMMAYA_LOOKUP_TOPK`, `UMMAYA_NMC_FRESHNESS_MINUTES` | ✅ Registry reflects these exact names — no conflict. |
| #501 (observability) | Owns `LANGFUSE_*` + `UMMAYA_OTEL_ENDPOINT` runtime wiring | ✅ Epic #468 *catalogues* names only; code wiring deferred. Conditional-required rule added in registry with "owner: #501". |
...
| #465 (LiteLLM) | Will add 6 new env vars (`UMMAYA_LITELLM_MASTER_KEY`, etc.) | ✅ Registry schema designed to accept new rows without breaking audit regex. Entities in `data-model.md` support forward growth. |
| #458 (CLOSED — Kakao degradation) | Root cause: missing `UMMAYA_KAKAO_API_KEY` silently downgraded tool | ✅ This Epic's guard is the named regression guard (spec §User Story 1). |

No conflicts found.

## Infisical Free tier capacity check

Infisical Cloud Free (as of 2026-04): **3 workspaces** × **unlimited secrets** × **unlimited members**. UMMAYA needs 1 workspace (`UMMAYA`) × 2 environments (`test`, `prod`) × ~20 secrets. **Well within limits.** No paid-tier upgrade required for this Epic.
