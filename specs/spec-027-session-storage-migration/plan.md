...
|-----------|------------|------|
| I — CC + 2 swaps thesis | PASS | We are routing CC's session-storage helper to a KOSAX-owned root; not changing the JSONL schema, not changing function signatures (FR-013). |
| II — Permissions never lateral | N/A | Storage path Epic only. |
| III — Live tools cite agency policy | N/A | Internal storage. |
| IV — No KOSAX-invented permission classifications | N/A | |
...
**Method**: Read `.references/claude-code-sourcemap/restored-src/utils/sessionStorage.ts` (or its current location) for `getProjectsDir()` + `getProjectDir(projectDir)` definitions. Compare to KOSAX port at `tui/src/utils/sessionStorage.ts:203-205, 441-443`.

**Acceptance**: We have a confirmed reference for `<projects-root>/<sanitized-cwd>/<session_id>.jsonl`. The KOSAX port at `sessionStorage.ts:441-443` (`getProjectDir`) and `sessionStoragePortable.ts:329-331` (same name) call `sanitizePath(projectDir)`. We will leave the per-cwd sanitization helper alone — it is only used for legacy-discovery in US2.
...
**Method**: `grep -rn "getTranscriptPath\|getProjectsDir\|getProjectDir\|getSessionProjectDir\|getAgentTranscriptPath" tui/src/ src/kosax/`.
...
### R-3 — Decide US5 `~/.kosax/sessions/` resolution

**Question**: For the 3,572 stub JSONLs at `~/.kosax/sessions/`, which of {delete-only | symlink-to-canonical | repoint-Python-backend} is chosen?

**Method**: Inspect 5 random stub files for any non-metadata content (rule out partial-content). Inspect `src/kosax/session/store.py` for write call-sites. Inspect IPC envelope (`src/kosax/ipc/stdio.py:600,727`) for any callsite that depends on the legacy path.

**Acceptance**: One concrete decision documented with rationale. Default recommendation per spec.md Q3: **repoint Python `_DEFAULT_SESSION_DIR` to `~/.kosax/memdir/user/sessions/`** + add a one-shot `kosax --gc-empty-stubs` pass that audits each stub by reading its first JSON line and confirming `message_count == 0` *content-wise* before deletion (never by filename heuristic).
...
**Method**: Read 1 sample line from `~/.claude/projects/-Users-um-yunsang-KOSAX-tui/<largest>.jsonl` (TUI-written) vs `~/.kosax/sessions/<any>.jsonl` (Python-written). Diff field names. Then read `tui/src/commands/history.ts:loadSessionEntries` (the canonical reader) — what fields does it require?
...
SessionPathRoot:
  KOSAX_CANONICAL: ~/.kosax/memdir/user/sessions  # FR-002
  CC_LEGACY:        ~/.claude/projects               # FR-005 read-only
  ENV_OVERRIDE:     $KOSAX_MEMDIR_USER + /sessions  # FR-002
...
- `contracts/session-path-resolver.md` — Markdown contract for `getKosaxSessionsDir()`, env override semantics, fail-closed behavior (FR-011).
...
1. `export KOSAX_MEMDIR_USER=/tmp/test-memdir-$RANDOM`
...
7. `/resume` → assert both real KOSAX session + fake legacy session appear.
...
10. `unset KOSAX_MEMDIR_USER`.

### dispatch-tree.md

```
Phase 1 Setup (T001-T002): sonnet-foundational  (KOSAX path helper + env override)
...
- **T001** — Add `KOSAX_MEMDIR_USER` to `pydantic-settings` env catalog if it isn't already (Spec 027 may already have it; verify in R-2). Backend Python side.
- **T002** — Add `getKosaxSessionsDir()` helper to a new file `tui/src/utils/kosaxPaths.ts` (matching the Spec 1635 `uiL2Memdir.ts:25-26` pattern). TS side.
- **T003-T004** — Repoint `getProjectsDir()` *body* in `sessionStorage.ts` and `sessionStoragePortable.ts` to call `getKosaxSessionsDir()`. The function name + signature stay identical (FR-013). The CC-shape-mimicking helper `getProjectDir(projectDir)` stays as a *legacy-discovery* helper used only by FR-005 dual-path enumeration.
- **T005-T006** — Add migration helper module + `/migrate-sessions` slash command registration.
- **T007** — Resolve `~/.kosax/sessions/` per R-3 decision. Most likely: change `src/kosax/session/store.py:32` to `_DEFAULT_SESSION_DIR = Path.home() / ".kosax" / "memdir" / "user" / "sessions"`. Add `--gc-empty-stubs` CLI flag.
...
|-------|----------|------------------------|
| 1a — pytest | `tests/test_session_store_path.py` | YES (covers Python `_DEFAULT_SESSION_DIR` repoint, `KOSAX_SESSION_DIR` env override, `--gc-empty-stubs`). |
...
| 2 — stdio JSONL probe | `specs/spec-027-session-storage-migration/scripts/probe-write-path.sh` | YES (asserts a tool-call session writes to KOSAX path, not CC path). |
...
**Bypass declaration**: This Epic touches `tui/src/**` extensively (sessionStorage.ts, sessionStoragePortable.ts, new kosaxPaths.ts, new migrateSessions command). NO bypass available — all verification layers required.
...
- `docs/requirements/kosax-migration-tree.md § L1-A A5` — same invariant from the requirements tree.
...
- Spec 035 `~/.kosax/memdir/user/consent/` — sibling dir under same root.
...
- **Q2 (resolved by spec.md Q2)**: Per-cwd subdirs at the KOSAX path? **No** — flat layout per FR-014.
- **Q3 (resolved by R-3 decision in research.md)**: `~/.kosax/sessions/` action? **Repoint + gc**.
- **Q4 (NEW, escalate if blocking)**: Should `getKosaxSessionsDir()` live in a new `tui/src/utils/kosaxPaths.ts` (preferred — single helper file for all canonical KOSAX paths) OR be inlined into `envUtils.ts` next to `getClaudeConfigHomeDir()` (simpler diff)? **Recommendation**: new file. `kosaxPaths.ts` becomes the home for any future `getKosaxConsentDir()` / `getKosaxPluginsDir()` factoring (DRY across Spec 035 / Spec 1636 helpers).

---

## Phase 6 — PR closure plan

- PR title: `feat(spec-027-session-storage-migration): route citizen sessions to ~/.kosax/memdir/user/sessions/ (closes Epic 3)`
- PR body cites: Spec 027 + `kosax-migration-tree.md § L1-A A5` + this plan.
