...
"""HMAC key loader / generator for the KOSAX consent ledger (Spec 033 FR-D04).

Manages the 32-byte HMAC-SHA-256 secret stored at ``~/.kosax/keys/ledger.key``
(configurable via ``KOSAX_PERMISSION_KEY_PATH`` env var).
...
def load_or_generate_key(path: Path) -> bytes:
    ...
    Args:
        path: Filesystem path for the HMAC key file.  Typically
              ``Path.home() / ".kosax" / "keys" / "ledger.key"``.
    ...
    _logger.info("kosax.permissions.ledger - initialised at %s", path)
    return key
...
def bootstrap_hmac_key(
    ...
    Args:
        key_path: Absolute path to the HMAC key file (``~/.kosax/keys/ledger.key``).
        key_registry_path: Absolute path to the key registry JSON
            (``~/.kosax/keys/registry.json``).
