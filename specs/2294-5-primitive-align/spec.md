...
| MCP-side primitive permission-UI downgrade pattern | Edge case for community plugin primitives that route through MCP rather than native registration; out of P3 scope | Phase 5 plugin DX follow-up | NEEDS TRACKING |
| `prompts/system_v1.md` 5-primitive citizen-friendly tone update | Optional Phase ζ from `cc-source-migration-plan`; gated on shadow-eval results from Spec 026 to avoid prompt-drift regressions | Phase ζ (optional) | NEEDS TRACKING |
