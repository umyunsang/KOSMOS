...
**Purpose**: Specify how `Infisical/secrets-action@v1` is integrated into `ci.yml` so that (a) no long-lived GitHub Encrypted Secret is required, (b) every `UMMAYA_*` env var needed by CI tests is injected via OIDC-fetched Infisical secrets, (c) the `UMMAYA_DATA_GO_KR_KEY` typo is fixed.
...
Claim pins:
  ...
  workflow_ref     = umyunsang/UMMAYA/.github/workflows/ci.yml@*
  actor_type       = User
...
In every CI job that needs UMMAYA secrets, insert the following step **before** any step that imports application code or runs tests:
...
- `project-slug` — **required**: human-readable Infisical project slug (`ummaya-3f-zs` for this repo). The action does not accept a `project-id` / UUID input.
...
| Var | Required for |
|-----|--------------|
| `UMMAYA_FRIENDLI_TOKEN` | LLM live suite (SC-004) |
| `UMMAYA_KAKAO_API_KEY` | Kakao Local tool tests |
| `UMMAYA_DATA_GO_KR_API_KEY` | data.go.kr tool tests — **NOTE** this is the correct name; prior typo `UMMAYA_DATA_GO_KR_KEY` (FR-050) MUST be eliminated |
| `UMMAYA_JUSO_CONFM_KEY` | JUSO geocoding live tests |
| `UMMAYA_SGIS_KEY` / `UMMAYA_SGIS_SECRET` | SGIS live tests |
| `LANGFUSE_PUBLIC_KEY` / `LANGFUSE_SECRET_KEY` | Langfuse smoke (#501, conditional) |
| `UMMAYA_OTEL_ENDPOINT` | OTLP dry-run (#501, conditional) |
...
1. Locate any occurrence of `UMMAYA_DATA_GO_KR_KEY` (line 53 at time of spec).
2. Replace with `UMMAYA_DATA_GO_KR_API_KEY`.
...
1. Edit secret in Infisical dashboard (`UMMAYA` project → `test` env → `UMMAYA_FRIENDLI_TOKEN`).
