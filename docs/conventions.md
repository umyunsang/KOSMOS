...
Detailed development conventions for KOSAX. `AGENTS.md` summarizes the cross-cutting rules; this file is the long form.
...
All environment variables must be prefixed `KOSAX_`.

| Variable | Purpose |
|---|---|
| `KOSAX_FRIENDLI_TOKEN` | FriendliAI Serverless API token |
| `KOSAX_DATA_GO_KR_KEY` | data.go.kr API key |
| `KOSAX_LOG_LEVEL` | stdlib logging level (default `INFO`) |
...
- Default level `INFO`, configurable via `KOSAX_LOG_LEVEL`
...
KOSAX uses [Conventional Commits](https://www.conventionalcommits.org/).
...
```bash
TASK_ID=$(gh api graphql -f query='query{repository(owner:"umyunsang",name:"KOSAX"){issue(number:TASK_NUM){id}}}' --jq '.data.repository.issue.id')
gh api repos/umyunsang/KOSAX/issues/EPIC_NUM/sub_issues --method POST -f sub_issue_id="$TASK_ID"
```
...
```bash
gh api repos/umyunsang/KOSAX/issues/EPIC_NUM/sub_issues --jq '.[].number' | while read num; do
...
2. Regenerate `prompts/manifest.yaml` via `uv run python -m kosax.context.prompt_loader --regenerate-manifest`.
