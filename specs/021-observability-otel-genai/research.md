...
- **Rationale**:
  - UMMAYA의 LLM 호출은 FriendliAI Serverless에 `httpx.AsyncClient`로 직접 요청(OpenAI-compatible endpoint). `openai` Python SDK의 `ChatCompletion` 객체가 존재하지 않기 때문에 auto-instrumentor가 훅 걸 대상이 원천적으로 없다.
...
- **Decision**: 세 종류 span의 이름·속성 셋은 OTel GenAI semconv v1.40을 따른다.
  - `invoke_agent ummaya-query` — 속성: `gen_ai.operation.name=invoke_agent`, `gen_ai.agent.name=ummaya-query`, `gen_ai.conversation.id=<session_id>`
...
- **Alternatives considered**:
  ...
  - 자체 attribute key(`ummaya.llm.*`): Langfuse 자동 인식 손실. 기각.
...
- **Alternatives considered**:
  - 자체 feature flag(`UMMAYA_OTEL_ENABLED`): OTel 표준 변수와 이중 경로 발생. 기각.
...
- **Reference**: OTel GenAI spec — streaming usage aggregation convention. 내부 레퍼런스: `src/ummaya/llm/usage.py`의 기존 누산 패턴.

### D7. 429 retry counter를 별도 metric으로 추가

- **Decision**: FriendliAI가 `429 Too Many Requests`를 반환해 LLM 재시도가 발생할 때마다 `ummaya_llm_rate_limit_retries_total` counter를 1 증가. `Retry-After` 헤더 존중 기존 로직(spec 019)은 변경 없음.
...
- **Decision**: `src/ummaya/observability/event_logger.py`의 `_ALLOWED_METADATA_KEYS` frozenset(`tool_id`, `step`, `decision`, `error_class`, `model`)을 `otel_bridge.py`에서 import하여 span 속성 prefilter에도 사용한다. 중복 whitelist 정의 금지.
- **Rationale**: 두 곳에 whitelist를 두면 한쪽만 업데이트되는 drift 위험. Constitution II(fail-closed) 유지.
- **Reference**: 기존 `src/ummaya/observability/event_logger.py:45-47` 주석 "(AC-A10)".
...
| Streaming usage aggregation | OTel GenAI spec + `src/ummaya/llm/usage.py` 누산 | LLM Client |
