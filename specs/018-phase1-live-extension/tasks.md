...
- Repo root: `/Users/um-yunsang/KOSAX/`
...
- [ ] T002 [P] Create new test module `tests/live/test_live_observability.py` with SPDX header, module docstring noting required env vars (`KOSAX_FRIENDLI_TOKEN`, `KOSAX_DATA_GO_KR_API_KEY`), and pytest imports. File ends with no test functions yet.
...
- [ ] T003 Add `kakao_api_key` session-scoped fixture to `tests/live/conftest.py`. Reads `KOSAX_KAKAO_API_KEY`; on unset/whitespace-only, calls `pytest.fail("set KOSAX_KAKAO_API_KEY to run live geocoding tests")` — exact string per FR-004/contracts/fixtures.md. No silent skip, no xfail, no formatting variation.
...
**Independent Test**: `uv run pytest -m live -v tests/live/test_live_geocoding.py` — all 7 pass with valid `KOSAX_KAKAO_API_KEY` and Kakao Local API activated. Verifies SC-001.
...
- [ ] T017 [P] Verify SC-004 (hard-fail contract). Run `unset KOSAX_KAKAO_API_KEY && uv run pytest -m live -v tests/live/test_live_geocoding.py 2>&1 | grep -F "set KOSAX_KAKAO_API_KEY to run live geocoding tests"`. Expect: exact string in failure output; exit code non-zero; no `SKIPPED` or `XFAIL` lines for geocoding tests. Document the command output in the PR body.
...
- **Exact hard-fail string** — `set KOSAX_KAKAO_API_KEY to run live geocoding tests` (no period, no prefix). SC-004 greps for this literal.
