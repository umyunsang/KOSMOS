...
The backend emits exactly one `AdapterManifestSyncFrame` after `register_all_tools()` completes successfully and `build_routing_index()` validates. The frame is the **first non-handshake frame** the TUI receives. Implementation in `src/ummaya/ipc/adapter_manifest_emitter.py`:
...
async validateInput(input, context) {
  ...
  if (backendEntry) {
    const citation = backendEntry.policy_authority_url
      ...
      : null;
    setUmmayaCitations(context, citation);
    return { result: 'allow' };
  }
