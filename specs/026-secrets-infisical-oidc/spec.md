# Feature Specification: Secrets & Config — Infisical OIDC + 12-Factor + UMMAYA_* Registry

**Feature Branch**: `feat/468-secrets-config`
**Created**: 2026-04-17
**Status**: Draft
**Input**: User description: Epic #468 — Secrets & Config: Infisical OIDC + 12-Factor + UMMAYA_* registry. Two converging failure modes motivate this work: (1) secret hygiene — long-lived GitHub Encrypted Secrets are the retired 2025 pattern; 2026 standard is short-lived OIDC federation; (2) env-var registry drift — Epic #458 (closed 2026-04-16) burned a day misdiagnosing a missing `UMMAYA_KAKAO_API_KEY` as an external FriendliAI CJK-decoder defect. UMMAYA has no authoritative registry, no fail-fast startup guard, and no `.env.example`. Scope: (A) Infisical Cloud Free OIDC migration; (B) canonical `docs/configuration.md` registry; (C) fail-fast startup guard `src/ummaya/config/guard.py`; (D) 12-Factor cleanup + audit scripts.

## User Scenarios & Testing *(mandatory)*

### User Story 1 — Fail-fast startup guard prevents silent mis-configuration (Priority: P1)

A contributor clones UMMAYA for the first time, runs the CLI without populating `.env`, or loses a secret during a rotation window. Today, the process boots, the LLM accepts the session, and only downstream tool calls silently degrade — which is exactly how Epic #458 (closed 2026-04-16) consumed a day of wrong-root-cause investigation before the root cause (missing `UMMAYA_KAKAO_API_KEY`) was found. With the guard in place, the CLI exits within 100 ms and prints a single-line message listing every missing required variable plus a stable pointer to `docs/configuration.md`.
...
1. **Given** a fresh clone with no `.env` and no shell `UMMAYA_*` exports, **When** the contributor runs the CLI entry point, **Then** the process exits with a non-zero status within 100 ms and `stderr` contains exactly one line naming every missing required variable plus the path `docs/configuration.md`.
...
3. **Given** a single required variable is unset (e.g., `UMMAYA_KAKAO_API_KEY`), **When** the CLI starts, **Then** the single-line message names exactly that variable and no other.
4. **Given** a conditional-required variable is unset but its activation flag (`UMMAYA_ENV=prod`) is absent, **When** the CLI starts in `dev` mode, **Then** the guard does not treat the conditional variable as missing.
5. **Given** a live-suite test fixture runs, **When** the fixture boots UMMAYA, **Then** the guard is invoked and fails the suite with the same single-line message if any live-suite-required variable is missing.

---

### User Story 2 — Canonical env-var registry replaces scattered defaults (Priority: P2)

A contributor adding a new tool adapter — or a reviewer auditing a PR — needs a single authoritative answer to "what `UMMAYA_*` variables exist, which are required, what the default is, which module consumes them, and where to get the credential." Today, the answer is split across `src/ummaya/settings.py` (7 fields), `src/ummaya/llm/config.py` (6 fields with their own alias pattern), `src/ummaya/cli/config.py` (3 fields plus `UMMAYA_THEME`), `src/ummaya/permissions/credentials.py` (per-tool override pattern + legacy `UMMAYA_API_KEY` fallback), and scattered `os.environ.get()` calls. The Epic body's nine-row table is a subset of the actual surface — discovered during spec research, `grep -rhoE "UMMAYA_[A-Z_]+"` returned 17 distinct names in `src/` alone and another eight test-only variables. The new `docs/configuration.md` enumerates every variable with columns `Variable | Required | Default | Range | Consumed by | Source doc`, and a CI-enforced audit script fails the build the moment code and registry diverge.
...
**Independent Test**: Review `docs/configuration.md` and confirm every variable returned by `grep -rhoE "UMMAYA_[A-Z_]+" src/ --include="*.py"` is present with all six schema columns filled. Invoke `scripts/audit-env-registry.py`; exit code is `0` when code and registry agree, non-zero with a diff-style report when they disagree.

**Acceptance Scenarios**:

1. **Given** the current `src/` tree, **When** `scripts/audit-env-registry.py` is run, **Then** the script exits `0` and prints no drift.
2. **Given** a contributor adds `os.environ.get("UMMAYA_NEW_THING")` to a source file without updating `docs/configuration.md`, **When** CI runs the audit, **Then** CI fails and the report names `UMMAYA_NEW_THING` as present in code but absent from registry.
...
4. **Given** a reviewer opens `docs/configuration.md`, **When** they search for the canonical source of any `UMMAYA_*` name referenced in any PR, **Then** the registry returns exactly one row with all six schema columns.
...
The CI operator (today, the project owner; tomorrow, any collaborator) needs to run the test suite — including the 33-test live suite — without storing any long-lived API token as a GitHub Encrypted Secret. Today, `.github/workflows/ci.yml` injects `UMMAYA_DATA_GO_KR_KEY`, `UMMAYA_KOROAD_API_KEY`, `UMMAYA_FRIENDLI_TOKEN` as hard-coded placeholder strings for mocked tests, while the real live-suite keys would need to live as Encrypted Secrets. After migration, `Infisical/secrets-action@v1` exchanges the GitHub Actions OIDC token for a short-lived Infisical service identity token at job start, fetches the current `UMMAYA_*` values from Infisical project `ummaya-ci`, and injects them as environment variables scoped to the job. No secret material ever lives in `.github/workflows/*.yml` or in the GitHub repository Settings → Secrets page.
...
2. **Given** the operator rotates `UMMAYA_FRIENDLI_TOKEN` inside Infisical only, **When** the next CI run is triggered, **Then** the run succeeds with no source or workflow file change in git.
...
5. **Given** the GitHub repository `Settings → Secrets and variables → Actions` page, **When** an auditor inspects it after migration, **Then** no `UMMAYA_*` token is stored there; at most a bootstrap `INFISICAL_CLIENT_ID` exists, and preferably nothing at all if pure OIDC federation is used.

---

### Edge Cases

- **`.env` is a symlink**: The contributor's `.env` is a symlink into a secret-management tool (e.g., macOS Keychain-backed mount, 1Password CLI shim, a sibling worktree). The guard and the stdlib loader (`src/ummaya/_dotenv.py`) MUST read through the symlink without following, overwriting, rewriting, or warning about it. No code path in this Epic's scope may write to `.env`.
- **Variable with empty string value**: `UMMAYA_KAKAO_API_KEY=""` is semantically the same as "unset" and MUST be treated as missing by the guard.
- **Variable with whitespace-only value**: `UMMAYA_KAKAO_API_KEY="   "` MUST be treated as missing after `.strip()`.
- **Conditional-required variable activation**: When `UMMAYA_ENV=prod` or `UMMAYA_ENV=ci`, `UMMAYA_OTEL_ENDPOINT`, `LANGFUSE_PUBLIC_KEY`, and `LANGFUSE_SECRET_KEY` graduate from "optional" to "required". When `UMMAYA_ENV` is absent or equals `dev`, they stay optional.
- **Legacy `UMMAYA_API_KEY` fallback**: The permissions module's global fallback pattern is an existing escape hatch. The registry MUST document it as `deprecated` and forbid its use for newly added tools, without breaking current tool lookups.
- **Per-tool `UMMAYA_<TOOL_ID>_API_KEY` override**: An intentional escape hatch for per-tool credential isolation (e.g., the shared `data.go.kr` key being overridden by a KOROAD-specific key). The registry MUST document the pattern as a family with a single schema row, not enumerate every possible tool id.
- **Shell export wins over `.env`**: CI/CD, systemd, and Docker injection set variables in the process environment before Python starts. The loader MUST never overwrite a pre-existing `os.environ` entry — a property the current `src/ummaya/_dotenv.py` already guarantees and this Epic MUST preserve.
...
- **FR-005**: The guard MUST treat conditional-required variables as required only when their activation flag (`UMMAYA_ENV ∈ {prod, ci}` for observability variables) is set; otherwise they remain optional.
...
- **FR-010**: Project MUST ship a new file `docs/configuration.md` as the single authoritative registry of every `UMMAYA_*` variable and every explicitly allowlisted non-`UMMAYA_` variable (currently: `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`).
...
- **FR-012**: The registry MUST enumerate, at minimum, every variable discovered by `grep -rhoE "UMMAYA_[A-Z_]+" src/ --include="*.py"` at the time of merge. Test-only variables (`UMMAYA_AUTH_TEST_TOOL_API_KEY`, `UMMAYA_SKIP_PERF`, etc.) MAY be grouped under a dedicated "Test-only" subsection.
- **FR-013**: The registry MUST document the legacy fallback `UMMAYA_API_KEY` with `Required: ❌ (deprecated)` and a note forbidding its use for newly added tools.
- **FR-014**: The registry MUST document the per-tool override pattern `UMMAYA_<TOOL_ID>_API_KEY` as a single family row, not one row per tool id.
- **FR-015**: The registry MUST document conditional-required variables (`UMMAYA_OTEL_ENDPOINT`, `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`) with `Required: ⚠️` and a note specifying the activation condition (`UMMAYA_ENV ∈ {prod, ci}`).
...
- **FR-020**: Project MUST ship `scripts/audit-env-registry.py` that (a) parses the `docs/configuration.md` registry table, (b) greps the `src/` tree for every `UMMAYA_*` string literal, (c) greps `.env.example` for every `UMMAYA_*` entry, and (d) exits non-zero with a diff-style report when code, registry, and example diverge.
...
- **FR-023**: Project MUST ship `scripts/audit-secrets.sh` that greps `.github/workflows/ci.yml` for long-lived secret patterns (assignment of literal token strings, `secrets.UMMAYA_*` references that aren't the bootstrap identity) and exits non-zero on any match.
...
- **FR-030**: `.github/workflows/ci.yml` MUST replace every long-lived `UMMAYA_*` secret reference with a single `Infisical/secrets-action@v1` step that exchanges the GitHub Actions OIDC token for a short-lived Infisical service-identity token and fetches the job's variables from the Infisical project `ummaya-ci`.
- **FR-031**: The migration MUST NOT retain any `UMMAYA_*` value as a GitHub Encrypted Secret. A bootstrap `INFISICAL_CLIENT_ID` (or equivalent non-secret identity identifier) is permitted only if pure OIDC federation cannot be configured.
...
- **FR-040**: Every environment variable read by `src/` code MUST start with the prefix `UMMAYA_`, with the single documented exception family `LANGFUSE_*` (owned by the observability vendor's SDK convention).
- **FR-041**: The `src/ummaya/_dotenv.py` loader MUST continue to honour the "shell wins over `.env`" rule; this Epic MUST NOT regress that property.
...
- **FR-043**: The registry MUST explicitly name `LANGFUSE_*` as the only permitted non-`UMMAYA_*` prefix and explain why (vendor SDK default).

#### F. Defects fixed (mandatory sub-scope)

- **FR-050**: `.github/workflows/ci.yml` currently injects `UMMAYA_DATA_GO_KR_KEY` as a job environment variable, but `src/ummaya/settings.py` and `src/ummaya/permissions/credentials.py` both read `UMMAYA_DATA_GO_KR_API_KEY`. This typo MUST be fixed in this Epic (rename the workflow env var to match the code).
- **FR-051**: Epic #468 body references `UMMAYA_KAKAO_REST_KEY` as living in `docs/tool-adapters.md`. The actual stale occurrence is at `docs/design/mvp-tools.md:642`. The spec MUST surface this location correction; `docs/design/mvp-tools.md:642` MUST be updated to the canonical name `UMMAYA_KAKAO_API_KEY` (FR-051 is in-scope file surface; other prose edits to `docs/tool-adapters.md` remain limited to typo fixes only, per the Lead constraint).
- **FR-052**: Any other documentation occurrence of `UMMAYA_KAKAO_REST_KEY` discovered during the registry audit MUST be rewritten to `UMMAYA_KAKAO_API_KEY` as a single atomic find-and-replace commit inside the allowed file surface.
...
- **NFR-005** (Portability — stdlib-only loader): `src/ummaya/_dotenv.py` MUST NOT acquire a new third-party dependency; the AGENTS.md hard rule against unsolicited dependency additions applies.
- **NFR-006** (Extensibility): Adding a new `UMMAYA_*` variable MUST be a three-file change — registry, `.env.example`, and the consuming module — with no schema migration and no registry re-ordering.
- **NFR-007** (Backwards compatibility): Existing production invocations that populate `UMMAYA_*` via the shell or CI environment MUST continue to work without modification through this Epic's rollout.
...
- **Environment Variable Registry**: The canonical table of every `UMMAYA_*` (and allowlisted `LANGFUSE_*`) variable. Each row has: variable name, required level (required / conditional / optional / deprecated), default value (or `—`), accepted range, consuming module path, and source-of-credential doc. Persisted as `docs/configuration.md`; machine-parsed by `scripts/audit-env-registry.py`.
- **Startup Guard**: A boundary validator invoked once per process at CLI entry (and per-fixture in live tests). Inputs: the in-process required-name list, the activation flag `UMMAYA_ENV`, and `os.environ`. Outputs: exit status 0 (quiet) or non-zero with a single `stderr` line.
...
- **SC-003** (SC-3 from Epic): `docs/configuration.md` documents every `UMMAYA_*` variable (and allowlisted `LANGFUSE_*`) present in `src/`. Running `scripts/audit-env-registry.py` returns exit `0` and no drift.
...
- **SC-006** (SC-6 from Epic — #458 regression guard): Starting UMMAYA with an empty `.env` fails within 100 ms with a single-line `stderr` message listing every missing required variable plus `docs/configuration.md`. Verified by `tests/config/test_guard.py` using `time.monotonic()`.
...
- Infisical Cloud Free tier has sufficient headroom for (a) two projects (`ummaya-prod`, `ummaya-ci`), (b) the current ~10 `UMMAYA_*` and `LANGFUSE_*` secret count, and (c) the audit-log retention needed for a student portfolio project. If this assumption is violated during plan Phase 0, work halts per the stop conditions in the Lead prompt.
...
- `UMMAYA_ENV` is the activation flag for conditional-required observability variables. Valid values: `dev` (default, observability optional), `ci` (observability required), `prod` (observability required). No other value is recognised; unknown values fall through to `dev` semantics.
...
- The canonical `UMMAYA_*` names defined by Epic #507 (`UMMAYA_KAKAO_API_KEY`, `UMMAYA_LOOKUP_TOPK`, `UMMAYA_NMC_FRESHNESS_MINUTES`) are authoritative for this registry. If #507 renames any of them before this Epic merges, the registry is updated in lockstep.
- The `LANGFUSE_*` prefix is the only permitted exception to the `UMMAYA_` hard rule. Any future vendor-SDK exception requires an explicit addition to the registry's exception list.
...
- **Rewriting `src/ummaya/safety/`** — owned by a different Epic; the guard lives under `src/ummaya/config/` and must not reach into `safety/`.
...
| LiteLLM-specific secrets (`UMMAYA_LITELLM_MASTER_KEY`, `UMMAYA_LLM_BASE_URL`, `UMMAYA_SESSION_TOKEN_BUDGET`, `UMMAYA_USER_DAILY_BUDGET_USD`, `UMMAYA_MAX_FAILED_TOOL_INVOCATIONS`, `UMMAYA_LLM_FALLBACK_ENABLED`) | Variables are not yet consumed by any `src/` code; adding rows now would violate the "registry mirrors reality" invariant | Epic #465 (LiteLLM proxy) | #465 |
| Observability secret wiring (`UMMAYA_OTEL_ENDPOINT`, `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`) code paths | This Epic catalogues the names and marks them conditional-required; the actual observability code lives in #501 | Epic #501 (Observability) | #501 |
...
| Conversion of `security.yml`, `sbom.yml`, `eval.yml`, `pr-lint.yml` to Infisical injection | Those workflows currently use no `UMMAYA_*` secrets, so migration is a no-op; revisit only if they acquire a secret need | Epic #467 follow-up | #743 |
| Deprecation removal of legacy `UMMAYA_API_KEY` global fallback | Removing the fallback requires a cross-tool refactor to eliminate any remaining caller; out of scope for a secrets/config Epic | Separate refactor Epic | #744 |
...
|--------|---------------|------------|------------|
| `.github/workflows/ci.yml` injects `UMMAYA_DATA_GO_KR_KEY`, but `src/ummaya/settings.py` and `src/ummaya/permissions/credentials.py` both read `UMMAYA_DATA_GO_KR_API_KEY` | Typo; placeholder value is never consumed by production code paths, so CI passed "by accident" | Rename the workflow env var to `UMMAYA_DATA_GO_KR_API_KEY` as part of the Infisical migration edit | FR-050, SC-001 |
| Epic #468 body claims a stale `UMMAYA_KAKAO_REST_KEY` reference lives in `docs/tool-adapters.md`, but `grep` finds the only hit at `docs/design/mvp-tools.md:642` | Registry body location error; the stale name survived because no audit existed | Rewrite `docs/design/mvp-tools.md:642` to the canonical `UMMAYA_KAKAO_API_KEY`; scan-and-replace any other documentation hit surfaced by the audit | FR-051, FR-052, SC-003 |

## Cross-Epic Contracts

| Epic | Relationship | Contract |
|------|--------------|----------|
| #458 (CLOSED, 2026-04-16) | Regression guard | User Story 1 + SC-006 exist specifically to prevent the "missing `UMMAYA_KAKAO_API_KEY` silently degrades tool calls" class of bug that consumed a day on #458 |
| #507 | Upstream name authority | Registry rows for `UMMAYA_KAKAO_API_KEY`, `UMMAYA_LOOKUP_TOPK`, `UMMAYA_NMC_FRESHNESS_MINUTES` MUST match the canonical names #507 defines. If #507 renames, this Epic updates in lockstep |
| #501 | Catalogue-only | This Epic catalogues `UMMAYA_OTEL_ENDPOINT`, `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY` as conditional-required; the consuming code and wiring live in #501 |
...
|-------------------------------------|--------------------------------------------------|
| `src/ummaya/config/` (new module, including `guard.py`) | `.env` (symlink — absolute no-write) |
| `tests/config/` (new test package) | `docker/`, `.devcontainer/` (Epic #467) |
| `docs/configuration.md` (new) | `prompts/`, `src/ummaya/safety/` (other Epics) |
| `docs/tool-adapters.md` (typo fix only, if any `UMMAYA_KAKAO_REST_KEY` survives) | `.github/workflows/docker.yml`, `.github/workflows/shadow-eval.yml` (Epic #467) |
| `docs/design/mvp-tools.md:642` (FR-051 one-line rewrite) | Sibling worktrees `/Users/um-yunsang/UMMAYA-467`, `/Users/um-yunsang/UMMAYA-585`, `/Users/um-yunsang/UMMAYA-466` (parallel work) |
...
- AGENTS.md § Hard rules — "Env vars prefixed `UMMAYA_`. Never commit `.env` or `secrets/`." — anchors FR-040, FR-042.
