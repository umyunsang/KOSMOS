...
Brief: Add `CC reference: services/api/claude.ts:<line-range>` citations to every handler in `tui/src/ipc/llmClient.ts`. Add `// SKIPPED — KOSAX-N/A: <reason>` comments for the 4 channels K-EXAONE doesn't emit. NO functional changes — only comment additions.
