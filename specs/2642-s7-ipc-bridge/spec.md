...
**Author**: Lead Opus (KOSAX Epic F · S7 slice)
...
1. **`remote/` 4-file DROP + `directConnectManager` dead type-stub cleanup.**
   ...
   WebSocket bridge. KOSAX swap-1 (LLM = K-EXAONE on FriendliAI) and
   swap-2 (Tool = GovAPITool) replace claude.ai entirely; the cloud
   session surface has no KOSAX use-case. The 4 files were never ported
...
2. **`notification_push` IPC arm — CC baseline verification + SWAP
   ...
   Ghostty / bell), all in-process. KOSAX's `notification_push` arm
   (`src/kosax/ipc/frame_schema.py:768`) is a **swap-2 add-on**
...
4. **`mcpb-compat.ts` ADR-009 (KOSAX-original innovation).**
   ...
   this is a KOSAX-original optimization. Audit § 2.5 + § 5 Finding 5
...
KOSAX = CC-original harness + 2 swaps. Every action above is justified
under that thesis:

| Action | Swap class | Justification |
|---|---|---|
| 1. `remote/` DROP + `directConnectManager` cleanup | swap-2 (claude.ai sync removed) | claude.ai backed remote sessions are not part of KOSAX surface |
...
| 3. codec.ts ↔ Python envelope drift CI | swap-implementation transport | stdio JSONL is KOSAX's swap-implementation; field-level parity guards swap correctness |
| 4. mcpb-compat.ts ADR | KOSAX-original innovation (perf optimization) | byte-identical CC default does not apply — CC has no equivalent; ADR captures the deviation |
...
**As a** KOSAX maintainer auditing the TUI for swap-2 leftovers,
...
- `tui/src/screens/REPL.tsx` retains a comment: `// KOSAX-2642 / Epic F · S7 — directConnect/remote/ DROPPED (claude.ai sync swap-out).`
- `bun typecheck` (KOSAX narrow) PASS.
...
**so that** future drift audits can compare KOSAX's IPC surface to
CC's call signatures without re-discovering this finding.

**Acceptance**:
- `src/kosax/ipc/frame_schema.py:NotificationPushFrame` docstring extended with explicit "CC parity: NO equivalent — CC uses terminal OSC via `ink/useTerminalNotification.ts` (iTerm2/Kitty/Ghostty/bell), in-process. This arm is KOSAX swap-2 (Spec 031 SubscriptionHandle for Korean civic push channels)."
...
schema diverges from `src/kosax/ipc/frame_schema.py`'s `_BaseFrame`
field constraints,
...
- A negative fixture `tests/ipc/fixtures/codec_drift_negative.ts` — a 30-line text file that re-declares the envelope with `correlation_id` made optional — proves the test fails when fed the negative fixture (toggled by a `KOSAX_IPC_PARITY_DRIFT_FIXTURE=1` env var that swaps the codec.ts path before parsing; default off, so production CI uses the real codec.ts).
...
**I want** ADR-009 to capture the KOSAX-original lazy-load shim
...
- Add comment `// KOSAX-2642 / Epic F · S7 — directConnect/server/ DROPPED (claude.ai sync swap-out).`
...
| SC-011 | TUI Layer 5 tmux-capture smoke shows healthy chat boot | `specs/2642-s7-ipc-bridge/scripts/smoke-2642.sh` runs and `snap-NNN-*.txt` includes `KOSAX` branding + `tool_registry: 24 entries verified` (or current count) |
...
- `docs/requirements/kosax-migration-tree.md` — L1-A § A6 (Error recovery) + L1-B § B1 (Tool wrapping = work unit).
- `specs/cc-migration-audit/scope-S7-ipc-bridge.md` — full S7 audit
  (44 CC files vs 74 KOSAX files mapping).
...
- `tui/src/mcpb-compat.ts` — KOSAX-original lazy shim (FR-010 of Spec 2293).
...
| Negative fixture not isolated → real CI uses it | L | env-var gate `KOSAX_IPC_PARITY_DRIFT_FIXTURE=1`; tests/ipc/conftest.py asserts default OFF |
