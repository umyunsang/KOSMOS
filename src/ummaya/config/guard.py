...
"""Fail-fast startup guard for required UMMAYA_* environment variables.
...
_DOC_URL: Final[str] = "https://github.com/umyunsang/UMMAYA/blob/main/docs/configuration.md"
...
_REQUIRED_VARS: Final[tuple[RequiredVar, ...]] = (
    ...
    RequiredVar(
        name="UMMAYA_OTEL_ENDPOINT",
        consumer="ummaya.observability.otel (#501)",
        required_in=frozenset({"prod"}),
        doc_anchor="#ummaya_otel_endpoint",
    ),
)
...
def verify_startup() -> None:
    ...
    if diag is None:
        return
    message = (
        f"UMMAYA config error [env={diag.env}]: "
