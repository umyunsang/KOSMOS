...
- Backend: `httpx ≥ 0.27` (FriendliAI HTTP client), `pydantic ≥ 2.13` (frame_schema models + LLM tool I/O), `pydantic-settings ≥ 2.0` (env catalog), `opentelemetry-sdk` + `opentelemetry-semantic-conventions` (Spec 021 GenAI spans, extended with `ummaya.frame.kind` per Spec 032).
...
- Consent receipts: `~/.ummaya/memdir/user/consent/` (existing memdir per Spec 027 + Spec 035).
- Session JSONL: `~/.ummaya/memdir/user/sessions/<session_id>.jsonl` (existing, unchanged).
...
| **IV. Government API Compliance** | ✅ Demo scenario uses an existing adapter validated under Spec 1637-p6-docs-smoke (memory `reference_koroad_portal` for emergency / KOROAD coverage). No new adapters. No live `data.go.kr` calls in CI tests; PTY scenario harness uses fixture-backed mock when `UMMAYA_LIVE_API=0`. |
| **V. Policy Alignment** | ✅ Surfaces PIPA gauntlet (Principle 8/9 — citizen consent over single conversational window). `ummaya-migration-tree.md § L1-A/B/C` pillars unchanged; this Epic only completes their TUI binding. |
...
tui/src/hooks/
└── useApiKeyVerification.ts   ← rewritten as UMMAYA-aware: returns 'valid' when
                                  UMMAYA_FRIENDLI_TOKEN present, never calls anthropic.com
...
**Structure Decision**: UMMAYA-native multi-process layout (Backend Python + TUI TS over stdio NDJSON). Matches existing Spec 032 / 287 / 1633 / 1634 conventions. No new top-level directories.
...
2. Copy the relevant function / module / type into the UMMAYA path
3. Adapt to the UMMAYA shape — applying the rewrite boundary (Constitution §I)
   - services/api/* → UMMAYA Python backend over stdio JSONL
   - tools/* → thin renderers over UMMAYA's 5-primitive surface
   - net-new domain layers (Korean IME, public-API adapters, PIPA permission, swarm mailbox) → UMMAYA-original with header `// UMMAYA-original — no upstream analog`
...
```
- [ ] T0NN [P?] [USx] <action> in <UMMAYA path>
       Ref: .references/claude-code-sourcemap/restored-src/src/<file>:<lines>
       Action: copy → adapt — <UMMAYA deviation in 1 line>
       OR: UMMAYA-original (closest CC pattern: <reference>)
```

This pattern is **load-bearing for every Phase below**. Task bodies missing a Ref line (or a justified UMMAYA-original claim) will be flagged in `/speckit-analyze`. The pattern is what makes "≥ 90% CC fidelity" measurable rather than aspirational.
...
- Read `tui/src/query/deps.ts:queryModelWithStreaming` (the UMMAYA-1633 P2 stub adapter): confirm `UmmayaToolDefinition[]` is already prepared, just not delivered across the IPC boundary.
...
- Read `src/ummaya/ipc/frame_schema.py` lines 109-200: confirm `_BaseFrame` is the right superclass; `_ROLE_KIND_ALLOW_LIST` and `_TERMINAL_KINDS` extension points.
...
- Read `src/ummaya/permissions/pipeline.py:PermissionPipeline.evaluate()` signature.
...
- Read `ummaya.observability.semconv` GenAI attributes (Spec 021).
- Read Spec 028 OTEL collector config for `ummaya.frame.kind` attribute.
- Decision: span hierarchy is `ummaya.session > ummaya.turn > ummaya.frame{kind} > gen_ai.{request,response}`. Tool calls are children of `ummaya.turn`, parented by frame_seq order.
...
**Decision**: TUI `bun run tui` spawns `uv run python -m ummaya.ipc.mcp_server` once at startup (parallel to the main `ummaya --ipc stdio` bridge spawn) and reuses the connection for the session lifetime. Cold-start budget remains 500 ms per `mcp.ts:COLD_BUDGET_MS` but is paid during boot, not during the first tool turn.
...
### ADR-0004 — Span hierarchy: `ummaya.session > ummaya.turn > ummaya.frame{kind}` with `correlation_id` propagation

**Decision**: 
- `ummaya.session` span opens at `ummaya --ipc stdio` startup, closes at `session_event{event=exit}`.
- `ummaya.turn` span opens at each `ChatRequestFrame` receive, closes at the final `assistant_chunk{done=True}` (or terminal `error` / `tool_result{is_final=True}` chain).
- Per-frame spans (`ummaya.frame.tool_call`, `ummaya.frame.tool_result`, `ummaya.frame.permission_request`, `ummaya.frame.permission_response`) child of `ummaya.turn`, ordered by `frame_seq`.
- Existing GenAI spans (`gen_ai.client.operation` per Spec 021) become children of `ummaya.turn`.
- `correlation_id` (UUIDv7) is the trace-correlation key — emitted on every span attribute set (`ummaya.correlation_id`).
...
- Spec 028 OTEL collector already accepts `ummaya.frame.kind`. No collector config change needed.
...
- Crash recovery: TUI persists history to `~/.ummaya/memdir/user/sessions/<session_id>.jsonl` per existing Spec 027 — this is the only persistence path; backend has nothing to replay from.
...
| **Consent Receipt** | `~/.ummaya/memdir/user/consent/<receipt_id>.json` | Until citizen revokes |
...
1. From `UMMAYA-wiring` worktree: `unset ANTHROPIC_API_KEY`, `unset ANTHROPIC_AUTH_TOKEN` (defence in depth — verify FR-004 absence regression).
2. Set `UMMAYA_FRIENDLI_TOKEN` + `UMMAYA_DATA_GO_KR_API_KEY`.
...
8. Inspect `/tmp/ummaya-tui.log` (UMMAYA_TUI_LOG_LEVEL=DEBUG) for `gen_ai.*` + `ummaya.frame.*` span lines.
...
|---|---|
| I. Reference-Driven Development | ✅ All ADRs cite CC restored-src as primary; UMMAYA-original deviations (stdio bridge integration) documented |
