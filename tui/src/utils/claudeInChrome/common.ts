      ...
      case 'macos': {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            logForDebugging(
              `[UMMAYA in Chrome] Detected browser: ${config.name}`,
            )
            return browserId
          }
        } catch (e) {
...
export async function openInChrome(url: string): Promise<boolean> {
  ...
  if (!browser) {
    logForDebugging('[UMMAYA in Chrome] No compatible browser found')
    return false
  }
...
function getSocketName(): string {
  // NOTE: This must match the one used in the UMMAYA in Chrome MCP
