...
**Created**: 2026-04-20
**Status**: Draft
**Input**: User description: "Epic H #1302 — Onboarding + 브랜드 port. 참조: docs/tui/component-catalog.md (Epic H row), docs/design/brand-system.md §1-§2, docs/tui/accessibility-gate.md §7, ADR-006 Part A-9 (kosaxCore/orbitalRing), .references/claude-code-sourcemap/restored-src/src/components/LogoV2/ + Onboarding.tsx. 목표: 브랜드 팔레트 값 확정 + Onboarding 스플래시 + LogoV2 REWRITE 시각 사양."

**Epic**: [#1302 — Onboarding + brand port (binds ADR-006 A-9)](https://github.com/umyunsang/KOSAX/issues/1302)
**Parent Initiative**: [#2 — Phase 2 Multi-Agent Swarm](https://github.com/umyunsang/KOSAX/issues/2)
...
- `assets/kosax-logo.svg`, `assets/kosax-banner-dark.svg` (authoritative palette extraction source per ADR-006 A-9)
...
### User Story 1 — Citizen sees the KOSAX orbital-ring splash on first launch (Priority: P1)

A first-time citizen user launches the KOSAX TUI from a terminal. Instead of the current placeholder screen that inherits the Claude Code "Welcome to Claude Code" wordmark on a cyan (`rgb(0,204,204)`) background, the user sees the KOSAX galaxy splash: a navy deep-space background, a glowing central asterisk ("kosaxCore") surrounded by an orbital ring, with the wordmark "KOSAX" and subtitle "KOREAN PUBLIC SERVICE MULTI-AGENT OS", and four satellite nodes around the ring indicating the Phase 1 ministry adapters. The splash communicates the 은하계 metaphor — one conversational window unifying many ministry APIs — without requiring the citizen to read any documentation.

**Why this priority**: The splash is the first citizen-facing surface in the entire session. Shipping the TUI with Claude Code brand residue (`Welcome to Claude Code` wordmark, `claude` / `clawd` tokens, placeholder cyan background) directly contradicts the KOSAX mission stated in `docs/vision.md § What is original to KOSAX` and the ADR-006 A-9 commitment to a citizen-legible Korean public-service brand. Without this, no downstream citizen-facing claim (Korea AI Action Plan 공공AX Principle 9, PIPA transparency, "single conversational window") is honestly testable.

**Independent Test**: Launch the TUI in a fresh session. Verify that within the first frame (a) the wordmark is "KOSAX", not "Claude Code", (b) the background anchor colour is navy `#0a0e27`, not cyan `rgb(0,204,204)`, (c) the orbital-ring and core gradients render using the ADR-006 A-9 palette, (d) four labelled satellite nodes are visible for the Phase 1 ministries.

**Acceptance Scenarios**:

1. **Given** a fresh KOSAX session with no prior onboarding state, **When** the TUI renders its first frame, **Then** the splash displays the "KOSAX" wordmark in the `wordmark` token (`#e0e7ff`), the "KOREAN PUBLIC SERVICE MULTI-AGENT OS" subtitle in the `subtitle` token (`#94a3b8`), against a navy background (`#0a0e27` → `#1a1040` gradient anchor).
2. **Given** the splash is rendered, **When** the user observes the centre of the screen, **Then** a glowing "kosaxCore" asterisk (`#818cf8` → `#6366f1` gradient) is visible, surrounded by an "orbitalRing" arc (`#60a5fa` → `#a78bfa` gradient).
...
4. **Given** the splash is rendered in a terminal with `NO_COLOR=1` or `KOSAX_REDUCED_MOTION=1`, **When** the first frame renders, **Then** a static-text equivalent is emitted (no shimmer animation) that still conveys the wordmark, subtitle, and ministry roster in a screen-reader-consumable text stream.
...
4. **Given** the header comment of `tui/src/theme/dark.ts`, **When** a human or automated reviewer inspects it, **Then** the comment explicitly notes "KOSAX 브랜드 토큰으로 리네이밍됨 (ADR-006 A-9)" in addition to the original "Source: .references/..." line.

---

### User Story 5 — TUI renders the LogoV2 REWRITE family with the KOSAX metaphor (Priority: P2)

The eight Epic H-owned LogoV2 / root-logo catalog rows (AnimatedAsterisk, CondensedLogo, Feed, FeedColumn, feedConfigs, LogoV2, WelcomeV2, FastIcon) render in the TUI using KOSAX tokens and KOSAX content semantics — not Claude Code vendor content. Specifically: `AnimatedAsterisk` renders the kosaxCore glyph with the `rainbow_*` shimmer animation engine ported from CC; `CondensedLogo` renders a KOSAX wordmark + session/model header line; `LogoV2` renders the orbital-ring splash composition; `WelcomeV2` renders the citizen welcome screen; `FastIcon` becomes `KosaxCoreIcon` (renamed file) and renders the kosaxCore asterisk instead of the CC fast-mode lightning glyph; `Feed` + `FeedColumn` + `feedConfigs` render ministry-availability and recent-session feeds instead of CC-specific activity / changelog / guest-pass feeds.
...
**Independent Test**: For each of the eight catalog rows, run the REWRITE-target file under a visual-regression fixture and confirm: (a) no CC-branded identifier appears in the rendered output (no "Claude Code", "Clawd", "guest pass", "Opus"), (b) the KOSAX token surface is referenced (via `useTheme()` → KOSAX tokens), (c) the `[ag-logov2]` / `[ag-onboarding]` / `[ag-logo-wordmark]` accessibility-gate rows pass (WCAG 1.4.3 + 4.1.2 + reduced-motion fallback).

**Acceptance Scenarios**:

1. **Given** the REWRITE target `tui/src/components/onboarding/LogoV2/AnimatedAsterisk.tsx`, **When** the component renders, **Then** it emits the kosaxCore asterisk glyph (not the CC teardrop) using the `rainbow_*` shimmer engine unchanged from CC, and no CC-specific rainbow cycle ordering is hard-coded.
2. **Given** the REWRITE target `tui/src/components/chrome/KosaxCoreIcon.tsx` (renamed from `FastIcon.tsx`), **When** the component renders, **Then** the file emits a kosaxCore asterisk in the `kosaxCore` token colour and NOT the CC fast-mode `chromeYellow` lightning glyph.
3. **Given** the REWRITE target `tui/src/components/onboarding/LogoV2/LogoV2.tsx`, **When** the component renders on first launch, **Then** it composes the splash (wordmark + subtitle + orbitalRing + kosaxCore + satellite-node row + session metadata) and does NOT import `Clawd`, `GuestPassesUpsell`, `EmergencyTip`, `VoiceModeNotice`, `Opus1mMergeNotice`, `ChannelsNotice`, or `OverageCreditUpsell`.
4. **Given** the REWRITE target `tui/src/components/onboarding/LogoV2/Feed.tsx` + `feedConfigs.tsx`, **When** the splash renders, **Then** the feed column shows (a) a "최근 세션" column bound to KOSAX session history and (b) a "부처 상태" column bound to ministry-adapter availability; it does NOT render CC recent-activity, guest-pass, or referral feeds.
...
- **Reduced-motion environment** (`NO_COLOR=1` or `KOSAX_REDUCED_MOTION=1`) — the splash emits a static-text equivalent on a single render; the orbital-ring shimmer and the kosaxCore pulse animation are skipped; the ministry-node row remains fully legible.
...
- **Terminal width < 50 columns** — the splash renders as a single "KOSAX — 한국 공공서비스 대화창" text line plus the ministry list; this is below the practical TUI threshold but must degrade without erroring.
...
- **Missing `kosax-logo-dark.svg` asset** — `docs/design/brand-system.md § 1` cross-reference points to a file not present on disk (only `kosax-logo.svg` and `kosax-banner-dark.svg` exist). The TUI does NOT render SVG; the hex values are extracted from the existing SVG files; this spec does not require authoring the missing file but documents the reference-drift as an assumption.
...
- **FR-001**: System MUST define the canonical KOSAX brand palette whose hex values are drawn from `assets/kosax-logo.svg` and `assets/kosax-banner-dark.svg`. The palette anchors are: background `#0a0e27` → `#1a1040`; kosaxCore `#818cf8` → `#6366f1`; orbitalRing `#60a5fa` → `#a78bfa`; wordmark `#e0e7ff`; subtitle `#94a3b8`; four `agentSatellite{MINISTRY}` accents drawn from `{#34d399, #f472b6, #93c5fd, #c4b5fd}`.
...
- **FR-006**: System MUST ADD the following ten identifiers to `ThemeToken`, in the order and naming specified by `docs/design/brand-system.md § 2` grammar: `kosaxCore`, `kosaxCoreShimmer`, `orbitalRing`, `orbitalRingShimmer`, `wordmark`, `subtitle`, `agentSatelliteKoroad`, `agentSatelliteKma`, `agentSatelliteHira`, `agentSatelliteNmc`. Identifier casing conforms to the § 2 BNF (camelCase `MetaphorRole` + optional TitleCase `Variant`).
...
- **FR-008**: System MUST update the header comment of `tui/src/theme/tokens.ts` and `tui/src/theme/dark.ts` to explicitly note "KOSAX 브랜드 토큰으로 리네이밍됨 (ADR-006 A-9)" alongside the existing "Source: .references/claude-code-sourcemap/..." line.

### Functional Requirements — `dark.ts` palette map

- **FR-009**: System MUST REPLACE the placeholder `background: 'rgb(0,204,204)'` in `tui/src/theme/dark.ts` with the KOSAX navy anchor `'rgb(10,14,39)'` (equivalent to `#0a0e27`). The gradient endpoint `#1a1040` is consumed by the LogoV2 REWRITE composition, not by the `background` token.
- **FR-010**: System MUST bind each of the ten new `ThemeToken` identifiers to its FR-001 hex value in `dark.ts` using the `rgb(...)` form consistent with the existing file. The shimmer variants (`kosaxCoreShimmer`, `orbitalRingShimmer`) MUST use a lightened shade drawn from the 16-hex palette present in `assets/kosax-logo.svg` (plan phase selects among `#a5b4fc`, `#c7d2fe`, `#6ee7b7`, `#f9a8d4`).
...
- **FR-017**: System MUST rewrite `tui/src/components/onboarding/LogoV2/LogoV2.tsx` to render the splash composition: wordmark + subtitle + orbitalRing + kosaxCore + satellite-node row + session metadata. The rewrite MUST NOT import `Clawd`, `GuestPassesUpsell`, `EmergencyTip`, `VoiceModeNotice`, `Opus1mMergeNotice`, `ChannelsNotice`, or `OverageCreditUpsell`. The layout-mode switch (condensed vs. full) inherited from CC `logoV2Utils.ts` MUST be preserved and parameterised on `useTerminalSize().columns`.
- **FR-018**: System MUST rewrite `tui/src/components/onboarding/LogoV2/AnimatedAsterisk.tsx` to render the kosaxCore asterisk glyph using the `rainbow_*` shimmer engine. The CC `chromeYellow` binding and the teardrop-asterisk glyph MUST be removed.
- **FR-019**: System MUST rewrite `tui/src/components/onboarding/LogoV2/CondensedLogo.tsx` to render a KOSAX wordmark header + a compressed session-metadata line (model / effort / coordinator-mode from Spec 033). The CC "Claude Code" wordmark and Clawd / GuestPassesUpsell references MUST be removed.
- **FR-020**: System MUST rewrite `tui/src/components/onboarding/LogoV2/WelcomeV2.tsx` to render a KOSAX welcome screen. The "Welcome to Claude Code" wordmark and the Apple-terminal-specific ASCII art for CC branding MUST be replaced with a "KOSAX에 오신 것을 환영합니다" wordmark and the kosaxCore + orbitalRing visual.
- **FR-021**: System MUST port `tui/src/components/onboarding/LogoV2/FeedColumn.tsx` verbatim (palette-only swap) — this row is a generic multi-feed column layout with no CC-specific logic. The PORT includes renaming any imported token identifier to its KOSAX equivalent.
- **FR-022**: System MUST rewrite `tui/src/components/onboarding/LogoV2/Feed.tsx` and `tui/src/components/onboarding/LogoV2/feedConfigs.tsx` to emit exactly two feed columns on the splash: "최근 세션" (bound to KOSAX session history, sourced from memdir Session tier) and "부처 상태" (bound to adapter-availability signals from the Spec 022 registry). The four CC feed factories (`createRecentActivityFeed`, `createWhatsNewFeed`, `createProjectOnboardingFeed`, `createGuestPassesFeed`) and the `OverageCreditFeed` MUST be deleted.
- **FR-023**: System MUST rewrite `tui/src/components/FastIcon.tsx` as `tui/src/components/chrome/KosaxCoreIcon.tsx` (file renamed). The rewrite removes the `chromeYellow` lightning glyph and emits the kosaxCore asterisk glyph tinted with the `kosaxCore` token. Existing consumers of `FastIcon` in the TUI are updated to import `KosaxCoreIcon` as part of this PR.

### Functional Requirements — Reduced-motion, screen-reader, and contrast guarantees

- **FR-024**: Every REWRITE file listed in FR-017 through FR-023 MUST honour `NO_COLOR=1` and `KOSAX_REDUCED_MOTION=1` env flags. Under either flag, animation (shimmer, asterisk pulse) is skipped and a static-text equivalent renders that carries the same information. This satisfies `docs/tui/accessibility-gate.md § 1.1` pathway 2.
...
- **KosaxThemeToken** — the TypeScript type alias exported from `tui/src/theme/tokens.ts`. Fields: union of `MetaphorRole` + `SemanticRole` identifiers, each typed `string` (hex value). The type is the upstream contract consumed by every component `useTheme()` call.
- **LogoV2RewriteComponent** — a REWRITE catalog row whose target lives under `tui/src/components/onboarding/LogoV2/` or `tui/src/components/chrome/`. Each row carries: CC source path, REWRITE verdict, KOSAX target path, token surface it consumes, accessibility-gate anchor, reduced-motion behaviour.
...
- **SC-007**: With `NO_COLOR=1` or `KOSAX_REDUCED_MOTION=1` set, every REWRITE row in FR-017 — FR-023 emits a static-text equivalent on the first frame; no shimmer, no pulse, no asterisk animation is attempted.
...
- **PIPA role binding inherits from project memory.** Per `memory/project_pipa_role.md` (2026-04-19 entry), KOSAX is a PIPA § 26 processor (수탁자) by default, with the controller-level carve-out applying only at the LLM synthesis stage. The onboarding consent prompt wording in FR-013 and FR-015 reflects this split; the plain-language Korean summary clarifies that KOSAX forwards the citizen's request to ministry portals on their behalf.
...
- **`kosax-logo-dark.svg` is a doc-drift reference.** `docs/design/brand-system.md § 1` cross-references an `assets/kosax-logo-dark.svg` file that does not exist on disk (the actual assets are `kosax-logo.svg`, `kosax-banner-dark.svg`, etc.). Because the TUI does not render SVG and the palette hex values are extracted from existing SVG sources, this spec does not require authoring the missing file. A follow-up Brand Guardian doc fix (a one-line correction in § 1) is tracked under the `brand-system-doc-fix` Deferred row below.
...
- **Korean label conventions.** Citizen-facing Korean labels (`최근 세션`, `부처 상태`, `KOSAX에 오신 것을 환영합니다`, ministry names) are final at spec scope. Minor copy edits are allowed in the plan / implement phases provided they do not alter the meaning or break a screen-reader narration.
...
- **SVG-to-terminal rendering.** The TUI is Ink + React + Bun over a character grid; SVG assets are consumed as colour sources, not rendered. Any proposal to render the KOSAX SVG logo as a terminal image is permanently out of scope (no `kitty-image-protocol` integration, no `term-img`).
- **Sound / voice feedback during onboarding.** KOSAX has no audio subsystem (§ 8 Voice & tone concerns prose, not audible voice). Adding audio cues to the splash or consent step is permanently out of scope.
- **Browser or mobile equivalent.** KOSAX is terminal-only; an "onboarding web page" or "mobile splash" is permanently out of scope (AGENTS.md hard rule — TypeScript is allowed only for the TUI layer).
...
| Full mass-rename of the 69 legacy CC-branded tokens in `tokens.ts` allow-list | Epic M `.brand-guardian-allowlist.txt` tracks this under Deferred row 10; this spec only introduces the 10 KOSAX additions and 7 CC deletes, not the 69-token mass rename | Epic M #1310 Deferred row 10 | #1542 |
...
| `brand-system.md § 1` doc-fix: replace `kosax-logo-dark.svg` cross-reference with an existing file name | Single-line doc fix bundled into Epic H PR — resolved inline (tasks.md T036) | Epic H #1302 (this spec) | Resolved inline — tasks.md T036 |
