...
Korean IME strategy for the KOSAX TUI. See ADR-005 for the full decision rationale.

## Strategy Selector

The active IME strategy is controlled by the `KOSAX_TUI_IME_STRATEGY` environment variable.
...
export KOSAX_TUI_IME_STRATEGY=fork
bun run tui
```
...
When `KOSAX_TUI_IME_STRATEGY=fork` (or the variable is unset), the TUI uses
...
trade-off documented in ADR-005 — the KOSAX TUI does not use any Ink 7-only
...
At present, selecting `KOSAX_TUI_IME_STRATEGY=readline` causes the
`useKoreanIME` hook to throw immediately at invocation time with the message:

```
KOSAX_TUI_IME_STRATEGY=readline not yet implemented;
...
1. Set `KOSAX_TUI_IME_STRATEGY=readline` to confirm you get the stub error
   message (this verifies the env var is being read correctly).
2. File a bug at the KOSAX issue tracker with the terminal name and OS version.
