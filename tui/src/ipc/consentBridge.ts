...
//
// The bridge is the TS-side analog of ummaya.plugins.consent_bridge.IPCConsentBridge;
...
//   (getOrCreateUmmayaBridge) so `_handleConsentRevokeResponse` is called
...
// - Default timeout: 5 s (UMMAYA_CONSENT_REVOKE_TIMEOUT_MS env var overrides).

import { getOrCreateUmmayaBridge } from './bridgeSingleton.js'
...
const DEFAULT_TIMEOUT_MS = (() => {
  const env = globalThis.process?.env?.['UMMAYA_CONSENT_REVOKE_TIMEOUT_MS']
