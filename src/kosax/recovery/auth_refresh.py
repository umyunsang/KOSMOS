...
- Credential resolution is centralised in :mod:`kosax.permissions.credentials`
  ...
  ``address_to_grid``) correctly discover ``KOSAX_KAKAO_API_KEY`` here,
  and data.go.kr-backed tools discover ``KOSAX_DATA_GO_KR_API_KEY``.
"""
...
from kosax.permissions.credentials import candidate_env_vars, resolve_credential
...
async def attempt_auth_refresh(tool_id: str) -> bool:
    ...
    :func:`kosax.permissions.credentials.resolve_credential`, which tries
