...
| # | Decision | Reference (Primary) | Reference (Secondary) | Why it fits UMMAYA Layer 4 |
|---|---|---|---|---|
...
| D2 | **Coordinator owns Synthesis — never delegates** | `docs/vision.md § Layer 4 → Coordinator workflow` (UMMAYA canon) | Anthropic Cookbook orchestrator-workers example | Synthesis must integrate cross-ministry dependencies (e.g., residence transfer must precede vehicle registration). Delegating synthesis to a "meta-worker" would make the dependency graph opaque to the citizen-facing coordinator. Enforced by FR-004 + FR-038. |
...
- **Infisical OIDC** (Spec 468 follow-on): agent env vars live in the standard `UMMAYA_*` namespace; no secret material is introduced (the mailbox stores message bodies, which may contain citizen data, but that is a Layer 3 PII concern, not a secrets-management concern).
...
| Epic #501 attribute boundary table not yet merged | Medium | `ummaya.agent.*` names might need renaming later | FR-031 requires submission to #501 before any collector deploys; rename cost is string-constant scope only. |
