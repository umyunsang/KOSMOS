...
A fully-functional KOSAX backend process that boots the complete mock
...
JSONL stdio loop (same pattern as :mod:`kosax.ipc.mcp_server`).

This module is the canonical target for ``KOSAX_BACKEND_CMD`` during
smoke-testing and offline demos:

    KOSAX_BACKEND_CMD="uv run python -m kosax.ipc.demo.mock_backend" bun run tui

Design decisions (research.md Decision 5):
- Reuses the stdio loop from ``mcp_server._run_loop`` (DRY principle).
- Imports ``kosax.tools.mock`` EXPLICITLY after ``register_all_tools()`` so
...
Entry point::
    ...
    uv run python -m kosax.ipc.demo.mock_backend

Module docstring notes ``KOSAX_BACKEND_CMD`` invocation so any reader of the
        ...
        if method == "initialize":
            return {
                ...
                "result": {
                    ...
                    "serverInfo": {"name": "kosax-mock-backend", "version": "0.1.0"},
                },
            }
...
async def main() -> None:
    ...
    4. Import ``kosax.tools.mock`` — triggers __init__.py side-effects that register all
       verify and submit mock surfaces into their per-primitive sub-registries.
       Subscribe adapters are intentionally not imported until KOSAX has an
    ...
    from kosax.ipc.adapter_manifest_emitter import emit_manifest  # noqa: PLC0415
    ...
    # Step 4: Import kosax.tools.mock to register all 20 mock surfaces.
    ...
    try:
        from kosax.ipc.adapter_manifest_emitter import _build_entries  # noqa: PLC0415
