...
- Path resolution: ``KOSAX_PROMPTS_DIR`` env var → ``<repo_root>/prompts``.
...
# Source: data-model.md § 2 + src/kosax/primitives/verify.py
...
def _locate_prompts_dir() -> Path:
    ...
    Resolution order:
    1. ``KOSAX_PROMPTS_DIR`` environment variable (if set and non-empty).
    ...
    3. Fall back to ``<repo_root>/prompts`` where repo_root is three levels up
       from this file (``src/kosax/tools/verify_canonical_map.py``).
    """
    env_dir = os.environ.get("KOSAX_PROMPTS_DIR", "").strip()
    if env_dir:
        return Path(env_dir)
    ...
    # src/kosax/tools/ → src/kosax/ → src/ → repo_root
