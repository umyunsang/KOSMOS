...
// real_classification_url; 0% contain any KOSAX-invented phrase from the
// blocklist below. All assertions use the synthetic KOROAD adapter fixture.
//
// Coverage:
//  1. Happy-path: validateInput populates kosaxCitations on context.
//  2. Blocklist: renderToolResultMessage output contains no KOSAX-invented phrases.
...
// ---------------------------------------------------------------------------
// SC-003 blocklist — phrases KOSAX must never invent.
// ---------------------------------------------------------------------------
const FORBIDDEN_KOSAX_INVENTED_PHRASES: readonly string[] = [
  '안전한 권한 등급',
  '본 시스템은',
  'KOSAX는 다음과 같이',
  '권한 등급 1',
...
// ---------------------------------------------------------------------------
// Happy-path: active primitives populate kosaxCitations on context.
...
describe('validateInput — happy path (citation populates context)', () => {
  test('LookupPrimitive mode=fetch: result true + kosaxCitations populated', async () => {
    ...
    const citations = (context as unknown as Record<string, unknown>).kosaxCitations as { real_classification_url: string; policy_authority: string }[]
  ...
  test('SubmitPrimitive: result true + kosaxCitations populated', async () => {
    const context = buildContext([syntheticAdapter])
  ...
  test('VerifyPrimitive: result true + kosaxCitations populated', async () => {
    const context = buildContext([syntheticAdapter])
...
// ---------------------------------------------------------------------------
// Blocklist: renderToolResultMessage output must not contain KOSAX-invented phrases.
...
describe('renderToolResultMessage — blocklist assertion (SC-003 0% rule)', () => {
  function assertNoForbiddenPhrases(rendered: unknown, label: string) {
    ...
    for (const phrase of FORBIDDEN_KOSAX_INVENTED_PHRASES) {
