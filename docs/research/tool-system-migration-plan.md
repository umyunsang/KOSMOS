# UMMAYA Tool System Migration Plan

> Scope: Close the structural gap between Claude Code 2.1.88 (CC) Tool system and UMMAYA public-API harness. Preserve harness invariants (active primitive envelope, Pydantic v2 schemas, Layer-3 auth gate) while porting CC file layout, deferred-loading mechanism, and per-tool Result components.
>
...
**Recommended migration path** (one paragraph): Preserve UMMAYA's primitive-first envelope and Pydantic-v2 security stack as the source-of-truth, but re-layer the per-adapter code into a CC-style `src/ummaya/tools/<AdapterName>/` bundle (`adapter.py`, `prompt.py`, `UI.tsx`, `permissions.py`). Introduce a thin `UmmayaTool` protocol over the existing `GovAPITool` + primitive dispatchers so the TUI can look up per-adapter React renderers and decision-reason text the way CC's `renderToolResultMessage` does, without re-plumbing the backend envelope. Keep `PrimitiveDispatcher` as a fall-through default. Promote live adapters first, port seed-stubs (NMC/NFA/SSIS) into the new layout while still fail-closed on auth, and phase in CC-style per-tool `UI.tsx` for the eight Phase-1 adapters. Mock adapters stay ministry-grouped under `src/ummaya/tools/mock/` because their evidence docs (`docs/mock/<system>/README.md`) are already ministry-scoped and byte/shape-mirror bound.

---

## 2. UMMAYA Current Tool System

### 2.1 Directory tree (counts from `find … | wc -l`)

```
src/ummaya/tools/                  (70 .py files)
...
src/ummaya/permissions/            ~25 files incl. pipeline_v2.py, modes.py, ledger.py, steps/
...
Source-of-truth: `src/ummaya/tools/register_all.py:33-115` (registration order).
...
- **Permissions**: Spec 033 v2 pipeline (`src/ummaya/permissions/pipeline_v2.py:1`). Ordered gauntlet `killswitch → mode → rule → prompt`, AAL backstop pre-dispatch (FR-F02), PermissionMode spectrum `{default, plan, acceptEdits, bypassPermissions, dontAsk}` (`modes.py:21`) ported from CC 2.1.88's own `PermissionMode.ts`. Shift+Tab cycle limited to low/mid-risk modes.
...
- **Audit**: `ToolCallAuditRecord` schema lives in `src/ummaya/security/audit.py`; Spec 024 invariants I1–I4 enforced via `GovAPITool` V1–V5 model validators (`models.py:231`). Persistence is **deferred** (harness-only ledger is implemented at `src/ummaya/permissions/ledger.py` with HMAC-chained entries, but per-tool append is out of scope until Spec 024 Phase 2).
- **Observability**: Spec 021 OTEL spans emitted by `ToolExecutor.dispatch` (`executor.py:314`) — `gen_ai.operation.name`, `gen_ai.tool.name`, `gen_ai.tool.call.id`, `ummaya.tool.adapter`, `ummaya.tool.outcome`. Safety overlay: ingress scanner+redactor runs on every adapter output before envelope normalization (`executor.py:250-275`).
- **Ministry-scope guard**: `main_router.resolve_with_scope_guard` reads the citizen's Spec 035 consent memdir and refuses any `<ministry>_*` tool_id whose ministry is opted-out **before** any network call (`src/ummaya/tools/main_router.py:207`).
...
`ToolPermissionContext` (`Tool.ts:123`) is deeply immutable and carries `mode: PermissionMode`, `alwaysAllow/Deny/AskRules`, `additionalWorkingDirectories`, `isBypassPermissionsModeAvailable`. CC's `PermissionMode` is the exact superset UMMAYA already ports in `permissions/modes.py:21`.
...
| # | Concern | CC approach | UMMAYA today | Gap | Impact |
|---|---------|-------------|--------------|-----|--------|
| G1 | Tool layout | 1 dir per tool, 6–18 files, colocated | 1 dir per ministry, 1 py per adapter, no per-tool dir | UMMAYA has **no per-tool dir** | **H** |
| G2 | Tool interface | Single `Tool<Input,Output,P>` with 40+ optional methods | `GovAPITool` pydantic record + external `ToolExecutor` callable | No unified TS-facing interface; UMMAYA has active primitive dispatchers instead | **M** (by design, aligns with main-verb-primitive) |
...
| G4 | Output schema | Optional Zod | Pydantic v2 discriminated union (5 variants for lookup, 6 for resolve_location, 4 for verify) | UMMAYA stricter; migration should keep UMMAYA discriminators | L |
| G5 | Permission gate | `tool.checkPermissions(input, ctx)` + `useCanUseTool` | Layer-3 gate in `ToolExecutor.invoke` + Spec 033 pipeline_v2 external to adapter | UMMAYA centralises; CC is per-tool. Port CC's `checkPermissions()` **optional** hook per adapter so tools can declare preapproved values (e.g. domain whitelist for WebFetch-style citizen services) | M |
...
| G7 | Rendering — result | Per-tool `renderToolResultMessage(output, progressMsgs, opts)` | Generic `PrimitiveDispatcher` sub-dispatches on `payload.subtype` | UMMAYA underfits: 10 live adapters share 5 renderers. Port per-adapter variants (e.g. `HiraHospitalResultMessage`, `KoroadHazardResultMessage`) **on top of** PrimitiveDispatcher fallback | M |
...
| G9 | Deferred loading | `shouldDefer=true` + `ToolSearchTool({query:"select:X"})` schema hydration | `lookup(mode="search")` returns ranked candidates, `lookup(mode="fetch")` runs adapter | Semantically equivalent; syntactically not. Keep UMMAYA envelope; **do not** port CC's `shouldDefer` flag — the main-verb primitive supersedes it | L |
| G10 | Audit | Not enforced at the interface (CC is developer-domain, low-stakes) | `ToolCallAuditRecord` schema I1-I4; `GovAPITool` V1-V6 validators; Layer-3 gate | UMMAYA stricter — no port needed. Keep | L |
...
| G12 | `userFacingName` | Per-tool function | `name_ko` field on `GovAPITool` | Port — add `userFacingNameForAdapter(tool_id, partialInput)` to the UMMAYA bridge layer for consistency | L |
| G13 | Search hint | `searchHint?: string` (free-text) | `search_hint: str` (bilingual Korean+English required) | UMMAYA stricter — keep; extend schema to `{ko: list[str], en: list[str]}` already used by `AdapterRegistration.search_hint` | L |
...
| G15 | File watchers / autoload | `isEnabled()`, `feature()` bundle-time gate | `register_all.register_all_tools()` — static | Port dynamic enablement only if MCP adapters land in UMMAYA | L |
| G16 | MCP support | First-class (`MCPTool`, `McpAuthTool`, `ReadMcpResourceTool`, `ListMcpResourcesTool`) | None | Explicit non-goal today (UMMAYA MCP will be a separate spec); document the gap | L |

---

## 5. Live API Permission Matrix

Evidence: `.env.example:1-30` (keys UMMAYA currently holds), `register_all.py:33-115`, `docs/mock/<system>/README.md`, `MEMORY.md § reference_koroad_portal`.
...
| Provider | Proposed tool_id | Public spec | UMMAYA status | Mock viability (5-pt) |
|----------|-----------------|-------------|---------------|-----------------------|
...
### 6.2 `UmmayaTool` protocol (bridge layer)

Introduce a TypeScript protocol in `tui/src/tools/UmmayaTool.ts` (new file, **not** yet written — this is the proposal):

```ts
// Shape-compatible with CC's Tool<Input, Output, P>, narrowed to UMMAYA primitives.
...
Backend side: add a thin `ummaya.tools.metadata.manifest()` generator that serialises every `GovAPITool` + `AdapterRegistration` into `tui/src/tools/generated/manifest.ts` at build time (one-way: Python → TS), so the TS side always reflects the source-of-truth Pydantic registry. No runtime dependency on Python in TS.
...
| AgentTool / TaskCreateTool / TaskGetTool / TaskListTool / TaskOutputTool / TaskStopTool / TaskUpdateTool | Developer task orchestration (replaced by UMMAYA agents coordinator/worker) |
...
| MCPTool / McpAuthTool / ListMcpResourcesTool / ReadMcpResourceTool | MCP integration — out of scope until UMMAYA MCP spec lands |
...
**Retain/port**: `AskUserQuestionTool` (becomes a **new UMMAYA primitive candidate**, or folded into the permission prompt pipeline); `ToolSearchTool` already has a UMMAYA analogue as `lookup(mode="search")`; `WebFetchTool` pattern informs how a future `public_info_fetch` citizen tool could be built (with a preapproved domain list restricted to `.go.kr`).
...
|------|-------|---------------|
| T1: Port `koroad_accident_search` to `src/ummaya/tools/koroad_accident_search/` bundle + TUI `UI.tsx` | Sonnet-A | 5 py + 2 tsx = 7 |
...
Mock python modules remain in `src/ummaya/tools/mock/<ministry>/` — evidence docs at `docs/mock/<system>/README.md` are ministry-scoped and rebundling would break the mock-drift fixture recording process.

#### Wave C-2.4 — Bridge layer + manifest generation

| Task | Owner | Files |
|------|-------|-------|
| T15: `tui/src/tools/UmmayaTool.ts` protocol | Sonnet-A | 1 |
| T16: `src/ummaya/tools/metadata.py::manifest()` generator + `tui/src/tools/generated/manifest.ts` build step | Sonnet-B | 3 |
...
Candidate list (live API exists, no UMMAYA adapter yet — public spec verified, evidence-backed):
...
1. `UmmayaTool` protocol MUST exist (T15) so UI.tsx files have a type target — **elevate T15 to Wave C-2.0**.
...
- T15 (UmmayaTool protocol), T16 (manifest generator). 4 files total. Blocks all downstream waves.
...
- `/Users/um-yunsang/UMMAYA/src/ummaya/tools/main_router.py`
- `/Users/um-yunsang/UMMAYA/src/ummaya/tools/lookup.py`
- `/Users/um-yunsang/UMMAYA/src/ummaya/tools/envelope.py`
- `/Users/um-yunsang/UMMAYA/src/ummaya/tools/models.py`
- `/Users/um-yunsang/UMMAYA/src/ummaya/tools/registry.py`
- `/Users/um-yunsang/UMMAYA/src/ummaya/tools/register_all.py`
- `/Users/um-yunsang/UMMAYA/src/ummaya/tools/executor.py`
- `/Users/um-yunsang/UMMAYA/src/ummaya/tools/mvp_surface.py`
- `/Users/um-yunsang/UMMAYA/src/ummaya/primitives/submit.py`
- `/Users/um-yunsang/UMMAYA/src/ummaya/permissions/pipeline_v2.py`
- `/Users/um-yunsang/UMMAYA/src/ummaya/permissions/modes.py`
- `/Users/um-yunsang/UMMAYA/tui/src/components/primitive/index.tsx`
- `/Users/um-yunsang/UMMAYA/tui/src/components/primitive/types.ts`
- `/Users/um-yunsang/UMMAYA/tui/src/hooks/useCanUseTool.ts`
- `/Users/um-yunsang/UMMAYA/.references/claude-code-sourcemap/restored-src/src/Tool.ts`
- `/Users/um-yunsang/UMMAYA/.references/claude-code-sourcemap/restored-src/src/tools.ts`
- `/Users/um-yunsang/UMMAYA/.references/claude-code-sourcemap/restored-src/src/tools/FileReadTool/FileReadTool.ts`
- `/Users/um-yunsang/UMMAYA/.references/claude-code-sourcemap/restored-src/src/tools/WebFetchTool/WebFetchTool.ts`
- `/Users/um-yunsang/UMMAYA/.references/claude-code-sourcemap/restored-src/src/tools/ToolSearchTool/ToolSearchTool.ts`
- `/Users/um-yunsang/UMMAYA/docs/mock/*/README.md`
- `/Users/um-yunsang/UMMAYA/docs/scenarios/README.md`
- `/Users/um-yunsang/UMMAYA/.env.example`
