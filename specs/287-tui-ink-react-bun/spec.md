...
This spec governs the port of Claude Code 2.1.88's Ink + React terminal UI onto the UMMAYA Python backend. The migration thesis (per `project_tui_architecture.md` memory and Epic #287): keep Claude Code's Ink + React + `useSyncExternalStore` rendering spine, its IPC/streaming chunk protocol, its virtualized list, its permission-gauntlet UX, its theme + session commands — **replace only the backend boundary**. Claude Code's Anthropic API client (`services/api/`) becomes UMMAYA's Python backend over stdio JSONL; Claude Code's `tools/*` developer-domain implementations become thin Ink renderers over UMMAYA's 5-primitive surface (Spec 031 / #1052).
...
**Depends on**: #507 (Spec 022 rendering contract), #1052 (Spec 031 five-primitive), #13 (agent swarm coordinator), #14 (specialist role labels), #468 (UMMAYA_TUI_* env registry), #501 (observability span emission).
...
A citizen launches the UMMAYA TUI via `bun run tui`. The TypeScript process spawns the UMMAYA Python backend as a child process (`uv run ummaya-backend --ipc stdio`) and communicates via newline-delimited JSON (JSONL) over stdin/stdout. Streaming assistant text renders chunk-by-chunk in real time.
...
1. **Given** `bun run tui` is invoked, **When** the TypeScript process starts, **Then** it spawns `uv run ummaya-backend --ipc stdio` via `Bun.spawn` (stdin/stdout/stderr only — no extra fds per oven-sh/bun#4670) within 2 seconds.
...
3. **Given** `UMMAYA_TUI_THEME` is set to `dark` | `light` | `default`, **When** the TUI starts, **Then** the theme token set matching the env var is applied to all Box/Text components.
...
- What happens when a `subscribe` stream never sends a terminal event? → The TUI times out after `UMMAYA_TUI_SUBSCRIBE_TIMEOUT_S` seconds (default: 120) and renders `<StreamClosed reason="timeout" />`.
...
- **FR-001**: The TUI MUST spawn the Python backend as `uv run ummaya-backend --ipc stdio` via `Bun.spawn`, using only stdin/stdout/stderr (no extra fds — oven-sh/bun#4670 limitation).
...
- **FR-004**: The TUI MUST detect backend process crash (non-zero exit or stderr flush) within ≤ 5 s and surface a crash notice in the terminal. Crash notices MUST redact all `UMMAYA_`-prefixed environment variable values (per #468 guard patterns).
...
- **FR-010**: The IPC bridge MUST log every frame receive/send at `DEBUG` level using the `UMMAYA_TUI_LOG_LEVEL` env var; default is `WARN`.
...
- **FR-039**: The theme engine MUST support three built-in themes: `default`, `dark`, `light`. Theme MUST be selected via `UMMAYA_TUI_THEME` env var and documented in `docs/configuration.md`.
- **FR-040**: Theme tokens MUST be a named set (`ThemeToken`) consumed by all Box/Text components — no inline hex colors.
- **FR-041**: `UMMAYA_TUI_THEME` MUST be registered in the #468 env var registry.
...
- **FR-053**: The TUI MUST emit `gen_ai.tool_loop.iteration` spans with `ummaya.session.id` correctly attributed, routed through the #501 OTEL collector.
...
- The Phase 1 Python CLI (`src/ummaya/cli/`) is retained unchanged as a fallback + CI smoke target. This Epic delivers an additional front-end.
- `.references/claude-code-sourcemap/restored-src/` is read-only; UMMAYA-local adaptations live only in `tui/`.
...
- **Web UI / mobile UI**: UMMAYA is a terminal-based platform; a web front-end would require a fundamentally different rendering stack.
...
| B — Dispatcher + Theme + Session | Command dispatcher lifted from `commands/`, Korean+English localization, `/save` `/sessions` `/resume` `/new`, 3 built-in themes, `UMMAYA_TUI_THEME` env var |
...
| Buddy mode (`buddy/` from Claude Code) | Not mapped to any UMMAYA role; no equivalent backend | Phase 3+ | #1286 |
| KAIROS assistant mode (`assistant/` from Claude Code) | No equivalent backend; design TBD | Phase 3+ | #1287 |
| Remote sessions (`remote/` from Claude Code) | UMMAYA runs local; remote orchestration is a separate initiative | Future Initiative | #1288 |
...
- **Epic #287**: `gh issue view 287 --repo umyunsang/UMMAYA` — primary scope source; SC-1..SC-9 copied verbatim
...
- **`docs/vision.md § Layer 6`**: UMMAYA Layer 6 human interface boundary; permission-delegation flow
