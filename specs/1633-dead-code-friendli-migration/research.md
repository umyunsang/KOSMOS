...
**Rationale**:
- Canonical tree `docs/requirements/kosax-migration-tree.md § L1-A A1` pins **"K-EXAONE 단일 고정"** — the K-EXAONE brand. `K-EXAONE-236B-A23B` is the literal K-EXAONE release; `EXAONE-4.0-32B` belongs to a different brand family (EXAONE 4.0), not K-EXAONE. Brand-name alignment with the tree's normative language dominates unrelated generation-novelty arguments.
...
- `src/kosax/llm/config.py:37` **already uses** `LGAI-EXAONE/K-EXAONE-236B-A23B` as its default, set before this Epic. Retaining it avoids otherwise-unnecessary Python config churn and preserves the baseline that Spec 019/020/021 live-validation work already exercised.
...
- `KOSAX_FRIENDLI_MODEL` env-var-driven multi-model switching. **Rejected** — violates canonical tree's "단일 고정" directive (L1-A A1).
...
- **`src/kosax/llm/config.py:37` — NO change** (default already `"LGAI-EXAONE/K-EXAONE-236B-A23B"`).
...
- `main.tsx`, `context.ts`, `commands.ts`, `QueryEngine.ts`, `setup.ts`, tool files (`NotebookEditTool.ts`, `FileWriteTool.ts`, `FileEditTool.ts`, `PowerShellTool/pathValidation.ts`, `GlobTool.ts`) — total 10 files reference `filesApi`. These are **CC developer-tool paths** that the P3 tool-system rewrite will either delete or replace with KOSAX primitives. For this Epic, stub out the import sites with no-op equivalents (an empty `{}` returns) so the tool files keep compiling until P3.
...
**Alternatives considered**:
- **Delete**: rejected because cache metrics are useful for the KOSAX OTEL observability layer (Spec 021) and for FP&A cost tracking.
...
5. Emit OTEL `gen_ai.client.invoke` span with `gen_ai.system=friendli_exaone`, `gen_ai.request.model=LGAI-EXAONE/K-EXAONE-236B-A23B`, `kosax.prompt.hash=<sha256>`.

**KOSAX type replacement**:
- Define `tui/src/ipc/llmTypes.ts` with `KosaxMessageStreamParams`, `KosaxRawMessageStreamEvent`, `KosaxContentBlockParam`, `KosaxTextBlockParam` as structural supersets of the Anthropic SDK types currently imported. This lets us delete `@anthropic-ai/sdk` imports without rewriting QueryEngine's control flow.
...
Anthropic-specific error codes (from `tui/src/services/api/errors.ts` CC reference) → KOSAX envelope (LLM · Tool · Network) via FriendliAI HTTP + stdio IPC transport signals:

| Anthropic code | HTTP | FriendliAI equivalent | KOSAX envelope | Retry? |
|---|---|---|---|---|
...
**Decision**: Retry targets = `{ 429, 500, 502, 503, 504 }`. Max attempts = 3. Backoff = exponential (1s, 2s, 4s) with `Retry-After` header override when present. Consistent with `src/kosax/llm/client.py` Python-side retry.
...
| `kosax.prompt.hash` | SHA-256 of `prompts/system_v1.md` | **required by SC-008** (Spec 026) |
| `kosax.correlation_id` | `envelope.correlation_id` | Spec 032 correlation |
| `kosax.transaction_id` | `envelope.transaction_id` | Spec 032, may be null for streaming |

Python backend (`src/kosax/llm/client.py`) already emits these same attributes on its side — we get dual-emission (TS + Python) naturally joined by `correlation_id` at the OTEL collector.

**Rationale**:
- Spec 021 already defined the semconv; this spec just wires the TS emission point.
- Spec 026 already defines `kosax.prompt.hash`; `PromptLoader` is the Python authority and forwards the hash via IPC metadata (see Decision 9).
...
**Rationale**:
- No KOSAX user-facing account concept exists — FRIENDLI_API_KEY is env-var only, held by the user.
...
**Alternatives considered**:
- **Keep `/login` as a help-only stub**: rejected — "login" has no meaning in the KOSAX model.
...
Initial `grep -c '@anthropic-ai/sdk' tui/src` returned **137** imports across ~30 files. A significant fraction are **type-only imports** (`import type {...}` from `@anthropic-ai/sdk/resources/*`). The TS LLMClient in Decision 5 replaces these with KOSAX-scoped types in `tui/src/ipc/llmTypes.ts`. The mechanical substitution is ~137 edits × 1 line each plus 1 new KOSAX types file.
...
2. **Code edits (Python)**: none — `src/kosax/llm/config.py:37` default is already `LGAI-EXAONE/K-EXAONE-236B-A23B` (matches Decision 1).
...
- `src/kosax/llm/client.py` + `config.py` — Python backend FriendliAI implementation (target for Spec 032 IPC bridging)
- `docs/vision.md § 28-44` — thesis (CC is first reference)
- `docs/requirements/kosax-migration-tree.md § L1-A` — canonical LLM-layer decisions (A1-A7)
...
- Spec 026 (Prompt Registry) — PromptLoader + `kosax.prompt.hash`
