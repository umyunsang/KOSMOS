...
TASKS: list[Task] = [
    ...
    Task(
        ...
        "**Acceptance**: artifact files exist; observably show `tui/src/commands/plugin/index.tsx` (CC marketplace) being invoked under `/plugin install` rather than KOSAX path."
    ),
    Task(
    ...
    Task(
        ...
        2, "", False, "S",
        "Add `frame.kind == \"plugin_op\"` branch after `session_event` arm in `src/kosax/ipc/stdio.py`. Wrap handler call in try/except → ErrorFrame fanout matching the existing `chat_request` / `tool_result` / `permission_response` arms.\n\n"
    ...
    Task(
        ...
        2, "", True, "M",
        "Create `src/kosax/ipc/plugin_op_dispatcher.py` (NEW) with `handle_plugin_op_request(frame, *, registry, executor, write_frame, consent_bridge, session_id)` entry point + `handle_install` / `handle_uninstall` / `handle_list` private routers (signatures only; bodies stub `NotImplementedError`).\n\n"
    ...
    Task(
        ...
        2, "", True, "M",
        "Extend `src/kosax/tools/registry.py:ToolRegistry` with: `_inactive: set[str]` field + `set_active(tool_id, active: bool) -> None` + `is_active(tool_id) -> bool` + `deregister(tool_id) -> None` methods. Filter `_inactive` from `core_tools` / `all_tools` / `to_openai_tool` / BM25 corpus rebuild.\n\n"
    ...
    Task(
        ...
        3, "US1", False, "M",
        "Implement `handle_install` in `src/kosax/ipc/plugin_op_dispatcher.py`: build progress_emitter closure that wraps each phase tick into a `PluginOpFrame(op=\"progress\", progress_phase=N, progress_message_ko=..., progress_message_en=...)` + `write_frame(...)`. On `install_plugin` return, emit terminal `plugin_op_complete` with `result` + `exit_code` + `receipt_id`.\n\n"
    ...
    Task(
        ...
        3, "US1", True, "M",
        "Create `src/kosax/plugins/uninstall.py` (NEW) with `uninstall_plugin(plugin_id, *, registry, executor, progress_emitter=None) -> UninstallResult` 3-phase flow: deregister → rmtree → write `PluginConsentReceipt(action_type=\"plugin_uninstall\")`. Idempotent on already-removed plugin.\n\n"
    ...
    Task(
        ...
        3, "US1", False, "S",
        "Implement `handle_uninstall` in `src/kosax/ipc/plugin_op_dispatcher.py` mirroring T008 with the 3-phase progress emitter from T010. Reuse the same `_allocate_consent_position` flock for receipt position assignment.\n\n"
    ...
    Task(
        ...
        3, "US1", False, "S",
        "In `src/kosax/ipc/stdio.py`, pass `_ensure_tool_registry()` + `_ensure_tool_executor()` + `write_frame` + freshly-constructed `IPCConsentBridge(write_frame=write_frame, pending_perms=_pending_perms, session_id=frame.session_id)` into the T003 if-elif arm's `handle_plugin_op_request` call.\n\n"
        "Reference: contracts/dispatcher-routing.md § Dispatch logic.\n\n"
    ...
    Task(
        ...
        4, "US2", False, "S",
        "In TUI ChatRequestFrame builder (likely `tui/src/services/api/` or `tui/src/ipc/bridge.ts` outbound path), if `pluginsModifiedThisSession === true`, set `frame.tools = []` to defer to backend's `registry.export_core_tools_openai()` fallback at `src/kosax/ipc/stdio.py:1192-1195`. Reset flag after emit.\n\n"
    ...
    Task(
        "T021", "[US3] CRITICAL: Swap commands.ts:133 import to KOSAX plugin.ts",
        5, "US3", False, "S",
        "**CRITICAL wire-up**: In `tui/src/commands.ts`, change line 133 from `import plugin from './commands/plugin/index.js'` to `import plugin from './commands/plugin.js'`. This is the single line that activates the entire KOSAX citizen plugin path; all subsequent US3 tasks depend on it.\n\n"
    ...
    Task(
        "T023", "[US3] Replace KOSAX_PLUGIN_REGISTRY env-var stub with IPC round-trip",
        5, "US3", False, "M",
        "In `tui/src/commands/plugins.ts`, replace the `KOSAX_PLUGIN_REGISTRY` env-var stub (lines 32-51) with: emit `plugin_op_request:list` with fresh correlation_id → await matching `plugin_op_complete` + reassembled payload → parse `PluginListEntry[]` → return as `PluginEntry[]`.\n\n"
    ...
    Task(
        ...
        6, "US4", True, "M",
        "Author fixture catalog + bundle under `specs/1979-plugin-dx-tui-integration/scripts/fixtures/`: `catalog.json` (CatalogIndex schema with 1 entry pointing at file:// URLs), `seoul-subway.tar.gz` (containing `manifest.yaml` + `adapter.py` + minimal Pydantic v2 input/output schemas), `seoul-subway.intoto.jsonl` (SLSA provenance compatible with `KOSAX_PLUGIN_SLSA_SKIP=true` test path).\n\n"
...
DEFERRED: list[Deferred] = [
    Deferred(
        ...
        "- memory `feedback_kosax_scope_cc_plus_two_swaps`\n"
    ...
    Deferred(
        ...
        "2. New SHA-256 hash for the `kosax.ipc.schema.hash` OTEL attribute\n"
        ...
        "- docs/requirements/kosax-migration-tree.md § UI-E.3"
    ),
    Deferred(
        "[Deferred] SC-001 live environment validation against kosax-plugin-store",
        "## Originating Epic\n\n#1979 (Plugin DX TUI integration)\n\n"
        "## Deferral reason\n\n"
        "Epic #1979 SC-001 (≤30s install) is measured against fixture catalog under `file://` URLs only. Live network latency variance from the `kosax-plugin-store` GitHub catalog + bundle download is not quantified. The fixture path is known to be ≤ 13s (analysis.md quickstart measurement); live network adds variance.\n\n"
        "## Target\n\n"
        "Post-P5 fixture→live calibration Epic. Measures SC-001 against actual `kosax-plugin-store/seoul-subway` catalog + bundle URLs; calibrates the 30s ceiling against real network conditions.\n\n"
        "## Reference\n\n"
...
def create_issue(title: str, body: str, labels: list[str]) -> int:
    ...
    url = gh(*args)
    # URL format: https://github.com/umyunsang/KOSAX/issues/2208
