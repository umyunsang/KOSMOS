...
Wires a validated :class:`~ummaya.plugins.manifest_schema.PluginManifest`
into the existing Spec 022 :class:`~ummaya.tools.registry.ToolRegistry`
+ Spec 1634 :class:`~ummaya.tools.executor.ToolExecutor`. The plugin
...
``ummaya.plugin.install`` carrying the ``ummaya.plugin.id`` attribute
(Spec 021 UMMAYA extension; Migration tree § L1-A A7). A failure
...
Auto-discovery walks ``UMMAYA_PLUGIN_INSTALL_ROOT`` (Spec 035
...
from ummaya.plugins.exceptions import (
    ManifestValidationError,
    PluginRegistrationError,
)
from ummaya.plugins.manifest_schema import PluginManifest
...
def _import_adapter_module(module_path: str, *, plugin_root: Path | None) -> ModuleType:
    ...
    1. ``plugin_root`` provided → load from
       ...
       installed under ``~/.ummaya/memdir/user/plugins/<id>/`` work
    ...
    if plugin_root is not None:
        ...
        spec_name = f"_ummaya_plugin_{plugin_root.name}_{module_path.replace('.', '_')}"
...
def register_plugin_adapter(
    ...
    *,
    ...
    with _tracer.start_as_current_span("ummaya.plugin.install") as span:
        span.set_attribute("ummaya.plugin.id", plugin_id)
        span.set_attribute("ummaya.plugin.version", manifest.version)
        span.set_attribute("ummaya.plugin.tier", manifest.tier)
        span.set_attribute("ummaya.plugin.tool_id", expected_tool_id)
        span.set_attribute("ummaya.plugin.permission_layer", manifest.permission_layer)
        ...
        span.set_attribute("ummaya.plugin.slsa_verification", slsa_verification)
    ...
    *,
    ...
    Args:
        ...
        strict: When True, raise on the first registration failure.
            ...
            still surfaced via OTEL ``ummaya.plugin.install`` span with
            ``status=ERROR``.
