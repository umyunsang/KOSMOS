#!/usr/bin/env bash
# KOSAX Epic #2112 — P1 Dead Anthropic Model Matrix Removal · Audit Script
...
c3() {
  ...
  #   src/kosax/llm/config.py:37          (FR-012 anchor)
...
c4() {
  local hits
  hits=$(rg -n 'temperature: float = 1\.0|top_p: float = 0\.95|presence_penalty: float = 0\.0|max_tokens: int = 1024' src/kosax/llm/client.py 2>/dev/null | wc -l | tr -d ' ')
  if [[ "$hits" -ge "8" ]]; then
...
c5() {
  local hits
  hits=$(rg -n 'class RetryPolicy|_compute_rate_limit_delay|_is_rate_limit_envelope|_complete_with_retry|_stream_with_retry' src/kosax/llm/client.py 2>/dev/null | wc -l | tr -d ' ')
  if [[ "$hits" -ge "5" ]]; then
...
c6() {
  local hits
  hits=$(rg -n 'KOSAX_K_EXAONE_THINKING|chat_template_kwargs' src/kosax/llm/client.py 2>/dev/null | wc -l | tr -d ' ')
  if [[ "$hits" -ge "2" ]]; then
