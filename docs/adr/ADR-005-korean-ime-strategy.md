...
UMMAYA's primary user demographic is Korean citizens interacting with Korean
...
React 19.2 pin but requires UMMAYA-original reimplementation of Ink's input
...
A strategy-selector environment variable `UMMAYA_TUI_IME_STRATEGY` is
...
- **T104**: Implement the `UMMAYA_TUI_IME_STRATEGY` selector hook; wire the
...
| FR-016 | IME strategy must be swappable via env var without code change | `UMMAYA_TUI_IME_STRATEGY` env var provides the swap mechanism |
...
- React 18 is a stable, well-understood target; no known regressions for the
  UMMAYA TUI feature set.
- The `UMMAYA_TUI_IME_STRATEGY` env var ensures Option (b) can be activated
...
- **React 18 instead of React 19.2**: We lose React 19.2 Suspense improvements
  ...
  no concurrent-mode rendering, this is an acceptable downgrade. The UMMAYA TUI
...
2. **High UMMAYA-original maintenance**: Ink's `useInput` state machine covers
   ...
   UMMAYA-original code surface that diverges from upstream with every Ink
...
4. **Benefit does not justify cost**: The sole benefit is staying on Ink 7 +
   React 19.2. UMMAYA v1 does not use any Ink 7-only APIs that would be lost
