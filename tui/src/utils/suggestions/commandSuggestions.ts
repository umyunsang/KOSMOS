...
/**
 * Checks whether input is an exact slash command with no arguments.
 */
export function isExactSlashCommandInput(
  input: string,
  commands: Command[],
): boolean {
  const trimmed = input.trim()
  if (!isCommandInput(trimmed)) return false
  if (hasCommandArgs(input)) return false

  const commandName = trimmed.slice(1)
  return commandName.length > 0 && hasCommand(commandName, commands)
}
