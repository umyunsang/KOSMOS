...
def check_auth_default(ctx: CheckContext) -> CheckOutcome:
    ...
    (it was a KOSAX-invented field; AGENTS.md § CORE THESIS forbids invention).
    ...
    For KOSAX-internal synthetic surfaces (resolve_location / lookup) ``policy``
    ...
    if ctx.manifest.adapter.policy is None:
        return failed(
            ko=(
                ...
                "(AGENTS.md § CORE THESIS: KOSAX는 권한 정책을 발명하지 않음)"
            ),
            en=(
...
def check_pii_default(ctx: CheckContext) -> CheckOutcome:
    ...
    (KOSAX-invented field). The symmetry check now uses ``derived pipa_class``
    ...
    When ``adapter.policy is None`` (KOSAX-internal surfaces) the check
    ...
    if a.policy is None:
        return passed()
    from kosax.tools.policy_derivation import derive_pipa_class_default
