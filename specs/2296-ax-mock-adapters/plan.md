...
**Technical approach**: Extend the existing per-primitive sub-registry pattern (`kosax.primitives.{verify,submit,subscribe}._ADAPTER_REGISTRY` from Spec 031) with the new `mock_*_module_*` adapters; introduce one new `IPCFrame` arm (`AdapterManifestSyncFrame`) in `kosax.ipc.frame_schema` reusing the Spec 032 envelope; cache the synced manifest in a new TS-side `tui/src/services/api/adapterManifest.ts` module; modify the four `*Primitive.validateInput` methods to consult the cached manifest before falling back to `context.options.tools.find`. Audit ledger gains three new event kinds appended to the existing Spec 035 consent JSONL (`delegation_issued/used/revoked`).
...
- Append-only on disk: Spec 035 consent ledger at `~/.kosax/memdir/user/consent/` gains `delegation_*` event kinds; session JSONL transcripts at `~/.kosax/memdir/user/sessions/` (Spec 027 unchanged)
...
**Target Platform**: Single-user terminal (KOSAX TUI); Python backend speaks JSONL frames over stdio to TS-side TUI process
...
- Each Mock adapter MUST cite an agency-published policy URL — no KOSAX-invented classifications (FR-025, Constitution § II)
- Mock backend for PTY smoke is a real Python process speaking JSONL — NOT `KOSAX_BACKEND_CMD=sleep 60` (FR-021, closes Codex P1 PTY-coverage gap)
**Scale/Scope**:
...
| Per-primitive `_ADAPTER_REGISTRY` pattern for new mock adapters | Spec 031 prior art (`kosax.tools.mock.__init__.py` 6 verify + 2 submit + 3 subscribe) | Pydantic AI schema-driven registry |
...
│   ├── register_all.py                                          # MODIFY — import kosax.tools.mock for production registry boot
...
**Structure Decision**: Multi-package monorepo (Python backend `src/kosax/` + TS TUI `tui/src/`). New code lives in three locations matching the existing layering: (a) Python adapter implementations under `src/kosax/tools/mock/` extending the Spec 031 sub-registry pattern; (b) Python IPC frame variant in `src/kosax/ipc/frame_schema.py` extending the 20-arm union; (c) TS adapter-manifest cache + four primitive `validateInput` modifications under `tui/src/`. Tests parallel each layer.
...
4. **Mock-fixture backend pattern for PTY smoke**: Spawn a real Python process via `KOSAX_BACKEND_CMD=python -m kosax.ipc.demo.mock_backend` (new module). The process speaks the same JSONL frames the production backend speaks, but with deterministic mock fixtures for the US1 chain.
