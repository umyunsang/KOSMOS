...
"""NDJSON emit/parse helpers for the KOSAX IPC frame protocol (Spec 032 T012).
...
def compute_schema_file_hash(schema_path: Path) -> str:
    ...
    Used at backend startup to emit ``kosax.ipc.schema.hash`` as an OTEL
...
def attach_envelope_span_attributes(
    ...
    *,
    ...
    Sets ``kosax.ipc.correlation_id`` (always) and, when populated,
    ``kosax.ipc.transaction_id`` and ``kosax.ipc.tx.cache_state``.  The
    ...
    Args:
        ...
        tx_cache_state: ``"miss"`` / ``"hit"`` / ``"stored"`` from the
            :class:`~kosax.ipc.transaction_lru.TransactionLRU` path when an
            irreversible-tool frame is being emitted; ``None`` otherwise.
    """
    span = trace.get_current_span()
    if not span.is_recording():
        return
    span.set_attribute(KOSAX_IPC_CORRELATION_ID, frame.correlation_id)
    tx_id = getattr(frame, "transaction_id", None)
    if tx_id is not None:
        span.set_attribute(KOSAX_IPC_TRANSACTION_ID, tx_id)
    if tx_cache_state is not None:
        span.set_attribute(KOSAX_IPC_TX_CACHE_STATE, tx_cache_state)
...
def emit_schema_hash_resource_attribute(schema_path: Path | None = None) -> str | None:
    """Emit ``kosax.ipc.schema.hash`` on the current OTEL span at backend startup.
    ...
    if span.is_recording():
        span.set_attribute(KOSAX_IPC_SCHEMA_HASH, digest)
    logger.info(
