...
| **UMMAYA files in slice (total)** | **452** | 446 components + 3 screens + 3 top-level + 1 outputStyles + 1 moreright |
...
| **MIGRATE-FOR-SWAP** (CC ≠ UMMAYA) | **60** | 57 components + REPL.tsx + interactiveHelpers + replLauncher |
| **PORT** (CC has, UMMAYA missing) | **6** | All P2 / DROP-CANDIDATE in spirit — see § 4 |
| **DROP-CANDIDATE** (UMMAYA-only) | **64** | All UMMAYA-original UI L2 / brand / legitimate features — see § 6 |

**Headline**: This slice is in the **best shape of the entire audit**.
**85%** of files where CC and UMMAYA overlap are **byte-identical**.
...
hypothesis assumed. UMMAYA components/ has **more** files than CC's
...
(UMMAYA = CC + 2 swaps; payments / 1P telemetry are the third thing
correctly removed per `feedback_ummaya_scope_cc_plus_two_swaps`).
...
find tui/src/components -type f \( -name '*.ts' -o -name '*.tsx' \) \
  | sed 's|.*/components/||' | sort > ummaya.txt
comm -23 cc.txt ummaya.txt   # CC-only → PORT candidates
comm -13 cc.txt ummaya.txt   # UMMAYA-only → DROP candidates
comm -12 cc.txt ummaya.txt   # in both → bucket via SHA-256
```
...
in is "every file in `comm -12 cc.txt ummaya.txt` whose SHA-256 matches
...
## 4. Bucket B — PORT (6 files in CC, missing in UMMAYA)
...
|---|---|---:|---|---|
| 1 | `Feedback.tsx` | — | CC `/feedback` slash-command UX (telemetry submission to anthropic.com) | **DROP-CONFIRMED**. UMMAYA has no Anthropic feedback endpoint; we collect feedback via GitHub Issues. |
| 2 | `grove/Grove.tsx` | — | CC's "Grove" growth-experiment shelf | **DROP-CONFIRMED**. Pure 1P growth surface. |
| 3 | `LogoV2/GuestPassesUpsell.tsx` | — | "Guest passes" sales pitch overlay (claude.ai paid plan upsell) | **DROP-CONFIRMED**. UMMAYA is open-source / no paid tier. |
...
The CC files import from `@anthropic-ai/sdk/resources/...`; the UMMAYA
...
| File | Change |
|---|---|
| `LogoV2/Opus1mMergeNotice.tsx` | "Opus now defaults to 1M context" promo string removed/changed (UMMAYA has no Opus). |
| `IdeOnboardingDialog.tsx` | `Welcome to Claude Code for {ideName}` → `Welcome to UMMAYA for {ideName}`. |
| `HelpV2/HelpV2.tsx` | `Claude Code v${VERSION}` → `UMMAYA v${VERSION}`. |
...
UMMAYA-equivalent or feature-flagged off), Korean string injections,
or `sdk-compat` imports. **Justified** but each file should carry a
short `// SWAP:` or `// UMMAYA:` comment marker for traceability. Some
...
These are the swap surfaces where UMMAYA legitimately diverges, each
...
| `LogoV2/feedConfigs.tsx` | 23 | Brand-feed config (feed slots removed for UMMAYA). |
...
| `permissions/ExitPlanModePermissionRequest/ExitPlanModePermissionRequest.tsx` | 51 | Plan-mode permission dialog adapted for UMMAYA permission gauntlet (3-tier color, Korean labels). Verify in-file comment. **Action**: confirm. |
...
| `Messages.tsx` | 56 | **Spec 2521 multi-tool layout (`createStreamingThinkingLayoutMessage` + `getStreamingThinkingInsertIndex` + `isSameAssistantToolStack`)** + **Phase 4 `useFrameCommitTracker` OTEL hook** for `ummaya.tui.frame_commit` event. Both in-file comments. Justified. |
...
character per `docs/requirements/ummaya-migration-tree.md § Mascot`).
Each file carries an SPDX header + UMMAYA comment block citing the
...
## 6. Bucket D — DROP-CANDIDATE / UMMAYA-only legitimate features (64 files)

These are UMMAYA-original UI L2 / brand / spec-driven features added on
...
### 6.6 Prompt-input extensions (2 files) — **KEEP — Spec 287 / ummaya-migration-tree UI-B.2 + B.6**

`PromptInput/CtrlOToExpand.tsx` (CC has expand/collapse but as inline
helper; UMMAYA extracted to component),
...
`chrome/UmmayaCoreIcon.tsx`, `ReplHeader.tsx`, `Spinner/types.ts`.

### 6.8 UMMAYA-original infrastructure (11 files) — **KEEP**
...
- `FeedbackSurvey/utils.ts` (UMMAYA feedback collection scaffold).
- `mcp/types.ts` (UMMAYA MCP type module).
...
| W2 | Brand string substitution | `Claude Code` → `UMMAYA` (visible UI text only) | `IdeOnboardingDialog.tsx`, `HelpV2/HelpV2.tsx`, `Welcome*.tsx` |
...
| W4 | Brand glyph | `✻` preserved (per `ummaya-migration-tree § Mascot`) | All files using `Asterisk` / `AnimatedAsterisk` |
...
| W6 | Color palette | CC primary blue → UMMAYA violet (`#a78bfa` body, `#4c1d95` background) | Token definitions in `tui/src/theme/tokens.ts`; component-level usage threaded via theme |
| W7 | Anthropic 1P-business surfaces removed | `Settings/Usage`, `Passes`, `Feedback`, `LogoV2/*Upsell`, `grove/Grove` (no UMMAYA replacement) | 6 PORT-DROPPED files in § 4 |
...
| W12 | OTEL frame_commit instrumentation | `useFrameCommitTracker(conversationId)` hook added (Phase 4) | `Messages.tsx` (and any future `ummaya.tui.frame_commit` consumer) |
...
**Verified**: all 8 above exist in UMMAYA and either match SHA-256
...
any of them would violate CORE THESIS ("UMMAYA = CC + 2 swaps;
...
Set-difference output (reproducible):
  ...
  /tmp/ummaya-components.txt      (sorted UMMAYA enumeration, 446 entries)
  /tmp/missing-in-ummaya.txt      (CC-only, 7 entries)
  /tmp/extra-in-ummaya.txt        (UMMAYA-only, 64 entries)
