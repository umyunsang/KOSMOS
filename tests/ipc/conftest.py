...
- ``ipc_env_overrides``: monkeypatched env vars for KOSAX_IPC_* knobs.
"""
...
# ---------------------------------------------------------------------------
# Environment overrides for KOSAX_IPC_* knobs
# ---------------------------------------------------------------------------


@pytest.fixture(autouse=False)
def ipc_env_overrides(monkeypatch: pytest.MonkeyPatch) -> Generator[None, None, None]:
    """Set small KOSAX_IPC_* defaults suitable for fast unit tests.

    Opt-in per test/module by requesting this fixture.
    Values are restored after each test.
    """
    overrides = {
        "KOSAX_IPC_RING_SIZE": "16",
        "KOSAX_IPC_TX_CACHE_CAPACITY": "8",
        "KOSAX_IPC_HEARTBEAT_INTERVAL_MS": "500",
        "KOSAX_IPC_HEARTBEAT_DEAD_MS": "1500",
    }
...
def _kosax_ipc_parity_drift_fixture_default_off(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Force ``KOSAX_IPC_PARITY_DRIFT_FIXTURE`` unset by default.
