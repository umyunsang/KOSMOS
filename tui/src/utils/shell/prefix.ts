...
// UMMAYA: services/api/claude.js deleted by Spec 1633 P1+P2. queryHaiku not available — FriendliAI backend handles all LLM calls.
...
async function getCommandPrefixImpl(
  ...
  // UMMAYA: queryHaiku (Anthropic Haiku API) removed by Spec 1633 P1+P2.
  ...
  if (process.env.NODE_ENV !== 'test') {
    const message = `[${toolName}Tool] LLM-based prefix extraction not available in UMMAYA (Spec 1633).`
