...
- [x] Methodology section present and aligns with AGENTS.md CORE THESIS ("UMMAYA = CC + 2 swaps") and memory `feedback_cc_source_migration_pattern`
...
- [x] Step C (UMMAYA-only behavior-mirror) defined for files lacking direct CC source
...
- [x] Edge cases identified (6 cases covering OAuth carryover, model ID rename, signature_delta byte-copy, UMMAYA-only files, byte-copy reintroducing previously-deleted code)
...
- **Methodology change** (2026-05-01 user directive): replaced "audit-and-fix" with "byte-copy + bounded swap". Every diff between UMMAYA and CC must now justify itself by category; unjustified diffs revert.
- **Step A byte-copy is destructive** for the Procedure-A files: it overwrites the current UMMAYA state. Justified state (e.g., the `_ensure_tool_registry` register fix from 2026-05-01) is REINTRODUCED via Step B / Step C swap commits with retroactive labeling. Nothing is silently lost.
