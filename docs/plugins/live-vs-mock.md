...
├─ 예 → Authentication 흐름이 UMMAYA partnership 으로 발급 가능한가?
│       ├─ 예 → ✅ Live tier
...
async def adapter(payload: LookupInput) -> dict:
    api_key = os.environ["UMMAYA_<MINISTRY>_API_KEY"]   # 하드코딩 금지
...
`tier=mock` scaffold (`ummaya plugin init <name> --tier mock`) 가 위 패턴을 자동 emit.
...
| 분류 | 위치 | Plugin 가능? |
|---|---|---|
| Live tier | `ummaya-plugin-store/ummaya-plugin-<name>` | ✓ |
| Mock tier (mirror 가능) | `ummaya-plugin-store/ummaya-plugin-<name>` + `docs/mock/` 출처 cite | ✓ |
| OPAQUE (시나리오만) | `docs/scenarios/<name>.md` | ✗ — scenario 문서만 |

UMMAYA host 는 OPAQUE 항목을 plugin 으로 받지 않습니다. 이는 PII 처리 invariant chain (Spec 024 V2-V4) 을 우회 가능한 attack surface 가 됩니다.
