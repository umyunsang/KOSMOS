...
"""Structured event logger for KOSAX observability.

``ObservabilityEventLogger`` is the single emit point for all structured
observability events in KOSAX.  It:
...
Usage::
    ...
    from kosax.observability import ObservabilityEventLogger, ObservabilityEvent

    logger = ObservabilityEventLogger()
    logger.emit(ObservabilityEvent(event_type="llm_call", success=True))

The underlying Python logger name is ``kosax.events``.  Consumers can
...
from kosax.observability.events import EventType, ObservabilityEvent

_events_logger = logging.getLogger("kosax.events")
...
class ObservabilityEventLogger:
    """Fail-safe structured event emitter for KOSAX observability.

    Args:
        logger: Optional custom Python logger.  Defaults to ``kosax.events``
