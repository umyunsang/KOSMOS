...
A citizen typing a natural-language question realises partway through the agent's tool-loop execution that they asked the wrong thing ("아, 내가 잘못 말했네"). Today on ministry portals this requires closing the browser tab and re-authenticating with a public certificate (공동인증서). In UMMAYA the citizen presses `ctrl+c` once; the active agent loop aborts, any in-flight tool call is cancelled, the session and authentication state persist, and the citizen can re-ask immediately in the same session.

**Why this priority**: Without a reliable interrupt, every mistaken query forces the citizen back through PIPA re-consent and AAL authentication. This is a pre-launch blocker — a conversational AX surface without one-key interrupt is functionally worse than the very DX portal flows UMMAYA claims to replace.
...
A citizen finishes their session and wants to exit. Ministry portals auto-logout silently after 10 minutes; the explicit logout button's location differs per portal. In UMMAYA the citizen presses `ctrl+d` once; the session closes cleanly, all pending audit records are flushed to durable storage, any held consent tokens are marked as session-complete, and the TUI exits with status code 0.
...
1. **Given** `~/.ummaya/keybindings.json` contains `{"ctrl+r": null}`, **When** `ctrl+r` is pressed, **Then** no action fires.
...
- **FR-010**: The Permission Mode status indicator in the TUI MUST update within 200 ms of a successful cycle and emit an OTel span attribute `ummaya.permission.mode` on the change.
...
- **FR-023**: The TUI MUST read an optional user-override file at `~/.ummaya/keybindings.json` at startup. Missing or unreadable file MUST degrade silently to defaults.
...
- **FR-033**: Every successful Tier 1 activation MUST emit an OTel span with attribute `ummaya.tui.binding=<action-name>`; reserved bindings (`agent-interrupt`, `session-exit`) MUST additionally emit an audit record per Spec 024.
- **FR-034**: Blocked activations (IME gate, Permission Mode block, consent scope) MUST emit a span with `ummaya.tui.binding.blocked.reason=<reason>` for post-hoc analysis; no audit record is emitted for benign blocks (IME still composing is not an audit event).
...
- **User Override File** (`~/.ummaya/keybindings.json`): JSON object mapping chord strings to either `null` (disable) or action names (remap). Schema frozen in this spec; editor UI deferred.
...
- **SC-009**: At least 80% of the CC `defaultBindings.ts` binding schema shape (chord string format, action naming convention, context labels) is preserved in the UMMAYA registry to keep future Tier 2/3 ports mechanical.
...
- **Image paste binding** — UMMAYA is a Korean-public-API harness; citizen queries are text-only. No image-input flow exists in the product vision (`docs/vision.md`).
- **External editor binding** (`ctrl+e` in CC) — citizens do not edit source files; this is a DX-only affordance.
- **Model picker binding** (`meta+p` in CC) — UMMAYA uses K-EXAONE via FriendliAI as a single model; no picker surface exists.
