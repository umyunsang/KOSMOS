...
기존 KOSMOS `GovAPITool` 메타데이터 schema 그대로. 13 어댑터 + `resolve_location` 모두 이 schema 인스턴스. 사용자 디렉티브 "도메인 독립" + Constitution III (Pydantic v2 strict) 정합.
...
각 도메인의 17 광역시도 short reference table. KOSMOS 코드 안 mirror dict (`grid_coords.py:REGION_TO_GRID`, `koroad/code_tables.py:SidoCode` 등) 의 description 인라인 형태.
...
class ShortReference(BaseModel):
    ...
    source_doc: str  # 출처 docs path (예: "/tmp/kosmos-domain-docs/kma_asos.txt")
    last_verified: date
...
- `source_doc` MUST `/tmp/kosmos-domain-docs/*.txt` 또는 `docs/api/*.md` 의 절대 경로 또는 KOSMOS 코드 모듈 경로 (`src/kosmos/tools/kma/grid_coords.py`)
...
```python
# src/kosmos/tools/kma/grid_coords.py — 변경 X (mirror dict 그대로)
REGION_TO_GRID = {
    "서울": (60, 127),
    "부산": (98, 76),
    # ... 17 광역시도
...
# src/kosmos/tools/koroad/code_tables.py — 변경 X (mirror IntEnum 그대로)
...
| Mirror dict KOSMOS 코드 안 보존 (분리 X) | research.md Decision 1 | `data/agency-codes/` 디렉토리 부재 검증 |
