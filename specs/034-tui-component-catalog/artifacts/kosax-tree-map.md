# KOSAX Current TUI Tree Map

**Source**: `find tui/src/components -type f \( -name "*.tsx" -o -name "*.ts" \)` on 2026-04-20 at KOSAX HEAD `34c48f4`.
**Consumer**: T010–T014 (`KOSAX target` column for PORT/REWRITE rows).
...
## 2 · CC → KOSAX target-directory mapping (PORT/REWRITE column hint)

| CC family | KOSAX target parent | Rationale |
|---|---|---|
...
- When a new top-level subdir is needed (e.g., `chrome/`, `dialogs/`, `onboarding/`, `shortcuts/`, `mcp/`), spell the path in the `KOSAX target` column and note in rationale: `creates new subdir tui/src/components/<X>/ (first PORT into family)`.
...
- **PORT, implementation complete**: the one KOSAX file that already exists covers CC's core dialog surface. Mark `—` Task sub-issue.
...
IPC hardening lives in `src/kosax/ipc/` (backend) and TUI-side `src/ipc/`. CC has no `components/ipc/*` family; A is not a likely Owning Epic for any `src/components/*` file. If a row nonetheless cites A (e.g., a component rendering connection status), apply §R3 re-parent.

## 4 · Part D-3 carve-out reminder (FR-033)

`tui/src/components/primitive/*` (18 files listed in §1) are KOSAX-original citizen-domain primitives from Spec 287. They have **no CC analog** and do **not** appear in the catalog. Reverse-check only: no CC component should have `KOSAX target = tui/src/components/primitive/<X>.tsx`.
