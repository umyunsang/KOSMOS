...
- `KOSAX_FRIENDLI_TOKEN` + `KOSAX_DATA_GO_KR_API_KEY` set in `.env` for live smoke.
...
1. Step A byte-copy commits per Procedure-A file (using `cp <cc_source_path> <kosax_path>` then `git commit -m "byte-copy(2521): import CC <path>"`)
...
```sh
pkill -f "bun.*tui|kosax.*ipc" 2>/dev/null
find src/kosax/ipc/__pycache__ src/kosax/llm/__pycache__ -name '*.pyc' -delete 2>/dev/null
...
```sh
# llm-provider swaps (Anthropic SDK → KOSAX IPC)
...
# tool-domain (CC dev tools → KOSAX primitives) — likely 0 commits in this Epic
...
Cause: a function in a Procedure-B file (e.g., `src/kosax/llm/client.py`) lacks `CC reference: ...` in its docstring.
...
report a SHA mismatch. Use this step-by-step procedure to bring KOSAX back in sync.
...
- Possibly new unjustified hunks if the CC file changed in regions that were previously
  byte-identical (i.e., no KOSAX swap was needed there before, but now the CC and KOSAX
  files differ)
...
2. Resolve the conflict by editing the conflicted file:
   ...
   - Reject the CC change if KOSAX's IPC bridge already handles the new CC behavior through
...
2. **One swap commit per changed swap point** (re-applies each SWAP category with updated
   citations):
   ```
   swap/llm-provider(2521): route claude.ts through KOSAX IPC adapter [CC vX.Y.Z refresh]
