...
``infra/otel-collector/config.yaml`` correctly:
  ...
  (b) hashes ``kosax.location.query`` to its SHA-256 hex digest
...
- ``KOSAX_LANGFUSE_OTLP_AUTH_HEADER`` set in the environment (or ``.env``)
  containing the Base64-encoded ``pk-lf-xxx:sk-lf-xxx`` credential
- ``KOSAX_OTEL_COLLECTOR_PORT`` (default: ``4318``) matches the running collector
...
_AUTH_HEADER = os.environ.get("KOSAX_LANGFUSE_OTLP_AUTH_HEADER", "")
_COLLECTOR_PORT_RAW = os.environ.get("KOSAX_OTEL_COLLECTOR_PORT", "4318")
try:
...
def _require_live_stack() -> None:
    ...
    if _COLLECTOR_PORT is None:
        pytest.skip(
            f"KOSAX_OTEL_COLLECTOR_PORT={_COLLECTOR_PORT_RAW!r} is not a valid integer — "
            "fix or unset it before running live PII redaction tests."
        )
    if not _AUTH_HEADER:
        pytest.skip(
            "KOSAX_LANGFUSE_OTLP_AUTH_HEADER is unset — "
...
def _emit_test_span(trace_id_hex: str) -> None:
    ...
    The span carries:
      ...
      - kosax.location.query = "서울역"  (must be hashed by collector)
    """
    ...
    provider = TracerProvider(
        resource=Resource.create({"service.name": "kosax-pii-redaction-test"})
    )
    ...
    tracer = provider.get_tracer("kosax.test.pii_redaction")
    with (
        link_ctx,
        ...
        span.set_attribute("kosax.location.query", "서울역")
        span.set_attribute("kosax.test.marker", "spec028-pii-smoke")
...
def test_location_query_hashed_by_collector(_require_live_stack: None) -> None:
    """(b) kosax.location.query is replaced with its SHA-256 hash (SC-003 part 2).
    ...
    # Find kosax.location.query in any observation
    location_query_values: list[str] = []
    for observation in trace.get("observations", []):
        attrs = observation.get("attributes", {}) or {}
        if "kosax.location.query" in attrs:
            location_query_values.append(str(attrs["kosax.location.query"]))

    assert location_query_values, (
        "kosax.location.query attribute not found in Langfuse trace. "
        "It should be present (as a hash), but not the raw value."
    )

    for value in location_query_values:
        assert value == _EXPECTED_HASH, (
            f"kosax.location.query must equal SHA-256('서울역') = {_EXPECTED_HASH!r}.\n"
            f"  Got: {value!r}\n"
            "  If this is the raw string, the collector's hash action did not fire."
        )
        assert value != "서울역", (
            "kosax.location.query must NOT be the raw '서울역' string after collector processing."
        )
