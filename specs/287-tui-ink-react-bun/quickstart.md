...
**Audience**: UMMAYA contributor who wants to run the TUI locally, verify the IPC bridge, and exercise the permission-gauntlet + primitive renderers without touching live `data.go.kr` APIs.
...
- Python 3.12+ with `uv` (existing UMMAYA baseline)
...
```bash
# From repo root
uv run ummaya-backend --ipc stdio &        # Starts Python backend in stdio IPC mode
...
1. `tui/src/main.tsx` invokes `Bun.spawn(["uv", "run", "ummaya-backend", "--ipc", "stdio"], {stdio: ["pipe","pipe","pipe"]})`.
...
**Crash demo**: `kill -9 <backend-pid>` → TUI shows `<CrashNotice />` within 5 s with UMMAYA_* env vars redacted.
...
Every IPC frame emits a `ummaya.ipc.frame` OTEL span (child of the session span from Spec 021):
...
Span attributes: `ummaya.session.id`, `ummaya.frame.kind`, `ummaya.frame.direction`, `ummaya.ipc.latency_ms`.
...
| `assistant_chunk` render is choppy (> 50 ms) | Debug logging enabled | Set `UMMAYA_TUI_LOG_LEVEL=WARN` |
...
| `bun run tui` hangs on start | Backend spawn slower than 2 s on cold cache | Run `uv sync` + `uv run python -c "import ummaya"` once to warm import cache |
...
```bash
UMMAYA_TUI_THEME=default             # default | dark | light
UMMAYA_TUI_LOG_LEVEL=WARN            # DEBUG | INFO | WARN | ERROR
UMMAYA_TUI_SUBSCRIBE_TIMEOUT_S=120   # subscribe stream timeout (seconds)
UMMAYA_TUI_IME_STRATEGY=fork         # fork | readline  (value set by ADR)
```

All `UMMAYA_TUI_*` values are redacted from crash notices per #468 guard pattern (FR-004).
