...
- `kosax plugin init <name>` CLI · scaffolding
...
- `kosax-plugin-seoul-subway/` — 지하철 실시간 도착 (Live, Seoul Open Data Plaza)
- `kosax-plugin-post-office/` — 우체국 택배 (Live, 우정사업본부)
- `kosax-plugin-nts-homtax/` — 홈택스 (Mock, 권한 미보유)
- `kosax-plugin-nhis-check/` — 건강검진 (Mock)
...
- `kosax-plugin-store` (신규 GitHub org) — 공식 카탈로그
- `tui/src/commands/plugin-install.ts` — `kosax plugin install <name>` · SLSA 서명 검증
- `src/kosax/plugins/registry.py` — 플러그인 auto-discovery · manifest 검증
- `src/kosax/plugins/manifest_schema.py` — 매니페스트 Pydantic (primitive · permission · PIPA 서약)
...
- 플러그인 OTEL span 강제 (`kosax.plugin.id` attribute)
...
- Epic P0, P1+P2, P3 완료
- `src/kosax/tools/registry.py` 에 self-classify 메타데이터 wired in (P3)

### Related decisions

`docs/requirements/kosax-migration-tree.md § L1-B B8`
