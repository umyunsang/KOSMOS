# ADR-008: ummaya-plugin-store GitHub Organization + Vendored slsa-verifier

**Status**: Accepted
**Date**: 2026-04-26
**Epic**: #1636 (P5 Plugin DX 5-tier)
**Affected**: External GitHub repos (`ummaya-plugin-store/*`) · `~/.ummaya/vendor/slsa-verifier/` · `scripts/bootstrap_slsa_verifier.sh` · `src/ummaya/plugins/slsa.py` · `src/ummaya/plugins/installer.py`
...
1. **Where do example plugin repos live?** Migration tree § B8 mandates
   ...
   `umyunsang/<name>` (lead's personal namespace), `umyunsang/ummaya-plugin-<name>`
...
### R-2 → standalone repos under `ummaya-plugin-store/`

Plugin repos live in a dedicated GitHub organization
`ummaya-plugin-store` (free public org, created 2026-04-26 via web UI).
Naming convention: `ummaya-plugin-store/ummaya-plugin-<name>` where
...
| Repo | Role |
|---|---|
| `ummaya-plugin-store/ummaya-plugin-template` | Tier 1 scaffold (`is_template: true`) |
| `ummaya-plugin-store/ummaya-plugin-seoul-subway` | Tier 3 Live example |
| `ummaya-plugin-store/ummaya-plugin-post-office` | Tier 3 Live example |
| `ummaya-plugin-store/ummaya-plugin-nts-homtax` | Tier 3 Mock example |
| `ummaya-plugin-store/ummaya-plugin-nhis-check` | Tier 3 Mock example |
| `ummaya-plugin-store/index` | Tier 5 catalog index |

### R-3 → vendored slsa-verifier under `~/.ummaya/vendor/`

`slsa-verifier` Go binary is downloaded on first install via
`scripts/bootstrap_slsa_verifier.sh` into
`$UMMAYA_PLUGIN_VENDOR_ROOT/slsa-verifier/<platform>-<arch>/slsa-verifier`
(default `~/.ummaya/vendor/`). Platform matrix supported:
darwin-amd64, darwin-arm64, linux-amd64, linux-arm64. Pin: v2.6.0
(latest stable as of 2026-04-26).

The host's `ummaya.plugins.slsa.verify_artifact()` shells out via
...
`UMMAYA_PLUGIN_SLSA_SKIP=1` environment variable opt-in dev override
...
- **Source-URI semantics**: SLSA v1.0 provenance attestations bind
  ...
  predictable URI prefix `github.com/ummaya-plugin-store/ummaya-plugin-<name>`
...
- **Future delegation**: When UMMAYA scales to multiple maintainers,
...
- **No-pip constraint**: AGENTS.md hard rule "Never add a runtime
  ...
  install-time, not bundled with UMMAYA source.

- **Platform safety**: Bootstrap script downloads the matching binary
  on first plugin install. Host code in `ummaya.plugins.slsa` returns
...
- **`umyunsang/ummaya-plugin-<name>` (personal namespace)** —
  Rejected. SLSA source URI semantics + future delegation.
- **Monorepo `umyunsang/ummaya-plugins/<name>`** — Rejected. SLSA
...
- **JavaScript verifier (`@slsa-framework/slsa-verifier-action`)** —
  Rejected. Couples UMMAYA install path to Node.js, which the host
...
- **Skip SLSA verification entirely** — Rejected. Plugin install is
  ...
  `UMMAYA_PLUGIN_SLSA_SKIP=1` dev escape hatch documents itself
  in the consent receipt.
- **Bundle slsa-verifier into the UMMAYA wheel** — Rejected. ~10 MB
...
- Each `ummaya-plugin-store/<repo>` has a SLSA-verifiable source
...
- Vendored verifier path is overridable via `UMMAYA_PLUGIN_VENDOR_ROOT`
...
- Future fork of UMMAYA must create their own org or fork the
...
- `ummaya-plugin-store` namespace is reserved on GitHub but does
...
- **R-2 created** (2026-04-26):
  - <https://github.com/ummaya-plugin-store/ummaya-plugin-template>
  - <https://github.com/ummaya-plugin-store/ummaya-plugin-seoul-subway>
  - <https://github.com/ummaya-plugin-store/ummaya-plugin-post-office>
  - <https://github.com/ummaya-plugin-store/ummaya-plugin-nts-homtax>
  - <https://github.com/ummaya-plugin-store/ummaya-plugin-nhis-check>
  - <https://github.com/ummaya-plugin-store/index>

- **R-3 implemented**:
  - `scripts/bootstrap_slsa_verifier.sh` (T063) — vendor download.
  - `src/ummaya/plugins/slsa.py` (T011) — verify_artifact wrapper.
  - `src/ummaya/plugins/installer.py` (T014) — phase 3 SLSA.
  - `src/ummaya/plugins/tests/test_installer_slsa.py` (T011) — 8 tests
