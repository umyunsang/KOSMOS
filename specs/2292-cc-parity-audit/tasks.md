...
- 모든 경로는 repo root (`/Users/um-yunsang/UMMAYA`) 기준 상대경로
...
- [X] T002 [P] Add `specs/2292-cc-parity-audit/scripts/_common.sh` shared helpers (repo root resolution, UMMAYA_DIR / CC_DIR constants, `LC_ALL=C` enforcement) — sourced by all R-step shell scripts
...
- [X] T003 Implement `specs/2292-cc-parity-audit/scripts/enumerate-files.sh` — `find` + `diff -rq` + `comm` + `LC_ALL=C sort` 로 `data/enumerated-{keep-byte-identical,import-candidate,modified,ummaya-only,cc-only}.txt` 5 파일 산출 (quickstart.md § R1 + research.md § R-1 구현)
...
- [X] T008 [US1] Generate `specs/2292-cc-parity-audit/data/suspicious-transfer.json` — Suspicious 부분집합을 data-model.md § SuspiciousTransferList 라우팅 룰 (services/api/* → Epic β #2293, src/ummaya/permissions/* → out-of-scope, 그 외 → uncategorized) 로 분류
