...
from kosax.tools.kma.projection import KMADomainError, latlon_to_lcc
...
def _keyword_doc_matches_query(query: str, doc: object) -> bool:
    ...
    "존재하지않는주소" it may still return a popular POI; KOSAX must surface
...
async def _kakao_geocode(
    ...
    *,
    ...
    from kosax.tools.geocoding.kakao_client import (  # noqa: PLC0415
        search_address,
        search_keyword,
    )
...
async def _kakao_adm_cd_from_coords(
    ...
    *,
    ...
    from kosax.tools.geocoding.kakao_client import coord_to_region_code  # noqa: PLC0415
...
async def _kakao_address_coords(
    ...
    *,
    ...
    """
    try:
        from kosax.tools.geocoding.kakao_client import search_address  # noqa: PLC0415
...
async def _kakao_keyword_coords(
    ...
    *,
    ...
    """
    try:
        from kosax.tools.geocoding.kakao_client import search_keyword  # noqa: PLC0415
...
async def _kakao_coords(
    ...
    *,
    ...
    endpoints as six methods — the wrapper-side fanout is KOSAX' addition
...
async def _kakao_adm_cd(
    ...
    *,
    ...
    try:
        ...
        return None
    except Exception as exc:  # noqa: BLE001 — Codex P1: KOSAX_KAKAO_API_KEY 부재
...
async def _juso_adm_cd(
    ...
    *,
    ...
    """
    from kosax.settings import settings  # noqa: PLC0415
    from kosax.tools.geocoding.juso import lookup_adm_cd  # noqa: PLC0415

    confm_key = settings.juso_confm_key
    if not confm_key:
        logger.debug("juso: KOSAX_JUSO_CONFM_KEY not set, skipping")
        return None
...
async def _sgis_adm_cd(
    ...
    *,
    ...
    from kosax.settings import settings  # noqa: PLC0415
    from kosax.tools.geocoding.sgis import lookup_adm_cd_by_coords  # noqa: PLC0415

    consumer_key = settings.sgis_key
    consumer_secret = settings.sgis_secret
    if not consumer_key or not consumer_secret:
        logger.debug("sgis: KOSAX_SGIS_KEY/SECRET not set, skipping")
        return None
...
async def resolve_location(  # noqa: C901
    ...
    *,
    ...
    if want == "adm_cd":
        ...
        # JUSO (KOSAX_JUSO_CONFM_KEY) → Kakao address.b_code →
        # Kakao coord2regioncode → SGIS (KOSAX_SGIS_KEY).
...
async def resolve_location_v4(
    ...
    *,
    ...
    Evidence:
        /tmp/kosax-evidence/geocoding-evidence.md — 4 scenarios (서울 강남구,
