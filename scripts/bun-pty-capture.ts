...
//
// KOSAX-original — Bun-native PTY harness for end-to-end TUI verification.
//
...
async function run(): Promise<void> {
  ...
  const cols = Number(process.env.KOSAX_DEBUG_COLS ?? 180);
  const rows = Number(process.env.KOSAX_DEBUG_ROWS ?? 60);
  const sampleFrames = process.env.KOSAX_PTY_SAMPLE_FRAMES !== '0';
  const maxFrames = Number(process.env.KOSAX_PTY_MAX_FRAMES ?? 500);
