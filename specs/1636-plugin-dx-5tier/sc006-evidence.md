...
```
SC-006 success criterion: every kosax-plugin-store/<name> ships with
...
|---|---|---|---|---|
| `kosax-plugin-seoul-subway` | live | 1 | 4 | ✅ 4 passed in 0.11s |
| `kosax-plugin-post-office` | live | 1 | 4 | ✅ 4 passed in 0.12s |
| `kosax-plugin-nts-homtax` | mock | 2 | 4 | ✅ 4 passed in 0.52s |
| `kosax-plugin-nhis-check` | mock | 2 | 4 | ✅ 4 passed in 0.52s |
...
for repo in kosax-plugin-seoul-subway kosax-plugin-post-office \
           kosax-plugin-nts-homtax kosax-plugin-nhis-check; do
  echo "=== $repo ==="
  git clone --quiet https://github.com/kosax-plugin-store/$repo.git
  cd $repo
  uv sync --quiet
...
- ⏳ **CI plugin-validation.yml self-run**: GitHub Actions reusable workflow 호출은 첫 PR 생성 시 트리거 — 본 evidence 는 로컬 검증만 cover. CI 부분은 첫 외부 contributor PR 또는 `gh workflow run plugin-validation.yml --repo kosax-plugin-store/kosax-plugin-<name>` 수동 trigger 후 별도 측정 필요.
...
2. **`TOOL = _build_tool()` eager construction** — kosax 가 venv 에 없는 standalone 환경에서 `from kosax.tools.models import GovAPITool` 가 즉시 실행되어 ModuleNotFoundError. PEP 562 `__getattr__` lazy 패턴으로 전환.
...
- `kosax-plugin-store/kosax-plugin-seoul-subway@078f93c`
- `kosax-plugin-store/kosax-plugin-post-office@665a1c9`

Mock tier 두 repo (nts-homtax, nhis-check) 는 T023 이후 cli_init 으로 생성되어 처음부터 4/4 통과.

## 향후 강화

- **plugin-validation.yml self-run**: `kosax-plugin-store/<repo>` 의 `.github/workflows/plugin-validation.yml` 가 `umyunsang/KOSAX@main` 의 reusable workflow 를 호출. PR 생성 또는 manual trigger 시 50/50 자동 검증.
- **catalog regeneration**: `scripts/regenerate_catalog.py` 가 `kosax-plugin-store/index` 의 `index.json` 을 자동 갱신.
