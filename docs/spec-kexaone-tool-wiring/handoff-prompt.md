...
1. `AGENTS.md` — UMMAYA 룰 (Conventional Commits, English source, no Co-Authored-By 등)
2. `docs/vision.md` — 6-layer 아키텍처
3. `docs/requirements/ummaya-migration-tree.md § L1-A.A3 + § P3` — Tool protocol = K-EXAONE native function calling, P3 phase 정의
4. `docs/spec-streaming-ui-projection/epic-plan.md` — 직전 epic, paint chain root cause + 디버깅 패턴
5. `src/ummaya/llm/_cc_reference/claude.ts:1900-2304` — 직전 commit `33478d4`로 cp 된 CC streaming/agentic baseline
6. **메모리 파일들** (`/Users/um-yunsang/.claude/projects/-Users-um-yunsang-UMMAYA/memory/`):
...
cd ~/UMMAYA
git log -3 --oneline
# 33478d4 feat(llm): UMMAYA-1633 P3 — wire K-EXAONE thinking via CC reference
# a7fc8f6 fix(tui): UMMAYA-1633 P3 — assistant message paint chain unblocked
# f459bfb feat(tui): UMMAYA-1633 P3 — stream-event projection for incremental paint
git status                  # clean (또는 docs/* dirty만 OK)
ls .env                     # UMMAYA_FRIENDLI_TOKEN 필수
...
### 4.1 Backend 누락 (`src/ummaya/ipc/stdio.py`)
...
| Whitelist (line 1278-1284) | 하드코딩된 primitive list | primitives 카탈로그(`src/ummaya/primitives/__init__.py` 또는 `manifest.yaml`)에서 single source of truth로 끌어오기 |
...
| `src/utils/permissions/permissions.ts` | 1486 | permission gauntlet 본체 (Spec 033와 매핑) | `src/ummaya/llm/_cc_reference/permissions.ts` |
...
DEST=src/ummaya/llm/_cc_reference
...
위 § 5의 9개 파일 cp. `src/ummaya/llm/_cc_reference/README.md` 작성: 파일별 1-line description + UMMAYA 매핑.
...
- `registry.export_core_tools_openai()` 가 active primitives + MVP 보조를 OpenAI function shape로 반환 (현재 정의는 `src/ummaya/tools/registry.py:373-378`, UMMAYA-1978 T053b의 `_dispatch_primitive` 가 사용 가능한지 확인)
...
CC reference: `_cc_reference/permissions.ts` (1486 lines, UMMAYA Spec 033와 매핑 검토 필요).
...
a7fc8f6 fix(tui): UMMAYA-1633 P3 — assistant message paint chain unblocked
f459bfb feat(tui): UMMAYA-1633 P3 — stream-event projection for incremental paint
148b0d1 docs(epic): UMMAYA streaming UI projection — plan + handoff prompt
34304f6 feat(backend): UMMAYA-1633 P3 — parse K-EXAONE inline <tool_call> XML
```
...
backend stderr 가 PTY로 forward 안 되는 issue 있음 — backend 디버깅 필요시 `open("/tmp/ummaya-be.log", "a")` 패턴으로 file에 직접 write (이전 세션에서 검증된 우회).
