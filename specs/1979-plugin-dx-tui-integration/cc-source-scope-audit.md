...
**Date**: 2026-04-29
**Authority**: AGENTS.md § CORE THESIS (3차 정정 final) + `feedback_kosax_is_ax_gateway_client.md` + `delegation-flow-design.md § 12`
...
```
┌──────────────────────────────────────────────────────────────────────┐
│  TUI (1,884 CC files vs 2,090 KOSAX files)                          │
│                                                                       │
...
│      └─ 그 외                                  77                     │
│                                                                       │
│  ➕ KOSAX-only ADDITIONS                    274 files (13.1%)        │
...
| `services/skills/`, `services/files/`, `services/mcp/` | KOSAX 변경 (i18n 적용 등) | **MINOR PATCH** — 수정 점 검증 |
| 그 외 ~30 파일 | 미세 변경 | **AUDIT** — 각각 검증 |

#### 1.2.2 utils/ — 62 파일 (i18n/theme/Korean IME 적용)
KOSAX-specific 한국어 환경 통합 변경. 대부분 정당한 변경.

대표:
- `utils/markdown.ts`, `utils/format.ts` — 한국어 처리 추가
- `utils/keychain.ts`, `utils/secureStorage.ts` — 보안 storage 변경 (CC는 macOS keychain, KOSAX는 file-based)
...
- `components/Splash.tsx`, `components/Logo.tsx` — KOSAX 브랜딩 (UFO 마스코트, 보라색 #a78bfa)
...
#### 1.2.5 tools/ — 9 파일
- `tools/Tool.ts` — Tool 인터페이스 (KOSAX minor adjustment)
...
**작업**: KEEP. 단 일부 (kosaxPendingConsent dead state) 정리 후속.

#### 1.3.3 commands/ (46 files) — 한국어 slash commands
- `/onboarding`, `/plugin`, `/plugins`, `/export`, `/consent`, `/agents`, `/help` 등 KOSAX 추가
...
- 시민 보조: CalculatorTool, DateParserTool, TranslateTool, ExportPDFTool — kosax-migration-tree § C.6 MVP 7
- 검토 후 삭제 후보 6개:
  - `MonitorTool` — KOSAX-original, 시민 use case 불명확
...
| `services/api/bootstrap.ts`, `services/oauth/`, `services/policyLimits/`, `services/remoteManagedSettings/`, `services/rateLimitMocking.ts`, `services/mockRateLimits.ts`, `services/claudeAiLimitsHook.ts`, `services/analytics/datadog.ts` | 16 | claude.ai 인증/결제/통계 (KOSAX = K-EXAONE) |
| `migrations/` | 11 | Anthropic 모델 migration (Sonnet45→46, Opus1m→Sonnet45 등) — KOSAX는 K-EXAONE 단일 |
| `remote/` (SessionsWebSocket, RemoteSessionManager 등) | 4 | Anthropic 원격 세션 (KOSAX는 local만) |
| `tools/AgentTool/built-in/{planAgent,claudeCodeGuideAgent,verificationAgent,exploreAgent}` | 4 | Anthropic 사내 agent (KOSAX는 한국 부처 agent) |
| `commands/login/`, `commands/logout/` | 4 | claude.ai 인증 (KOSAX = K-EXAONE FriendliAI Tier 1) |
...
| M-3 | (KOSAX Modified utils/permissions 잔재) | Spec 033 정리 |
| M-4 | system prompt 갱신 (`prompts/system_v1.md` Python backend) | 5-primitive citizen UX + OPAQUE hand-off rule |

---

## 2. Backend Python (`src/kosax/`) — 카테고리별 분류

### 2.1 디렉토리별 통계
- 19 디렉토리, 259 .py files
- KOSAX-original full (CC 대응 없음)

### 2.2 KEEP — 보존 영역
- `agents/`, `cli/`, `config/`, `context/`, `engine/`, `eval/`, `ipc/`, `llm/`, `memdir/`, `observability/`, `recovery/`, `safety/`, `security/`, `session/`, `_canonical/` — KOSAX 인프라, 보존
...
#### 2.3.1 `permissions/` (25개 파일) — KOSAX-original Spec 033 인프라
...
`GovAPITool` 모델에서 KOSAX-invented 필드 제거 + `AdapterRealDomainPolicy` citation 추가:
...
| `src/kosax/primitives/delegation.py` (DelegationToken + DelegationContext) | schema | ~80 | High |
...
**작업**: `src/kosax/tools/rate_limiter.py` 이미 존재 — 검토 후 per-tool-id 인덱싱 확인/수정.
...
### Phase β — KOSAX-original UI 정리 마무리 (1 sprint)
...
- 6 KOSAX-only 검토 삭제 후보 (Monitor/ReviewArtifact/SuggestBackgroundPR/Tungsten/VerifyPlanExecution/Workflow)
...
- `src/kosax/permissions/` 25 파일 정리 (~20 DELETE, 5 KEEP/부분)
- `AdapterRealDomainPolicy` Pydantic 모델 신설 (`src/kosax/tools/models.py` 수정)
- 18 어댑터 메타 마이그레이션 (KOSAX-invented 필드 → citation_url)

**위험**: 낮음 (이미 frontend에서 해본 패턴).

### Phase ε — AX-infrastructure mock 어댑터 신설 (2 sprints)
3차 thesis 핵심 — Singapore APEX + 공공마이데이터 base mock 신설:
- `src/kosax/primitives/delegation.py` (DelegationToken/Context schema)
...
- `docs/research/policy-mapping.md` 신설 — KOSAX adapter ↔ Singapore APEX/X-Road/EUDI/마이나 매핑 표
...
| 위험 | 영향 | 완화 |
|---|---|---|
| Phase α에서 modified 212 파일 중 잘못된 KOSAX 변경 발견 | 작업 양 증가 | spot-check, 의심 파일만 정밀 audit |
...
tui/src/keybindings/ (대부분, KOSAX-only 제외)
...
```
src/kosax/primitives/delegation.py                                     (~80 LOC)
src/kosax/tools/mock/verify_module_simple_auth.py                      (~150)
src/kosax/tools/mock/verify_module_modid.py                            (~150)
src/kosax/tools/mock/verify_module_kec.py                              (~150)
src/kosax/tools/mock/verify_module_geumyung.py                         (~150)
src/kosax/tools/mock/verify_module_any_id_sso.py                       (~120) [디지털원패스 후속]
src/kosax/tools/mock/submit_module_hometax_taxreturn.py                (~100)
src/kosax/tools/mock/submit_module_gov24_minwon.py                     (~100)
src/kosax/tools/mock/submit_module_public_mydata_action.py             (~100)
src/kosax/tools/mock/lookup_module_hometax_simplified.py               (~100)
src/kosax/tools/mock/lookup_module_gov24_certificate.py                (~80)
...
### Phase δ (백엔드 정리):
```
src/kosax/permissions/modes.py
src/kosax/permissions/models.py (ConsentDecision/PermissionRule)
src/kosax/permissions/pipeline.py
src/kosax/permissions/pipeline_v2.py
src/kosax/permissions/prompt.py
src/kosax/permissions/rules.py
src/kosax/permissions/bypass.py
src/kosax/permissions/mode_bypass.py
src/kosax/permissions/mode_default.py
src/kosax/permissions/synthesis_guard.py
src/kosax/permissions/aal_backstop.py
src/kosax/permissions/killswitch.py
src/kosax/permissions/cli.py (검토)
src/kosax/permissions/adapter_metadata.py (검토)
src/kosax/permissions/credentials.py (검토)
src/kosax/permissions/session_boot.py (검토)
src/kosax/permissions/steps/ (검토)
src/kosax/tools/mock/verify_digital_onepass.py [서비스 종료]
```

### 보존 (Spec 035 영수증 ledger):
```
src/kosax/permissions/ledger.py        # 시민 영수증 (KEEP)
src/kosax/permissions/action_digest.py # 영수증 hash (KEEP)
src/kosax/permissions/hmac_key.py      # 영수증 HMAC (KEEP)
src/kosax/permissions/canonical_json.py # JSON canonicalize (KEEP)
src/kosax/permissions/audit_coupling.py # 영수증 ↔ 어댑터 coupling (KEEP)
src/kosax/permissions/ledger_verify.py  # 영수증 무결성 (KEEP)
src/kosax/permissions/otel_emit.py      # OTEL (KEEP)
src/kosax/permissions/otel_integration.py (KEEP)
```
...
### Phase δ:
- `src/kosax/tools/models.py` — `GovAPITool` 모델 6 필드 제거 + 4 필드 추가
- `src/kosax/tools/koroad/{accident_search,accident_hazard_search,code_tables}.py` (3)
- `src/kosax/tools/kma/{forecast_fetch,kma_*}.py` (8)
- `src/kosax/tools/hira/hospital_search.py` (1)
- `src/kosax/tools/nmc/{emergency_search,freshness}.py` (2)
- `src/kosax/tools/nfa119/emergency_info_service.py` (1)
- `src/kosax/tools/ssis/{codes,welfare_eligibility_search}.py` (2)
...
- ➕ **ADD 274 파일 보존 + 신규 ~17 파일** — KOSAX infrastructure + 시민 도구 + 3차 thesis mock
