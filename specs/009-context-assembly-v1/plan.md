...
**Project Type**: Library module (`src/ummaya/context/`) consumed by `QueryEngine`
...
**Complexity Justification**: No constitution violations. The `src/ummaya/context/` sub-package is the fourth Python package under `src/ummaya/`, joining `engine/`, `llm/`, and `tools/`. This is not an extra project; it is a lateral peer package that respects layer separation per `AGENTS.md § Directory layout`.
...
| Token budget accounting | Existing `ummaya.engine.tokens.estimate_tokens()` | Reuse for consistency with `PreprocessingPipeline`; avoids dual estimation paths with divergent heuristics. |
...
| Module isolation | `AGENTS.md § Directory layout` | New sub-package `src/ummaya/context/` keeps context assembly isolated from `engine/` and `llm/`. |
...
### Module structure: `src/ummaya/context/`
...
**Files**:
- `src/ummaya/context/models.py` — `SystemPromptConfig`, `ContextLayer`, `ContextBudget`, `AssembledContext`
- `src/ummaya/context/__init__.py` — export all public types
- `src/ummaya/context/builder.py` — `ContextBuilder` with all three public methods stubbed; `build_system_message()` returns a minimal static system message
...
**Files**:
- `src/ummaya/context/system_prompt.py` — `SystemPromptAssembler`; produces deterministic text from `SystemPromptConfig`; logs `WARNING` when no core tools are registered
- `src/ummaya/context/builder.py` — wire `build_system_message()` to `SystemPromptAssembler`; wire `build_assembled_context()` tool_definitions to core-then-situational ordering
...
**Files**:
- `src/ummaya/context/attachments.py` — `AttachmentCollector`; each section is a private method that returns `str | None`; the collector joins non-None sections with `\n\n`; returns `None` when result is empty
- `src/ummaya/context/builder.py` — wire `build_turn_attachment()` to `AttachmentCollector`
...
**Files**:
- `src/ummaya/context/budget.py` — `BudgetEstimator`; sums `estimate_tokens()` over all layers and tool definitions; produces `ContextBudget`
- `src/ummaya/context/builder.py` — wire `build_assembled_context()` budget computation to `BudgetEstimator`
- `src/ummaya/engine/engine.py` — replace `system_prompt` parameter with `context_builder`; insert `build_turn_attachment()` call in `run()`; handle `is_over_limit` → yield `StopReason.api_budget_exceeded`
- `src/ummaya/engine/models.py` — add `active_situational_tools` to `QueryState`
...
| `estimate_tokens()` reuse | Existing `ummaya.engine.tokens.estimate_tokens()` — consistency with `PreprocessingPipeline` | Avoids dual estimation paths (FR-007) |
...
| `src/ummaya/context/` new sub-package | `AGENTS.md § Directory layout` — layer separation | Isolated from `engine/` and `llm/` (NFR-005) |
