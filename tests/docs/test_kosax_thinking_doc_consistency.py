...
"""P1-4 doc-drift regression test — KOSAX_K_EXAONE_THINKING default consistency.

Verifies that every canonical document that mentions ``KOSAX_K_EXAONE_THINKING``
records the correct default value (``true``, matching Spec 2521 FR-010 and
the implementation in ``src/kosax/llm/client.py``).
...
References:
- Spec 2521 FR-010: ``KOSAX_K_EXAONE_THINKING`` env default MUST remain ``true``
- src/kosax/llm/client.py:973 — canonical implementation source of truth
...
# Root of the KOSAX repository — resolved relative to this test file.
_REPO_ROOT = Path(__file__).parent.parent.parent

# Documents that must state "default `true`" (not "default `false`") for
# KOSAX_K_EXAONE_THINKING.  Only include canonical policy/L1-A docs, not
...
# Pattern that matches an occurrence of "KOSAX_K_EXAONE_THINKING" alongside
# either "default `false`" or "default false" (stale phrasing to detect).
_STALE_PATTERN = re.compile(
    r"KOSAX_K_EXAONE_THINKING[^\n]*default\s+`?false`?",
    re.IGNORECASE,
)
...
def test_implementation_default_is_true() -> None:
    """src/kosax/llm/client.py must hard-code default 'true' for the env var."""
    client_py = _REPO_ROOT / "src" / "kosax" / "llm" / "client.py"
    ...
    assert 'os.environ.get("KOSAX_K_EXAONE_THINKING", "true")' in content, (
        f"client.py must use default='true' for KOSAX_K_EXAONE_THINKING.\nSearched in: {client_py}"
    )
...
def test_doc_does_not_contain_stale_false_default(doc_path: Path) -> None:
    """No canonical doc should say 'KOSAX_K_EXAONE_THINKING ... default false'."""
    ...
    if "KOSAX_K_EXAONE_THINKING" not in content:
        return

    matches = _STALE_PATTERN.findall(content)
    assert not matches, (
        f"Found stale 'default false' phrasing for KOSAX_K_EXAONE_THINKING in "
