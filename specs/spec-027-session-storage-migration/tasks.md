...
### T001 — Add canonical UMMAYA path env var to backend pydantic-settings catalog

**Files**:
- `src/ummaya/config.py` (or wherever `BaseSettings` lives — verify in R-2 mapping).

**Action**:
- Confirm `UMMAYA_MEMDIR_USER` is declared in the pydantic-settings env catalog. If absent (Spec 027 may have added it; otherwise Spec 1635 P4 introduced it for `uiL2Memdir.ts:25`), add it as `UMMAYA_MEMDIR_USER: Path = Path.home() / ".ummaya" / "memdir" / "user"` with description: `"USER-tier memdir root (Spec 027). Sessions, consent, ministry-scope, plugins, onboarding, preferences live here."`
- Confirm `UMMAYA_SESSION_DIR` continues to exist for back-compat with the existing Python override at `src/ummaya/session/store.py:42`.

**Acceptance**:
- `uv run python -c "from ummaya.config import settings; print(settings.ummaya_memdir_user)"` prints the expected default.
- Setting `UMMAYA_MEMDIR_USER=/tmp/test` env then re-running prints `/tmp/test`.

**Reference**: Spec 1635 P4 `tui/src/utils/uiL2Memdir.ts:25-26` (pattern to mirror).

---

### T002 — Create `tui/src/utils/ummayaPaths.ts` with `getUmmayaSessionsDir()` helper [P]

**Files**:
- `tui/src/utils/ummayaPaths.ts` (NEW, ~40 LOC).

**Action**:
- New module exporting:
  - `getUmmayaUserTierRoot()`: returns `process.env['UMMAYA_MEMDIR_USER'] ?? join(homedir(), '.ummaya', 'memdir', 'user')` (string).
  - `getUmmayaSessionsDir()`: returns `join(getUmmayaUserTierRoot(), 'sessions')`.
  - `getUmmayaTranscriptPath(sessionId)`: returns `join(getUmmayaSessionsDir(), '<sessionId>.jsonl')`.
- Each helper memoized via `lodash-es/memoize` keyed off `process.env['UMMAYA_MEMDIR_USER']` (matches `getClaudeConfigHomeDir` pattern at `tui/src/utils/envUtils.ts:7-14`).
...
**Acceptance**:
- `bun test tui/src/utils/__tests__/ummayaPaths.test.ts` — 3 tests pass: default path, env override, memoization clears on env change.
...
**Action**:
- Change `getProjectsDir()` body from `return join(getClaudeConfigHomeDir(), 'projects')` to `return getUmmayaSessionsDir()`. Add `import { getUmmayaSessionsDir } from './ummayaPaths.js'` at top.
- Add comment block above the function: `// UMMAYA Spec 027 path migration: returns the canonical USER-tier sessions dir. The CC native '<config-home>/projects' layout is now legacy-discovery only via getCCLegacyProjectsDir() below.`
...
- Audit lines 4122-4173 and 3986-4031 (existing `readdir(projectsDir, ...)` enumeration call-sites) — those become *legacy*-readers; add a new sibling enumeration that walks the UMMAYA path, then merge results (T008 covers tests).
...
- Manual smoke: launch TUI, type a prompt, exit, verify new JSONL appears under `~/.ummaya/memdir/user/sessions/` not `~/.claude/projects/`.
...
**Action**:
- Identical change to T003 but for the portable variant. Add `import { getUmmayaSessionsDir } from './ummayaPaths.js'`.
...
- Behavior: enumerate every `<sanitized-cwd>/<session_id>.jsonl` under `getCCLegacyProjectsDir()`. For each:
  - Compute UMMAYA dest = `getUmmayaTranscriptPath(sessionId)`.
...
- Emit OTEL spans per FR-012 with attributes `ummaya.session.path_root`, `ummaya.session.migration_action`.
...
- 8 unit tests (Bun `bun:test`):
  ...
  2. 3 legacy files, no collisions → 3 copied, UMMAYA dir contains 3.
  3. 3 legacy files, 1 collision (same `session_id` already at UMMAYA) → 2 copied, 1 skip-collision.
...
**Files**:
- `src/ummaya/session/store.py` (lines 32, 42 — change default + comment).
- `src/ummaya/cli/app.py` (add CLI flag handler).
- `tests/session/test_store_path.py` (NEW, ~80 LOC).

**Action**:
- Change `_DEFAULT_SESSION_DIR = Path.home() / ".ummaya" / "sessions"` → `_DEFAULT_SESSION_DIR = Path.home() / ".ummaya" / "memdir" / "user" / "sessions"`.
- Update docstring at line 4 to cite Spec 027 invariant + this Epic.
- `UMMAYA_SESSION_DIR` env override stays for back-compat (some tests rely on it; verify in R-2 grep).
- Add `ummaya --gc-empty-stubs` CLI flag: enumerates `~/.ummaya/sessions/` (the NOW-legacy path), reads each `<uuid>.jsonl`, parses first line, asserts `entry_type == 'metadata'` AND `data.message_count == 0` AND total file lines == 1, then `unlink` only those that pass all three checks. Logs `N stubs collected, M bytes freed`. Refuses to delete any file that does not pass content inspection.
...
- Smoke: `ummaya --gc-empty-stubs` reports the expected stub count from the audited dev machine (≈3,572 if no stubs have been added since).
...
- `tui/src/test-utils/dualPathFixtures.ts` (NEW, ~40 LOC; helper to seed both legacy + UMMAYA paths from a temp dir).

**Action**:
- Use `UMMAYA_MEMDIR_USER` + `CLAUDE_CONFIG_DIR` env redirects to point at temp dirs.
- Pre-seed: 1 JSONL at legacy CC path, 1 at UMMAYA path, 1 with colliding `session_id` at both.
- Invoke `listSessionsImpl()` (or whichever surface `/resume` ultimately calls — verify in R-2).
- Assert: 2 unique entries (collision deduplicated), UMMAYA-tier copy preferred on tie, sorted by `last_active_at` desc.

**Acceptance**: 5 tests pass (legacy-only, UMMAYA-only, both, collision, ENOENT-on-legacy).
...
### T009 — `/fork` writes new JSONL to UMMAYA path regardless of parent path [P with T008]
...
- Assert: new file appears under UMMAYA path, contains 25 entries, header includes `parent_session_id == <legacy parent uuid>`.
- Assert: subsequent appends to the fork go to the UMMAYA path (mock 3 follow-up appends, re-inspect file).

**Acceptance**: 3 tests pass (fork-from-legacy, fork-from-ummaya, follow-up-appends-stay-ummaya).
...
**Action**:
- tmux scenario:
  1. Pre-seed 2 fake legacy sessions under `~/.claude/projects/-Users-um-yunsang-UMMAYA-tui/`.
  ...
  8. Inspect `~/.ummaya/memdir/user/sessions/` — assert exactly 3 files (1 from the new chat + 2 migrated).
- vhs `.tape`:
...
- `bun typecheck` clean (UMMAYA narrows to `src/stubs/**` only — verify no new TS errors leak in).
