...
``src/ummaya/ipc/frame_schema.py`` (covered by the existing
...
Drift fixture is opt-in only via ``UMMAYA_IPC_PARITY_DRIFT_FIXTURE=1``;
...
from ummaya.ipc.frame_schema import _BaseFrame
...
_DRIFT_ENV_VAR = "UMMAYA_IPC_PARITY_DRIFT_FIXTURE"
...
def run_codec_envelope_parity_check(codec_path: pathlib.Path | None = None) -> None:
    ...
    assert not extra, (
        ...
        f"_BaseFrame in src/ummaya/ipc/frame_schema.py."
    )
