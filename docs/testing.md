...
KOSAX testing conventions and expectations. `AGENTS.md` summarizes the rules; this file is the long form.
...
Every source module under `src/kosax/<area>/<module>.py` gets a parallel `tests/<area>/test_<module>.py`.
...
uv run pytest --cov=src/kosax   # with coverage
```
...
1. Set the API key: `export KOSAX_DATA_GO_KR_KEY=...`
...
| 2. **stdio JSONL probe** | "Does the backend invoke tools when given a citizen prompt?" — bypasses the TUI render entirely | `subprocess.Popen(['uv','run','kosax','--ipc','stdio'])` + line-based JSONL frames | `*.jsonl` | ✓ grep |
...
expect(stdout.lastFrame()).toContain('KOSAX')
stdin.write('/help\r')
...
|---|---|---|
| `smoke-keyframe-1-boot.png` | After `bun run tui` settles | KOSAX branding, boot-guard line (`tool_registry: N entries verified ...`), prompt rendered |
...
Env KOSAX_BACKEND_CMD "sleep 60"
...
- `Ctrl+Left` / `Ctrl+Up` / `Ctrl+Right` / `Ctrl+Down` chord support — required for KOSAX keybinding tier-1 smoke (Spec 287 / 1979 keybinding wiring) where word-jump and history-navigation chords need a tape capture.
...
6. **Trusting one's own expect run** — same machine, warm cache; a flash humans see on cold start may not reproduce. **Countermeasure**: vary `KOSAX_*` startup env between runs, diff frame sets.
...
1. **Backend SSE chunk pacing** (`kosax.llm.client._pace_text_chunk`, `KOSAX_LLM_STREAM_PACE_MS`). K-EXAONE on FriendliAI Serverless emits SSE deltas. Splitting them server-side into 8-char sub-chunks with 80–150 ms sleep extends the *gap between paragraphs* but Ink's React reconciler folds the setStates back into a single commit at paint time. (`/tmp/tdb-md-fix/raw.cast` frame_0294, `/tmp/tdb-final/raw.cast` frame_0572.)
...
7. **`KOSAX_K_EXAONE_THINKING=false`** to force K-EXAONE's content channel instead of `reasoning_content`. Inter-paragraph cadence improved (5 s gap between two answer paragraphs in `/tmp/tdb-thinking-off/raw.cast`) but each paragraph still arrives as a single 117-/617-byte SSE chunk. **This identifies the dominant root cause**: K-EXAONE on FriendliAI Serverless emits paragraph-granularity SSE deltas, not Anthropic-style per-token deltas. No frontend or pacing layer can dilate a single SSE delta — the chunk is paragraph-shaped on the wire.

**Honest UX claim**: KOSAX gets the citizen visible streaming **between paragraphs** (5 s cadence on `enable_thinking=False`, longer on `True` because the reasoning channel buffers first); paragraph-internal token-by-token reveal **is not achievable** at the LLM-provider tier we run on. CC against Anthropic doesn't see the same fold because Anthropic's token-stream API delivers ~5-30-byte deltas at 50-100 ms intervals — comfortably outside Ink's throttle and aligned with React's commit cadence.

**Mitigations the citizen can opt into**:
- `KOSAX_LLM_STREAM_PACE_MS=80` — backend pacing on, paragraph-cadence streaming made more deliberate.
- `KOSAX_K_EXAONE_THINKING=false` — first-token latency drops to <10 s and the answer arrives on the content channel, but the agentic loop loses the model's CoT signal (multi-step tool chaining degrades).
