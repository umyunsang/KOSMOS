...
**위치**: `src/ummaya/tools/models.py` (기존 파일에 추가)
...
class AdapterRealDomainPolicy(BaseModel):
    ...
    UMMAYA는 권한을 invent 하지 않는다 (.specify/memory/constitution.md § II).
...
- **frozen=True**: 인스턴스 불변 — 향후 UMMAYA-invented 가변 enum 패턴 reintroduction 차단.
...
| 필드 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `path` | string | ✅ | `src/ummaya/permissions/...` 상대경로 |
...
| 필드 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `path` | string | ✅ | `src/ummaya/permissions/<name>.py` |
...
| `adapter_path` | string | ✅ | `src/ummaya/tools/koroad/...` |
| `agency` | string | ✅ | 예: `도로교통공단 (KOROAD)` |
| `removed_fields` | array<string> | ✅ | 제거된 UMMAYA-invented 필드 list |
...
| Entity | 위치 | 비고 |
|---|---|---|
| AdapterRealDomainPolicy | `src/ummaya/tools/models.py` (코드) + 단위 테스트 `tests/tools/test_adapter_real_domain_policy.py` | Pydantic v2 strict |
