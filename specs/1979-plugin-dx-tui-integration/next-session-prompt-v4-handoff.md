...
- 6 UMMAYA-only Tool 디렉토리 (Monitor / ReviewArtifact / SuggestBackgroundPR / Tungsten / VerifyPlanExecution / Workflow) 의 9 file deletion
- 5 callsite cleanup (cli/print, commands/insights, components/Feedback, utils/mcp/dateTimeParser, utils/sessionTitle)
- `services/api/claude.ts` modify (Spec 2077 UMMAYA-needed pure utility exports preserved; verifyApiKey/queryHaiku/queryWithModel functions removed)
...
- `src/ummaya/tools/models.py`: `AdapterRealDomainPolicy` Pydantic v2 모델 추가 (frozen=True, extra="forbid", 4 fields)
...
`auth_level` / `is_irreversible` / `requires_auth` / `dpa_reference` 토큰이 UMMAYA-needed 인프라에서 광범위 합법 사용 중:
- `src/ummaya/tools/permissions.py` — Spec 025 v6 invariant (auth_type ↔ auth_level 매트릭스)
- `src/ummaya/security/audit.py` — Tool security audit (Spec 024)
- `src/ummaya/plugins/cli_init.py` — Plugin init template (auth_level="AAL1" default)
- `src/ummaya/plugins/checks/q3_security.py` — Plugin Q3-V3/V4/V6 invariant 검사
- `src/ummaya/plugins/tests/test_namespace_invariant.py` — Plugin test fixtures

`SessionContext` (auth_level: int field) 도 UMMAYA-needed:
- `src/ummaya/recovery/auth_refresh.py` — credential resolution
- `src/ummaya/cli/{app,repl}.py` — REPL session bootstrap

→ **spec.md FR-008 의 grep gate 가 너무 광범위; spec 보정 필요 (어댑터 metadata 의 UMMAYA-invented `auth_level` 만 잡도록 정밀화)**.

### 미완 사항

- **잔재 17 source file 그대로** (`src/ummaya/permissions/` 의 aal_backstop / adapter_metadata / bypass / cli / killswitch / models / modes / pipeline / pipeline_v2 / prompt / rules / session_boot / synthesis_guard + steps/ 디렉토리)
...
1. spec.md 보정 — FR-008 grep gate 를 어댑터 metadata 영역 (`src/ummaya/tools/`) 으로 좁히고 UMMAYA-needed 영역 (`src/ummaya/security/audit.py`, `src/ummaya/plugins/checks/q3_security.py` 등) 을 명시 제외.
...
```bash
$ git worktree list
/Users/um-yunsang/UMMAYA         bc523b7 [main]
/Users/um-yunsang/UMMAYA-w-2293  10d9754 [2293-ui-residue-cleanup]
/Users/um-yunsang/UMMAYA-w-2295  553bb62 [2295-backend-permissions-cleanup]
```
...
- `auth_level` 광범위 합법 사용 발견은 Initiative #2290 전체 thesis 검토 입력. UMMAYA thesis 의 "기관 정책 cite only" 와 Spec 024/025 의 "auth_level 매트릭스 invariant" 가 양립할 수 있는 형태로 spec 보정 필요.
...
**Epic δ 재진입**:
```
Lead solo: spec.md FR-008 보정 (auth_level grep gate 를 src/ummaya/tools/ adapter metadata 영역으로만 좁힘)
            ↓
