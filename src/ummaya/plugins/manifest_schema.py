...
"""Pydantic v2 manifest schema for UMMAYA plugins.
...
existing Spec 022/031 :class:`~ummaya.tools.registry.AdapterRegistration`
...
* ``_v_pipa_hash`` — ``acknowledgment_sha256`` byte-equals the canonical
  hash from :mod:`ummaya.plugins.canonical_acknowledgment`
  (FR-014, R-1 Q6-PIPA-HASH).
* ``_v_otel_attribute`` — ``otel_attributes["ummaya.plugin.id"]``
...
from ummaya.plugins import canonical_acknowledgment
from ummaya.tools.models import AdapterRealDomainPolicy  # noqa: F401 — re-exported via __all__
...
(Q8-NO-ROOT-OVERRIDE). ``subscribe`` is deferred until UMMAYA has a real
...
class PIPATrusteeAcknowledgment(BaseModel):
    ...
    acknowledgment_sha256: str = Field(
        ...
        description=(
            ...
            "ummaya.plugins.canonical_acknowledgment.CANONICAL_ACKNOWLEDGMENT_SHA256."
        ),
    )


class PluginManifest(BaseModel):
    """Top-level Pydantic v2 manifest contract for a UMMAYA plugin release.
    ...
    otel_attributes: dict[str, str] = Field(
        description=(
            ...
            "(Spec 021 extension). MUST contain key 'ummaya.plugin.id' "
    ...
    def _v_otel_required_key(cls, value: dict[str, str]) -> dict[str, str]:
        if "ummaya.plugin.id" not in value:
            raise ValueError(
                "otel_attributes must contain key 'ummaya.plugin.id' (Spec 021 UMMAYA extension)."
            )
        return value
    ...
    def _v_otel_attribute(self) -> PluginManifest:
        """otel_attributes['ummaya.plugin.id'] must equal plugin_id (FR-021)."""
        observed = self.otel_attributes.get("ummaya.plugin.id")
        if observed != self.plugin_id:
            raise ValueError(
                f'otel_attributes["ummaya.plugin.id"] must equal plugin_id '
