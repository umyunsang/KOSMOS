...
# Boundary marker — kept identical to ``kosax.ipc.stdio._DYNAMIC_BOUNDARY_MARKER``
# and ``kosax.llm.client``'s slicing literal so all three sites agree.
    ...
    def build(self, ctx: PromptAssemblyContext) -> SystemPromptManifest:
        ...
        for name, fn in self._registry.items():
            try:
                ...
                logger.warning(
                    "kosax.prompt_assembler injector %r raised %s; skipping",
                    name,
