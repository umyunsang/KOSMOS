...
//   Output: { pdf_path: string }   ← absolute path under ~/.ummaya/memdir/user/exports/
//
...
const outputSchema = lazySchema(() =>
  z.object({
    pdf_path: z
      .string()
      .describe(
        'Absolute path of the exported file under ~/.ummaya/memdir/user/exports/. ' +
 ...
 *
 * Priority:
 *   1. UMMAYA_MEMDIR_ROOT env var (testing / integration override)
 *   2. DEFAULT_MEMDIR_ROOT (~/.ummaya/memdir) from memdir/io.ts
 */
export function exportsDir(
  root: string = process.env.UMMAYA_MEMDIR_ROOT ?? DEFAULT_MEMDIR_ROOT,
): string {
