...
**Canonical reference**: docs/requirements/kosax-migration-tree.md § L1-C C6
...
|---|---|---|---|
| TodoWriteTool | delete-in-followup | #1757 | Developer-session task-tracking mechanism with no citizen-facing utility; replaced by in-session LLM reasoning within the KOSAX citizen harness. |
...
- **Citizen relevance**: None. Citizens interact with KOSAX to query public services — they do not manage LLM-internal task lists. Progress tracking in the citizen harness is surfaced through the conversation transcript and `subscribe` handle status, not an internal todo ledger. The tool description is saturated with "coding" context that would confuse K-EXAONE about its citizen role.
...
- **Citizen relevance**: None. The KOSAX closed-13 tool surface (FR-020) eliminates the deferred-tool pattern: all LLM-visible tools ship fully specified in the initial context. ToolSearchTool only makes sense when the tool count is large enough to require lazy schema loading, which the P3 closed set explicitly avoids.
- **Decision**: delete-in-followup
- **Target Epic**: #1757
...
- **Current behavior**: Directory `tui/src/tools/MonitorTool/` does not exist in the KOSAX codebase. The tool was listed in FR-019 as undecided but was not ported from CC 2.1.88 during P0.
...
- **Current behavior**: Directory `tui/src/tools/WorkflowTool/` does not exist in the KOSAX codebase. Same situation as MonitorTool.
- **Citizen relevance**: Cannot evaluate — no code exists.
- **Decision**: delete-in-followup
- **Target Epic**: #1757
- **Rationale**: Tool directory is absent; #1757 should confirm no stray import references remain and close the item.
...
- **Citizen relevance** (shared): Legitimate Phase-2 potential — a citizen might ask KOSAX to remind them of a permit renewal deadline or send a daily traffic-hazard summary. However, the durable-cron path writes to `.claude/` (a CC-specific path that conflicts with the KOSAX `~/.kosax/memdir/` session convention) and the cron lifecycle must be scoped within the ministry-agent swarm design (UI-D). The tool description also references internal scheduler implementation details that should not be LLM-visible to citizens in this form.
- **Decision**: defer-to-P4 (all three)
- **Target Epic**: #1635
- **Rationale**: Citizen-schedulable reminders are a valid capability but belong in P4 UI-D ministry-agent context; the durable-cron path must be migrated from `.claude/` to `~/.kosax/memdir/` and the tool descriptions must be rewritten for citizen-primary Korean context before these tools can be registered.
...
- **Citizen relevance**: Not directly citizen-facing, but is the foundation of the UI-D ministry-agent swarm (where the KOSAX coordinator spawns sub-agents per ministry). Needs the swarm design to stabilize in P4 before the task schema can be adapted to KOSAX's `~/.kosax/memdir/` storage and Korean-primary UX context.
- **Decision**: defer-to-P4
- **Target Epic**: #1635
...
- **Citizen relevance**: Foundational to the UI-D ministry-agent swarm (one team per multi-ministry request), but the storage path (`~/.claude/`) conflicts with KOSAX's `~/.kosax/memdir/` convention and the tool description is entirely developer-oriented. Must be migrated and rewritten before citizen-facing use.
- **Decision**: defer-to-P4
- **Target Epic**: #1635
- **Rationale**: Ministry-agent swarm bootstrap tool; storage path and UX must be migrated to KOSAX conventions in P4 before this can be registered.
