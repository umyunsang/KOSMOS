...
Routes incoming :class:`~ummaya.ipc.frame_schema.PluginOpFrame` frames with
``op="request"`` to one of three backend handlers:

- ``request_op="install"`` → :func:`handle_install` → wraps
  :func:`ummaya.plugins.installer.install_plugin` with a progress-emitter
...
- ``request_op="uninstall"`` → :func:`handle_uninstall` → mirrors the install
  flow with the 3-phase :func:`ummaya.plugins.uninstall.uninstall_plugin`.
...
async def handle_plugin_op_request(
    ...
    *,
    ...
    Args:
        ...
        registry: :class:`ummaya.tools.registry.ToolRegistry` (typed loosely
            to avoid import-time circular dependency at the dispatcher seam).
        executor: :class:`ummaya.tools.executor.ToolExecutor`.
        write_frame: Async callable that serialises an :class:`IPCFrame` and
            writes it to stdout (existing :func:`ummaya.ipc.stdio.write_frame`).
        consent_bridge: :class:`ummaya.plugins.consent_bridge.IPCConsentBridge`
    ...
    Raises:
        ValueError: When ``frame.op`` is not ``"request"`` or ``frame.request_op``
            ...
            :func:`ummaya.ipc.stdio` ErrorFrame fanout.
    """
    if frame.op != "request":
...
async def handle_install(
    ...
    *,
    ...
    """
    from ummaya.plugins.installer import install_plugin  # noqa: PLC0415
...
async def handle_uninstall(
    ...
    *,
    ...
    from ummaya.plugins.uninstall import uninstall_plugin  # noqa: PLC0415
