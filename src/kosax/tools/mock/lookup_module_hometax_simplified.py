...
_ACTUAL_ENDPOINT: Final = "https://api.gateway.kosax.gov.kr/v1/lookup/hometax_simplified"
...
async def handle(
    ...
    *,
    ...
    discriminated-union validator (``kosax.tools.envelope.normalize``). The
...
def register(registry: object, executor: object) -> None:
    ...
    Called by ``register_all_tools()`` in ``src/kosax/tools/register_all.py``.
