...
**Context**: Spec SC-003 says the registry will contain "exactly 28 entries (12 Live + 15 Mock + 1 meta = `resolve_location`)". Phase 0 reading of `src/ummaya/tools/register_all.py` and `src/ummaya/tools/mock/__init__.py` reveals that:

1. The main `ToolRegistry` boot path (`register_all_tools()` at `src/ummaya/tools/register_all.py:48`) registers exactly **14 entries**: 12 Live agency tools + 2 MVP-surface entries (`resolve_location` + `lookup`).
2. The 6 existing verify mocks register to a separate per-primitive sub-registry at `ummaya.primitives.verify._ADAPTER_REGISTRY` via `register_verify_adapter("family", invoke)` (see `verify_mobile_id.py:61`).
3. The 2 existing submit mocks register to `ummaya.primitives.submit._ADAPTER_REGISTRY`. The 3 existing subscribe mocks register to `ummaya.primitives.subscribe._ADAPTER_REGISTRY`.
4. The mocks register only when `ummaya.tools.mock` is **explicitly imported**. The production paths (`src/ummaya/cli/app.py:230` and `src/ummaya/ipc/mcp_server.py:246`) do **not** import the mock package; only test fixtures do. Production registries today contain **zero mocks**.
...
- New `mock_verify_module_modid` mirrors the **LLM-callable secure-wrapping** module the AX gateway is expected to expose to UMMAYA — a different shape (OAuth2.1 + scope-bound DelegationToken)
...
**Reference files**: `src/ummaya/tools/mock/__init__.py:42-51` · spec FR-001/FR-005/FR-006 · `delegation-flow-design.md § 12.7` (mock catalog re-design)

---

## Decision 3 — IPC frame: NEW arm, joins union as 21st discriminator value

**Context**: Spec FR-015 requires the backend to emit an `adapter_manifest_sync` frame at boot. Spec Assumptions section says "NEW arm" preferred. Phase 0 confirmed the existing `IPCFrame` discriminated union has 20 arms (`src/ummaya/ipc/frame_schema.py:943-965`).
...
**Reference files**: `src/ummaya/ipc/frame_schema.py:943-965` · Spec 032 `frame_schema.py` · spec FR-015 + Assumptions
...
**Context**: Spec FR-021 requires the PTY smoke harness to spawn a Mock-fixture backend (NOT `UMMAYA_BACKEND_CMD=sleep 60`). Codex P1 #2395 specifically called out the `sleep 60` placeholder as the reason the PTY smoke could not catch the dispatch path.

**Decision**: Add a new module `src/ummaya/ipc/demo/mock_backend.py` that:

1. Boots the full `ToolRegistry` + per-primitive sub-registries with all mocks registered (i.e., explicitly imports `ummaya.tools.mock`)
...
The PTY smoke launches the TUI with `UMMAYA_BACKEND_CMD="python -m ummaya.ipc.demo.mock_backend"`. End-to-end: real TUI ↔ real Python process speaking real frames ↔ real registry with real mock adapters. The only "mock" element is the recorded fixture data.
...
**Context**: Today, `src/ummaya/cli/app.py:230` and `src/ummaya/ipc/mcp_server.py:246` call `register_all_tools(registry, executor)` but do not import `ummaya.tools.mock`. Production therefore has zero mocks visible to the LLM. The US1 chain (citizen → 종합소득세 신고) cannot work in production unless the mocks are registered.

**Decision**: `register_all_tools()` gains a single line:

```python
import ummaya.tools.mock  # noqa: F401 — side-effect: registers all 20 mock surfaces
```
...
- UMMAYA at student-tier has no Live agency channels for these domains, so Mock is the only available reference.
- The transparency fields make the Mock status loudly visible to operators, auditors, and the LLM itself.

**Reference files**: `src/ummaya/tools/register_all.py:107` (single insertion point) · spec § Assumptions ("the Mock backend used by the PTY smoke is a pure-Python process that imports `ummaya.tools.registry`")
...
**Decision**: Add `src/ummaya/tools/transparency.py` with a single function:
...
**Context**: Spec 035 defined the consent-receipt JSONL ledger format at `~/.ummaya/memdir/user/consent/`. FR-012/013/014 add three new event kinds (`delegation_issued`, `delegation_used`, `delegation_revoked`) to the same ledger.

**Decision**: Extend the existing `LedgerEvent` discriminated union (in `src/ummaya/memdir/consent_ledger.py`, currently containing consent-receipt event kinds) with three new Pydantic models. Each carries a `kind: Literal["delegation_issued" | "delegation_used" | "delegation_revoked"]` discriminator, a `ts: datetime` field, and event-specific payload fields. The append path is unchanged: open file in append mode, `json.dumps()` one line, fsync. No new file, no new directory.
...
| PTY smoke backend implementation? | **Real Python process** at `ummaya.ipc.demo.mock_backend` speaking JSONL frames, NOT `sleep 60` (Decision 5) |
...
| Per-primitive sub-registry for new mocks | Spec 031 prior art (`ummaya.tools.mock.__init__.py`) | Pydantic AI schema-driven registry pattern |
