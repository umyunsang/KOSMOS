...
- Path resolution: ``UMMAYA_PROMPTS_DIR`` env var → ``<repo_root>/prompts``.
...
def _locate_prompts_dir() -> Path:
    ...
    Resolution order:
    1. ``UMMAYA_PROMPTS_DIR`` environment variable (if set and non-empty).
    ...
    3. Fall back to ``<repo_root>/prompts`` where repo_root is three levels up
       from this file (``src/ummaya/tools/verify_canonical_map.py``).
    """
    env_dir = os.environ.get("UMMAYA_PROMPTS_DIR", "").strip()
    if env_dir:
        return Path(env_dir)
    ...
    # src/ummaya/tools/ → src/ummaya/ → src/ → repo_root
