...
**Storage**: User-tier memdir under `~/.ummaya/memdir/user/` (existing infrastructure from Spec 027 and Spec 035). New paths added by this epic: `~/.ummaya/memdir/user/onboarding/state.json` (resumable step state, FR-002), `~/.ummaya/memdir/user/preferences/a11y.json` (accessibility toggles, FR-005). Permission receipts continue to live in `~/.ummaya/memdir/user/consent/` (Spec 035), append-only.
