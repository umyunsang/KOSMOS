...
Source of truth for the KOSAX TUI <-> Python backend NDJSON protocol.
...
class ChatMessageFunctionCall(BaseModel):
    ...
    Mirrors ``kosax.llm.models.FunctionCall`` (the LLM-client-internal model)
...
class AssistantChunkFrame(_BaseFrame):
    ...
    ``kosax/llm/_cc_reference/claude.ts:2053-2169``). The ``delta`` field
...
class PermissionResponseFrame(_BaseFrame):
    ...
    receipt_id: str | None = Field(
        default=None,
        description=(
            "Consent receipt UUID written to ~/.kosax/memdir/user/consent/<id>.json "
...
class ErrorFrame(_BaseFrame):
    ...
    message: str = Field(
        description=(
            ...
            "MUST NOT contain KOSAX_*-prefixed env var values (FR-004 redaction rule)."
        )
    )
    details: dict[str, object] = Field(
        description="Structured error details. KOSAX_* env var values MUST be redacted."
    )
...
class NotificationPushFrame(_BaseFrame):
    ...
    KOSAX swap-2 add-on, not a CC-divergence regression).
    """
...
class ConsentRevokeRequestFrame(_BaseFrame):
    ...
    - ``receipt_id``: The ``rcpt-<id>`` value referencing the receipt file at
      ``~/.kosax/memdir/user/consent/<receipt_id>.json``.
