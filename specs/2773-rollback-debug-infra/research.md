...
- UMMAYA thesis: `docs/vision.md` defines UMMAYA as a Claude Code harness migration; tool loop, permission gauntlet, context assembly, and TUI remain the reference substrate.
- Canonical requirements: `docs/requirements/ummaya-migration-tree.md` requires the CC agent loop to be preserved and exposes only the five national-infrastructure primitives.
- CC restored source:
...
- Ink testing library exposes both `lastFrame()` and the full `frames` array; UMMAYA audits must inspect the sequence, not only the last frame: <https://github.com/vadimdemedes/ink-testing-library>
...
- MCP tool results may contain structured and unstructured content; UMMAYA should keep adapter envelopes structured and visible to the UI layer: <https://modelcontextprotocol.io/specification/draft/server/tools>
...
3. Treat nested primitive result errors as CC tool errors. A UMMAYA envelope such as `{kind:"lookup", result:{kind:"error"}}` is semantically equivalent to CC's `is_error` tool result and must render through `UserToolErrorMessage`.
