...
//     KOSAX-1633 inline stubs (CLAUDE_AI_INFERENCE_SCOPE/CLAUDE_AI_PROFILE_SCOPE/
//     OAUTH_BETA_HEADER/getOauthConfig). KOSAX uses FriendliAI; no Anthropic
//     OAuth tokens exist.
//   • swap/anti-anthropic-1p(2641) — every public entry-point throws via the
//     dead-call gate below unless KOSAX_ENABLE_DEAD_TEAM_MEM_SYNC is set
...
//     KOSAX-neutral naming where applicable; CC team-memory contract names
...
// or shell environments template booleans as `KOSAX_ENABLE_DEAD_*=0` or
...
function assertNotDeadCall(entryName: string): void {
  if (!isDeadCallOverrideExplicit(process.env.KOSAX_ENABLE_DEAD_TEAM_MEM_SYNC)) {
    throw new Error(
      `[KOSAX] services/teamMemorySync.${entryName}: dead in KOSAX — ` +
        'claude.ai team memory is not part of the L1-A K-EXAONE harness. ' +
        "Set KOSAX_ENABLE_DEAD_TEAM_MEM_SYNC='1' (literal) only when " +
