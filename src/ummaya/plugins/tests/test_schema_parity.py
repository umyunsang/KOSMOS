...
source-of-truth is :class:`ummaya.plugins.manifest_schema.PluginManifest`
...
Regeneration recipe (when the diff is intended):
    ...
    uv run python -c "
    import json, pathlib
    from ummaya.plugins import PluginManifest
    s = PluginManifest.model_json_schema()
    s['\\$schema'] = 'https://json-schema.org/draft/2020-12/schema'
    s['\\$id'] = 'https://ummaya.dev/schemas/plugin-manifest/1.0.0.json'
...
def _live_schema() -> dict[str, Any]:
    ...
    schema["$id"] = "https://ummaya.dev/schemas/plugin-manifest/1.0.0.json"
    return schema
...
class TestManifestSchemaParity:
    ...
    def test_schema_pins_canonical_id_and_dialect(self) -> None:
        ...
        assert disk.get("$id") == ("https://ummaya.dev/schemas/plugin-manifest/1.0.0.json")
