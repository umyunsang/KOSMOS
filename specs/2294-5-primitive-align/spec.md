...
| Adapter manifest IPC sync — primitive `validateInput` resolves backend adapter IDs | Spec assumption mismatch surfaced by Codex P1 review of PR #2394: TS-side `context.options.tools` holds 14 KOSMOS tools, not the 18 backend agency adapters; `validateInput` therefore returns `AdapterNotFound` for real adapter IDs | Epic ε #2296 (piggyback on new mock-adapter registration) or dedicated successor Epic | #2395 |
