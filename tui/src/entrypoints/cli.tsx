...
// KOSAX-1978 T003 boot tracer — opt-in via KOSAX_BOOT_TRACE=1.
...
const __kBoot = (label: string): void => {
  if (process.env.KOSAX_BOOT_TRACE === '1') {
    try {
      require('fs').writeSync(2, `[KOSAX-BOOT-TRACE] ${label}\n`);
    } catch {
      /* stderr torn down */
    }
  }
...
if (process.env.KOSAX_BOOT_TRACE === '1') {
  ...
  (process as unknown as { exit: typeof process.exit }).exit = ((code?: number) => {
    try {
      ...
      require('fs').writeSync(2, `[KOSAX-BOOT-TRACE] process.exit(${code}) called\n${stk}\n`);
    } catch {}
  ...
  process.on('unhandledRejection', (reason: unknown) => {
    try {
      ...
      require('fs').writeSync(2, `[KOSAX-BOOT-TRACE] unhandledRejection: ${e?.stack ?? String(reason)}\n`);
    } catch {}
  });
  // eslint-disable-next-line custom-rules/no-top-level-side-effects
  process.on('uncaughtException', (err: Error) => {
    try {
      require('fs').writeSync(2, `[KOSAX-BOOT-TRACE] uncaughtException: ${err.stack ?? err.message}\n`);
    } catch {}
  });
}
...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.VERSION} (KOSAX)`);
    return;
  }
  ...
  if (feature('BRIDGE_MODE') && (args[0] === 'remote-control' || args[0] === 'rc' || args[0] === 'remote' || args[0] === 'sync' || args[0] === 'bridge')) {
    ...
    // Bridge is a remote control feature - no policy gate in KOSAX.
