...
// UMMAYA-original — Spec 032 T053 (TUI side of Scenario B).
//
// Quickstart § 2.2 companion for `src/ummaya/ipc/demo/session_backend.py`.
//
...
//      (`uv run python -m ummaya.ipc.demo.session_backend`).
...
async function main(argv: readonly string[]): Promise<number> {
  ...
  const child = spawn(
    ...
    [
      ...
      'python',
      '-m',
      'ummaya.ipc.demo.session_backend',
      '--session-id',
      args.session,
      '--total-frames',
