...
`src/kosax/ipc/stdio.py:_handle_chat_request` consumes at line ~1099-1101 (existing) plus Step 4 fallback:
...
- `function.name` MUST match an entry in the backend's `_ensure_tool_registry()` — backend silently drops unknown entries and logs a `kosax.tool.unknown_in_frame` OTEL span event.
...
- `kosax.tools.frame.count` (int, gauge) — number of entries received in `frame.tools`.
- `kosax.tools.frame.fallback_used` (bool) — true when fallback to registry was triggered.
- `kosax.tools.frame.unknown_dropped` (int) — number of unknown entries silently dropped.
