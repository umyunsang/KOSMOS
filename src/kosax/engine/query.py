...
"""Standalone per-turn query loop for the KOSAX Query Engine.
...
from kosax.engine.preprocessing import PreprocessingPipeline
from kosax.engine.tokens import estimate_tokens
from kosax.llm.errors import BudgetExceededError, StreamInterruptedError
from kosax.llm.models import ChatMessage, FunctionCall, ToolCall, ToolDefinition
...
from kosax.tools.errors import ToolNotFoundError
...
async def query(ctx: QueryContext) -> AsyncIterator[QueryEvent]:  # noqa: C901
    ...
    span = _tracer.start_span("invoke_agent kosax-query")

    # Required attributes (contracts § Span 1)
    span.set_attribute(GEN_AI_OPERATION_NAME, "invoke_agent")
    span.set_attribute(GEN_AI_AGENT_NAME, "kosax-query")
