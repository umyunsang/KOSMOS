...
| `docs/requirements/ummaya-migration-tree.md § L1-A A6` (Error recovery: simple network retry only) | Confirms `directConnect`/`remote/` retry surface has no role in UMMAYA — DROP justified |
...
**Verdict**: `notification_push` is a UMMAYA swap-2 add-on transporting
...
it) is a UMMAYA-original architectural decision that future agents
...
tui/src/server/directConnectManager.ts (UMMAYA-stubbed; type-only deps to deleted remote/)
...
tui/src/hooks/useDirectConnect.ts (UMMAYA-stubbed; no-op hook)
```
...
Edit `src/ummaya/ipc/frame_schema.py:NotificationPushFrame.__doc__`:
...
1. Read `tui/src/ipc/codec.ts` as text (or `UMMAYA_IPC_PARITY_DRIFT_FIXTURE=1` → fixture path).
...
def test_drift_negative_fixture_triggers_failure(monkeypatch):
    monkeypatch.setenv("UMMAYA_IPC_PARITY_DRIFT_FIXTURE", "1")
    with pytest.raises(AssertionError, match="correlation_id"):
        run_codec_envelope_parity_check()
```

`conftest.py` guard ensures `UMMAYA_IPC_PARITY_DRIFT_FIXTURE` defaults
unset:
```python
@pytest.fixture(autouse=True)
def _guard_drift_fixture_env_default_off(monkeypatch):
    if os.environ.get("UMMAYA_IPC_PARITY_DRIFT_FIXTURE") == "1":
...
3. `wait_for_pane "UMMAYA" 5`.
4. `tmux send-keys -t ummaya-2642 "/help" Enter`.
5. `wait_for_pane "Available commands" 10`.
6. `tmux send-keys -t ummaya-2642 "" "C-c"` twice.
...
```bash
UMMAYA_IPC_PARITY_DRIFT_FIXTURE=1 uv run pytest \
...
| TG-B: `notification_push` SWAP doc + test | src/ummaya/ipc/frame_schema.py (edit), tests/ipc/test_notification_push_swap_parity.py (new) | Sonnet teammate |
...
- `tui/src/mcpb-compat.ts` (UMMAYA-original lazy shim)
