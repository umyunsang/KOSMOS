# Codex Goal: KOSMOS Final Packaging Readiness

## Ultimate Goal

Make KOSMOS packaging-ready as a client-side reference implementation for Korea's national AX infrastructure.

KOSMOS is not a demo app and this goal is not "fix one module." The real product target is a conversational execution surface where a citizen asks for an outcome and KOSMOS decomposes, routes, verifies, asks permission, calls the right Korean public-service tools, renders inspectable evidence, and either completes the action or gives a legally/technically honest handoff.

Canonical thesis:

- KOSMOS = Claude Code original harness + two swaps.
...
3. `docs/vision.md`
4. `docs/requirements/kosmos-migration-tree.md`
5. `.agents/skills/kosmos-reference-first/SKILL.md`
...
- Skill file: `.agents/skills/kosmos-reference-first/SKILL.md`

This skill is mandatory for:

- Finding CC restored-source reference paths.
- Finding KOSMOS canonical docs and active spec artifacts.
...
- Source path or URL.
- Source authority level: CC source, KOSMOS canonical doc, local adapter doc, official primary source, upstream open source, community secondary source.
- Decision supported by that source.
- Unknowns or blocked evidence.

## Hard Rules

- Use `kosmos-reference-first` before planning or editing runtime behavior.
- Use `kosmos-reference-first` before research-material discovery for KOSMOS runtime, adapter, TUI, tool-loop, workflow, or verification decisions.
...
- Privileged actions must be evidence-graded mock unless KOSMOS has official credential, legal authority, and documented permission policy.
- Permission classification must cite agency policy; KOSMOS must not invent policy classes.
...
- Use env vars prefixed `KOSMOS_`; never commit `.env`, `secrets/`, or local Codex config.
...
Use the LLMOps rendering verification infrastructure researched for KOSMOS. Do not perform rendering checks as isolated screenshot review. Every real-use scenario must be evaluated as a correlated execution trace from model reasoning to final terminal pixels.
...
- LLM streaming chunks, including `kosmos.llm.chunk` timing when available.
...
- `kosmos.tui.frame_commit` render events and frame hashes when available.
...
1. Research: invoke `$kosmos-reference-first`, then read the relevant KOSMOS docs, CC restored source, local adapter docs, official API docs, and only then secondary open-source/community references for implementation insight. For OpenAI/Codex behavior, also invoke `openai-docs`.
...
Build and run broad real-use scenarios from KOSMOS' ultimate goal. Include both happy paths and failure/recovery paths.
...
- KOSMOS still implements the CC harness migration thesis with only the sanctioned swaps.
...
Start by invoking `$kosmos-reference-first` and reading the source-of-truth hierarchy above. Then write the research-source ledger and reference bootstrap note before editing anything. Reproduce the highest-risk current flows with real-use TUI/PTY captures, starting with permission flow and public-service adapter parameter recovery. Continue the RALF loop until KOSMOS is packaging-ready by the acceptance criteria, or stop only when a blocker is proven with exact file references, command output, and evidence.
