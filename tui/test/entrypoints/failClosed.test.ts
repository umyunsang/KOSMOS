...
// T020 — TUI fail-closed boot: refuses to run without FRIENDLI_API_KEY / KOSAX_FRIENDLI_TOKEN
...
// The KOSAX-rewritten init.ts should export `checkRequiredEnv()` or similar.
...
//
// We try to dynamically detect whether a KOSAX-specific env-check export
...
> {
  // T011 is expected to export `checkKosaxEnv` or `assertFriendliKey` from
...
describe('TUI fail-closed boot without FRIENDLI_API_KEY (T020, § 5)', () => {
  ...
  //
  // Gated behind KOSAX_T020_SPAWN_TEST=1 because spawning main.tsx is
  // heavyweight and requires the TUI to have the fail-closed check wired
  // (T011). Enable in CI once T011 lands.
  // -------------------------------------------------------------------------

  const spawnTestEnabled = process.env.KOSAX_T020_SPAWN_TEST === '1'
