...
const repoRoot = path.resolve('/Users/um-yunsang/KOSAX')
...
const kosaxReferenceSkillPath = path.join(repoRoot, '.agents/skills/kosax-reference-first/SKILL.md')

const fullPrompt = fs.readFileSync(promptPath, 'utf8')
const objective = [
  'Make KOSAX packaging-ready as the client-side reference implementation for Korea national AX infrastructure.',
  ...
  'Use the KOSAX LLMOps rendering-flow infrastructure: join LLM chunks, IPC frames, tool dispatch, permission events, render commits, PTY/frame artifacts, expanded logs, and scenario audits by trace/correlation ids.',
  'Use the kosax-reference-first skill for research material discovery before implementation decisions.',
...
const initializeId = request('initialize', {
  clientInfo: {
    name: 'kosax_goal_runner',
    title: 'KOSAX Goal Runner',
    version: '0.1.0',
  },
  capabilities: {
...
rl.on('line', (line) => {
  ...
  if (msg.result?.thread?.id && !threadId) {
    ...
    request('thread/name/set', {
      threadId,
      name: 'KOSAX final packaging readiness loop',
    })
