...
// ═══════════════════════════════════════════════════════════════════════
// KOSAX version is sourced from tui/package.json — the single source of
...
// bump the user-visible version. The previous "2.1.88-kosax" hardcode was
// a residue of the CC 2.1.88 source-map import; KOSAX is a separate
// project with its own release cadence (github.com/umyunsang/KOSAX).
//
// BUILD_TIME is injected from the env var KOSAX_BUILD_TIME at runtime
...
;(globalThis as any).MACRO = {
  VERSION: pkg.version,
  VERSION_CHANGELOG: 'https://github.com/umyunsang/KOSAX/releases',
  BUILD_TIME: process.env.KOSAX_BUILD_TIME ?? new Date(0).toISOString(),
  FEEDBACK_CHANNEL: 'https://github.com/umyunsang/KOSAX/issues',
  ISSUES_EXPLAINER:
    'Please open a GitHub issue at https://github.com/umyunsang/KOSAX/issues',
  PACKAGE_URL: 'https://github.com/umyunsang/KOSAX',
  NATIVE_PACKAGE_URL: 'https://github.com/umyunsang/KOSAX',
}
...
// ═══════════════════════════════════════════════════════════════════════
// Debug hooks — OFF by default. Enable with `KOSAX_DEBUG_PRELOAD=1`.
// ═══════════════════════════════════════════════════════════════════════
if (process.env.KOSAX_DEBUG_PRELOAD === '1') {
  process.stderr.write(
    `[KOSAX/PRELOAD] loaded, pid=${process.pid}\n` +
      `[KOSAX/TTY] stdin=${process.stdin.isTTY} stdout=${process.stdout.isTTY} stderr=${process.stderr.isTTY}\n`,
  )
  process.on('unhandledRejection', (reason: unknown) => {
    try {
      require('fs').writeSync(
        2,
        `[KOSAX/DEBUG] unhandledRejection: ${
  ...
  process.on('uncaughtException', (err: Error) => {
    try {
      require('fs').writeSync(
        2,
        `[KOSAX/DEBUG] uncaughtException: ${err.stack || err.message}\n`,
      )
    } catch {
      /* stderr torn down */
    }
  ...
  process.on('beforeExit', (code: number) => {
    try {
      require('fs').writeSync(2, `[KOSAX/DEBUG] beforeExit(${code})\n`)
    } catch {
      /* stderr torn down */
    }
  })
  process.on('exit', (code: number) => {
    try {
      require('fs').writeSync(
        2,
        `[KOSAX/DEBUG] exit(${code}) exitCode=${process.exitCode}\n`,
      )
    } catch {
      /* stderr torn down */
    }
  ...
  // dies (boot crashes were silent — KOSAX_FORCE_INTERACTIVE=1 prevents
  ...
  if (!(process as any)._origExit) {
    ...
    process.exit = ((code?: number) => {
      try {
        ...
        require('fs').writeSync(
          2,
          `[KOSAX/DEBUG] process.exit(${code}) called\n${stack}\n`,
        )
      } catch {
        /* stderr torn down */
      }
