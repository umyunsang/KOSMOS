...
There are no open `NEEDS CLARIFICATION` markers in spec.md. The migration tree (`docs/requirements/kosax-migration-tree.md § UI L2`) is the canonical source for every UI choice and was approved 2026-04-24.
...
| Korean primary + English fallback (FR-004) | `tui/src/i18n/{ko,en}.ts` (existing KOSAX infrastructure) | i18n table already in place; this epic only adds onboarding + a11y keys. | New i18n framework — rejected: existing module satisfies the FR. |
...
| Config overlay + .env isolated editor (FR-030) | `.references/claude-code-sourcemap/restored-src/src/components/InvalidConfigDialog.tsx` (overlay primitive) | CC overlay primitive supports inline edit; .env isolation is a KOSAX-original safeguard. | Edit .env inline — rejected: secret leakage risk in scrollback. |
...
| History 3-filter search (FR-033) | `.references/claude-code-sourcemap/restored-src/src/components/HistorySearchDialog.tsx` | Direct port, KOSAX adds Layer filter. | New search UI — rejected: CC fidelity. |
...
| `kosax.ui.surface` OTEL attribute (FR-037) | Spec 021 GenAI extension + Spec 028 collector pipeline | Adding a span attribute is a Spec 021 extension point; collector ingests by default (FR-038 friendly). | New collector route — rejected: would violate FR-038 (zero new egress). |
...
None at Phase 0 close. All design decisions trace to either (a) a `.references/claude-code-sourcemap/restored-src/` construct, (b) a `docs/wireframes/*.mjs` visual contract, or (c) an already-shipped KOSAX spec (Spec 021/027/028/032/033/034/035). Spec.md ships zero `NEEDS CLARIFICATION` markers and the migration tree resolves every category-level question.
...
- **D-3**: REPL streaming chunk size is fixed at ≈ 20 tokens, configurable via env var `KOSAX_TUI_STREAM_CHUNK_TOKENS` (default 20) for ops latitude — the spec FR (FR-008) reads "approximately" and the env var stays within "approximately" semantics.
...
- **D-5**: A11y toggles persist to a new memdir path `~/.kosax/memdir/user/preferences/a11y.json`. This is a Spec 035 follow-up entry in deferred-items §VI but the path itself is owned by this epic (writes only, no schema change to Spec 035 storage).
...
- **D-7**: `kosax.ui.surface` OTEL attribute is emitted from a new helper `tui/src/observability/surface.ts` that wraps the existing Spec 021 emit path. No new collector route; the attribute rides existing spans.
