...
// Save and restore UMMAYA_TUI_LOCALE across tests
const SAVED_LOCALE = process.env['UMMAYA_TUI_LOCALE']
...
afterEach(() => {
  if (SAVED_LOCALE !== undefined) {
    process.env['UMMAYA_TUI_LOCALE'] = SAVED_LOCALE
  } else {
    delete process.env['UMMAYA_TUI_LOCALE']
  }
})
...
describe('parseLangCommand — valid locales', () => {
  it('switches to Korean (ko)', () => {
    ...
    expect(process.env['UMMAYA_TUI_LOCALE']).toBe('ko')
  })
...
describe('getCurrentLocale', () => {
  it('returns ko by default when UMMAYA_TUI_LOCALE is unset', () => {
    delete process.env['UMMAYA_TUI_LOCALE']
    expect(getCurrentLocale()).toBe('ko')
  })

  it('returns en when UMMAYA_TUI_LOCALE=en', () => {
    process.env['UMMAYA_TUI_LOCALE'] = 'en'
    expect(getCurrentLocale()).toBe('en')
  })

  it('returns ko as fallback for unrecognised locale values', () => {
    process.env['UMMAYA_TUI_LOCALE'] = 'ja'
    expect(getCurrentLocale()).toBe('ko')
  })
})
