...
**Date**: 2026-04-29
**기준점**: UMMAYA = CC 원본 + 2 swaps. CC 소스맵은 `.references/claude-code-sourcemap/restored-src/` (research-only, 수정 금지). 모든 개발은 CC 원본을 base로 UMMAYA 트리(`tui/src/`, `src/ummaya/`)에서 진행.

---

## 0. 원칙 (한 문단)

CC 원본을 토대로 한다는 것은: **CC가 이미 검증한 모든 인프라 (REPL, ToolRegistry, PermissionRequest, agentic loop, slash commands, message stream, IPC, theme, keybindings)는 byte-identical로 보존**, 두 가지만 바꾼다 — (a) `services/api/` 의 Anthropic 클라이언트를 UMMAYA Python 백엔드로 가는 IPC 브릿지로 (이미 Spec 1633에서 진행 중), (b) `tools/` 디렉토리의 개발자 중심 도구(Bash/Read/Write/Edit/Grep/Glob/Notebook/REPL/PowerShell/LSP/TodoWrite)를 UMMAYA 5-primitive 시민 중심 surface(`lookup/submit/verify/subscribe` + `resolve_location`)로. 그 외 모든 CC tool (AskUserQuestion, Brief, Agent, MCP, Task lifecycle, Team, Skill, Schedule, ToolSearch, EnterPlanMode 등 30개+)은 CC 원본 그대로 보존.

---

## 1. 현재 상태 정확 매핑

CC `restored-src/src/tools/` 43개 vs UMMAYA `tui/src/tools/` 57개를 diff한 결과 (2026-04-29):

### 1.1 Class A — CC 원본 byte-identical 또는 거의 동일 (43개, 보존 대상)

CC와 UMMAYA 양쪽에 모두 존재하는 tool. 대부분 byte-identical (단순 SDK import 경로 차이만 존재).

| Tool | CC role | UMMAYA 처리 |
|---|---|---|
...
| `ConfigTool` | 설정 변경 | **보존** — UMMAYA_* env 노출 |
| `EnterPlanModeTool` / `ExitPlanModeTool` | plan mode 토글 | **보존** — UMMAYA도 plan mode 지원 |
...
| `ListMcpResourcesTool` / `MCPTool` / `McpAuthTool` / `ReadMcpResourceTool` | MCP 통합 | **보존** — UMMAYA 플러그인 시스템 (Spec 1636) base |
...
| `SkillTool` | skill 호출 | **보존** — UMMAYA 스킬 시스템 |
| `SleepTool` | wait/delay | **보존** — workflow utility |
| `SyntheticOutputTool` | 합성 출력 | **보존** — 테스트/시뮬레이션 |
| `TaskCreateTool` / `TaskGetTool` / `TaskListTool` / `TaskOutputTool` / `TaskStopTool` / `TaskUpdateTool` | task 라이프사이클 | **보존** — UMMAYA 다중 작업 추적 |
...
| `CalculatorTool` | 산술 계산 | **유지** — 시민 보조 도구 (ummaya-migration-tree § C.6 MVP 7) |
| `DateParserTool` | 날짜 파싱 | **유지** — § C.6 MVP 7 |
| `ExportPDFTool` | 대화 → PDF | **유지** — § E.4 |
| `TranslateTool` | 한국어↔English | **유지** — § C.6 MVP 7 |
| `MonitorTool` | 외부 시스템 모니터 | **삭제 후보** — UMMAYA-original, 시민 use case 불명확 |
...
**UMMAYA**: 이 디렉토리 전체를 **IPC 브릿지로 교체**. 이미 Spec 1633에서 진행 중:
- CC `services/api/claude.ts` (3419 LOC) → UMMAYA `tui/src/ipc/llmClient.ts` (stdio bridge)
- CC `verifyApiKey()`, `queryHaiku()`, `queryWithModel()` → UMMAYA Python 백엔드로 IPC 호출
- 4 callsite 잔재 정리 필요 (memory `project_tui_anthropic_residue`)

**작업**: Spec 1633 dead-code elimination 마무리 + 연결 포인트만 UMMAYA IPC.
...
**UMMAYA Spec 1979 감사 결과**: 모든 파일 byte-identical (SDK import 2줄 차이만 존재).
...
**작업 2**: UMMAYA 5-primitive 권한 UI는 **CC `FallbackPermissionRequest`를 base로**:
...
**UMMAYA**: 보존. Spec 1633의 services/api/ 교체만 적용되면 자동으로 UMMAYA K-EXAONE으로 라우팅.

### 2.4 `screens/REPL.tsx` (보존, 마운트 포인트만 정리)

**CC 원본**: 거대한 REPL 컴포넌트. CC와 UMMAYA 모두 ~5500 LOC.

**현재 상태**: Spec 1979에서 UMMAYA-original 마운트 포인트(`ummayaPendingConsent`, `ummayaShowBypassConfirm`, `UmmayaActivePermissionGate`) 모두 제거됨. CC 원본의 마운트 포인트만 활성.
...
**작업**: CC slash commands는 보존. UMMAYA 추가 commands는 별도 파일로 등록.

### 2.6 `permissions/` (UMMAYA-original, 이미 Spec 1979에서 삭제됨)

Spec 033 5-mode spectrum, PIPA §15(2) ConsentDecision, AAL hint, dontAsk — 전부 삭제 완료.

**작업**: 백엔드 Python `src/ummaya/permissions/` (25개 파일)도 같은 운명 — 후속 epic에서 정리.
...
**목표**: UMMAYA의 모든 CC-ported 파일을 CC 원본과 byte-identical 상태로 정렬. 차이는 SDK import 경로 변환만.
...
| α-3 | `tui/src/screens/REPL.tsx` vs CC. UMMAYA-original 마운트 포인트 잔재 확인 | grep `ummaya*` → 정당한 UMMAYA 추가만 남음 |
...
**목표**: UMMAYA `LookupPrimitive`, `SubmitPrimitive`, `VerifyPrimitive`, `SubscribePrimitive` 4개를 **CC의 `Tool.ts` 인터페이스 정확히 따르도록** 재구현.
...
export const LookupPrimitive: Tool = {
  ...
  call: async function* (input, context) {
    // route to src/ummaya/tools/<domain>/<adapter>.py via IPC
...
### Phase δ — 백엔드 Python `src/ummaya/permissions/` 정리 (1 sprint)

**목표**: 프론트엔드에서 삭제한 UMMAYA-invented permission 시스템의 **백엔드 미러 25개 파일**도 같은 운명.

| Task | 작업 |
|---|---|
| δ-1 | `src/ummaya/permissions/modes.py` (5-mode PermissionMode), `models.py` (ConsentDecision, ToolPermissionContext, PermissionRule), `pipeline_v2.py`, `pipeline.py`, `prompt.py`, `rules.py`, `bypass.py`, `mode_bypass.py`, `mode_default.py`, `synthesis_guard.py`, `aal_backstop.py`, `killswitch.py` 등 UMMAYA-invented 파일 삭제 |
...
| δ-5 | `ledger.py`, `action_digest.py`, `hmac_key.py`, `canonical_json.py`, `audit_coupling.py` — 영수증 시스템은 보존 검토 (Spec 035 시민 영수증 ledger는 유효한 UMMAYA 추가) |
...
### 의미 2: UMMAYA-original 발명물 삭제 (이미 Spec 1979에서 완료)

`permissions/` 디렉토리 전체 (25 UMMAYA-only 파일) — Spec 1979 Wave 1-3에서 삭제됨.
...
CC가 정의한 `Tool` 인터페이스 (name/description/inputSchema/call/render*) **그대로 따름**. 5-primitive는 그 인터페이스의 인스턴스일 뿐이며, 호출 시 백엔드 `src/ummaya/tools/<domain>/<adapter>.py` 로 IPC 라우팅.
...
권한 UI는 이 메타를 읽어 **CC `<FallbackPermissionRequest>`** 또는 **CC `<PermissionPrompt>`** 에 그대로 dispatch. UMMAYA는 새 권한 컴포넌트를 만들지 않음.

---

## 5. 5분 ramp-up — 신규 학부생 기여자가 즉시 따라할 수 있는 절차

> "UMMAYA 어떻게 시작해요?" 라는 질문에 대한 정답.

1. **CC 원본 읽기** — `.references/claude-code-sourcemap/restored-src/` 클론본을 읽는다. 이게 토대.
2. **`tui/src/`와 비교** — 동일 경로의 UMMAYA 파일을 읽는다. 다르면 `diff` — 차이점이 SDK import 1-2줄이면 정상, 그 외 차이는 UMMAYA의 의도적 변경 (보존 가치 검증 필요).
...
5. **한국 정부 어댑터 추가 시 확인** — `src/ummaya/tools/koroad/koroad_accident_search.py`를 reference로 복사. `real_domain_policy` 메타에 citation URL 필수.
...
- **canonical references**:
  ...
  - `docs/requirements/ummaya-migration-tree.md` (L1 pillars + UI L2 + P0-P6)
