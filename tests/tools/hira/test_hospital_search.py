...
from kosax.tools.models import LookupCollection, LookupError, LookupFetchInput  # noqa: A004
...
class TestHiraHospitalSearchHappy:
    ...
    async def test_handle_returns_collection_dict(
        ...
        monkeypatch.setenv("KOSAX_DATA_GO_KR_API_KEY", "test-key-hira")
    ...
    async def test_lookup_fetch_returns_lookup_collection(
        ...
        with (
            ...
            mp.setenv("KOSAX_DATA_GO_KR_API_KEY", "test-key-hira")
...
class TestHiraHospitalSearchDistanceSort:
    """D-fix: HIRA does not sort server-side; KOSAX sorts by distance ASC.
