...
- [X] T003 #2484 [P] Create new module `src/kosax/tools/verify_canonical_map.py` per `data-model.md § 2`: exports `resolve_family(tool_id) -> str | None` and `get_canonical_map() -> Mapping[str, str]`; lazy `lru_cache`d loader parses `<verify_families>` markdown block from `prompts/system_v1.md` (path resolved via `KOSAX_PROMPTS_DIR` env, fallback to `<repo_root>/prompts`); raises `RuntimeError` on <10 entries (FR-008b). Acceptance: `python -c "from kosax.tools.verify_canonical_map import get_canonical_map; print(len(get_canonical_map()))"` prints `10`.
...
- [X] T005 #2486 Extend `src/kosax/tools/mvp_surface.py:243` `_VerifyInputForLLM` per `contracts/verify-input-shape.md` I-V1 through I-V8: add `tool_id: str | None` and `params: dict[str, object] | None` LLM-visible fields; preserve legacy `family_hint`/`session_context`; add `@model_validator(mode="before")` that translates `tool_id`→`family_hint` via `verify_canonical_map.resolve_family(...)` and packs `params`→`session_context`; raises `ValueError("unknown verify tool_id: <value>")` on unknown tool_id (FR-008a/8b/10). Acceptance: `uv run mypy src/kosax/tools/mvp_surface.py` PASS + `uv run ruff check src/kosax/tools/` PASS.
...
- [X] T008 #2489 [P] [US1] Create `tui/src/tools/_shared/dispatchPrimitive.ts` + `dispatchPrimitive.test.ts` per `data-model.md § 4` + `contracts/tui-primitive-dispatcher.md` I-D2/I-D3/I-D6/I-D7/I-D10: shared helper signature `dispatchPrimitive<O>(opts) -> Promise<ToolResult<O>>`; mints UUIDv7 callId; constructs ToolCallFrame; registers pending call with FR-006 timeout (default 30s, `KOSAX_TUI_PRIMITIVE_TIMEOUT_MS` override); sends via bridge; awaits resolution; OTEL `kosax.tui.primitive.timeout` attribute on timeout; passthrough error envelope. bun unit test covers ≥5 scenarios from I-D10. Acceptance: `bun test src/tools/_shared/dispatchPrimitive.test.ts` PASS.
- [X] T009 #2490 [P] [US1] Replace stub `call()` body in `tui/src/tools/LookupPrimitive/LookupPrimitive.ts:319-330` per `contracts/tui-primitive-dispatcher.md` I-D1: invoke `dispatchPrimitive({primitive: 'lookup', args: input, context, ...})`; return resolved result. Preserve existing `validateInput` adapter resolution + `kosaxCitations` context attachment (no regression to lookup-only path per US1 AC4 / FR-001). Acceptance: `bun typecheck` PASS + `bun test` no regression vs `main`.
...
- [X] T017 #2498 [US1] Create `tests/integration/test_tui_primitive_dispatch_e2e.py` per FR-016 + spec.md US1 AC1/AC2/AC3: ≤80 LOC pytest fixture spawning `bun run tui` subprocess, sending citizen prompt via stdin pipe, capturing stdout, asserting ≥3 `tool_call` markers + ≥3 `tool_result` markers + receipt-id regex match + 3 same-`delegation_token` ledger lines under `~/.kosax/memdir/user/consent/<YYYY-MM-DD>.jsonl`. Acceptance: `uv run pytest tests/integration/test_tui_primitive_dispatch_e2e.py -v` PASS on a working tree where T003-T014 are merged.
- [X] T018 #2499 [US1] **Lead-only task** (PARTIAL — receipt rendering blocked by #2516). Ran full smoke chain (`expect` + `vhs`) on 2026-04-30 with all of T003-T017 merged. Captured + committed all 6 artefacts (`smoke-citizen-taxreturn-pty.txt` + `smoke-citizen-taxreturn.gif` + 3 keyframe PNGs). **Lead Opus Read-tool visual verification of `smoke-keyframe-3-receipt.png`**: image shows KOSAX branding rendered, citizen prompt `종합소득세 신고해줘` accepted in input box, but **NO assistant response / NO receipt id** — consistent with PTY log showing 0 `tool_call` greps. Root cause: K-EXAONE on FriendliAI Tier 1 does NOT emit the chain despite `prompts/system_v1.md` v2 teaching it; this is an LLM-fidelity gap requiring `prompts/**` edits which FR-022 forbade. **Wiring + scaffolding work IS complete** (Phase 0a + 0b + 1a). LLM-side gap tracked as new sub-issue #2516; spec.md SC-001/SC-002 marked `[BLOCKED → #2516]`.
