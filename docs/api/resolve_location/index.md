...
- **Error envelope examples**:
  ...
  - JUSO key not configured: `resolve_location` silently skips JUSO (`UMMAYA_JUSO_CONFM_KEY` not set → `_juso_adm_cd` returns `None`) and falls through to SGIS; if all backends miss, surfaces `not_found`.
