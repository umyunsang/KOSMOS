...
export default async function run(h: Harness): Promise<void> {
  ...
  await h.waitForPane(/resolve_location|nmc_emergency_search|Invalid parameters|검색 오류|응급실|도구 결과/, 180)
  h.snapshot('post-tool-flow')

  h.sendKey('C-o')
  await h.waitForPane(/outbound_traces|request_url|response_status|status_code|Ctrl\+O|Error|검색 오류|응급실/, 30)
