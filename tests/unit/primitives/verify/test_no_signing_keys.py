...
FR-009 (harness-not-reimplementation): UMMAYA is a delegation-only harness.
...
_VERIFY_MODULE = _SRC_ROOT / "ummaya" / "primitives" / "verify.py"
...
def test_no_forbidden_pattern(verify_source: str, pattern: str, label: str) -> None:
    ...
    assert not matches, (
        ...
        f"UMMAYA is a harness — no CA/HSM/signing code is allowed here."
    )
