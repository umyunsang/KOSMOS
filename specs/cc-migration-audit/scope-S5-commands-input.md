...
- CC 파일수: **234** (commands 189 + hooks 104 + keybindings 14 + skills 20 + vim 5 + commands.ts 1) / 총 LOC: **55,124**
  ...
  - vim: 5 파일 / 1,513 LOC
- KOSAX 파일수 (대응 영역, TS): **364** (commands 223 + hooks 106 + keybindings 26 + skills 49 + vim 5 + commands.ts 1) / 총 LOC: **57,267**
...
| Bucket | 파일 수 | 핵심 |
| --- | --- | --- |
| **PORT** (CC 있음, KOSAX 없음) | **1** | `hooks/useTeleportResume.tsx` 만 누락 — claude.ai cloud teleport feature, KOSAX 무관. 정당 삭제. |
...
| **MIGRATE-FOR-SWAP** (둘 다, 발산) | **40** | commands 20 + commands.ts 1 + hooks 16 + keybindings 8 + skills 3 + vim 0. 모두 swap-1 (Anthropic SDK→KOSAX IPC) / swap-2 (도구 시스템) / Spec 288 키바인딩 확장 / Spec 1635 i18n 정당화. |
| **DROP-CANDIDATE** (KOSAX-only) | **130** | commands 46 + hooks 3 + keybindings 12 + skills 29 + vim 0. 분류: ① 한국 행정 신설 명령 (스펙 인용 명시) ② Spec 288 키바인딩 확장 (스펙 인용 명시) ③ "P0 reconstructed" 자동-스텁 (CC sourcemap gap, Epic #1633/#1979 추적) ④ KOSAX-original 후크 (Korean IME · reduced motion · IPC permission). 정당화 명시 안 된 항목 없음. |
...
### Commands.ts root (1)
- `commands.ts` (CC 754 / KOSAX 757 LOC) — **swap-1 정당화**: feedback / passes / login / logout / insights import 5건 제거 + 인라인 코멘트 (`// feedback command removed — claude.ai 1P telemetry submission, Anthropic-only (Spec 1633 / Epic #2293)`, `// passes command removed — claude.ai guest passes SaaS dead in KOSAX`, `// Anthropic claude.ai subscription check removed (KOSAX uses FriendliAI)`, `// Anthropic console API key check removed (KOSAX uses FriendliAI)`). 5번에 걸친 명시 citation. **회귀 후보 0.**
...
### Hooks 발산 (16)
- **swap-1 종속 (12)**: `notifs/useMcpConnectivityStatus.tsx`, `notifs/useNpmDeprecationNotification.tsx`, `notifs/useRateLimitWarningNotification.tsx`, `useApiKeyVerification.ts` (Anthropic API key → KOSAX FriendliAI 검증), `useAssistantHistory.ts`, `useCanUseTool.tsx` (KOSAX session-store 어댑터), `useDirectConnect.ts`, `usePromptsFromClaudeInChrome.tsx`, `useRemoteSession.ts`, `useSSHSession.ts`, `useTerminalSize.ts`, `useVirtualScroll.ts`.
...
## DROP-CANDIDATE (KOSAX-only) — 130개

### Commands KOSAX-only (46)
...
- `help.ts`, `help.tsx` (Spec 1635 / FR-029 / T061 — KOSAX-original 4-그룹 도움말)
...
- `plugin-init.ts` (Spec 1636 P5 — `kosax plugin init <name>`)
- `plugin.tsx`, `plugins.ts` (Spec 1635 / FR-031 / T066 + Spec 1979 T023)
- `new.ts`, `save.ts`, `sessions.ts`, `resume.ts` (KOSAX-original session commands → session_event IPC frame)
- `dispatcher.ts`, `index.ts`, `types.ts` (KOSAX-original command registry — CC 패턴 lift)
...
#### C. 미분류 (1)
- `install-github-app/types.ts` — CC 에는 없는 KOSAX 측 타입 분리. 회귀 위험 낮음, 다음 라운드 review.

### Hooks KOSAX-only (3)
- `useCanUseTool.ts` (.ts 변종, KOSAX session-store 어댑터, swap-2 종속, 명시 citation).
- `useKoreanIME.ts` (Spec 287, KOSAX-original Korean Hangul IME, swap-2 종속, 정당화 자명).
- `useReducedMotion.ts` (Spec 035 / Phase 0 R-8, NO_COLOR · KOSAX_REDUCED_MOTION 게이트).

### Keybindings KOSAX-only (12, 모두 Spec 288 명시 citation)
...
### Skills KOSAX-only (29, 전부 `claude-api/*` + `verify/*` SKILL.md.ts 번들)
...
→ CC 에는 없는 멀티-언어 SDK 문서 번들. KOSAX 가 추가한 자료. **회귀 후보 아니지만 KOSAX scope 정당화 의문** — claude-api SDK 문서가 KOSAX 번들 자산에 포함된 이유 사용자 결정 필요 (아래 § 사용자 결정 참고).

### Vim KOSAX-only (0)
없음.

## PORT (CC 있음, KOSAX 없음) — 1개

| CC 경로 | 정당 삭제 사유 |
| --- | --- |
| `hooks/useTeleportResume.tsx` | claude.ai cloud teleport 세션 재개 — KOSAX 무관 (KOSAX 는 로컬 sessions/ JSONL). KOSAX src 전체 grep 0 hit. **정당 삭제, 복원 불필요.** |

## swap 종속 명령 화이트리스트 (CC → KOSAX 정당 발산 enumerate)

### claude.ai 1P 정당 제거 (CC 9 파일 → KOSAX 0 파일)
...
### CC sourcemap gap 추적 (CC 2 파일 → KOSAX 0 파일)
- `commands/extra-usage/extra-usage-core.ts` — KOSAX 측에 누락. caller `extra-usage.tsx` 에 KOSAX 측 로직 인라인됐을 가능성.
- `commands/rename/generateSessionName.ts` — KOSAX 측에 누락. `rename.ts` 가 KOSAX-original 분기일 가능성.
- `commands/review/reviewRemote.ts` — KOSAX 측에 누락. swap-1 종속 (claude.ai remote review API).
→ 다음 라운드 audit 에서 caller-graph 검증 권고.

### 한국 행정 신설 명령 (KOSAX-only 신설 14, 전부 명시 citation)
| 명령 | Spec | FR/Task |
| --- | --- | --- |
...
| `/save · /sessions · /resume · /new` | KOSAX-original | session_event IPC frame |
...
2. **claude.ai 1P 명령 제거 (5종 9 파일) 모두 commands.ts 인라인 코멘트로 박제** — feedback / login / logout / passes / insights. swap-1 정당 발산 명시 citation 의 모범 사례. KOSAX 가 발산을 박제하는 패턴이 정착되었다는 신호.
...
5. **Skills KOSAX-only 29 파일 (claude-api/* SKILL doc bundle + verify/SKILL.md.ts) 정당화 의문** — 멀티-언어 (csharp/go/java/php/python/ruby/typescript) Anthropic API SDK 문서가 KOSAX skills/bundled 에 포함되어 있음. KOSAX 가 K-EXAONE on FriendliAI 하나만 사용하는데 Anthropic SDK 문서를 번들해야 하는 이유 불명확. 사용자 결정 필요.

## 사용자 결정 필요 사항

1. **Skills `claude-api/` 멀티-언어 SDK 문서 번들 (29 파일, 약 1,600 LOC) 처리**: KOSAX 가 Anthropic SDK 를 사용하지 않는데 번들 자산으로 7개 언어의 Anthropic API 문서를 가지고 있다. (a) 전부 제거, (b) "claude-api" → "kexaone-api" 로 swap-2 마이그레이션, (c) 보존 (bundled skills 시스템의 fixture 데이터로만 활용) 중 결정.
2. **CC sourcemap gap 추적 3 파일 (`extra-usage-core.ts` / `rename/generateSessionName.ts` / `review/reviewRemote.ts`) 의 KOSAX 측 caller-graph 재검증** 다음 audit 라운드에 포함할지 여부.
