...
- Python (new adapter only): `pytest` + `pytest-asyncio` for `kosax-backend --ipc stdio` round-trip.
...
- KOSAX-original TypeScript: ≤ 40 files (5-primitive renderers, IPC bridge, env config, entrypoint).
- Python additions: 1 entrypoint module (`src/kosax/ipc/stdio.py`) + 1 CLI flag (`--ipc stdio`) on existing `kosax-backend` command.
...
├── src/
│   ├── main.tsx                        # Entrypoint — spawns kosax-backend --ipc stdio
│   ├── ipc/
...
│   │   └── crash-detector.ts           # ≤5 s crash detection + KOSAX_* redaction (FR-004)
...
│   │   └── provider.tsx                # KOSAX_TUI_THEME env var reader (FR-041)
│   ├── components/
│   │   ├── primitive/                  # KOSAX-original renderers (Spec 031 envelopes)
...
src/kosax/cli/
└── __main__.py                         # Add --ipc stdio flag routing to kosax.ipc.stdio
...
src/kosax/config/env_registry.py       # Register KOSAX_TUI_THEME, KOSAX_TUI_LOG_LEVEL,
                                        # KOSAX_TUI_SUBSCRIBE_TIMEOUT_S (FR-041, #468)
```

**Structure Decision**: Dual-language workspace. `tui/` is the **only** TypeScript directory — AGENTS.md hard rule permits TypeScript exclusively for the TUI layer. Python backend keeps its existing `src/kosax/` layout; a single new `src/kosax/ipc/` subpackage adds the stdio multiplexer without touching any existing module. TUI tests live inside `tui/tests/`; Python IPC tests live in the existing `tests/` root. No monorepo tooling (no Turborepo/Nx/pnpm workspaces) — `bun install` is invoked from `tui/` and `uv sync` from the repo root. Top-level `Makefile` / `package.json` at repo root remain untouched; a future ADR can add a top-level orchestrator if needed.
