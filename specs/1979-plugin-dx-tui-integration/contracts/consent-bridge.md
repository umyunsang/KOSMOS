...
**Surface**: `src/ummaya/plugins/consent_bridge.py` (NEW) wrapping `src/ummaya/plugins/installer.py:_default_consent_prompt` seam (lines 219-229) at runtime.
...
- On `asyncio.TimeoutError`:
  ...
  - OTEL attribute `ummaya.permission.decision = "timeout"` on the `ummaya.plugin.install` span.
...
- `src/ummaya/plugins/installer.py:191` (`ConsentPrompt` type alias)
- `src/ummaya/plugins/installer.py:219-229` (`_default_consent_prompt` stub being replaced)
- `src/ummaya/ipc/stdio.py:521` (`_pending_perms` dict — Spec 1978 D2 invariant)
- `src/ummaya/ipc/stdio.py:814` (existing `asyncio.wait_for` permission timeout pattern)
