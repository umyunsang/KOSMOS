...
**Location**: `src/ummaya/tools/retrieval/backend.py`
...
class Retriever(Protocol):
    ...
    ``ToolRegistry`` and ``ummaya.tools.search`` depend only on this
...
- Consumed by: `ummaya.tools.search.search_tools()`.

---

## 2. `BM25Backend` (new — wraps existing `BM25Index`)

**Location**: `src/ummaya/tools/retrieval/bm25_backend.py`

**Fields**:
| Field | Type | Default | Constraint |
|---|---|---|---|
| `_index` | `BM25Index` | — | Composed verbatim from `ummaya.tools.bm25_index`; behaviour unchanged. |
...
**Relationships**: Satisfies `Retriever`. Used standalone when `UMMAYA_RETRIEVAL_BACKEND=bm25` (default). Composed by `HybridBackend`.

---

## 3. `DenseBackend` (new)

**Location**: `src/ummaya/tools/retrieval/dense_backend.py`
...
**Location**: `src/ummaya/tools/retrieval/hybrid.py`
...
| `_rrf_k` | `int` | 60 | Constant from Cormack SIGIR 2009; overridable via `UMMAYA_RETRIEVAL_FUSION_K`. |
...
**Relationships**: Satisfies `Retriever`. Consumed by factory when `UMMAYA_RETRIEVAL_BACKEND=hybrid`.

---

## 5. `RetrievalManifest` (new — Pydantic v2 model)

**Location**: `src/ummaya/tools/retrieval/manifest.py`
...
**Location**: file `eval/retrieval_queries_adversarial.yaml`; loader in `src/ummaya/eval/retrieval.py`.
...
**Location**: `src/ummaya/tools/retrieval/degrade.py`
...
Not reprinted; reference `src/ummaya/tools/models.py`. V6 `auth_type`/`auth_level` consistency invariant stands.
...
ummaya.tools.search.search_tools(query, registry)
...
- Every new surface is internal to `src/ummaya/tools/retrieval/` or `src/ummaya/eval/`; the public tool contract is unchanged.
