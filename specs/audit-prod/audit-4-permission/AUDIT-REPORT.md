...
| **P0-2** | CRITICAL | Backend gate | **Allow path bypasses Spec 033 ledger entirely.** `_check_permission_gate` (stdio.py:1539-1559) writes a JSON receipt file but never calls `kosax.permissions.ledger.append()`. No HMAC seal, no SHA-256 chain, no key_id, no fcntl lock, no WORM mode. PIPA §22-2 audit-trail obligation violated. | After full scenario: `~/.kosax/consent_ledger.jsonl` does not exist; `~/.kosax/keys/registry.json` does not exist; `~/.kosax/permissions.json` does not exist; only the pre-existing 2026-04-22 onboarding receipt is on disk. |
| **P0-3** | CRITICAL | Backend revoke | **Revoke path uses ad-hoc hashlib SHA-256 instead of canonical ledger.** `_handle_consent_revoke_request` (stdio.py:3192-3221) writes to `~/.kosax/memdir/user/consent/ledger.jsonl` (NOT the canonical Spec 033 path) with no HMAC seal, no prev_hash chain, no key_id, no fcntl lock — entries are forgeable in plain text. | stdio.py:3200 `record_hash = _hashlib.sha256(ledger_json.encode("utf-8")).hexdigest()` — no `kosax.permissions.ledger` import in the entire file. |
...
| # | Severity | Surface | Finding |
|---|---|---|---|
| P2-1 | MED | i18n | `permissionEn` strings exist but are unreachable from `KOSAX_TUI_LOCALE` because the env var is read once at module load (KosaxPrimitivePermissionRequest.tsx:56) — locale switch requires a TUI restart. |
...
| permission_request → KosaxPrimitivePermissionRequest mount | via toolUseConfirmQueue | YES — visible | ✅ |
...
| receipt JSON write | `~/.kosax/memdir/user/consent/<id>.json` | YES (when allowed) | ✅ |
...
- `KosaxPermissionRequestAdapter.handleDecision('deny')` calls `toolUseConfirm.onReject()` then `onReject()` then `onDone()`.
...
Citizen-facing fallback wording would depend on the LLM's prompt; **no deterministic guidance is shown to the citizen by KOSAX itself** (no toast, no banner, no "alternative options" panel).  Production should add a Korean wording fallback (e.g. "권한이 거부되어 작업을 중단했습니다. 다른 방법을 시도하려면 …") in the message timeline at deny-time, independent of the LLM.

## Summary verdict

**PRODUCTION READY: NO.**

The Permission Gauntlet is the **single most safety-critical surface** in KOSAX — it is the boundary between a citizen's request and a side-effecting / irreversible call against a Korean public-service module.  The current build:
...
2. P0-2 + P0-3 fix (allow / deny / withdraw all flow through `kosax.permissions.ledger.append`).
...
KOSAX_BACKEND_LOG_FILE=/tmp/audit-4.log \
