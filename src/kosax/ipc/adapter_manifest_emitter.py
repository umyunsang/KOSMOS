...
1. **Extra manifest registry** (``_EXTRA_REGISTRY``):
   Adapters that do not have a :class:`~kosax.tools.registry.AdapterRegistration`
...
2. **Submit primitive sub-registry**:
   ``kosax.primitives.submit._ADAPTER_REGISTRY`` →
...
from kosax.ipc.frame_schema import AdapterManifestEntry, AdapterManifestSyncFrame
...
def register_manifest_entry(entry: AdapterManifestEntry) -> None:
    ...
    or similar).  For example::
        ...
        from kosax.ipc.adapter_manifest_emitter import register_manifest_entry
        from kosax.ipc.frame_schema import AdapterManifestEntry
...
def _build_entries(  # noqa: C901, ANN401 — three-source walker, refactor deferred
    ...
    *,
    ...
    Args:
        registry: A :class:`kosax.tools.registry.ToolRegistry` instance.
    ...
    try:
        from kosax.primitives.submit import (
...
def emit_manifest(
    ...
    registry: Any,  # kosax.tools.registry.ToolRegistry
    *,
    pid: int | None = None,
) -> None:
    ...
    Args:
        ...
        registry:      Fully booted :class:`~kosax.tools.registry.ToolRegistry`.
