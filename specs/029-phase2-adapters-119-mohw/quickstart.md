...
- Git worktree at `/Users/um-yunsang/UMMAYA-15`, branch
  `feat/15-phase2-adapters-119-mohw`
- `.env` populated with `UMMAYA_DATA_GO_KR_API_KEY` (already present from earlier
...
- `src/ummaya/tools/ssis/codes.py` — §1
- `src/ummaya/tools/nfa119/emergency_info_service.py` — §§2, 3, 4
- `src/ummaya/tools/ssis/welfare_eligibility_search.py` — §§5, 6, 7
...
```bash
# Add register() calls to src/ummaya/tools/register_all.py.
# Modify src/ummaya/security/audit.py — apply the TOOL_MIN_AAL diff from
...
from ummaya.tools.register_all import register_all
...
```bash
git -C /Users/um-yunsang/UMMAYA-15 add \
    src/ummaya/tools/nfa119 \
    src/ummaya/tools/ssis \
    src/ummaya/security/audit.py \
...
git -C /Users/um-yunsang/UMMAYA-15 commit -m "feat(029): nfa_emergency_info_service + mohw_welfare_eligibility_search (interface-only)"
```
