...
4. Wire `tui/src/commands/plugins.ts` (existing T066 entry point — Spec 1635) to populate `PluginEntry[]` from a `plugin_op_request:list` round-trip rather than the current `KOSAX_PLUGIN_REGISTRY` env-var stub.
...
**Project Type**: Conversational multi-agent platform. Backend = Python `kosax.*` packages over stdio JSONL; TUI = Bun + Ink React.
...
**Scale/Scope**: ~28 tasks across 6 phases (A–F). Affects ~6 files in `src/kosax/ipc/` + `src/kosax/plugins/` + ~4 files in `tui/src/commands/` + `tui/src/screens/REPL.tsx` integration.
...
| **§IV Government API Compliance** | ✅ PASS | FR-026 forbids live `data.go.kr` in CI. Tests use `file://` catalog + fixture bundles per `installer.py:_default_catalog_fetcher`. No `KOSAX_*` key handling change. |
...
tui/src/
├── commands.ts                   # Modify line 133: import KOSAX plugin from './commands/plugin.js' (singular)
├── commands/
│   ├── plugin.ts                 # No change — already emits sendPluginOp; remove H7 deferred suffix from acknowledgement strings
│   ├── plugins.ts                # Modify: replace KOSAX_PLUGIN_REGISTRY env-var stub with plugin_op_request:list IPC round-trip
│   └── plugin/                   # CC marketplace residue — unreachable after commands.ts:133 swap; documented for future cleanup Epic
├── components/
│   └── plugins/
│       └── PluginBrowser.tsx     # No change — Spec 1635 T065 already KOSAX-aligned; receives PluginEntry[] from plugins.ts
├── ipc/
...
**Structure Decision**: Single project layout (already established for KOSAX). Backend additions concentrated in `src/kosax/ipc/` + `src/kosax/plugins/` + one method addition to `src/kosax/tools/registry.py`. TUI additions are minimal: one import swap + one command-handler logic replacement. No new top-level packages.
...
| V2 — Use existing `PluginBrowser.tsx` (Spec 1635 T065) verbatim; only rewire data source in `commands/plugins.ts` | The existing component is already KOSAX-aligned with `PluginEntry` shape, ⏺/○ glyphs, Space/i/r/a keystrokes, Korean-primary i18n, and `kosax.ui.surface=plugins` OTEL emission. | "Build a new `CitizenPluginStore.tsx`" rejected because the existing component matches every UI-E.3 spec line. Authoring a parallel component would duplicate logic and introduce drift risk between the two surfaces. |
