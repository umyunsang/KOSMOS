...
This spec covers the **local developer workflow only**: running OTel Collector Contrib + the existing Langfuse v3 stack from `docker-compose.dev.yml` to visualize UMMAYA span trees during KSC 2026 demo sessions. It does **not** cover the production deployment scenario described in the Epic body (Fly.io/Railway, Langfuse Cloud Hobby, TLS, CI integration). Those are deferred — see the Deferred Items table.

The two goals of this spec are:

1. Add an `otelcol-contrib` sidecar to the existing `docker-compose.dev.yml` so that UMMAYA spans travel `UMMAYA app → Collector → Langfuse` instead of `UMMAYA app → Langfuse` directly. This layer allows PII attribute redaction and batch buffering before traces reach Langfuse.
...
A student demonstrator running UMMAYA on a MacBook opens one terminal, runs `docker compose -f docker-compose.dev.yml up -d`, then starts the UMMAYA CLI in another terminal. Within 10 seconds of an agent call, the Langfuse UI at `http://localhost:3000` shows the trace with a three-level span tree: `invoke_agent ummaya-query` → `chat` → `execute_tool <tool_id>`.
...
2. **Given** the full stack is running, **When** a UMMAYA agent session emits a trace (one `invoke_agent` + one `chat` + one `execute_tool`), **Then** the `invoke_agent` span and its two children appear under the same trace ID in Langfuse within 10 seconds.
...
2. **Given** a `resolve_location` span includes a raw `ummaya.location.query` string, **When** the span passes through the collector, **Then** the stored attribute is the SHA-256 hash of the query, not the raw string.

---

### Edge Cases

- **Collector container not running**: UMMAYA app falls back to warning-only log (inherited from spec 021 FR-010). No user-visible error.
...
- **Port 4318 (OTLP HTTP) in use**: The collector exposes OTLP HTTP on port 4318 by default. If the port is occupied, the operator sets `UMMAYA_OTEL_COLLECTOR_PORT` to override.
...
- **FR-003**: The `otelcol` service exposes OTLP HTTP on port 4318 (`${UMMAYA_OTEL_COLLECTOR_PORT:-4318}:4318`). gRPC port 4317 is not exposed.
...
- **FR-006**: The `attributes/pii_redact` processor applies the following rules via the OTel Collector `attributes` processor `delete` action:
  ...
  - Hash `ummaya.location.query` via `hash` action (SHA-256 output stored back in the same key)
- **FR-007**: The `batch` processor is configured with `timeout: 5s` and `send_batch_size: 512`.
- **FR-008**: The `otlphttp` exporter sends spans to the Langfuse OTLP ingest endpoint. The URL is `${UMMAYA_LANGFUSE_OTLP_ENDPOINT:-http://langfuse-web:3000/api/public/otel/v1/traces}`. The exporter sets the `Authorization` header to `Basic <base64(pk-xxx:sk-xxx)>` sourced from `${UMMAYA_LANGFUSE_OTLP_AUTH_HEADER:-}`. The header `x-langfuse-ingestion-version: 4` is also set for real-time preview. If `UMMAYA_LANGFUSE_OTLP_AUTH_HEADER` is empty, the exporter sends without auth (acceptable for local dev with anonymous project).
...
#### UMMAYA app configuration

- **FR-009**: When `OTEL_EXPORTER_OTLP_ENDPOINT` is not set in the UMMAYA app environment, the app logs a warning and disables export (inherited from spec 021 FR-010). For local dev, the operator sets `OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4318`.
- **FR-010**: The `.env.example` file is updated with the following new variables and their descriptions:
  - `UMMAYA_OTEL_COLLECTOR_PORT` — host port for OTLP HTTP receiver (default 4318)
  - `UMMAYA_LANGFUSE_OTLP_ENDPOINT` — Langfuse OTLP ingest URL for the collector exporter (default `http://langfuse-web:3000/api/public/otel/v1/traces`)
  - `UMMAYA_LANGFUSE_OTLP_AUTH_HEADER` — `Basic <base64(pk-xxx:sk-xxx)>` for Langfuse OTLP auth (empty = anonymous)
...
- **OTLP ingest endpoint**: the URL the UMMAYA Python app points `OTEL_EXPORTER_OTLP_ENDPOINT` to — `http://localhost:4318` in dev (the collector's exposed port).
...
- **Production OTLP collector deployment** — Fly.io / Railway hosting, TLS termination (`otel.ummaya.<domain>`), public collector endpoint.
...
- **CI OIDC injection** of `UMMAYA_LANGFUSE_OTLP_AUTH_HEADER` via Infisical (depends on #468 being stable).
- **Per-environment Langfuse project isolation** (`ummaya-dev`, `ummaya-ci`, `ummaya-prod`).
...
|---|---|---|
| Production OTLP collector deployment (Fly.io / Railway, TLS, `otel.ummaya.<domain>`) | Cost-constrained; local dev is the KSC demo target. Production infra requires #468 Infisical OIDC to be stable first. | #897 |
...
| CI pipeline `UMMAYA_LANGFUSE_OTLP_AUTH_HEADER` injection via Infisical OIDC | Depends on #468 (OIDC) being stable. | #903 |
| Per-environment Langfuse project isolation (`ummaya-dev`, `ummaya-ci`, `ummaya-prod`) | Not meaningful until CI integration is in scope. | #907 |
...
|---|---|---|
| `UMMAYA_OTEL_COLLECTOR_PORT` | `4318` | Host port for the `otelcol` OTLP HTTP receiver |
| `UMMAYA_LANGFUSE_OTLP_ENDPOINT` | `http://langfuse-web:3000/api/public/otel/v1/traces` | URL the collector exporter sends spans to (inside compose network) |
| `UMMAYA_LANGFUSE_OTLP_AUTH_HEADER` | `` (empty) | `Basic <base64(pk-xxx:sk-xxx)>` for Langfuse OTLP auth |
...
| `.env.example` | Add three new `UMMAYA_` variables with inline comments |
