...
**Module**: `src/ummaya/permissions/models.py`
...
| `api_key` | `"api_key"` | `UMMAYA_DATA_GO_KR_API_KEY` must be set and non-empty |
...
Written to `logging.getLogger("ummaya.permissions.audit")` after every invocation.
...
`PermissionPipeline.run()` returns the existing `ummaya.tools.models.ToolResult`. No new result type.
...
**Required change**: `ToolResult.error_type` Literal must be extended to include `"permission_denied"` in `src/ummaya/tools/models.py` during Phase 4.
