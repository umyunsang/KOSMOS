...
**Purpose**: Create the structured event emission infrastructure that all later phases depend on. Extends the existing `src/ummaya/observability/` files — does **not** replace them.

- [ ] T001 Extend `EventType` Literal in `src/ummaya/observability/events.py` — add `"permission_decision"` and `"llm_call"` to the union; update module docstring
- [ ] T002 Create `ObservabilityEventLogger` class in `src/ummaya/observability/event_logger.py` (new file) with `emit(event: ObservabilityEvent) -> None`, level-map keyed on `(event_type, success)`, PII key whitelist enforcement, and fail-safe try/except
- [ ] T003 Add `ObservabilityEventLogger` to re-export list in `src/ummaya/observability/__init__.py`

**Checkpoint**: `from ummaya.observability import ObservabilityEventLogger` succeeds. `emit()` emits structured JSON at the correct log level for each event type.
...
- [ ] T005 [US3] Add `metrics: MetricsCollector | None = None` parameter to `UsageTracker.__init__` in `src/ummaya/llm/usage.py`; in `debit()` increment `llm.input_tokens` and `llm.output_tokens` after budget logic, wrapped in try/except
- [ ] T006 [US3] Add `metrics: MetricsCollector | None = None` and `event_logger: ObservabilityEventLogger | None = None` parameters to `LLMClient.__init__` in `src/ummaya/llm/client.py`; pass `metrics` through to `UsageTracker` at construction
- [ ] T007 [US3] In `LLMClient.complete()` in `src/ummaya/llm/client.py` — bracket `_do_request` with monotonic timer; call `metrics.observe("llm.call_duration_ms", ms)` and `metrics.increment("llm.call_count")` on success; `metrics.increment("llm.error_count")` on failure; emit `ObservabilityEvent(event_type="llm_call")` via `event_logger`; all wrapped in try/except
- [ ] T008 [US3] In `LLMClient.stream()` in `src/ummaya/llm/client.py` — start timer before `async with`; record `llm.call_duration_ms` and `llm.call_count` at `event.type == "done"`; increment `llm.error_count` on `StreamInterruptedError`; emit `llm_call` event; all wrapped in try/except
...
- [ ] T010 [US2] Add `metrics: MetricsCollector | None = None` and `event_logger: ObservabilityEventLogger | None = None` parameters to `PermissionPipeline.__init__` in `src/ummaya/permissions/pipeline.py`
- [ ] T011 [US2] In `PermissionPipeline._run_pre_execution_steps()` in `src/ummaya/permissions/pipeline.py` — after each `step_result`, increment `permission.decision_count{step, decision}`; on step exception branch emit `decision=deny`; all wrapped in try/except (AC-A1)
- [ ] T012 [US2] In `PermissionPipeline._run_pre_execution_steps()` and `_execute_step6()` in `src/ummaya/permissions/pipeline.py` — after each decision metric, emit `ObservabilityEvent(event_type="permission_decision")` via `event_logger` with `metadata={"step": int, "decision": str, "reason": str}` (PII-clean); wrapped in try/except (AC-A5, AC-A7 partial)
- [ ] T013 [US2] Expose trip indicator from `src/ummaya/permissions/steps/refusal_circuit_breaker.py` — extend `record_denial()` return value or add a query function so `PermissionPipeline` can detect circuit trips; increment `permission.refusal_circuit_trips` on trip (AC-A3 refusal circuit)
- [ ] T014 [US2] In `PermissionPipeline.run()` in `src/ummaya/permissions/pipeline.py` — wrap body in `try/finally` after monotonic timer start; record `permission.pipeline_duration_ms` histogram on all exit paths (allow, deny, not-found, bypass); wrapped in try/except (AC-A2)
...
- [ ] T017 [US2] Add `event_logger: ObservabilityEventLogger | None = None` parameter to `ToolExecutor.__init__` in `src/ummaya/tools/executor.py`; after dispatch completes (both success and failure), emit `ObservabilityEvent(event_type="tool_call")` with `tool_id`, `duration_ms`, `success`, `metadata={"error_type": ...}`; wrapped in try/except (AC-A6)
- [ ] T018 [US2] Add `event_logger: ObservabilityEventLogger | None = None` parameter to `RecoveryExecutor.__init__` in `src/ummaya/recovery/executor.py`; emit `ObservabilityEvent(event_type="retry")` after retry batch with `metadata={"attempt": N, "error_class": str}`; emit `event_type="circuit_break"` on `if not breaker.allow_request()` branch with `metadata={"circuit_state": str(breaker.state)}`; both wrapped in try/except (AC-A7)
...
- [ ] T020 [US1] Register `"metrics"` `SlashCommand` entry in `COMMANDS` dict in `src/ummaya/cli/models.py` with description `"Show session metrics snapshot (counters, histograms, gauges)"`
- [ ] T021 [US1] Add `metrics: MetricsCollector | None = None` parameter to `REPLLoop.__init__` in `src/ummaya/cli/repl.py`; store as `self._metrics`
- [ ] T022 [US1] Add `elif name == "metrics": self._cmd_metrics()` dispatch branch in `REPLLoop._dispatch_command()` in `src/ummaya/cli/repl.py`
- [ ] T023 [US1] Implement `REPLLoop._cmd_metrics()` in `src/ummaya/cli/repl.py` using `rich.table.Table` — render COUNTERS section, HISTOGRAMS section (columns: name, p50, p95, p99, count; values formatted as `{v:.0f} ms`), GAUGES section; omit empty sections; show "No metrics collected in this session." when snapshot has no data (AC-A8)
...
- [ ] T024 Inspect `src/ummaya/engine/` to determine how `PermissionPipeline` is constructed (inside `QueryEngine` or externally) — document the injection point before coding
- [ ] T025 In `src/ummaya/cli/app.py` `_run_repl()`, create `MetricsCollector()` and `ObservabilityEventLogger()` instances; pass `metrics` and `event_logger` into `RecoveryExecutor`, `ToolExecutor`, `LLMClient`, and `PermissionPipeline` (or `QueryEngine` if pipeline is internal); pass `metrics` to `REPLLoop`
- [ ] T026 Update `src/ummaya/context/builder.py` or `QueryEngine` constructor (wherever `PermissionPipeline` is assembled) to accept optional `metrics` and `event_logger` parameters so the shared instances flow through

**Checkpoint**: Running `uv run python -m ummaya` starts without error. Typing `/metrics` after one query shows non-zero counters for `tool.call_count`, `llm.input_tokens`, and `permission.decision_count`.
...
- **Phase 5 (REPL command, US1)**: Depends on T001–T003 (Phase 1 complete); independent of Phases 2–4; `MetricsCollector` is already in `src/ummaya/observability/metrics.py`
...
Agent A (Sonnet): "Phase 2 — LLM instrumentation (T004–T008) in src/ummaya/llm/"
Agent B (Sonnet): "Phase 3 — PermissionPipeline instrumentation (T009–T014) in src/ummaya/permissions/"
Agent C (Sonnet): "Phase 4 — ToolExecutor + RecoveryExecutor events (T015–T018)"
Agent D (Sonnet): "Phase 5 — /metrics REPL command (T019–T023) in src/ummaya/cli/"
```
...
5. **STOP and VALIDATE**: Run UMMAYA, issue one query, type `/metrics` — see `llm.input_tokens` and `llm.call_duration_ms`
