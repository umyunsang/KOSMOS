...
"""Theme system for the KOSAX CLI.
...
and a :func:`load_theme` loader that respects the ``KOSAX_THEME`` environment
variable with a ``KOSAX_CLI_`` prefix for consistency.

Usage::

    from kosax.cli.themes import load_theme
...
class Theme(BaseModel):
    """Colour/style definitions for the KOSAX CLI renderer.
...
def load_theme() -> Theme:
    ...
    Reads ``KOSAX_THEME`` (or its alias ``KOSAX_CLI_THEME``) from the
    ...
    """
    raw = os.environ.get("KOSAX_THEME") or os.environ.get("KOSAX_CLI_THEME", "")
    name = raw.strip().lower() or _DEFAULT_THEME_NAME
    if name not in _THEMES:
        logger.warning(
            "Unknown KOSAX_THEME %r — falling back to %r. Valid choices: %s",
            name,
            _DEFAULT_THEME_NAME,
            ", ".join(_THEMES),
        )
