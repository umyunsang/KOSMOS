...
> **Reference baseline rule** (`MEMORY.md feedback_cc_source_migration_pattern`): 모든 task 가 CC restored-src 또는 KOSAX 기존 모듈을 reference baseline 으로 명시. 새로 작성 X — 포팅 또는 wire 복구.
...
**Definition of done**: `bun typecheck` 통과 (KOSAX narrow `src/stubs/**`) + lint clean. (Smoke 는 본 task 에서 별도 X.)
...
### T004 — `KosaxPermissionRequest.tsx` dispatcher [sonnet-hook-components]

**File**: `tui/src/components/permissions/KosaxPermissionRequest.tsx` (NEW), `tui/test/components/permissions/KosaxPermissionRequest.test.tsx` (NEW)
...
**Deliverables**:
1. Functional component `<KosaxPermissionRequest>` (~ 80 LOC).
...
4. bypassMode=true 시 `<KosaxBypassReinforcement>` (T005) 를 wrapper 로 mount.
...
### T005 — `KosaxBypassReinforcement.tsx` dual-confirm [sonnet-hook-components]

**File**: `tui/src/components/permissions/KosaxBypassReinforcement.tsx` (NEW), `tui/test/components/permissions/KosaxBypassReinforcement.test.tsx` (NEW)
...
### T006 — REPL.tsx dead placeholder 제거 + KosaxPermissionRequest mount [sonnet-repl-wire]
...
2. `<KosaxPermissionRequest>` 를 FullscreenLayout `bottom` slot 또는 `toolPermissionOverlay` 위치에 mount.
...
**Definition of done**: `grep "PermissionGauntletModal × 2" tui/src/screens/REPL.tsx` = 0 hit (또는 historical-note 한 줄로) + KosaxPermissionRequest import + mount 라인 alive + `bun test` 회귀 0.
...
2. Layer 는 `aalToLayer(primitive, auth_level, is_irreversible)` 로 계산 (backend 가 layer 를 직접 보내주면 그것 사용 우선, KOSAX 측 fallback only).
3. `KosaxPermissionRequest` 의 `onApprove` 콜백에서 `sendFrame(permission_response, {request_id, decision})` 호출.
...
4. `mode === 'bypassPermissions'` 토글 시 KosaxPermissionRequest 에 prop drilling (T006 이미 wire).
...
4. **CC parity statement** — 신규 컴포넌트 3개 (`KosaxPermissionRequest`, `KosaxBypassReinforcement`, `aalToLayer`) 모두 CC reference 명시 + KOSAX 적응 설명.
