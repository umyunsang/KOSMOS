...
from kosax.tools.nmc.freshness import FreshnessResult
...
class NmcEmergencySearchInput(BaseModel):
    ...
    operations for coordinate lookup and regional list lookup. KOSAX models
...
async def handle(inp: NmcEmergencySearchInput) -> dict[str, Any]:
    ...
    if not settings.data_go_kr_api_key:
        return {
            ...
            "message": "KOSAX_DATA_GO_KR_API_KEY is not configured",
            "retryable": False,
        }
