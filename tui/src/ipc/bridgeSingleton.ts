...
// KOSAX-original — Epic #1633 FR-007/FR-017 bootstrap helper.
//
...
//
// The bridge is spawned lazily because cold-starting `uv run kosax --ipc
...
export function getOrCreateKosaxBridge(): IPCBridge {
...
export function getKosaxBridgeSessionId(): string {
  if (_sessionId === null) {
...
export async function closeKosaxBridge(): Promise<void> {
  if (_bridge !== null) {
    const b = _bridge
    _bridge = null
    await b.close()
