...
**Verdict**: REWRITE
**UMMAYA target**: `tui/src/components/onboarding/LogoV2/AnimatedAsterisk.tsx`
**Tokens consumed**: `ummayaCore`, `ummayaCoreShimmer`
...
An asterisk glyph (`*`) rendered in the `ummayaCore` colour, with a shimmer-cycle animation that cycles through `ummayaCoreShimmer` → `ummayaCore` → `ummayaCoreShimmer` at 6 fps (same frame budget as CC `useShimmerAnimation.ts`).

### Reduced-motion fallback

Static `*` in `ummayaCore` colour; no shimmer; no re-render triggered by the animation hook.

### CC content removed

- Teardrop-asterisk glyph (CC-specific design) — replaced with a standard `*` (U+002A).
- `chromeYellow` colour binding — replaced with `ummayaCore`.
...
**Verdict**: REWRITE
**UMMAYA target**: `tui/src/components/onboarding/LogoV2/CondensedLogo.tsx`
**Tokens consumed**: `wordmark`, `subtitle`, `ummayaCore`, `background`
**Accessibility anchor**: `[ag-logov2]`
...
One-line header: `[ummayaCore] UMMAYA — <model> · <effort> · <coordinatorMode>` rendered in the `wordmark` token on the `background` token.
...
**Verdict**: REWRITE
**UMMAYA target**: `tui/src/components/onboarding/LogoV2/Feed.tsx`
...
```typescript
type Props = {
  sessionHistory: UmmayaSession[]     // from memdir Session tier
...
- **Left column — "최근 세션"**: up to 5 recent UMMAYA session entries (query summary + timestamp). Each entry styled in `text` token; timestamp in `subtle`.
...
**UMMAYA target**: `tui/src/components/onboarding/LogoV2/FeedColumn.tsx`
...
None structurally; the PORT only renames imported token identifiers to their UMMAYA equivalents (no occurrences of `claude*` in the original file per inspection).
...
**Verdict**: REWRITE
**UMMAYA target**: `tui/src/components/onboarding/LogoV2/feedConfigs.tsx`
**Tokens consumed**: `agentSatellite*`, `text`, `subtle`

### Exports

```typescript
export function createUmmayaSessionHistoryFeed(sessionHistory: UmmayaSession[]): FeedConfig
export function createMinistryAvailabilityFeed(status: MinistryStatus[]): FeedConfig
```

### CC content removed

CC's 5 feed factories are entirely deleted. Two UMMAYA factories replace them.
...
**Verdict**: REWRITE
**UMMAYA target**: `tui/src/components/onboarding/LogoV2/LogoV2.tsx`
...
Single-line text: `UMMAYA — 한국 공공서비스 대화창`.
...
**Verdict**: REWRITE
**UMMAYA target**: `tui/src/components/onboarding/LogoV2/WelcomeV2.tsx`
**Tokens consumed**: `wordmark`, `subtitle`, `ummayaCore`

### Visual

```
  UMMAYA에 오신 것을 환영합니다  vN.N.N
  ────────────────────────────
          *  *  *
         *  ●  *
          *  *  *
...
Citizen welcome screen — ummayaCore asterisk cluster (static or shimmering per reduced-motion), with the Korean welcome message. Apple-Terminal-specific ASCII art branch from CC is deleted.

### CC content removed

- "Welcome to Claude Code" → "UMMAYA에 오신 것을 환영합니다".
- Apple-Terminal special-case ASCII poses → replaced with the ummayaCore cluster.
- Theme-specific light/dark branching: retained only the dark branch (per Phase 1 scope).

---

## § 8 · `UmmayaCoreIcon.tsx` (REWRITE from `FastIcon.tsx`, row 162)

**Verdict**: REWRITE + file rename
**CC source**: `FastIcon.tsx`
**UMMAYA target**: `tui/src/components/chrome/UmmayaCoreIcon.tsx`
**Tokens consumed**: `ummayaCore`, `ummayaCoreShimmer`
...
Single `*` (U+002A) glyph in `ummayaCore` token; shimmers to `ummayaCoreShimmer` when `shimmering === true` AND `useReducedMotion() === false`.

### Consumers to update

Every import of `FastIcon` in the existing TUI is updated to `UmmayaCoreIcon` as part of this REWRITE.
...
**Verdict**: REWRITE
**UMMAYA target**: `tui/src/components/onboarding/Onboarding.tsx`
...
- `logEvent('tengu_*')` analytics — replaced with UMMAYA OTEL span emission per Spec 021 (use `ummaya.onboarding.step` span name).

---

## § 10 · Traceability matrix

| # | CC source | UMMAYA target | Verdict | a11y anchor | FR | Test |
|---|---|---|---|---|---|---|
...
| 8 | `FastIcon.tsx` | `chrome/UmmayaCoreIcon.tsx` | REWRITE | `[ag-logo-wordmark]` r154 | FR-023 | `UmmayaCoreIcon.snap.test.tsx` |
