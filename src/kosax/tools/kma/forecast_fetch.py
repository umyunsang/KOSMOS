...
from kosax.tools._description_template import build_description_v4
from kosax.tools._outbound_trace import traced_async_client
from kosax.tools.errors import LookupErrorReason, _require_env
from kosax.tools.kma.projection import KMADomainError, latlon_to_lcc
from kosax.tools.models import (  # noqa: A004
...
# KOSAX canonical citizen-facing timezone for `meta.fetched_at`.
# Internal elapsed-time math (`t_start`) keeps UTC; only the citizen-visible
# stamp switches. See `src/kosax/tools/envelope.py` for the canonical rule.
...
async def _fetch(
    ...
    *,
    ...
    api_key = _require_env("KOSAX_DATA_GO_KR_API_KEY")
        ...
        if "xml" in content_type.lower() and "json" not in content_type.lower():
            return LookupError(
                ...
                message=(
                    ...
                    "Check KOSAX_DATA_GO_KR_API_KEY validity."
                ),
            )
    ...
    from kosax.tools.models import LookupMeta
