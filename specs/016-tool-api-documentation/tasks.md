...
- [ ] T001 Write `docs/tools/README.md` index — table of all 7 tools (tool ID, Korean name, provider, one-line description), links to each adapter page, Authentication section (`UMMAYA_DATA_GO_KR_API_KEY`), shared error code summary referencing `koroad.md`
...
**Independent Test**: Open `koroad.md` and confirm: (1) auth field shows `UMMAYA_DATA_GO_KR_API_KEY`, (2) `gu_gun` Required column shows `Yes`, (3) `GugunCode` table includes `BUSAN_HAEUNDAE = 350`, (4) full error code table present.

### Implementation for User Story 1

- [ ] T003 [US1] Fix auth field in `docs/tools/koroad.md` Overview table: change `UMMAYA_KOROAD_API_KEY` → `UMMAYA_DATA_GO_KR_API_KEY`
...
- [ ] T006 [US1] Expand `GugunCode` table in `docs/tools/koroad.md`: add all 25 Seoul gu entries, all 16 Busan gu entries (must include `BUSAN_HAEUNDAE = 350`), all 5 Daejeon entries, and one anchor entry per remaining sido; add note linking to `src/ummaya/tools/koroad/code_tables.py` for the complete 250+ entry list
...
- [ ] T017 [US3] Create `docs/tools/road-risk-score.md` — Overview table (endpoint `(none — composite)`, provider `UMMAYA (composite)`, rate limit 10/min, cache TTL 300 s, Personal Data No, `is_concurrency_safe: Yes`)
...
- [ ] T035 [US5] Spot-check all 7 doc Overview tables against current `GovAPITool` definitions in source (`src/ummaya/tools/`) — verify endpoint URL, rate_limit_per_minute, cache_ttl_seconds, requires_auth, is_personal_data match exactly
...
- [ ] T041 Structural validation of all code examples — run each `asyncio.run()` snippet with `UMMAYA_DATA_GO_KR_API_KEY` unset; confirm failure mode is `ConfigurationError` (not `ModuleNotFoundError` or `SyntaxError`)
