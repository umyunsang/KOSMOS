...
**Purpose**: Create the `src/ummaya/context/` sub-package and `tests/context/` directory.
No source code yet — just the file system skeleton that all later tasks populate.

- [ ] T001 Create `src/ummaya/context/__init__.py` with empty public export list
- [ ] T002 Create `tests/context/__init__.py` to make test directory a package

**Checkpoint**: `src/ummaya/context/` and `tests/context/` exist as importable packages.
...
- [ ] T003 Implement `SystemPromptConfig`, `ContextLayer`, `ContextBudget`, `AssembledContext` frozen models with all field validators and `ContextBudget.from_estimate()` class method in `src/ummaya/context/models.py`
- [ ] T004 Add `active_situational_tools: set[str] = field(default_factory=set)` to `QueryState` dataclass in `src/ummaya/engine/models.py`
...
- [ ] T006 [US1] Implement `SystemPromptAssembler` with the four mandatory sections (platform identity, language policy, tool-use policy, personal-data reminder) in `src/ummaya/context/system_prompt.py`
- [ ] T007 [US1] Implement `ContextBuilder.__init__()` accepting `config: SystemPromptConfig | None = None`, caching the assembled system message on first call, in `src/ummaya/context/builder.py`
- [ ] T008 [US1] Implement `ContextBuilder.build_system_message()` delegating to `SystemPromptAssembler`, returning cached `ChatMessage` on subsequent calls, in `src/ummaya/context/builder.py`
- [ ] T009 [US1] Update `src/ummaya/context/__init__.py` to export `ContextBuilder`, `SystemPromptConfig`, `ContextLayer`, `ContextBudget`, `AssembledContext`
...
- [ ] T012 [US3] Implement `ContextBuilder.build_assembled_context()` stub: calls `build_system_message()`, builds `tool_definitions` via `registry.export_core_tools_openai()` + active situational tools sorted by `id`, returns minimal `AssembledContext` (no attachment, no budget yet), in `src/ummaya/context/builder.py`
...
- [ ] T014 [US2] Implement `AttachmentCollector` with private section methods for resolved tasks, in-flight tool state, API health, and auth expiry warning in `src/ummaya/context/attachments.py`
- [ ] T015 [US2] Implement `ContextBuilder.build_turn_attachment()` delegating to `AttachmentCollector`, returning `None` for empty sessions, in `src/ummaya/context/builder.py`
...
- [ ] T017 [US5] Add reminder section logic to `AttachmentCollector`: inject reminder listing `resolved_tasks` and pending in-flight state when cadence condition is met in `src/ummaya/context/attachments.py`
...
- [ ] T019 [US4] Implement `BudgetEstimator` with `estimate_layer_tokens()` and `estimate_tool_defs_tokens()` functions producing `ContextBudget` via `ContextBudget.from_estimate()` in `src/ummaya/context/budget.py`
- [ ] T020 [US4] Wire `BudgetEstimator` into `ContextBuilder.build_assembled_context()`: sum tokens across all layers and tool definitions, produce complete `AssembledContext.budget`, in `src/ummaya/context/builder.py`
- [ ] T021 [US4] Add `build_assembled_context()` budget-near-limit WARNING log in `src/ummaya/context/builder.py`
...
- [ ] T023 [US6] Replace `system_prompt: str | None` parameter with `context_builder: ContextBuilder | None` in `QueryEngine.__init__()`, remove `_DEFAULT_SYSTEM_PROMPT` constant, initialize `QueryState.messages` with `[context_builder.build_system_message()]`, in `src/ummaya/engine/engine.py`
- [ ] T024 [US6] Insert `context_builder.build_turn_attachment(state, api_health=None)` call in `QueryEngine.run()` before appending the user message; prepend attachment `ChatMessage` if non-None; yield `api_budget_exceeded` when `is_over_limit` is True, in `src/ummaya/engine/engine.py`
...
Teammate A — US1 (Phase 3):
  src/ummaya/context/system_prompt.py  (T006)
  src/ummaya/context/builder.py        (T007, T008)
  tests/context/test_system_prompt.py  (T010)
  tests/context/test_builder.py        (T011)

Teammate B — US2 (Phase 5):
  src/ummaya/context/attachments.py    (T014)
  src/ummaya/context/builder.py        (T015) ← after Teammate A's builder.py is merged
  tests/context/test_attachments.py    (T016)

Teammate C — US4 (Phase 7):
  src/ummaya/context/budget.py         (T019)
  src/ummaya/context/builder.py        (T020, T021) ← after Teammate A's builder.py is merged
