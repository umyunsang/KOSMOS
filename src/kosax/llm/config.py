...
"""Configuration model for the KOSAX LLM client."""
...
class LLMClientConfig(BaseSettings):
    ...
    Environment variables (no global prefix — each field declares its own alias):
        KOSAX_FRIENDLI_TOKEN       — required API token
        KOSAX_FRIENDLI_BASE_URL    — API base URL
        KOSAX_FRIENDLI_MODEL       — model identifier
        KOSAX_LLM_SESSION_BUDGET   — per-session token budget
        KOSAX_LLM_TIMEOUT_SECONDS  — HTTP stream timeout budget
    """
    ...
    token: SecretStr = Field(
        ...,
        validation_alias="KOSAX_FRIENDLI_TOKEN",
    ...
    base_url: AnyHttpUrl = Field(  # type: ignore[assignment]
        ...
        validation_alias="KOSAX_FRIENDLI_BASE_URL",
        description="FriendliAI API base URL.",
    )
    model: str = Field(
        default="LGAI-EXAONE/K-EXAONE-236B-A23B",
        validation_alias="KOSAX_FRIENDLI_MODEL",
    ...
    session_budget: int = Field(
        ...
        # 1M context. KOSAX agentic loop emits a system prompt + Available
        ...
        # KOSAX_LLM_SESSION_BUDGET if a session needs further tuning.
        default=1_000_000,
        validation_alias="KOSAX_LLM_SESSION_BUDGET",
        description="Maximum tokens allowed per session.",
    )
    timeout: float = Field(  # type: ignore[pydantic-alias]
        default=300.0,
        validation_alias=AliasChoices(
            "KOSAX_LLM_TIMEOUT_SECONDS",
            "KOSAX_LLM_TIMEOUT",
            "timeout",
        ),
    ...
    def token_must_not_be_empty(cls, value: SecretStr) -> SecretStr:
        ...
        if not value.get_secret_value().strip():
            raise ValueError("KOSAX_FRIENDLI_TOKEN must not be empty")
        return value
