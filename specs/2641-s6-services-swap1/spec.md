...
S6 services 감사 (`scope-S6-services.md`) 가 swap-1 (Anthropic SDK → KOSAX IPC)
...
- swap/llm-provider(2641): constants/oauth → KOSAX-1633 inline stub (이미 적용
  된 상태를 헤더로 명시)
...
```ts
if (!process.env.KOSAX_ENABLE_DEAD_TEAM_MEM_SYNC) {
  throw new Error(
    '[KOSAX] services/teamMemorySync: dead in KOSAX — claude.ai team memory ' +
    'is not part of L1-A K-EXAONE harness. Set KOSAX_ENABLE_DEAD_TEAM_MEM_SYNC=1 ' +
...
이므로 gate 미적용 (외부 다른 KOSAX 코드가 import 가능하므로 — 단, 현재 그런
caller 는 0).
...
```ts
if (!process.env.KOSAX_ENABLE_DEAD_SETTINGS_SYNC) {
...
의 `feature()` 를 변경해도 KOSAX_ENABLE_DEAD_SETTINGS_SYNC 가 명시 set 되지
...
| SC-007 | Layer 5 tmux capture-pane boot smoke (`scripts/tui-tmux-capture.sh`) 가 boot 완료 후 services 초기화 path 무손상 캡처 + KOSAX branding 출력 + tool_registry 메시지 출력 |
...
- `docs/requirements/kosax-migration-tree.md` § L1-A LLM Harness (services 가 swap-1 의 일부)
