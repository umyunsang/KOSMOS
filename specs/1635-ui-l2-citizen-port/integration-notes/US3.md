...
|------|------|-------------|
| `tui/src/components/onboarding/PreflightStep.tsx` | T040 | Bun version + graphics protocol + KOSAX_* env-var ✓/✗ checks |
...
## T052 — OTEL kosax.ui.surface=onboarding emission
...
- `OnboardingState` lives at `~/.kosax/memdir/user/onboarding/state.json`
  (atomic-rename writes via `uiL2Memdir.ts`).
- `AccessibilityPreference` lives at `~/.kosax/memdir/user/preferences/a11y.json`
...
`parseLangCommand` mutates `process.env['KOSAX_TUI_LOCALE']`. Components that call
