...
"""PromptLoader — Prompt Registry for the UMMAYA platform.
...
CLI::
    ...
    python -m ummaya.context.prompt_loader --regenerate-manifest
    python -m ummaya.context.prompt_loader --emit-hashes
"""
...
from ummaya.context.prompt_models import PromptManifest
...
class PromptLoader:
    ...
    langfuse_enabled:
        ...
        ``UMMAYA_PROMPT_REGISTRY_LANGFUSE``.  Passing an explicit bool is
    ...
    def __init__(
        ...
        if langfuse_enabled is None:
            langfuse_enabled = (
                os.environ.get("UMMAYA_PROMPT_REGISTRY_LANGFUSE", "").strip().lower() == "true"
            )
    ...
    def get_hash(self, prompt_id: str) -> str:
        ...
        Used by the Context Assembly layer to stamp ``ummaya.prompt.hash`` on
    ...
    def _verify_langfuse(self) -> None:
        """When UMMAYA_PROMPT_REGISTRY_LANGFUSE=true, lazily import langfuse and
        ...
        try:
            ...
            raise PromptRegistryError(
                "UMMAYA_PROMPT_REGISTRY_LANGFUSE=true but the langfuse package is "
                "not installed. Install it with: uv sync --extra langfuse"
            ) from exc

        host = os.environ.get("UMMAYA_LANGFUSE_HOST", "")
        public_key = os.environ.get("UMMAYA_LANGFUSE_PUBLIC_KEY", "")
        secret_key = os.environ.get("UMMAYA_LANGFUSE_SECRET_KEY", "")
...
def main(argv: list[str] | None = None) -> None:
    """CLI entry point: python -m ummaya.context.prompt_loader [options]."""
    parser = argparse.ArgumentParser(
        prog="python -m ummaya.context.prompt_loader",
        description="UMMAYA Prompt Registry CLI utilities.",
    )
    parser.add_argument(
        "--regenerate-manifest",
        action="store_true",
