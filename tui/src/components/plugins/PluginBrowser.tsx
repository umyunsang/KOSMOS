...
// Spec 1979 T025 — Layer color glyphs per kosax-migration-tree.md UI-C.1.
...
function _layerColor(layer: 1 | 2 | 3, theme: ReturnType<typeof useTheme>): string {
  ...
  if (layer === 1) return (theme.success ?? theme.kosaxCore) || 'green';
...
export function PluginBrowser({
  ...
  return (
    <Box flexDirection="column" paddingX={1} paddingY={1}>
      {/* Title */}
      <Box marginBottom={1}>
        <Text bold color={theme.kosaxCore}>{'✻ '}</Text>
      ...
      {plugins.length === 0 && inflightInstalls.length === 0 ? (
        ...
        plugins.map((plugin, idx) => {
          ...
          const glyphColor = plugin.isActive ? theme.kosaxCore : theme.inactive;
          ...
          return (
            <Box key={plugin.id} marginBottom={0}>
              ...
              {/* Name + version */}
              <Box width={24} flexShrink={0}>
                <Text bold={isSelected} color={isSelected ? theme.kosaxCore : theme.text}>
                  {isSelected ? '› ' : '  '}
                  {plugin.name}
                </Text>
