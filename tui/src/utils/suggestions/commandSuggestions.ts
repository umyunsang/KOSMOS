...
// KOSAX P0-2: single-stack slash suggestions — only catalog commands are shown.
...
const KOSAX_CITIZEN_COMMAND_NAMES: ReadonlySet<string> = new Set(
...
/**
 * Filter a commands array to only those in the KOSAX citizen catalog.
 ...
 */
function filterToKosaxCommands(commands: Command[]): Command[] {
  return commands.filter(cmd => {
    const name = getCommandName(cmd)
    // Catalog contains multi-word entries like "consent list" (normalized below)
    const base = name.split(' ')[0]!
    return KOSAX_CITIZEN_COMMAND_NAMES.has(name) || KOSAX_CITIZEN_COMMAND_NAMES.has(base)
  })
}
...
export function generateCommandSuggestions(
  ...
  // KOSAX P0-2: restrict dropdown to citizen-facing catalog entries only.
  // This prevents CC dev commands (/speckit-*, /add-dir, /doctor, /review …)
  // from appearing in the autocomplete dropdown.
  const citizenCommands = filterToKosaxCommands(commands)
