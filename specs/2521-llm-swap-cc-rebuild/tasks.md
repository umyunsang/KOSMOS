...
- [X] T002 [P] Capture current KOSAX file SHA-256 baselines (`tui/src/services/api/claude.ts`, `tui/src/ipc/llmClient.ts`, `src/kosax/llm/client.py`, `src/kosax/ipc/stdio.py`) into `specs/2521-llm-swap-cc-rebuild/parity-matrix.md` File-level rows under "current state SHA". Cites FR-001.
...
- [X] T007 Document the four 2026-05-01 fixes in `specs/2521-llm-swap-cc-rebuild/parity-matrix.md` Swap commit log section: (a) `_ensure_tool_registry` lazy-init in `src/kosax/ipc/stdio.py`; (b) `<turn_order>` section in `prompts/system_v1.md` + manifest SHA-256 update; (c) `enable_thinking=true` default in `src/kosax/llm/client.py:858`; (d) partial `chunk.thinking` plumbing in `tui/src/ipc/llmClient.ts` + `tui/src/ipc/llmTypes.ts`. Each entry classified per the four SWAP categories. Cites FR-003 + FR-010 + FR-011.
- [X] T008 Enumerate the CC stream-event channels at `.references/claude-code-sourcemap/restored-src/src/services/api/claude.ts:1980-2295` into `specs/2521-llm-swap-cc-rebuild/parity-matrix.md` Stream-event channel rows. Each row has CC line, event kind/subtype, intended KOSAX handler (TBD). Cites FR-002 + plan § Phase 0 R-2.
- [X] T009 Update `specs/2521-llm-swap-cc-rebuild/parity-matrix.md` Procedure-B per-handler rows with current handler positions in `tui/src/ipc/llmClient.ts`, `src/kosax/llm/client.py`, `src/kosax/ipc/stdio.py`. Cites FR-005.
...
- [ ] T011 [US1] Step B `SWAP/llm-provider`: Replace `@anthropic-ai/sdk` imports with KOSAX-aliased imports from `tui/src/sdk-compat.ts`. Cohesion-merged sub-checklist:
  - import `Anthropic` types → import `Kosax*` aliases from `sdk-compat.ts`
  - replace SDK call sites (`new Anthropic({...}).messages.stream(...)`) with calls to KOSAX IPC adapter at `tui/src/ipc/llmClient.ts`
  ...
  Commit subject: `swap/llm-provider(2521): route claude.ts through KOSAX IPC adapter`. Update parity-matrix Swap-commit log. Cites FR-002 + FR-006.
...
- [ ] T013 [US1] Step B `SWAP/identifier-rename`: Replace Claude/Anthropic/claude.ai brand tokens with KOSAX/EXAONE/FriendliAI tokens in `tui/src/services/api/claude.ts` (citizen-visible strings only — internal type names like `Anthropic` import paths stay sdk-compat-aliased per T011). Pure rename diff. Commit subject: `swap/identifier-rename(2521): KOSAX brand tokens in claude.ts`. Update parity-matrix. Cites FR-006.
...
- [X] T017 [P] [US1] Add `// SKIPPED — KOSAX-N/A: <reason>` comments in `tui/src/ipc/llmClient.ts` for each CC-stream-event channel KOSAX does not handle: `signature_delta` (K-EXAONE doesn't emit), `citations_delta` (KOSAX uses tool_result envelopes), `connector_text_delta` (Anthropic-only), `server_tool_use` (KOSAX uses IPC primitive bridge). One SKIPPED comment per channel. Cites FR-002.

### Procedure-B — `src/kosax/llm/client.py` (citation-required)

- [X] T018 [P] [US1] Add `CC reference: services/api/claude.ts:1980-2295` citation block to `_stream_response` docstring in `src/kosax/llm/client.py`. Document the FriendliAI OpenAI-compat → AssistantChunkFrame mapping. Cite reasoning_content branch (line 788) explicitly to `services/api/claude.ts:2148`. Cites FR-005 + FR-007.
...
### Procedure-B — `src/kosax/ipc/stdio.py` (citation-required)

- [X] T020 [P] [US1] Add `CC reference: QueryEngine.ts (whole) + query.ts:120-410` citation block to `_handle_chat_request` in `src/kosax/ipc/stdio.py`. Document the per-turn agentic-loop pattern: fresh `message_id`, structured `tool_calls` dispatch, `role="tool"` injection, max_turns termination. Cites FR-005 + FR-008.
- [X] T021 [P] [US1] Add `CC reference: services/tools/toolOrchestration.ts:19-72 (runTools)` citation to `_dispatch_primitive` in `src/kosax/ipc/stdio.py`. Note the partition policy divergence (KOSAX = all-parallel via `asyncio.gather`; CC = partitioned by `concurrencySafe`). Cites FR-005 + Deferred Item "partitioning".
- [X] T022 [P] [US1] Add `CC reference: (no direct CC analog — KOSAX-only IPC adaptation)` annotation to `_ensure_tool_registry` in `src/kosax/ipc/stdio.py`. Document the `SWAP/llm-provider` justification: CC assumes registry populated at SDK construction; KOSAX populates lazily on first IPC dispatch via `register_all_tools`. Cites FR-005.
...
- [ ] T026 [US2] Implement `scripts/llm_swap_parity_audit.sh` byte-copy SHA verification (Procedure-A files): parse parity-matrix Step-A commit SHAs; for each, run `git show <byte-copy-sha>:<kosax-path> | sha256sum`; compare with CC source SHA. Mismatch → mark `byte_copy_sha_match=false` + add to outcome. Cites FR-004.
...
- [ ] T028 [US2] Implement audit script unjustified-hunk detection: compute `git diff <byte-copy-sha>..HEAD -- <kosax-path>` per Procedure-A file; for each hunk, find the SwapCommit that introduced it (via `git log -p --follow`). Hunks not owned by any SwapCommit → add to `unjustified_hunks`. Cites FR-004.
- [ ] T029 [US2] Implement audit script Procedure-B citation verification: for each of `tui/src/ipc/llmClient.ts`, `src/kosax/llm/client.py`, `src/kosax/ipc/stdio.py`, grep for functions/methods (regex per language); for each, verify presence of `CC reference:\s+\S+:\d+(-\d+)?` pattern within the function body or docstring. Missing → add to `missing_cc_citations`. Cites FR-005.
- [ ] T030 [US2] Implement audit script CC stream-event channel enumeration: parse `.references/claude-code-sourcemap/restored-src/src/services/api/claude.ts:1980-2295` via `awk`/`grep` extracting `case '...'` blocks; for each channel, verify either KOSAX handler OR `// SKIPPED — KOSAX-N/A:` comment exists. Cites FR-002.
...
- [ ] T038 [P] [US4] Read `specs/2292-cc-parity-audit/cc-parity-audit.md`; extract every `cleanup-needed` entry whose `kosax_path` matches one of the 4 in-scope files. Record list in `specs/2521-llm-swap-cc-rebuild/parity-matrix.md` "Cleanup-needed inflight" section. Cites FR-007 + SC-006.
...
- [ ] T042 Populate `specs/2521-llm-swap-cc-rebuild/parity-matrix.md` Stream-event channel rows fully — every (TBD) replaced with concrete KOSAX handler path (file:line) or skip reason. Verify alignment with audit-script enumeration step from T030. Cites FR-003 + SC-002.
...
- [ ] T044 [P] Update `prompts/manifest.yaml` SHA-256 if `prompts/system_v1.md` was modified during this Epic; verify Spec 026 boot guard still passes (`uv run python -c "from kosax.context.prompt_loader import PromptLoader; PromptLoader(...)"` succeeds). Cites FR-011 + SC-010.
...
T002 [P] (current KOSAX SHA baseline) - parallel with T003-T006
