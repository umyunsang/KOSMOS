...
- [ ] T005 [US1] importer cleanup — T003 산출 importer 의 caller block 삭제 또는 KOSMOS 등가 호출 교체 (Constitution II 강제 — caller 도 함께 cleanup)
- [ ] T006 [US1] 잔재 16 파일 + `steps/` 디렉토리 deletion — `git rm src/kosmos/permissions/{aal_backstop,adapter_metadata,bypass,cli,credentials,killswitch,mode_bypass,mode_default,models,modes,pipeline,pipeline_v2,prompt,rules,session_boot,synthesis_guard}.py && git rm -r src/kosmos/permissions/steps/`
- [ ] T007 [US1] `src/kosmos/permissions/__init__.py` 정정 — 잔재 모듈 export 줄 모두 제거; Spec 035 receipt 모듈만 export (8 파일)
- [ ] T008 [US1] Constitution II grep gate — `grep -rE 'pipa_class|auth_level|permission_tier|is_personal_data|is_irreversible|requires_auth|dpa_reference' src/kosmos/` 결과 0 행
