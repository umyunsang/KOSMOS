...
receipt under ``~/.ummaya/memdir/user/consent/<receipt_id>.json`` but never
called ``ummaya.permissions.ledger.append``. Receipts therefore had no
HMAC seal, no hash chain, no key_id — and the canonical Spec 033 PIPA
ledger at ``~/.ummaya/consent_ledger.jsonl`` did not even exist on first
boot. Similarly the Revoke path used an ad-hoc unsealed
``hashlib.sha256(json.dumps(entry))`` plus a parallel
``~/.ummaya/memdir/user/consent/ledger.jsonl`` file — entries forgeable.
...
from ummaya.permissions.action_digest import compute_action_digest, generate_nonce
from ummaya.permissions.ledger import append as ledger_append
...
def isolated_ledger(tmp_path: Path) -> tuple[Path, Path, Path]:
    """Spec 033 ledger triple in tmp_path — never touches ~/.ummaya."""
