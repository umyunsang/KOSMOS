...
**Project Type**: Single project (Python CLI + 모듈 라이브러리) — `src/ummaya/observability/` 아래에 OTel 레이어 확장.
...
src/ummaya/llm/client.py        # MODIFY — generate_stream 내 chat span 생성 + 429 counter
src/ummaya/tools/executor.py    # MODIFY — dispatch 내 execute_tool span 생성
src/ummaya/engine/query.py      # MODIFY — query() 진입점에 invoke_agent parent span 생성
...
**Structure Decision**: 단일 프로젝트 구조. 신규 모듈 3개(`tracing.py`, `otel_bridge.py`, `semconv.py`)를 기존 `src/ummaya/observability/` 패키지에 추가하여 **단일 진입점**(`src/ummaya/observability/__init__.py`)을 통해 노출. 기존 `event_logger.py`·`metrics.py`는 수정하지 않는다. 부착점 3개(`llm/client.py`, `tools/executor.py`, `engine/query.py`)에는 *optional* 의존(tracer 주입식)으로 추가하여, tracer가 없을 때 기존 동작 변화 없음.
