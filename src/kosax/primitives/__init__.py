...
"""KOSAX active primitive harness surface.
...
tool-call primitive until KOSAX has a real app runtime to own delivery.
"""
...
from kosax.primitives.submit import submit
from kosax.primitives.verify import verify
...
# hardcoded enumerations scattered in ``kosax.ipc.stdio``. The registry is
...
# from downstream callers (``from kosax.primitives import PRIMITIVE_REGISTRY``);
__all__ = [
    "lookup",
    "resolve_location",
    "submit",
...
# (``from kosax.primitives import PRIMITIVE_REGISTRY``) and the Spec 031
# active root verbs.
