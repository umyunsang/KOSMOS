...
"""REPL loop for the KOSAX CLI — citizen conversation interface."""
...
class REPLLoop:
    """Async REPL loop for citizen conversation with the KOSAX engine.
    ...
    - Session persistence via :class:`~kosax.session.manager.SessionManager`
    - Interrupt handling:
    ...
    async def run(self) -> None:
        ...
        while True:
            try:
                user_input = await prompt_session.prompt_async("KOSAX › ")
    ...
    async def _init_session(self) -> None:
        ...
        from kosax.session.manager import SessionManager  # noqa: PLC0415
