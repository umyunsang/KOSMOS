...
Port Claude Code 2.1.88's external PermissionMode spectrum (`default` / `plan` / `acceptEdits` / `bypassPermissions` / `dontAsk`) to UMMAYA's citizen-API harness, layer a persistent tri-state (`allow | ask | deny`) per-adapter rule store (Continue.dev-style `~/.ummaya/permissions.json`) on top, and anchor every data-access session to a hash-chained + HMAC-sealed PIPA consent ledger (`~/.ummaya/consent_ledger.jsonl`) whose record schema is Kantara Consent Receipt v1.1.0 + ISO/IEC 29184 notice-binding + UMMAYA scope extensions. Preserves Spec 024 `ToolCallAuditRecord` coupling via `consent_receipt_id` and Spec 025 V6 AAL backstop invariants — Permission v2 is a **layer on top** of those, never a replacement. Killswitch (FR-B01..B04) is NON-NEGOTIABLE per Constitution §II: `is_irreversible=True` adapters are never silent under any mode or rule, and `is_personal_data=True` + AAL≥2 calls are never allowed without a valid ledger receipt. Delivery factored into five parallel-safe workstreams (WS1 Mode engine + TUI keychord, WS2 Rule store + atomic write, WS3 Consent ledger + hash-chain, WS4 Killswitch + LLM synthesis guard, WS5 Integration with Spec 024/025/021) for Agent Teams dispatch at `/speckit-implement`. **Zero new runtime dependencies** (AGENTS.md hard rule) — stdlib `hashlib` / `hmac` / `secrets` / `json` / `pathlib` / `os.rename` only.
...
**Primary Dependencies**: `pydantic >= 2.13` (PermissionRule / ConsentDecision / ToolPermissionContext frozen models + discriminated unions, existing), `pydantic-settings >= 2.0` (env catalog — `UMMAYA_PERMISSION_*` knobs for timeout/TTL/key-path, existing), `opentelemetry-sdk` + `opentelemetry-semantic-conventions` (span attributes `ummaya.permission.mode` / `ummaya.permission.decision` / `ummaya.consent.receipt_id` from Spec 021, existing), `pytest` + `pytest-asyncio` (existing test stack). Stdlib `hashlib` (SHA-256 chain), `hmac` (HMAC-SHA-256 seal), `secrets` (key generation), `json` + `orjson`-free canonical JSON via **RFC 8785 JCS** (custom `ummaya.permissions.canonical_json` module, no external dep), `pathlib` + `os.rename` (atomic rule-store writes), `fcntl` (POSIX advisory lock on ledger append). **Zero new runtime dependencies** (AGENTS.md hard rule; SC-008 companion lint).
**Storage**:
- `~/.ummaya/permissions.json` — Continue.dev-style tri-state rule store, JSON object, schema-validated at boot, atomic writes via `tmpfile + os.rename`.
- `~/.ummaya/consent_ledger.jsonl` — append-only NDJSON, SHA-256 hash chain + HMAC-SHA-256 seal, WORM enforced in software (no update/delete API), POSIX advisory lock on append.
- `~/.ummaya/keys/ledger.key` — 32-byte HMAC secret, mode 0400, generated via `secrets.token_bytes(32)` on first boot, fail-closed on missing/wrong-mode.
...
**Project Type**: Library — new `src/ummaya/permissions/` package + new TUI `tui/src/permissions/` component tree. No net-new top-level packages outside these two.
...
| V | Policy Alignment | PIPA §15(2) 4-tuple enforced at prompt UI (FR-D03 + SC-006); §18(2) purpose-limitation enforced via FR-D08 re-prompt trigger; §22(1) individual-consent enforced via FR-D07; §26 controller/processor role per MEMORY `project_pipa_role` — UMMAYA = 수탁자 default, LLM synthesis = controller-level carve-out (FR-E01). AI 기본법 §27 high-impact AI safeguards via FR-E02 (banner + `/escalate` + explainability). Korea AI Action Plan Principle 5/8/9 preserved. ISMS-P 2.9.4 + 안전성 확보조치 §8 two-year retention encoded in FR-D10. | PASS |
...
│   ├── permissions-store.schema.json     # JSON Schema 2020-12 for ~/.ummaya/permissions.json
...
│   ├── ledger-verify.cli.md              # `ummaya permissions verify` CLI contract
...
```text
src/ummaya/permissions/                             # NEW package (WS1..WS5)
...
└── cli.py                                          # WS3 — `ummaya permissions verify` + `/permissions` backend
...
| Rule-store schema | [`contracts/permissions-store.schema.json`](./contracts/permissions-store.schema.json) | JSON Schema 2020-12 for `~/.ummaya/permissions.json` |
...
| Verify CLI contract | [`contracts/ledger-verify.cli.md`](./contracts/ledger-verify.cli.md) | `ummaya permissions verify` exit-code / output / failure-mode contract |
