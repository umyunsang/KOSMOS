...
- **Fully delivered** by the closed Epic's ship (e.g., `PermissionGauntletModal.tsx` delivered by #1441):
  - `Verdict = PORT`, `Owning Epic = <id> #<issue> (closed)`, `Rationale = "implementation complete; delivered by #<PR>"`, `Task sub-issue = —`, `UMMAYA target = tui/src/components/coordinator/PermissionGauntletModal.tsx`.
...
| `skills` | DISCARD (domain mismatch) | skill authoring is for CC users; UMMAYA adapters live server-side in Python |
