...
| kosax-only         | {actual['kosax-only']['actual']} | {actual['kosax-only']['baseline']} | {actual['kosax-only']['delta']:+d} |
...
"""
    ...
    transfer_section += "Suspicious 분류 0 건 — audit 결과 모든 modified 파일에 명확한 KOSAX 정당화 또는 알려진 Spec 1633 잔재. 후속 Epic 진입을 위한 transfer 는 다음과 같다:\n\n"
    ...
    spot_section += md_table_from_entries(
        spot_rows, ["idx", "kosax_path", "match", "sha256 (prefix)"]
    )
    spot_section += "\n\n</details>\n\n"
    ...
    import_section += md_table_from_entries(
        imp_rows, ["kosax_path", "verdict", "first import line changed"]
    )
    import_section += "\n\n</details>\n\n"
    ...
    repro_section += "```bash\n"
    repro_section += "cd /Users/um-yunsang/KOSAX  # or your repo root\n"
    ...
    exit_section += "- **Epic β #2293 (KOSAX-original UI residue cleanup)**: 진입 가능. 30 Cleanup-needed 항목이 task 입력 — 그 중 15개는 `tui/src/services/api/*` (claude.ts dispatcher Spec 1633 closure), 8개는 `queryHaiku` callsite, 3개는 `utils/permissions/*` (Spec 033 잔재).\n"
    ...
    exit_section += "- **Epic δ #2295 (Backend permissions/ cleanup)**: 본 audit (TUI-only) 산출이 직접 transfer 항목 0 건 — Epic δ 는 `src/kosax/permissions/` 백엔드 audit 별도 필요 (Out of Scope Permanent of this Epic α).\n"
