...
**Hash module**: `src/ummaya/plugins/canonical_acknowledgment.py` exports `CANONICAL_ACKNOWLEDGMENT_SHA256: str` computed at import time.
...
- **Hash exposure**: the canonical hash is published verbatim at the top of `docs/plugins/security-review.md` (e.g., "Current canonical SHA-256: `abc123...`") and printed by `ummaya plugin init` immediately before the contributor confirms acknowledgment.
...
- Memory `project_pipa_role` (UMMAYA PIPA stance — trustee by default, controller carve-out only at LLM synthesis step)
