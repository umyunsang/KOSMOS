...
"""Python entry-point fallback for ``ummaya plugin init``.
...
This module exists so non-TUI users can scaffold a plugin via:
    ...
    uvx ummaya-plugin-init <name> --tier live --layer 1 --no-pii
...
def _manifest_dict(opts: InitOptions) -> dict[str, object]:
    ...
    # ummaya.tools.policy_derivation. Layer 1 (green) maps to "read-only" gate → AAL1.
    ...
    return {
        ...
        "dpa_reference": None,
        "slsa_provenance_url": (
            f"https://github.com/ummaya-plugin-store/ummaya-plugin-{opts.name}/"
...
version = "0.1.0"
description = "UMMAYA plugin scaffolded by 'uvx ummaya-plugin-init'"
...
def _build_tool() -> Any:
    ...
    # policy.citizen_facing_gate via ummaya.tools.policy_derivation.
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="ummaya-plugin-init",
        description=(
            "Scaffold a UMMAYA plugin (Python entry-point fallback for the TUI command). "
