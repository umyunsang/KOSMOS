...
Routes incoming :class:`~kosax.ipc.frame_schema.PluginOpFrame` frames with
``op="request"`` to one of three backend handlers:

- ``request_op="install"`` → :func:`handle_install` → wraps
  :func:`kosax.plugins.installer.install_plugin` with a progress-emitter
...
- ``request_op="uninstall"`` → :func:`handle_uninstall` → mirrors the install
  flow with the 3-phase :func:`kosax.plugins.uninstall.uninstall_plugin`.
...
async def handle_plugin_op_request(
    ...
    *,
    ...
    Args:
        ...
        registry: :class:`kosax.tools.registry.ToolRegistry` (typed loosely
            to avoid import-time circular dependency at the dispatcher seam).
        executor: :class:`kosax.tools.executor.ToolExecutor`.
        write_frame: Async callable that serialises an :class:`IPCFrame` and
            writes it to stdout (existing :func:`kosax.ipc.stdio.write_frame`).
        consent_bridge: :class:`kosax.plugins.consent_bridge.IPCConsentBridge`
    ...
    Raises:
        ValueError: When ``frame.op`` is not ``"request"`` or ``frame.request_op``
            ...
            :func:`kosax.ipc.stdio` ErrorFrame fanout.
    """
    if frame.op != "request":
...
async def handle_install(
    ...
    *,
    ...
    """
    from kosax.plugins.installer import install_plugin  # noqa: PLC0415
...
async def handle_uninstall(
    ...
    *,
    ...
    from kosax.plugins.uninstall import uninstall_plugin  # noqa: PLC0415
