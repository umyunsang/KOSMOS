...
Verifies:
  ...
  5. UMMAYA_PERMISSION_KEY_REGISTRY_PATH env var is picked up by UmmayaSettings.
"""
...
from ummaya.permissions.hmac_key import HMACKeyFileModeError, load_or_generate_key
...
def test_boot_time_log_emitted(tmp_path: Path, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="ummaya.permissions.hmac_key"):
        load_or_generate_key(key_path)
...
# ---------------------------------------------------------------------------
# UmmayaSettings picks up UMMAYA_PERMISSION_KEY_REGISTRY_PATH
# ---------------------------------------------------------------------------


def test_settings_key_registry_path_env(tmp_path: Path, monkeypatch):
    """UMMAYA_PERMISSION_KEY_REGISTRY_PATH overrides the default registry path."""
    custom_path = tmp_path / "custom_registry.json"
    monkeypatch.setenv("UMMAYA_PERMISSION_KEY_REGISTRY_PATH", str(custom_path))
...
def test_settings_key_registry_path_default():
    """Default UMMAYA_PERMISSION_KEY_REGISTRY_PATH is ~/.ummaya/keys/registry.json."""
    from ummaya.settings import UmmayaSettings

    s = UmmayaSettings()
    expected = Path.home() / ".ummaya" / "keys" / "registry.json"
