...
**License**: Public standard (KFTC / FSS regulatory publication); UMMAYA mock is an independent re-implementation
...
KFTC publishes numbered revisions of the MyData API standard. The `spec_version` in `tests/fixtures/mydata/meta.json` is the canonical revision marker. UMMAYA CI does not automatically detect standard updates — the engineering team must monitor KFTC release notices at mydatacenter.or.kr. When the spec version changes, treat it as a `mock-drift` event and review all affected endpoint shapes.
