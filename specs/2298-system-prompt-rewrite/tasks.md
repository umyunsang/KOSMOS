...
- [x] T008 (#2454) [US1] Run boot fail-closed verification: `uv run python -c "from kosmos.context.prompt_loader import PromptLoader; PromptLoader.from_manifest('prompts/manifest.yaml'); print('OK')"` → must print `OK` and exit 0. If `PromptManifestIntegrityError` raises, T006 was performed incorrectly — re-run.
