...
**Storage**: N/A at feature layer. Session JSONL stays in `~/.ummaya/memdir/user/sessions/` (Spec 027); OTEL spans flow via Spec 028 OTLP collector to local Langfuse. No schema changes.
...
**Project Type**: Bi-lingual monorepo — TS TUI (`tui/`) + Python backend (`src/ummaya/`). No new project directories.
...
│   └── llmTypes.ts                # [NEW] Ummaya* type surface (contract § 2)
...
└── entrypoints/
    └── init.ts                    # [EDIT] replace initializeTelemetryAfterTrust with UMMAYA OTEL init
```
...
- `docs/requirements/ummaya-migration-tree.md § L1-A · § Execution Phase`
