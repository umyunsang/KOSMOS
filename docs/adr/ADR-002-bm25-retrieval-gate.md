...
2. **Pure-Python, zero system deps**: KOSAX CI runs under `uv`; any library
...
4. **30-query eval gate** (`eval/retrieval_queries.yaml`): 30 manually curated
   ...
   at 4K-tool scale; anything worse at KOSAX's smaller scale indicates a
...
- Apache-2.0 (`rank_bm25`) and MIT (`kiwipiepy`) licenses are compatible with
  KOSAX's Apache-2.0 project license.
...
- `src/kosax/tools/bm25_index.py` — BM25Okapi wrapper with deterministic tie-break
- `src/kosax/tools/tokenizer.py` — kiwipiepy thin wrapper with ASCII fast path
