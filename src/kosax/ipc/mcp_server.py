...
"""stdio-MCP server stub for the KOSAX tool surface (Spec 1634 FR-021).
...
live in :mod:`kosax.ipc.stdio` and are reused by the main KOSAX harness
...
to consume the KOSAX tool registry via MCP.
...
- ``tools/list`` returns an empty list → the client MUST treat as a failure;
  ...
  which is caught earlier by :func:`kosax.tools.register_all.register_all_tools`
...
from kosax.ipc.adapter_manifest_emitter import emit_manifest
...
from kosax.tools.routing_index import RoutingIndex

logger = logging.getLogger(__name__)

MCP_PROTOCOL_VERSION = "2025-06-18"
"""The MCP protocol version KOSAX speaks. Bump in lockstep with ``mcp.ts``
behind a single ADR per plan.md § Principle I reference mapping."""

SERVER_NAME = "kosax-backend"
...
class MCPServer:
    """Thin MCP-over-stdio server backed by the KOSAX tool registry."""
    ...
    async def _tools_call(self, rpc_id: Any, params: dict[str, Any]) -> dict[str, Any]:
        ...
        # primitive layer (kosax.primitives.*) for submit/verify
