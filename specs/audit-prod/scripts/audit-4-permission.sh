...
# ---------------------------------------------------------------------------
# Stage 2 — verify primitive (Layer ⓵ — should mount KosaxPrimitive Layer1)
...
# Extract the actual receipt_id from the kosax memdir, then attempt revoke.
# We do this AFTER the prior snapshots so the output is clean.
LATEST_RECEIPT="$(ls -t ~/.kosax/memdir/user/consent/*.json 2>/dev/null \
