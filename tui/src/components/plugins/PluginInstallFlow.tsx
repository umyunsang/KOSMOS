...
//
// Spec 1979 — KOSAX citizen plugin install/uninstall/list flow component.
// Audit-6 P1 fixes:
...
//
// KOSAX-internal Y/N/A direct-keystroke pattern (earlier dev iteration) is
...
import { getKosaxBridgeSessionId, getOrCreateKosaxBridge } from '../../ipc/bridgeSingleton.js';
...
function _exitCodeKo(exitCode: number | undefined | null, errorKind?: string | null): string {
  ...
  if (errorKind === 'slsa_skip_in_production') return 'KOSAX_PLUGIN_SLSA_SKIP은 production 환경에서 거부됩니다';
  ...
  return (
    <Box flexDirection="column" paddingX={1} paddingY={1}>
      ...
      {state.kind === 'completed' ? (
        <Box flexDirection="column">
          <Text color={theme.kosaxCore}>{state.summary}</Text>
