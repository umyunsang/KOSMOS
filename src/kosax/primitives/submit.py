...
"""T021/T022/T023 — ``submit`` primitive for the KOSAX Five-Primitive Harness.
...
exclusively in adapter modules under ``src/kosax/tools/mock/<ministry>/``.
...
T023 — deterministic ``transaction_id`` derivation:
    ``urn:kosax:submit:`` + SHA-256 over canonical JSON of
    ``{tool_id, params (sorted keys), adapter_nonce}``.
    Same inputs always produce the same URN. The ``adapter_nonce`` is sourced
    from :attr:`kosax.tools.registry.AdapterRegistration.nonce` so the
...
``kosax.security.audit.ToolCallAuditRecord`` shape-construction.
...
from kosax.primitives._errors import AdapterInvocationError, AdapterNotFoundError
...
def register_submit_adapter(registration: AdapterRegistration, invoke_fn: Any) -> None:
    ...
    ``src/kosax/tools/mock/data_go_kr/fines_pay.py``) as their last statement.
    """
...
def derive_transaction_id(
    ...
    *,
    ...
    """Derive a deterministic ``urn:kosax:submit:<sha256>`` transaction_id.
    ...
    Returns:
        A ``urn:kosax:submit:<sha256-hex>`` string (70 chars fixed length).
    """
    ...
    return f"urn:kosax:submit:{digest}"
...
async def submit(
    ...
    *,
    ...
    tracer = trace.get_tracer("kosax.primitives.submit")
    ...
    with tracer.start_as_current_span("gen_ai.tool_loop.iteration") as span:
        ...
        span.set_attribute("kosax.submit.session_id", session_id)
        ...
        span.set_attribute("kosax.submit.transaction_id", transaction_id)
        ...
        if isinstance(result, SubmitOutput):
            ...
            span.set_attribute("kosax.submit.status", result.status.value)
            return result
