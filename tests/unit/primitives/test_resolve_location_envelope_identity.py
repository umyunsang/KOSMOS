...
1. FR-017 (byte-identity): ``ummaya.primitives.resolve_location`` is the *same
   coroutine* as ``ummaya.tools.resolve_location.resolve_location`` — no wrapper
...
2. T031 (re-export contract): ``ummaya.primitives.__init__`` does NOT introduce
...
# ---------------------------------------------------------------------------
import ummaya.primitives as _primitives_module
...
from ummaya.tools.resolve_location import resolve_location as _canonical_rl
...
def test_primitives_resolve_location_is_same_object_as_canonical() -> None:
    ...
    assert _primitives_rl is _canonical_rl, (
        "ummaya.primitives.resolve_location must be the identical object to "
        "ummaya.tools.resolve_location.resolve_location — any wrapping would "
        "break byte-identity."
    )
...
def test_primitives_resolve_location_preserves_dunder_module() -> None:
    ...
    assert "ummaya.tools.resolve_location" in _primitives_rl.__module__
