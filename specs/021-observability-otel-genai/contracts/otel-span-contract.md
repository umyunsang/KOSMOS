...
This contract is the **observable agreement** between KOSAX and any OTLP-compatible backend (Langfuse v3, Datadog, Phoenix, Uptrace). Changing any name, attribute key, or status mapping in this document is a breaking change to downstream dashboards and requires a new spec.
...
## Span 1 — `invoke_agent kosax-query`
...
| `gen_ai.agent.name` | str | `kosax-query` |
...
**Retry rule**: HTTP 429 retries inside a single logical call remain within the **same** `chat` span. Do not create per-attempt spans. Each retry increments `kosax_llm_rate_limit_retries_total{provider, model}` (see Metrics).
...
| `gen_ai.tool.type` | str | `function` (all KOSAX tools are function-call) |
...
## Metric — `kosax_llm_rate_limit_retries_total`
...
Any `dict`-shaped metadata passing through `otel_bridge.filter_metadata` is prefiltered against the canonical frozenset imported from `src/kosax/observability/event_logger.py`:
