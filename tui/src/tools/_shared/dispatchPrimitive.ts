...
// KOSAX-original — Epic ζ #2297 Phase 0b · T008
//
...
const _tracer = trace.getTracer('kosax.tui.primitive', '0.1.0')
...
function _resolveTimeoutMs(override?: number): number {
  ...
  const env = process.env['KOSAX_TUI_PRIMITIVE_TIMEOUT_MS']
  if (env) {
...
export interface DispatchPrimitiveOpts {
  ...
  /** IPC bridge (injected). */
  bridge: IPCBridge
  /** Default 30_000 ms; env KOSAX_TUI_PRIMITIVE_TIMEOUT_MS overrides. */
  timeoutMs?: number
}
...
function _maybeEmitCheckpoint(
  ...
  if (process.env['KOSAX_SMOKE_CHECKPOINTS'] !== 'true') return
 ...
 *           OTEL span attribute `kosax.tui.primitive.timeout=true`.
...
export async function dispatchPrimitive<O = unknown>(
  ...
  // Step 1: OTEL span
  // ------------------------------------------------------------------
  const span = _tracer.startSpan(`kosax.tui.primitive.${opts.primitive}`, {
    attributes: {
      'kosax.tui.primitive.name': opts.primitive,
      'kosax.tui.primitive.timeout_ms': timeoutMs,
    },
  })
  ...
  if (!toolUseId) {
    span.setAttribute('kosax.tui.primitive.error', 'missing_tool_use_id')
    span.end()
    return {
      data: {
        ok: false as const,
  ...
  span.setAttribute('kosax.tui.primitive.dispatch_mode', 'register-and-await')
  span.setAttribute('kosax.tui.primitive.tool_use_id', toolUseId)
  ...
  try {
    ...
    span.setAttribute('kosax.tui.primitive.timeout', true)
    span.end()
    return {
      data: {
        ok: false as const,
  ...
  // backend envelope shape (src/kosax/ipc/stdio.py:1115):
  ...
  span.setAttribute(
    'kosax.tui.primitive.envelope_kind',
  ...
  // ``VerifyMismatchError`` from ``kosax.primitives.verify`` is dumped as
  ...
  // (populated by ``kosax.tools._outbound_trace`` on the backend) so the
