...
| **P0-2** | CRITICAL | Backend gate | **Allow path bypasses Spec 033 ledger entirely.** `_check_permission_gate` (stdio.py:1539-1559) writes a JSON receipt file but never calls `ummaya.permissions.ledger.append()`. No HMAC seal, no SHA-256 chain, no key_id, no fcntl lock, no WORM mode. PIPA §22-2 audit-trail obligation violated. | After full scenario: `~/.ummaya/consent_ledger.jsonl` does not exist; `~/.ummaya/keys/registry.json` does not exist; `~/.ummaya/permissions.json` does not exist; only the pre-existing 2026-04-22 onboarding receipt is on disk. |
| **P0-3** | CRITICAL | Backend revoke | **Revoke path uses ad-hoc hashlib SHA-256 instead of canonical ledger.** `_handle_consent_revoke_request` (stdio.py:3192-3221) writes to `~/.ummaya/memdir/user/consent/ledger.jsonl` (NOT the canonical Spec 033 path) with no HMAC seal, no prev_hash chain, no key_id, no fcntl lock — entries are forgeable in plain text. | stdio.py:3200 `record_hash = _hashlib.sha256(ledger_json.encode("utf-8")).hexdigest()` — no `ummaya.permissions.ledger` import in the entire file. |
...
| # | Severity | Surface | Finding |
|---|---|---|---|
| P2-1 | MED | i18n | `permissionEn` strings exist but are unreachable from `UMMAYA_TUI_LOCALE` because the env var is read once at module load (UmmayaPrimitivePermissionRequest.tsx:56) — locale switch requires a TUI restart. |
...
| permission_request → UmmayaPrimitivePermissionRequest mount | via toolUseConfirmQueue | YES — visible | ✅ |
...
| receipt JSON write | `~/.ummaya/memdir/user/consent/<id>.json` | YES (when allowed) | ✅ |
...
- `UmmayaPermissionRequestAdapter.handleDecision('deny')` calls `toolUseConfirm.onReject()` then `onReject()` then `onDone()`.
...
Citizen-facing fallback wording would depend on the LLM's prompt; **no deterministic guidance is shown to the citizen by UMMAYA itself** (no toast, no banner, no "alternative options" panel).  Production should add a Korean wording fallback (e.g. "권한이 거부되어 작업을 중단했습니다. 다른 방법을 시도하려면 …") in the message timeline at deny-time, independent of the LLM.

## Summary verdict

**PRODUCTION READY: NO.**

The Permission Gauntlet is the **single most safety-critical surface** in UMMAYA — it is the boundary between a citizen's request and a side-effecting / irreversible call against a Korean public-service module.  The current build:
...
2. P0-2 + P0-3 fix (allow / deny / withdraw all flow through `ummaya.permissions.ledger.append`).
...
UMMAYA_BACKEND_LOG_FILE=/tmp/audit-4-bun.log \
