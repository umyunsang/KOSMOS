...
submit / verify primitive dispatcher in :mod:`kosax.ipc.stdio` opens a trace
...
_outbound_traces_var: ContextVar[list[OutboundCallTrace] | None] = ContextVar(
    "kosax_outbound_traces", default=None
)
...
def _record_request_start(request: httpx.Request) -> None:
    ...
    request.extensions["_kosax_t_start"] = time.perf_counter()
...
def _emit_trace(request: httpx.Request, response: httpx.Response) -> None:
    ...
    t_start = request.extensions.get("_kosax_t_start")
