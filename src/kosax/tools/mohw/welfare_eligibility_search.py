...
Evidence: /tmp/kosax-evidence/koroad-mohw-evidence.md (lifeArray=007 → 21 services live).
...
from kosax.tools.errors import ToolExecutionError, _require_env
from kosax.tools.models import AdapterRealDomainPolicy, GovAPITool
from kosax.tools.mohw._short_references import (
...
def _parse_xml_response(xml_bytes: bytes) -> dict[str, Any]:
    ...
    Live SSIS envelope (verified 2026-05-04 with KOSAX_DATA_GO_KR_API_KEY):
        <wantedList>
...
async def handle(
    ...
    *,
    ...
    Raises:
        ConfigurationError: If KOSAX_DATA_GO_KR_API_KEY is not set.
