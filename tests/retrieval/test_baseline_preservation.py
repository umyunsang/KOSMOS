...
Programmatic invocation of ``kosax.eval.retrieval._build_registry()`` and
``_evaluate()`` against the committed 30-query set with
``KOSAX_RETRIEVAL_BACKEND`` unset (pure BM25 default path).
...
def test_recall_at_5_is_perfect(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.delenv("KOSAX_RETRIEVAL_BACKEND", raising=False)

    import yaml

    from kosax.eval.retrieval import _build_registry, _evaluate
