...
"""Full-backend channel probe — spawns the actual KOSAX stdio backend, sends
...
async def main() -> None:
    ...
    env = dict(os.environ)
    env["KOSAX_K_EXAONE_THINKING"] = enable_thinking
    ...
    tools = [
        {
            ...
            "function": {
                "name": n,
                "description": f"KOSAX {n} primitive (probe stub)",
    ...
    print(f"KOSAX_K_EXAONE_THINKING={env.get('KOSAX_K_EXAONE_THINKING')}")
    print("=" * 78)
