...
**Before** (lines 10-19):
```ts
// UMMAYA Spec 1633 / Epic #2293 — utils/mcp/dateTimeParser deleted (Anthropic
...
const parseNaturalLanguageDateTime = async (value: string, _formatHint?: string): Promise<DateParseResult> =>
  looksLikeISO8601(value)
    ? { success: true, value }
    : { success: false, error: 'UMMAYA: natural-language datetime parsing not available; please use ISO 8601 format (e.g. 2026-04-29T12:00:00Z)' }
```
...
**Callsite** (line 323-328) `parseNaturalLanguageDateTime(stringValue, schema.format, signal)` — already CC-shape compatible (3-arg form). UMMAYA-side currently uses `_formatHint` param name (CC: `format`), so the callsite signature works without further edit.
