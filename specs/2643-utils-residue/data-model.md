...
**Source-of-truth**: KOSAX-side stub (CC's interior is Spec 1633 deletion target; the type *shape* is preserved for callsite stability).
...
**Validation rules**:
- Always returned with `unavailable: true, shouldBlock: false` (KOSAX auto-mode = no-op per Spec 1633).
...
**Note**: This helper is also re-imported by CC's `commands/rename/generateSessionName.ts` — KOSAX keeps that helper inlined locally (Spec 1633 deletion of `generateSessionName.ts` itself), so KOSAX only needs one copy here.
...
**Behavior contract**: always returns `{unavailable: true, shouldBlock: false}` (KOSAX auto-mode = no-op per Spec 1633). Promise resolves synchronously without awaiting any I/O.
...
**Behavior contract**: always returns empty string `''`. CC signature compatibility only — not invoked for any meaningful classification in KOSAX.

## Migration Map (caller stability invariant)

| Caller | Before (KOSAX main) | After (Epic G) | Behavior |
|---|---|---|---|
...
All 6 principles still PASS. No new types violate Pydantic v2 strict typing (TypeScript-only Epic). No KOSAX-invented permission classifications introduced (`YoloClassifierResult` is byte-identical CC shape preserved for callsite stability, not a new policy invention).
