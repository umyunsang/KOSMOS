...
// Emits ummaya.ui.surface=history (FR-037).
//
// Session data is loaded from ~/.ummaya/memdir/user/sessions/ (Spec 027).
...
import { getUmmayaSessionsDir } from '../utils/ummayaPaths.js';
import {
  applyHistoryFilters,
...
/**
 * Sentinel value written as the first line of UMMAYA JSONL session files by
...
function collectAllSessionJsonl(sessionsDirPath: string): Array<{ filePath: string; sessionId: string }> {
  ...
  for (const entry of entries) {
    ...
    if (st.isDirectory()) {
      // Sanitized-cwd subdirectory (e.g. `-Users-um-yunsang-UMMAYA-tui/`).
...
function loadSessionEntries(): SessionHistoryEntry[] {
  ...
  // Resolved lazily so UMMAYA_MEMDIR_USER overrides (tests/env) are respected.
  const sessionsDir = getUmmayaSessionsDir();
  ...
  for (const { filePath, sessionId } of candidates) {
    ...
    try {
      ...
      // Render `ummaya session gc-stubs` would purge them — until then we
