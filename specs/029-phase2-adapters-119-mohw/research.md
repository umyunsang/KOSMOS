...
|---|---|---|
| Interface-only adapter pattern (stub handler + `Layer3GateViolation`) | NMC reference adapter | `src/ummaya/tools/nmc/emergency_search.py` |
| Full-HTTP adapter pattern (httpx client, envelope parsing, fail-closed) | HIRA reference adapter | `src/ummaya/tools/hira/hospital_search.py` |
| `GovAPITool` V1–V6 validator contract | Tool models | `src/ummaya/tools/models.py` §V1–V6 |
| `TOOL_MIN_AAL` drift gate | Security audit table | `src/ummaya/security/audit.py` |
| `LookupError` / `LookupCollection` envelopes | Envelope module | `src/ummaya/tools/envelope.py`, `errors.py` |
| XML/JSON dual-format upstream handling | HIRA Content-Type guard | `src/ummaya/tools/hira/hospital_search.py` |
...
| Layer 3 auth-gate contract (FR-025, FR-026, SC-006) | Executor short-circuit | `src/ummaya/tools/executor.py` (invoke) |
...
- `MohwWelfareEligibilitySearchInput` uses three `str | None` fields
  ...
  `src/ummaya/tools/ssis/codes.py`.
...
1. Validator V2 (`src/ummaya/tools/models.py` §V2) enforces the pattern
...
calls to SSIS, a DPA template document governing UMMAYA's §26 수탁자 relationship
...
The current `TOOL_MIN_AAL` table (`src/ummaya/security/audit.py`) lists only
