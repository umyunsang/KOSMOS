# S8 — State / Boot / Misc 감사 (CC → KOSAX 마이그레이션)
...
> **KOSAX target**: `tui/src/` + `src/kosax/`
> **Frame of reference**: KOSAX = CC + 2 swaps (LLM=K-EXAONE on FriendliAI, Tool=한국 부처 GovAPITool). S8 슬라이스는 swap 외 영역 — **대부분 PRESERVE-IDENTICAL** 이어야 정상.
...
| **C. KOSAX-ORIGINAL-justified** | KOSAX-only 신규 파일, swap 종속물 (i18n, brand, GovAPI, 5-step onboarding, K-EXAONE) | UI L2 / L1-B/C cite |
...
| Group | CC 개수 | KOSAX 개수 | 비고 |
|---|---|---|---|
| bootstrap | 1 | 1 | 매핑 1:1 |
| cli (top-level) | 7 | 6 | **`print.ts` 누락** (Spec 1633 명시 삭제 — main.tsx L1960 가 차단 메시지로 대체) |
| cli/handlers | 6 | 6 | 매핑 1:1 |
| cli/transports | 7 | 8 | KOSAX-only `Transport.ts` (인터페이스 추출, +1) |
| entrypoints | 5 | 6 | KOSAX-only `envGuard.ts` (Spec 1633 fail-closed FRIENDLI_API_KEY 게이트) |
| entrypoints/sdk | 3 | 9 | KOSAX-only `controlTypes.ts`/`coreTypes.generated.ts`/`runtimeTypes.ts`/`sdkUtilityTypes.ts`/`settingsTypes.generated.ts`/`toolTypes.ts` (+6) |
| memdir | 8 | 11 | KOSAX-only `consent.ts`/`io.ts`/`ministry-scope.ts` (+3, UI L2 동의 영수증) |
| state | 6 | 6 | 매핑 1:1 |
| context | 9 | 10 | KOSAX-only `PermissionReceiptContext.tsx` (+1, UI L2 영수증) |
| migrations | 11 | 0 | **전체 삭제** — 모두 Anthropic 모델/설정 마이그레이션 (Sonnet45→46, Opus, Fennec 등). main.tsx L181-185 명시 |
| voice | 1 | 1 | 매핑 1:1 |
| buddy | 6 | 6 | 매핑 1:1 |
| plugins | 2 | 2 | 매핑 1:1 |
| constants | 21 | 21 (≈) | KOSAX `oauth.ts` 누락 (CC 에는 있음) — KOSAX-only `querySource.ts` (+1) |
| schemas | 1 | 1+ui-l2/8 | UI L2 schemas 신규 (a11y, agent, error, onboarding, permission, slash-command, ufo) |
| types | 7 | 16 | KOSAX-only `connectorText.ts`/`fileSuggestion.ts`/`message.ts`/`messageQueueTypes.ts`/`notebook.ts`/`statusLine.ts`/`tools.ts`/`utils.ts` (+8 — CC 에서는 다른 위치에 흩어져 있던 type 들을 이 디렉토리로 통합한 변형으로 추정) |
| types/generated | 3 | 3 | 매핑 1:1 (단, **content 가 stub** — 아래 D. 회귀) |
| i18n | 0 | 5 | **KOSAX-ORIGINAL** (한국어 primary, swap 종속 정당) |
| Python `src/kosax/{memdir,observability,permissions,security}` | N/A | 30 + .py | **KOSAX-ORIGINAL** (백엔드 스택 — swap 의 server-side 대응) |
...
| **C. KOSAX-ORIGINAL-justified** | 24 | 22% |
| **D. REGRESSION** | 4 | 4% |
| **합계** | 108 | — |
...
| `entrypoints/cli.tsx` | +69 lines (boot tracer 삽입 only — CC 코드 100% 보존) | Spec 1978 T003 디버그 인프라 (KOSAX-ORIGINAL 추가, CC 본체 무손실) |
| `entrypoints/init.ts` | `policyLimits` + `remoteManagedSettings` import → no-op stub | Spec 1633 P1 — Anthropic enterprise feature dead |
| `entrypoints/agentSdkTypes.ts` | +7 lines: `HookEvent` + `SDKAssistantMessageError` 추가 only (CC 본체 보존) | swap 종속 (KOSAX hook event 표면) |
| `entrypoints/sdk/coreSchemas.ts` | 3 lines comment: `@anthropic-ai/sdk` → `KOSAX sdk-compat` 만 | sdk-compat alias |
| `cli/update.ts` | npm 패키지명: `@anthropic-ai/claude-cli` → `@kosax/tui` | brand swap |
...
| `migrations/*` (11 파일 전체 삭제) | Sonnet45→46, Opus, Fennec, Pro→Opus, autoUpdates, bypassPermissions, replBridge 등 모든 마이그레이션 제거 | **모두 Anthropic-only**. main.tsx L181-185 가 명시: "Update CURRENT_MIGRATION_VERSION when KOSAX-specific migrations needed". `CURRENT_MIGRATION_VERSION = 11` 그대로 유지하여 CC settings.json 호환 — 정당. |
...
| `constants/betas.ts` | CC 의 Anthropic beta header 상수 → KOSAX bridge stub (claude.ts 컴파일용 inert string) | Spec 2521 byte-copy bridge 정당 (단, 헤더 상수가 CC 와 다른 set 으로 교체됨) |

### 2.3 Bucket C — KOSAX-ORIGINAL-justified

| 파일 | 정당성 |
|---|---|
| `tui/src/i18n/{en.ts, ko.ts, keys.ts, index.ts, uiL2.ts}` | 한국어 primary 운영 — KOSAX swap 종속 (시민용 한국 행정 도구) |
...
| `tui/src/schemas/ui-l2/*.ts` (8 파일) | UI L2 새 표면 (a11y, agent, error, onboarding, permission, slash-command, ufo) — KOSAX-ORIGINAL Spec 035 + 1635 |
| `tui/src/cli/transports/Transport.ts` | KOSAX 가 추출한 Transport interface (CC 에서는 inline) — refactor 수준 |
| `tui/src/entrypoints/envGuard.ts` | FRIENDLI_API_KEY fail-closed gate (Spec 1633 T011) |
| `tui/src/entrypoints/sdk/{controlTypes.ts, coreTypes.generated.ts, runtimeTypes.ts, sdkUtilityTypes.ts, settingsTypes.generated.ts, toolTypes.ts}` | CC 의 `@anthropic-ai/sdk` 패키지가 export 하던 타입을 KOSAX-내부에서 재선언 — swap 의 dependency-removal 일부 (정당, 단 **D-2 의심** — 일부는 단순 누락 가능) |
...
| `tui/src/constants/querySource.ts` | KOSAX query source 상수 (Spec 2521 LLM swap 종속) |
| `src/kosax/memdir/*.py` (758 LOC) | Python 백엔드 — UI L2 동의 영수증 / ministry scope / onboarding 이벤트의 server-side 저장 (Spec 027 + Spec 035) |
| `src/kosax/observability/*.py` (1011 LOC) | Python OTEL bridge / semantic conventions / metrics — Spec 021 + Spec 028 (KOSAX 4-tier OTEL) |
| `src/kosax/permissions/*.py` (2271 LOC) | Permission 시스템 server-side — Spec 024 / 025 / 033 (HMAC ledger, canonical JSON, action digest) |
| `src/kosax/security/*.py` (455 LOC) | Audit + V12 dual-axis tool security spec — Spec 024 / 1979 |

### 2.4 Bucket D — REGRESSION (정당화 안 됨)

**D-1: `tui/src/types/generated/events_mono/*.ts` — 97% gutted to stub**

- `events_mono/claude_code/v1/claude_code_internal_event.ts`: **CC 865 LOC → KOSAX 21 LOC** (97% 손실)
- `events_mono/growthbook/v1/growthbook_experiment_event.ts`: 비슷한 비율로 stub
- `events_mono/common/v1/auth.ts`: **KOSAX 에서 디렉토리 자체 누락**
- KOSAX 헤더: `// KOSAX-1633 — protobuf events_mono stub.`
...
```ts
// constants/messages.ts (KOSAX) 1행:
// [P0 reconstructed · rebuild-stubs.ts · symbol-complete stub]
const __stub: any = new Proxy(function () {} as any, { ... });
export default __stub;
```

CC 의 `messages.ts` 는 단 1줄: `export const NO_CONTENT_MESSAGE = '(no content)'`. KOSAX 는 32줄짜리 Proxy stub 로 대체. 마찬가지로 `xml.ts` (CC 32 LOC of plain string consts → KOSAX 22 LOC of Proxy), `figures.ts` (CC 의 Unicode glyph 상수 vs KOSAX 의 stub-fallback), `types/logs.ts` (CC 의 `SerializedMessage` type vs KOSAX Proxy).
...
- main.tsx L1960 가 "KOSAX: --print / non-interactive (headless) mode is not supported" 메시지로 차단
- 정당성 cite: Spec 1633 / Epic #2293
- 그러나 `--print` (headless mode) 는 **swap 무관 기능** — CC harness 의 핵심 기능 중 하나 (CI/스크립트에서 LLM 출력만 stdout 으로). KOSAX 가 시민용이라도 headless 모드는 정책 분석 자동화에 필요.
...
| 섹션 (CC 라인) | 함수/구역 | KOSAX 처리 | 분류 |
|---|---|---|---|
| L1-70 | top-level imports + `feature()` 모듈 게이트 (KAIROS/COORDINATOR_MODE/TRANSCRIPT_CLASSIFIER 등) | 모두 삭제. KOSAX 는 `feature()` 호출 0건 | B (Spec 1633 cite) |
| L70-215 | teammate utils + 기타 module-level constants | 보존 (line 41-138 KOSAX) | A |
...
| L325-360 | `runMigrations()` | 보존, 본체는 `// If model-alias migrations are needed for KOSAX, add them here.` 주석으로 단순화 | B (모든 CC migration 이 Anthropic 모델용 → 정당) |
...
| L585-4611 | **`async function main()` 본체 — CC 4026 LOC, KOSAX 2356 LOC**  | 1670 LOC 삭제 | 혼합 (대부분 B, 일부 D-4 의심) |
| ↳ main 안의 onboarding | CC 의 1P `showSetupScreens` 호출 → KOSAX 의 5-step `OnboardingFlow` 추가 호출 (KOSAX-ORIGINAL +50 LOC) | C (UI L2) |
| ↳ main 안의 print mode | CC 의 `runPrint(..)` → KOSAX 의 차단 stderr 메시지 | D-3 |
| ↳ main 안의 file flag | CC 의 `--file` flow → KOSAX 의 unsupported 메시지 | B (Spec 1633) |
| ↳ main 안의 claude.ai MCP fetch / sessionDataUploader / fileDownloadPromise | KOSAX 에서 모두 nullified | B |
| ↳ main 안의 version 표시 | `MACRO.VERSION` → `${MACRO.VERSION} (KOSAX)` | C (brand) |
| L4611-4622 | `maybeActivateProactive()` | KOSAX 는 본체 비움 (`_options: unknown`) | D-4 의심 |
| L4622-4653 | `maybeActivateBrief()` | KOSAX 는 본체 비움 | D-4 의심 |
...
## 4. Python 백엔드 정당성 (`src/kosax/` 의 4 모듈)

S8 의 Python 측은 "TUI 는 client, Python 은 server" 이중 구조의 server-side 대응. 4 모듈 모두 KOSAX swap 의 server-side 표면으로 정당화됨.

| 모듈 | LOC | 정당성 cite |
|---|---|---|
| `memdir/` (consent_ledger, ministry_scope, user_consent, onboarding_events) | 758 | UI L2 (`docs/requirements/kosax-migration-tree.md § UI-A.5/C.3`) — 동의 철회 ledger, 부처 scope opt-in. Spec 027 mailbox 인프라 위에 빌드. 정당. |
| `observability/` (event_logger, events, metrics, otel_bridge, semconv, tracing) | 1011 | Spec 021 (4-tier OTEL: GenAI / Tool / Permission / Langfuse) + Spec 028 (OTLP collector). CC 의 client-side `services/analytics/` 가 KOSAX 에서 OTEL bridge 로 swap. 정당. |
...
CC 원본은 단순 string 상수 / type alias 인데 KOSAX 가 P0 reconstruction 단계에서 generic Proxy stub 으로 대체한 채 hydrate 되지 않음. 결과:
...
### P1-S8-4 (Major): `entrypoints/sdk/` 6 파일의 KOSAX-only re-declaration 검증 부족

`controlTypes.ts`, `coreTypes.generated.ts`, `runtimeTypes.ts`, `sdkUtilityTypes.ts`, `settingsTypes.generated.ts`, `toolTypes.ts` 가 모두 KOSAX-only. CC 는 이 타입들을 `@anthropic-ai/sdk` 패키지에서 가져옴. KOSAX 에서 재선언 자체는 정당하지만 type signature 가 CC 와 1:1 인지 cross-check 안 됨 → **silent type drift** 가능.

→ **수정**: CC `@anthropic-ai/sdk` 의 d.ts 와 KOSAX 재선언 비교 audit Sprint.

### P2-S8-5 (Minor): 마이그레이션 11개 전체 삭제 — `CURRENT_MIGRATION_VERSION = 11` 보존이 long-term 호환성 가져감

11개 마이그레이션이 모두 Anthropic 모델 alias / claude.ai 전용 — 삭제 정당. 그러나 `CURRENT_MIGRATION_VERSION = 11` 을 그대로 두는 결정은 향후 KOSAX-specific migration (예: K-EXAONE 모델 alias 변경) 도입 시 version space 충돌 위험. 12부터 KOSAX-original 로 시작하는 게 안전.
...
5. **P1-S8-4**: CC `@anthropic-ai/sdk` d.ts vs KOSAX 재선언 audit Sprint 발행?
