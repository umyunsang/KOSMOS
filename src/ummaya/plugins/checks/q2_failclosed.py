...
def check_auth_default(ctx: CheckContext) -> CheckOutcome:
    ...
    (it was a UMMAYA-invented field; AGENTS.md § CORE THESIS forbids invention).
    ...
    For UMMAYA-internal synthetic surfaces (resolve_location / lookup) ``policy``
    ...
    if ctx.manifest.adapter.policy is None:
        return failed(
            ko=(
                ...
                "(AGENTS.md § CORE THESIS: UMMAYA는 권한 정책을 발명하지 않음)"
            ),
            en=(
...
def check_pii_default(ctx: CheckContext) -> CheckOutcome:
    ...
    (UMMAYA-invented field). The symmetry check now uses ``derived pipa_class``
    ...
    When ``adapter.policy is None`` (UMMAYA-internal surfaces) the check
    ...
    if a.policy is None:
        return passed()
    from ummaya.tools.policy_derivation import derive_pipa_class_default
