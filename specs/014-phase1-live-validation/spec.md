...
1. **Given** valid `UMMAYA_FRIENDLI_TOKEN`, `UMMAYA_DATA_GO_KR_API_KEY`, and `UMMAYA_KOROAD_API_KEY` environment variables are set, **When** the developer runs `uv run pytest -m live`, **Then** all live-marked tests pass with real API responses. If any external API is unreachable, tests fail (not skip) to prevent false green results.
...
A user launches the UMMAYA CLI and conducts a Scenario 1 conversation ("safe route recommendation") using real K-EXAONE for reasoning and real data.go.kr/KOROAD APIs for data, verifying the full pipeline from user input through tool execution to Korean-language response.
...
- Valid API credentials (UMMAYA_FRIENDLI_TOKEN, UMMAYA_DATA_GO_KR_API_KEY, UMMAYA_KOROAD_API_KEY) are available and have sufficient quota for testing.
