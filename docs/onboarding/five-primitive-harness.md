...
- **Legacy 8-verb entries must not be re-introduced.** `src/ummaya/security/audit.py::TOOL_MIN_AAL` had 8 legacy IDs — they were removed at v1.2 GA. A lint test (T082) enforces this at CI time.
...
- [ ] Understand the active primitive surface in code: `src/ummaya/primitives/__init__.py`.
