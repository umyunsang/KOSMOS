...
- All 38 FRs trace to explicit decisions in `docs/requirements/kosax-migration-tree.md § UI L2`. No clarification markers were necessary because the migration tree is the canonical, user-approved source for every UI choice.
- Cross-cutting FR-034 references "Claude Code 2.1.88" as the canonical visual standard, which is a fixed, externally observable artifact (not an implementation framework choice for KOSAX).
