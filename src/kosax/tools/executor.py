...
"""Tool dispatcher for the KOSAX Tool System module.
...
from kosax.observability import (
    ERROR_TYPE,
...
from kosax.tools.envelope import make_error_envelope
from kosax.tools.errors import KosaxToolError, LookupErrorReason, ToolNotFoundError
...
def _log_adapter_exception(message: str, tool_id: str, exc: Exception) -> None:
    ...
    if isinstance(exc, KosaxToolError):
        logger.warning(
            "%s %s raised %s: %s",
            message,
            tool_id,
...
def _classify_adapter_exception(exc: Exception) -> tuple[LookupErrorReason, bool]:
    ...
    from kosax.tools.errors import Layer3GateViolation  # noqa: PLC0415
    from kosax.tools.kma.projection import KMADomainError  # noqa: PLC0415
...
class ToolExecutor:
    ...
    async def invoke(  # noqa: C901
        ...
        *,
        ...
        from kosax.tools.envelope import normalize  # noqa: PLC0415
        from kosax.tools.errors import EnvelopeNormalizationError  # noqa: PLC0415
        ...
        # Annotate the current execute_tool span with `kosax.tool.stage`
        ...
        _stage_span.set_attribute("kosax.tool.stage", "fetch")
        _stage_fetch_start = time.monotonic_ns()
        try:
            raw_output = await adapter(validated_input)
        except Exception as exc:
            _stage_span.set_attribute("kosax.tool.stage", "fetch_failed")
        ...
        _stage_span.set_attribute("kosax.tool.stage", "parse")
        ...
        from kosax.safety._ingress import apply_ingress_safety  # noqa: PLC0415
        from kosax.safety._span import emit_safety_event  # noqa: PLC0415
    ...
    async def dispatch(  # noqa: C901
        ...
        with _tracer.start_as_current_span(f"execute_tool {tool_name}") as span:
            ...
            try:
                ...
                # FR-017: emit kosax.tool.outcome exactly once per execute_tool span.
