...
.env.example                             # Fix: UMMAYA_DATA_GO_KR_KEY → UMMAYA_DATA_GO_KR_API_KEY
```
...
4. **Env var fix**: `.env.example` corrected to `UMMAYA_DATA_GO_KR_API_KEY`. Base URL documented with note on serverless vs standard endpoint.
...
|------|---------|---------|--------|
| `.env.example:18` | `UMMAYA_DATA_GO_KR_KEY=` | `UMMAYA_DATA_GO_KR_API_KEY=` | Rename variable |
