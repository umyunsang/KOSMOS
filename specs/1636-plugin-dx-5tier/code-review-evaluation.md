...
2. **Consent receipt 가 `~/.ummaya/consent/` 에 기록** — Spec 035 ledger (`~/.ummaya/memdir/user/consent/`) 와 분리되어 `/consent list` 에서 불가시.
...
| ID | 종류 | 위치 | 요지 | 검증 |
|---|---|---|---|---|
| C1 | 정합 | `installer.py:585` | Consent receipt 경로 `~/.ummaya/consent/` (data-model.md 명시는 `~/.ummaya/memdir/user/consent/`) → Spec 035 ledger 와 분리 | empirical: `print(consent_root)` 확인 |
...
| C6 | 보안 | `installer.py:402` + `settings.py:200` 주장 | `UMMAYA_PLUGIN_SLSA_SKIP=1` 의 production gate **존재하지 않음**. settings.py 주석 / security-review.md L3 약속 모두 vapor. 어떤 환경 변수 1개로 SLSA 건너뛰고 모든 layer 설치 가능. `slsa_state="skipped"` 는 consent receipt 에만 기록되고 OTEL span 에 미부착 | grep `.github/workflows/` → 0 matches |
...
| H3 | 보안 | adapter top-level code 권한 | 어댑터 모듈 import 가 UMMAYA process 권한으로 실행 (HMAC key / sessions JSONL / canonical hash 변조 등). `block_network` fixture 는 contributor's tests/conftest.py 한정 — 운영 환경 무영향 |
...
| D3 | 부재 CLI | `quickstart.ko.md:248-250` + `quickstart.md:230-232` | `ummaya-plugin-validate` CLI 가 `pyproject.toml:38-41` 에 미등록 (등록된 entry: `ummaya`, `ummaya-permissions`, `ummaya-plugin-init`). step 8 의 50/50 green-light 도달 불가 |
| D4 | env var typo | `quickstart.md:168` | 영문 source: `UMMAYA_DATA_GO_KR_KEY` (오타). `settings.py:55` 와 한국어 가이드: `UMMAYA_DATA_GO_KR_API_KEY` (정답). 영문 source 만 typo |
...
- DG2 `data-go-kr.md:209` 가 `ummaya plugin pipa-text` CLI 인용 — 존재하는 건 TUI slash command 만
...
5. **C6 fix**: `installer.py` 에 `if manifest.permission_layer == 3 and slsa_state == "skipped": return _EXIT_SLSA` + OTEL `ummaya.plugin.slsa_verification` attribute. `UMMAYA_ENV=production` 시 `plugin_slsa_skip=True` 거부.
...
8. **D3 fix**: `pyproject.toml` 에 `ummaya-plugin-validate = "ummaya.plugins.checks.framework:_cli_main"` entry 추가 + framework.py 에 CLI main 함수 작성.
9. **D4 fix**: `quickstart.md:168` `UMMAYA_DATA_GO_KR_API_KEY` 로 정정.
