...
"""Pydantic v2 data models for the KOSAX Tool System module."""
...
# checker (``src/kosax/plugins/checks/q3_security.py``) which enforces V6
...
# is the citizen authentication strength) and is NOT KOSAX-invented permission
...
# legitimate KOSAX-needed uses of the auth_level token outside adapter metadata.
...
Ministry = Literal[
    ...
    "KOSAX",  # harness-internal synthetic surface (resolve_location, lookup, mvp_surface)
    "OTHER",  # transitional escape hatch — CI emits warning
]


class AdapterRealDomainPolicy(BaseModel):
    """KOSAX adapter single permission representation — cite of agency published policy.

    KOSAX does NOT invent permission policy (.specify/memory/constitution.md § II).
...
class GovAPITool(BaseModel):
    ...
    # --- Policy citation (Epic δ #2295 — replaces KOSAX-invented security fields) ---
    # Spec 2295: AdapterRealDomainPolicy carries the agency's published policy URL
    # + citizen-facing gate category. KOSAX does NOT invent permission policy
    ...
    # mock adapters under ``src/kosax/tools/mock/*`` MUST set this to
    ...
    ``kosax.tools.registry``. Inline Literal avoids a circular import while
    ...
    # (KOSAX-internal surfaces with policy=None skip the chain).
    # ------------------------------------------------------------------
    ...
    def _validate_policy_security_invariants(self) -> GovAPITool:
        ...
        Pre-2295 KOSAX-invented chain (V1: pipa_class⇒auth_level, V2: dpa_reference,
        V4: irreversible⇒auth_level, V5: requires_auth) was tied to per-adapter
        KOSAX-invented fields. After Path B, those fields are derived from
        ...
        Skipped entirely when ``policy is None`` (KOSAX-internal).
        """
        ...
        from kosax.security.audit import TOOL_MIN_AAL
        from kosax.tools.policy_derivation import derive_min_auth_level
    ...
    def to_openai_tool(self) -> dict[str, object]:
        ...
        is consumed by ``kosax.llm.system_prompt_builder.build_system_prompt_with_tools``
...
# Evidence: /tmp/kosax-evidence/geocoding-evidence.md (4 scenarios, Kakao only).
# ---------------------------------------------------------------------------
...
class LookupSearchInput(BaseModel):
    ...
    """Per-call override; server-side clamp [1, 20]. If None, uses KOSAX_LOOKUP_TOPK default."""
...
class AdapterCandidate(BaseModel):
    ...
    real_classification_url: str | None = Field(
        ...
        description=(
            ...
            "(KOSAX does not invent permission classifications)."
        ),
    )
