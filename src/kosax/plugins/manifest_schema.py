...
"""Pydantic v2 manifest schema for KOSAX plugins.
...
existing Spec 022/031 :class:`~kosax.tools.registry.AdapterRegistration`
...
* ``_v_pipa_hash`` — ``acknowledgment_sha256`` byte-equals the canonical
  hash from :mod:`kosax.plugins.canonical_acknowledgment`
  (FR-014, R-1 Q6-PIPA-HASH).
* ``_v_otel_attribute`` — ``otel_attributes["kosax.plugin.id"]``
...
from kosax.plugins import canonical_acknowledgment
from kosax.tools.models import AdapterRealDomainPolicy  # noqa: F401 — re-exported via __all__
from kosax.tools.registry import AdapterRegistration
...
(Q8-NO-ROOT-OVERRIDE). ``subscribe`` is deferred until KOSAX has a real
...
class PIPATrusteeAcknowledgment(BaseModel):
    ...
    acknowledgment_sha256: str = Field(
        ...
        description=(
            ...
            "kosax.plugins.canonical_acknowledgment.CANONICAL_ACKNOWLEDGMENT_SHA256."
        ),
    )


class PluginManifest(BaseModel):
    """Top-level Pydantic v2 manifest contract for a KOSAX plugin release.
    ...
    otel_attributes: dict[str, str] = Field(
        description=(
            ...
            "(Spec 021 extension). MUST contain key 'kosax.plugin.id' "
    ...
    def _v_otel_required_key(cls, value: dict[str, str]) -> dict[str, str]:
        if "kosax.plugin.id" not in value:
            raise ValueError(
                "otel_attributes must contain key 'kosax.plugin.id' (Spec 021 KOSAX extension)."
            )
        return value
    ...
    def _v_otel_attribute(self) -> PluginManifest:
        """otel_attributes['kosax.plugin.id'] must equal plugin_id (FR-021)."""
        observed = self.otel_attributes.get("kosax.plugin.id")
        if observed != self.plugin_id:
            raise ValueError(
                f'otel_attributes["kosax.plugin.id"] must equal plugin_id '
