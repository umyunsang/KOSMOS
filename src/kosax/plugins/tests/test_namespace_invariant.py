...
The shim ``kosax.tools.registry.register_plugin_adapter`` is exercised
by the registration-flow test below so callers using the canonical
``kosax.tools.registry`` import path see identical behaviour to the
``kosax.plugins.registry`` direct path.
"""
...
from kosax.plugins import (
    CANONICAL_ACKNOWLEDGMENT_SHA256,
    PluginManifest,
)
from kosax.plugins.exceptions import PluginRegistrationError
from kosax.tools import registry as tools_registry
...
def _adapter(
    *, tool_id: str, primitive: AdapterPrimitive = AdapterPrimitive.lookup
    ...
    # arguments (extra="forbid" raises). Policy is optional (None = KOSAX-internal).
...
def _manifest_kwargs(**overrides: object) -> dict[str, object]:
    base: dict[str, object] = {
        ...
        "slsa_provenance_url": "https://github.com/kosax-plugin-store/x",
        "otel_attributes": {"kosax.plugin.id": "demo_plugin"},
        "search_hint_ko": "데모",
        "search_hint_en": "demo",
        "permission_layer": 1,
    }
...
class TestRegisterPluginAdapterShim:
    """Smoke-test the shim on kosax.tools.registry routes to the impl."""
    ...
    def test_shim_is_callable_and_routes_to_plugin_module(self) -> None:
        ...
        # kwargs, and calls into kosax.plugins.registry's implementation.
    ...
    def test_canonical_hash_constant_round_trips(self) -> None:
        ...
        # downstream plugins import it from kosax.plugins, the shim
    ...
    def test_model_construct_bypass_blocked_at_registry(self) -> None:
        ...
        tampered = PluginManifest.model_construct(
            **{**valid.__dict__, "otel_attributes": {"kosax.plugin.id": "wrong_id"}}
        )
        # Sanity: model_construct skipped the validator → the bad value sticks.
        assert tampered.otel_attributes["kosax.plugin.id"] == "wrong_id"
