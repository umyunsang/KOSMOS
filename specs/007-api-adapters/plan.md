...
The spec defines module paths under `src/kosax/tools/<provider>/`. This plan adopts those paths exactly (as required by `docs/tool-adapters.md § Naming`).
...
├── errors.py                            (existing — KosaxToolError hierarchy)
...
```python
import os
from kosax.tools.errors import KosaxToolError

class ConfigurationError(KosaxToolError):
...
**New Pydantic models** (all in `src/kosax/tools/<subpackage>/`):
...
**Deliverables**:
- `src/kosax/tools/koroad/__init__.py`
- `src/kosax/tools/koroad/code_tables.py`
- `src/kosax/tools/kma/__init__.py`
- `src/kosax/tools/kma/grid_coords.py`
- `src/kosax/tools/composite/__init__.py`
- `src/kosax/tools/errors.py` update — add `ConfigurationError`
...
**Deliverables**:
- `src/kosax/tools/koroad/koroad_accident_search.py`
...
async def _call(params: KoroadAccidentSearchInput, *, client=None) -> dict:
    api_key = _require_env("KOSAX_KOROAD_API_KEY")
    query = {
        "serviceKey": api_key,
        "_type": "json",
...
**Deliverables**:
- `src/kosax/tools/kma/kma_weather_alert_status.py`
- `src/kosax/tools/kma/kma_current_observation.py`
...
**Deliverables**:
- `src/kosax/tools/composite/road_risk_score.py`

**Implementation notes**:

The composite adapter imports the inner adapter functions directly:
```python
from kosax.tools.koroad.koroad_accident_search import _call as _koroad_call
from kosax.tools.kma.kma_current_observation import _call as _kma_obs_call
```
...
```bash
export KOSAX_KOROAD_API_KEY=<key>
export KOSAX_DATA_GO_KR_KEY=<key>
