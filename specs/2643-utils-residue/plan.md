...
1. **`utils/sessionTitle.ts` PORT** — byte-identical from CC + swap-1 wire to existing KOSAX `queryHaiku` (resolves a P1 broken import in `cli/print.ts:156`).
...
**Project Type**: TUI (Ink + React + Bun) within existing KOSAX monorepo; this Epic touches only the `tui/` subtree plus `docs/adr/` and audit cross-references.
...
| **II. Fail-Closed Security (NON-NEGOTIABLE)** | `yoloClassifier` stub maintains `unavailable=true` (auto-mode = no-op). No KOSAX-invented permission classifications introduced. `permissions.ts` callsite logic unchanged (FR-015). | **PASS** |
