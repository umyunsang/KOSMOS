# KOSAX MVP — Main Tools Precision Design
...
> **Expansion**: The main-tool axis was reset to an active primitive harness design. Current active primitives are `lookup`, `resolve_location`, `submit`, and `verify`; `subscribe` is deferred until KOSAX has an app/push-notification runtime. All ministry- and domain-specific knowledge (eligibility, payment, certificate issuance, application submission, slot reservation, notification handoff) collapses into adapters under `src/kosax/tools/<ministry>/<adapter>.py`; the main surface stays domain-agnostic. The previous 8-verb proposal and its Discussion have been retired.
...
| Finding | Source | Implication for KOSAX |
|---|---|---|
...
| **Anthropic Tool Search Tool (BM25)** ships as first-party primitive (2025-11) | Anthropic docs; Arcade, Spring, Stacklok replications | Pattern is validated. KOSAX reimplements client-side (FriendliAI/K-EXAONE has no server-side equivalent). |
...
| **Claude Code tool convention: 2–4 required params, mode enums over booleans, discriminated-union returns, fail-closed defaults via `buildTool()` factory** | `.references/claude-reviews-claude/docs/chapters/02-tool-system.md` | Direct blueprint — KOSAX adopts the same shape. |
...
KOSAX `lookup` therefore exposes **two operations** — `search` (retrieve candidates) and `fetch` (invoke the selected adapter with typed args). The LLM sees one tool with a `mode` discriminator, which is how Claude Code's `Grep` handles `output_mode: "content"|"files_with_matches"|"count"`.
...
- **Index source**: the in-process `ToolRegistry` (existing `src/kosax/tools/registry.py`). Build on startup; rebuild on adapter add/remove.
...
- This is the KOSAX equivalent of Anthropic's `defer_loading: true`, implemented client-side because FriendliAI/K-EXAONE has no server-side Tool Search primitive.
...
To validate the pattern end-to-end, MVP ships with **4 adapters** covering every canonical shape and every spatial-parameter convention KOSAX will ever encounter:
...
**NMC freshness threshold**: `hvidate` older than **30 minutes** emits `LookupError(reason="stale_data", retryable=False)`. Override via `KOSAX_NMC_FRESHNESS_MINUTES`.
...
Following Claude Code's static/dynamic split (`10-context-assembly.md:25-54`), the KOSAX system prompt adds these rules — *not* the tool description:
...
This preamble is the KOSAX analog of Claude Code's "Use FileRead instead of cat" rule — behavior couples to tool design via the system prompt, not via nested tool calls.
...
**Pattern D is the KOSAX sweet spot**: one `resolve_location` output feeds multiple adapters with different spatial conventions + the Layer 3 harness gate fires on the PII-flagged adapter, letting the LLM gracefully degrade to a non-PII alternative.

## 8. Non-MVP / deferred (tracked but not built)

### 8.1 Layer 3 harness posture (interface-only in MVP)

KOSAX is a **harness/framework**, not a turnkey citizen app. Layer 3 (auth, permission, consent) ships as **interface slots only** in MVP:
...
2. ~~**Default `top_k` for `lookup.search`**~~ **Resolved (2026-04-16)**: `min(5, len(registry))` with env override `KOSAX_LOOKUP_TOPK` clamped to `[1, 20]`. Adaptive floor handles MVP's 4-adapter phase; ceiling prevents context explosion at scale.
...
| `KOSAX_` env prefix | ✓ (`KOSAX_KAKAO_API_KEY`, `KOSAX_JUSO_CONFM_KEY`, `KOSAX_SGIS_KEY`, `KOSAX_SGIS_SECRET`, `KOSAX_LOOKUP_TOPK`, `KOSAX_NMC_FRESHNESS_MINUTES`) |
| Stdlib logging only | ✓ |
