# Feature Specification: TUI Component Catalog — CC → KOSAX Verdict Matrix + Brand-System Doctrine
...
1. **Given** a reviewer has the committed `docs/tui/component-catalog.md`, **When** they search for any CC component file path, **Then** they find exactly one row with verdict, owning Epic, CC source path, KOSAX target path (if PORT/REWRITE), and evidence citation.
...
**Independent Test**: Brand Guardian runs a grep on the proposed KOSAX tree for the banned patterns and finds zero matches in production source. Every token name matches the documented pattern in `docs/design/brand-system.md` §2.
...
**Why this priority**: Citizen-facing TUI accessibility is a KOSAX mission commitment (ADR-006 A-10 IME safety rule; vision.md). Without per-verdict accessibility gates in the catalog itself, later Epics will forget to apply them.
...
3. **Given** DISCARD verdicts, **When** the gate is consulted, **Then** no accessibility row is required (DISCARD means not in KOSAX tree).
...
Every DISCARD verdict must cite either ADR-006 Part D-1 (intentional exclusion), Part D-3 (KOSAX-original surface), or a specific domain-mismatch rationale. Unjustified DISCARDs silently lose features; justified DISCARDs provide permanent design-decision traceability.
...
1. **Given** CC `src/components/AutoUpdater.tsx` is marked DISCARD, **When** evidence is read, **Then** it cites "ADR-006 Part D-1 — CC-specific auto-updater; KOSAX uses uv/pip."
...
- **Existing KOSAX file without a CC counterpart**: This is a KOSAX-original extension (Part D-3 territory). Not in scope for verdict; documented as an explicit "no migration needed" row.
...
- **FR-002**: Each row MUST contain: CC source path, file count (1 for individual files; N for aggregated family entries where noted), verdict (PORT / REWRITE / DISCARD / DEFER), owning Epic (for PORT/REWRITE/DEFER), KOSAX target path (for PORT/REWRITE), rationale.
...
- **FR-004**: Each DISCARD row MUST cite ADR-006 Part D-1 (intentional exclusion) OR Part D-3 (KOSAX-original) OR a specific domain-mismatch rationale.
...
- **FR-009**: All new token names MUST follow the pattern {metaphorRole}{Variant}? where metaphorRole is drawn from {kosaxCore, orbitalRing, wordmark, subtitle, agentSatellite + MINISTRY, ...} and Variant is an optional modifier suffix (Shimmer, Muted, Hover, Active, etc.).
...
- **FR-015**: §1 MUST articulate the KOSAX (은하계) metaphor — scattered DX infrastructure unified into AX harness — and map visual elements to semantic roles (kosaxCore, orbitalRing, agentSatellite + MINISTRY).
...
- **FR-024**: Each Task sub-issue MUST contain: the CC source path, the KOSAX target path, a pointer back to the catalog row, and an acceptance checklist derived from the accessibility gate.
...
- **FR-031**: This Epic MUST NOT modify any `src/kosax/` source file.
- **FR-032**: Concrete color values in `dark.ts`, `default.ts`, `light.ts` are explicitly deferred to Epic H #1302.
- **FR-033**: KOSAX-original surfaces under `src/kosax/safety/`, `src/kosax/security/`, `src/kosax/primitives/`, `src/kosax/tools/*` are out of scope per ADR-006 Part D-3 — they MUST NOT appear in the verdict matrix.
...
- **Verdict**: One of PORT (palette-only swap; preserves logic), REWRITE (KOSAX mission-specific rebuild; logic changes allowed), DISCARD (excluded from KOSAX tree; evidence required), DEFER (Phase 3+ with unblock condition).
...
- **TokenName**: A string identifier in `tui/src/theme/tokens.ts` matching the pattern {metaphorRole}{Variant}?. Each token carries semantic weight tied to the KOSAX integration metaphor.
...
- Brand Guardian is a KOSAX reviewer role with authority to reject PRs that violate §2 token naming doctrine — implemented via the Agent Teams configuration at `/speckit-implement` time and via the grep CI gate at PR time.
...
- **KOSAX-original surfaces** under `src/kosax/safety/`, `src/kosax/security/`, `src/kosax/primitives/`, `src/kosax/tools/*` — no CC analog exists; migration is not applicable per ADR-006 Part D-3.
...
- **Mass file renames across KOSAX tree** — this Epic fixes the NAME CONTRACT only, not the physical rename. Downstream Epics perform renames under their own Tasks.
...
| Actual mass rename of CC-legacy tokens in KOSAX tree | Physical rename follows the NAME CONTRACT; implemented per Epic as each touches its files | All design-concerned Epics | #1480 |
...
| Ministry satellite icon set (SVG creation beyond existing assets) | New icons beyond `assets/kosax-*.svg` | Epic H #1302 | #1302 |
