...
// UMMAYA_ENABLE_DEAD_TEAM_MEM_SYNC env override is unset (the production
...
// UMMAYA process unintentionally — defense-in-depth atop the
...
const ENV_KEY = 'UMMAYA_ENABLE_DEAD_TEAM_MEM_SYNC'
...
describe('teamMemorySync dead-call gate (Spec 2641)', () => {
  ...
  test('isTeamMemorySyncAvailable throws without env override', () => {
    expect(() => isTeamMemorySyncAvailable()).toThrow(
      /isTeamMemorySyncAvailable: dead in UMMAYA/,
    )
  })
  ...
  test('pure helpers (createSyncState, hashContent) remain callable', () => {
    ...
    const hash = hashContent('hello ummaya')
  ...
  test('env override "0" keeps the gate closed (rejects truthy fallthrough)', () => {
    ...
    expect(() => isTeamMemorySyncAvailable()).toThrow(/dead in UMMAYA/)
  })
