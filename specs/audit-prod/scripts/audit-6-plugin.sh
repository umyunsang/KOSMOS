...
#   Tier 1 — kosax-plugin-template + `kosax plugin init` TUI + uvx fallback
...
#   Tier 5 — kosax-plugin-store/index catalog + 8-phase installer + SLSA verifier
...
#
# Hard rule: no live SLSA download in CI smoke. KOSAX_PLUGIN_SLSA_SKIP=true
...
#   Expected: "✗" + exit_code=1 + KOSAX branding for failure summary.
# ---------------------------------------------------------------------------
...
#   Uses cached bundle if present (~/.kosax/cache/plugin-bundles/) but
#   may catalog-miss if KOSAX_PLUGIN_CATALOG_URL is the live default.
