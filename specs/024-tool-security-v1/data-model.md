...
**Location**: `src/kosax/tools/models.py` (existing model; extended in this spec).
...
**Location**: `src/kosax/security/audit.py` (new module) as a `Final[dict[str, str]]` plus a dataclass for `public_path` metadata.
...
**Location**: `src/kosax/security/audit.py` (new module).
...
| `issuer` | `str` | Yes | Ministry or KOSAX-as-broker identifier. |
| `audience` | `str` | Yes | `kosax-agent` for the default agent audience. |
