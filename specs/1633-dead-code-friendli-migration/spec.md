...
|---|---|---|
| LLM provider | `@anthropic-ai/sdk` v0.x 가 TS 에서 직접 Anthropic API 호출 (137 imports) | TS 런타임에서 `@anthropic-ai/sdk` import **0**, 모든 LLM 호출이 Spec 032 stdio IPC 경유로 Python `LLMClient` → FriendliAI Serverless → EXAONE 로 라우팅 |
| 모델 ID | `claude-sonnet-4-6`, `claude-opus-4-7` 등 `getDefaultMainLoopModel()` 이 Anthropic 모델 반환 | `LGAI-EXAONE/EXAONE-236B-A23B` 단일 고정 (L1-A A1 "단일 고정" 결정) |
...
<!--
  ...
  US1 은 시민이 실제 EXAONE 응답을 처음 받는 end-to-end 시나리오 (product-level).
  US2/US3/US4 는 기여자 관점의 구조적 기준 (code-level measurable invariants) —
  포팅 clean-up 의 성패는 코드 수준 불변식으로만 검증 가능하기 때문.
-->

### User Story 1 — 시민이 TUI 로 EXAONE 응답을 받는다 (Priority: P1)

시민이 `FRIENDLI_API_KEY` 만 환경변수로 설정한 상태에서 KOSMOS TUI 를 실행하고, REPL 입력창에 "출산 보조금 신청 방법 알려줘" 를 입력한다. KOSMOS 는 CC 하네스의 agentic loop 를 그대로 사용하되 LLM 호출 엣지가 **Spec 032 stdio IPC 로 Python 백엔드를 경유**하여 FriendliAI Serverless 의 `LGAI-EXAONE/EXAONE-236B-A23B` 에 도달하고, EXAONE 의 첫 스트리밍 토큰이 5 초 이내에 시민 화면에 표시되기 시작한다. 시민은 "Anthropic" · "Claude" · "EXAONE" · "FriendliAI" 어느 단어도 볼 필요가 없다.
...
1. **Given** 시민이 `FRIENDLI_API_KEY=fr-...` 만 설정하고 `ANTHROPIC_API_KEY` 는 unset 인 상태, **When** 시민이 KOSMOS TUI 를 실행해 질의 "출산 보조금 신청 방법" 을 입력, **Then** Python 백엔드가 Spec 032 frame envelope 을 통해 LLM 요청을 수신하고 FriendliAI `/v1/chat/completions` 로 `model="LGAI-EXAONE/EXAONE-236B-A23B"` 호출을 발사하며, 첫 응답 토큰이 TUI 에 5 초 이내 스트리밍 표시된다.
...
- **FR-011**: `tui/src/utils/model/antModels.ts::getDefaultMainLoopModel()` (및 동등 함수) 반환값이 `"LGAI-EXAONE/EXAONE-236B-A23B"` 이어야 한다. Anthropic 모델 ID 는 TUI 어디에도 런타임 참조가 없다.
...
- **FR-020**: `tui/src/services/api/promptCacheBreakDetection.ts` 는 plan Phase 0 (research.md Decision 3) 에서 **keep + rewire** 로 결정됨 — FriendliAI 가 OpenAI-호환 응답의 `prompt_tokens_details.cached_tokens` 필드로 캐시 히트 메타데이터를 노출 (EXAONE 가격표에 cached-input $0.1/1M tier 존재로 확인). 기존 Anthropic `cache_read_input_tokens` 참조를 FriendliAI 필드로 재배선하여 유지한다.
...
- **SC-001**: 시민이 `FRIENDLI_API_KEY` 만 설정 후 TUI 를 실행해 첫 질의를 보내면 EXAONE 의 첫 스트리밍 토큰이 **5 초 이내** 에 TUI 에 표시된다 (mocked 또는 실제 FriendliAI 엔드포인트 대상, 로컬 Dev 환경).
...
- **SC-010**: TUI 런타임 코드에서 `getDefaultMainLoopModel()` 또는 그 호출 경로를 정적 분석한 결과 반환값 상수가 **`"LGAI-EXAONE/EXAONE-236B-A23B"` 단일 값**.
...
- 모델 ID `"LGAI-EXAONE/EXAONE-236B-A23B"` 는 `docs/requirements/kosmos-migration-tree.md § L1-A A1` 에서 "FriendliAI serverless + EXAONE 단일 고정" 으로 승인됨. FriendliAI 공개 모델 카탈로그에서의 정확한 ID 스펠링 확인은 plan Phase 0 에서 마무리.
...
- **Anthropic 이외의 LLM provider 지원**: L1-A A1 이 "FriendliAI serverless + EXAONE 단일 고정" 을 승인한 상태. 다중 provider 스위칭은 KOSMOS 의 디자인 원칙 위반이며 영구 제외.
