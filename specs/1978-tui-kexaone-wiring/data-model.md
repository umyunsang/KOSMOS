...
### `ChatMessage` (re-used from `kosax.llm.models`)
...
### `ToolDefinition` (re-used from `kosax.llm.models`)
...
**Storage**: append-only JSON file at `~/.kosax/memdir/user/consent/<receipt_id>.json` (Spec 035 + Spec 027 paths). Listed by `/consent list` slash command. Revocation marker is a sibling `<receipt_id>.json.revoked` (Spec 027 marker pattern).
...
| Persisted JSONL | `~/.kosax/memdir/user/sessions/<session_id>.jsonl` | TUI writer (Spec 027) |
...
Telemetry: `kosax.turn` span (ADR-0004). One turn may contain ≥0 tool calls and ≥0 permission requests.
...
**SHA-256 contract** (Spec 032 FR-037): `frame.schema.json` SHA-256 will change due to new arm. Backend `kosax.ipc.schema.hash` OTEL attribute updated at boot — verified by Phase C task `T021_schema_hash_update`.
