...
"""Central registry for KOSAX government API tools."""
...
from kosax.tools.models import AdapterRealDomainPolicy, GovAPITool, ToolSearchResult
from kosax.tools.policy_derivation import (
    AALLevel,
    PIPAClass,
...
from kosax.tools.retrieval.backend import Retriever, build_retriever_from_env
from kosax.tools.retrieval.bm25_backend import BM25Backend
from kosax.tools.retrieval.degrade import DegradationRecord
from kosax.tools.retrieval.dense_backend import DenseBackendLoadError
from kosax.tools.search import search_tools
...
class AdapterRegistration(BaseModel):
    ...
    construction time; see :mod:`kosax.security.v12_dual_axis`) remain the
    ...
    :mod:`kosax.security.v12_dual_axis` backstop flips ``V12_GA_ACTIVE = True``.
    """
    ...
    # policy is None. New registrations SHOULD populate this field; KOSAX-internal
    ...
    # (see :func:`kosax.primitives.submit.derive_transaction_id`). Adapters
    ...
    def _enforce_v12_dual_axis(self) -> AdapterRegistration:
        ...
        Delegates to :func:`kosax.security.v12_dual_axis.enforce`. No-op while
        ...
        """
        from kosax.security.v12_dual_axis import enforce as _enforce_v12
    ...
    # policy via ``policy_derivation`` instead of stored as KOSAX-invented
    ...
    def auth_level(self) -> AALLevel:
        ...
        None — KOSAX-internal synthetic surfaces (resolve_location, lookup).
        """
        if self.policy is None:
            return "AAL1"
...
class ToolRegistry:
    ...
    def __init__(self) -> None:
        ...
        # protocol and lets the environment (via KOSAX_RETRIEVAL_BACKEND)
        ...
        # legacy ``bm25_index`` attribute (e.g., kosax.tools.search) keep
    ...
    def register(self, tool: GovAPITool) -> None:
        ...
        # mapping (``_AUTH_TYPE_LEVEL_MAPPING``). KOSAX-internal synthetic
        # surfaces (policy=None) skip this check.
        if tool.policy is not None:
            from kosax.tools.models import _AUTH_TYPE_LEVEL_MAPPING
    ...
    def deregister(self, tool_id: str) -> None:
        ...
        Used by :func:`kosax.plugins.uninstall.uninstall_plugin` to fully
...
def register_plugin_adapter(
    ...
    *,
    ...
    """Plugin-adapter registration shim deferring to :mod:`kosax.plugins.registry`.

    The real implementation lives in ``kosax.plugins.registry`` so the plugin
    module owns its OTEL emission and module-import glue. This shim exists at
    the canonical ``kosax.tools.registry`` import path that downstream code
    ...
    Args:
        manifest: A validated :class:`kosax.plugins.manifest_schema.PluginManifest`.
        registry: Central :class:`ToolRegistry`.
        executor: :class:`kosax.tools.executor.ToolExecutor` instance.
    ...
    Returns:
        The registered :class:`kosax.tools.models.GovAPITool`.

    Raises:
        kosax.plugins.exceptions.PluginRegistrationError: On any failure
            in module import, symbol resolution, or invariant violation.
    """
    # Inline import to avoid a load-order cycle: kosax.plugins.registry
    # imports kosax.tools.registry (ToolRegistry). The shim lives at the
    # bottom of this module so a top-level import would re-enter before the
    # ToolRegistry class object is bound.
    from kosax.plugins.registry import (  # noqa: PLC0415
