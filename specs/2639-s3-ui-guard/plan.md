...
| Decision | Reference |
|---|---|
| **D1 — TeleportResumeWrapper DROP** | `specs/cc-migration-audit/decisions.md § S3 D1` (양쪽 DROP, NEVER-PORT 박제) · `specs/cc-migration-audit/scope-S3-components-screens.md § 4 #7 + § 10 NEVER-PORT 명단` · `AGENTS.md § CORE THESIS` (KOSAX = CC + 2 swap; 1P-Anthropic 비즈니스 surface 는 정당한 제3 제거) · `feedback_kosax_scope_cc_plus_two_swaps` 메모리 |
...
| **whitelist YAML schema** | `tui/src/sdk-compat.js` (W1 substitution path) · `docs/requirements/kosax-migration-tree.md § 마스코트 · 브랜딩` (W2/W3/W4/W6) · `prompts/system_v1.md` (한국어 i18n W5 source) · Spec 2519 PR (W10 Korean IME) · Spec 2521 docs (W9 K-EXAONE multi-tool layout) |
...
│      4. for each KOSAX file in slice:                                 │
│           a. compute sha256                                            │
│           b. lookup CC baseline by relative path                       │
│           c. if path missing in CC → PASS (KOSAX-only)                │
...
causes:
  ...
  W2: "Brand string substitution (Claude Code → KOSAX)"
  W3: "Mascot character swap (Clawd → UFO)"
  W4: "Brand glyph (✻ preserved)"
  W5: "Korean i18n string injection"
  W6: "Color palette (CC blue → KOSAX violet #a78bfa)"
...
entries:
  - path: tui/src/components/messages/UserToolResultMessage/utils.tsx
    cause: W1
    spec_ref: docs/requirements/kosax-migration-tree.md § L1-A
...
생성 명령:
```bash
cd /Users/um-yunsang/KOSAX/.references/claude-code-sourcemap/restored-src/src
{
...
} | sort | xargs shasum -a 256 > /Users/um-yunsang/KOSAX-w-2639/specs/2639-s3-ui-guard/fixtures/cc-baseline-shas.txt
```
...
failures = []
stats = {"total": 0, "byte_identical": 0, "whitelisted": 0, "kosax_only": 0, "failed": 0}

for kosax_path in slice_paths:
    rel = relative_to_tui_src(kosax_path)  # e.g., "components/App.tsx"
    actual_sha = sha256(read(kosax_path))
    cc_sha = baseline.get(rel)

    if cc_sha is None:
        stats["kosax_only"] += 1; continue
...
if failures:
    ...
    sys.exit(1)
print(f"PASS · {stats['byte_identical']} byte-identical · {stats['whitelisted']} whitelisted · {stats['kosax_only']} KOSAX-only · {stats['failed']} failed")
```
...
// Justification: Anthropic services/api/errors and secureStorage modules removed by KOSAX dead-code cleanup; constants and isMacOsKeychainLocked → false stub inlined to preserve component contract.
```
...
// Justification: Anthropic queryHaiku-driven auto-naming requires Anthropic SDK; KOSAX exits plan mode without auto-naming.
```
...
// Justification: Grove growth-experiment, Anthropic OAuth, and tengu_* analytics events all swap-1 dependent (claude.ai 계정 + Anthropic telemetry); KOSAX removes them and falls through to no-OAuth branches.
```
...
기대 frames:
- snap-000-boot.txt (KOSAX 브랜딩 + tool_registry verified)
