...
| 결정 | 값 |
|---|---|
| C1 Primitive | `find` · `locate` · `send` · `check` (4개 active) · `subscribe` 제거/보류. `check`는 모바일신분증·마이데이터·인증서·OmniOne CX/QR 같은 본인확인/위임 채널을 도구 호출로 감싸고, 현재 mock contract를 해커톤에서 live OmniOne-backed tool로 전환하는 대상이며, `send`는 그 위임 결과로 신청·제출·납부형 공공채널 어댑터를 호출 |
