...
"""Fail-fast startup guard for required KOSAX_* environment variables.
...
_DOC_URL: Final[str] = "https://github.com/umyunsang/KOSAX/blob/main/docs/configuration.md"
...
_REQUIRED_VARS: Final[tuple[RequiredVar, ...]] = (
    ...
    RequiredVar(
        name="KOSAX_OTEL_ENDPOINT",
        consumer="kosax.observability.otel (#501)",
        required_in=frozenset({"prod"}),
        doc_anchor="#kosax_otel_endpoint",
    ),
)
...
def verify_startup() -> None:
    ...
    if diag is None:
        return
    message = (
        f"KOSAX config error [env={diag.env}]: "
