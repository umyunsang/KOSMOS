...
FR-009 (harness-not-reimplementation): KOSAX holds NO private keys, performs
...
from kosax.tools.registry import NistAalHint, PublishedTier

logger = logging.getLogger(__name__)

_tracer = trace.get_tracer("kosax.primitives.verify")
...
# in src/kosax/tools/registry.py.
...
def register_verify_adapter(family: str, adapter: object) -> None:
    ...
    each ``src/kosax/tools/mock/verify_<family>.py``.
    """
...
async def verify(
    ...
    KOSAX is a harness.  All cryptographic validation, cert chain evaluation,
    ...
    with _tracer.start_as_current_span("gen_ai.tool_loop.iteration") as span:
        ...
        span.set_attribute("kosax.verify.family_hint", family_hint)
        ...
        span.set_attribute("kosax.verify.observed_family", str(observed))
        return result
