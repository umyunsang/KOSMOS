...
"""Mock verify adapter — kec (KEC 공동인증서 / Joint Certificate AX-channel).
...
shape for the Korea Electronic Certificate (공동인증서 / NPKI) identity channel.
Issues a DelegationToken with OAuth2.1 + mTLS + scope-bound bearer envelope.
...
from kosax.memdir.consent_ledger import DelegationIssuedEvent, append_delegation_issued
from kosax.primitives.delegation import DelegationContext, DelegationToken
from kosax.primitives.verify import KECContext, register_verify_adapter
from kosax.tools.transparency import stamp_mock_response
...
_ACTUAL_ENDPOINT: Final = "https://api.gateway.kosax.gov.kr/v1/verify/kec"
...
_TOOL_ID: Final = "mock_verify_module_kec"
_ISSUER_DID: Final = "did:web:kec.go.kr"
...
SEARCH_HINT: Final[dict[str, list[str]]] = {
    "ko": ["공동인증서", "공인인증서", "KEC", "NPKI", "KICA", "KOSCOM"],
    "en": ["joint certificate", "KEC", "NPKI", "public key certificate", "gongdong injeungseo"],
}
...
def invoke(session_context: dict[str, Any]) -> KECContext:
    """Issue a DelegationToken for the KEC (공동인증서) AX channel.
    ...
    """
    scope_list: list[str] = session_context.get("scope_list", ["verify:kec.identity"])
    scope_str = ",".join(scope_list)
    session_id: str = session_context.get("session_id", "mock-session-unknown")
    purpose_ko: str = session_context.get("purpose_ko", "공동인증서 신원확인")
    purpose_en: str = session_context.get(
        "purpose_en", "KEC joint-certificate identity verification"
    )
    ...
    return KECContext.model_validate(
        {
            "published_tier": "kec_aal3",
            "nist_aal_hint": "AAL3",
            "verified_at": now,
...
register_verify_adapter("kec", invoke)
