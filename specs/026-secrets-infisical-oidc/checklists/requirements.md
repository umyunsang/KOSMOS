# Specification Quality Checklist: Secrets & Config — Infisical OIDC + 12-Factor + KOSAX_* Registry
...
- [x] No implementation details leak into specification
    - `src/kosax/config/guard.py` appears as the **artifact path** in FR-003 / Key Entities, consistent with Epic body's allowed file surface. No guard source code, algorithm, or library choice is prescribed.
...
- The spec deliberately names the legacy `KOSAX_API_KEY` fallback and the `KOSAX_<TOOL_ID>_API_KEY` override pattern as first-class registry entries so the audit script can tolerate them without a special case.
