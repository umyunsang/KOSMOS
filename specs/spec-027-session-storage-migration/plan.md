...
|-----------|------------|------|
| I — CC + 2 swaps thesis | PASS | We are routing CC's session-storage helper to a UMMAYA-owned root; not changing the JSONL schema, not changing function signatures (FR-013). |
| II — Permissions never lateral | N/A | Storage path Epic only. |
| III — Live tools cite agency policy | N/A | Internal storage. |
| IV — No UMMAYA-invented permission classifications | N/A | |
...
**Method**: Read `.references/claude-code-sourcemap/restored-src/utils/sessionStorage.ts` (or its current location) for `getProjectsDir()` + `getProjectDir(projectDir)` definitions. Compare to UMMAYA port at `tui/src/utils/sessionStorage.ts:203-205, 441-443`.

**Acceptance**: We have a confirmed reference for `<projects-root>/<sanitized-cwd>/<session_id>.jsonl`. The UMMAYA port at `sessionStorage.ts:441-443` (`getProjectDir`) and `sessionStoragePortable.ts:329-331` (same name) call `sanitizePath(projectDir)`. We will leave the per-cwd sanitization helper alone — it is only used for legacy-discovery in US2.
...
**Method**: `grep -rn "getTranscriptPath\|getProjectsDir\|getProjectDir\|getSessionProjectDir\|getAgentTranscriptPath" tui/src/ src/ummaya/`.
...
### R-3 — Decide US5 `~/.ummaya/sessions/` resolution

**Question**: For the 3,572 stub JSONLs at `~/.ummaya/sessions/`, which of {delete-only | symlink-to-canonical | repoint-Python-backend} is chosen?

**Method**: Inspect 5 random stub files for any non-metadata content (rule out partial-content). Inspect `src/ummaya/session/store.py` for write call-sites. Inspect IPC envelope (`src/ummaya/ipc/stdio.py:600,727`) for any callsite that depends on the legacy path.

**Acceptance**: One concrete decision documented with rationale. Default recommendation per spec.md Q3: **repoint Python `_DEFAULT_SESSION_DIR` to `~/.ummaya/memdir/user/sessions/`** + add a one-shot `ummaya --gc-empty-stubs` pass that audits each stub by reading its first JSON line and confirming `message_count == 0` *content-wise* before deletion (never by filename heuristic).
...
**Method**: Read 1 sample line from `~/.claude/projects/-Users-um-yunsang-UMMAYA-tui/<largest>.jsonl` (TUI-written) vs `~/.ummaya/sessions/<any>.jsonl` (Python-written). Diff field names. Then read `tui/src/commands/history.ts:loadSessionEntries` (the canonical reader) — what fields does it require?
...
SessionPathRoot:
  UMMAYA_CANONICAL: ~/.ummaya/memdir/user/sessions  # FR-002
  CC_LEGACY:        ~/.claude/projects               # FR-005 read-only
  ENV_OVERRIDE:     $UMMAYA_MEMDIR_USER + /sessions  # FR-002
...
- `contracts/session-path-resolver.md` — Markdown contract for `getUmmayaSessionsDir()`, env override semantics, fail-closed behavior (FR-011).
...
1. `export UMMAYA_MEMDIR_USER=/tmp/test-memdir-$RANDOM`
...
7. `/resume` → assert both real UMMAYA session + fake legacy session appear.
...
10. `unset UMMAYA_MEMDIR_USER`.

### dispatch-tree.md

```
Phase 1 Setup (T001-T002): sonnet-foundational  (UMMAYA path helper + env override)
...
- **T001** — Add `UMMAYA_MEMDIR_USER` to `pydantic-settings` env catalog if it isn't already (Spec 027 may already have it; verify in R-2). Backend Python side.
- **T002** — Add `getUmmayaSessionsDir()` helper to a new file `tui/src/utils/ummayaPaths.ts` (matching the Spec 1635 `uiL2Memdir.ts:25-26` pattern). TS side.
- **T003-T004** — Repoint `getProjectsDir()` *body* in `sessionStorage.ts` and `sessionStoragePortable.ts` to call `getUmmayaSessionsDir()`. The function name + signature stay identical (FR-013). The CC-shape-mimicking helper `getProjectDir(projectDir)` stays as a *legacy-discovery* helper used only by FR-005 dual-path enumeration.
- **T005-T006** — Add migration helper module + `/migrate-sessions` slash command registration.
- **T007** — Resolve `~/.ummaya/sessions/` per R-3 decision. Most likely: change `src/ummaya/session/store.py:32` to `_DEFAULT_SESSION_DIR = Path.home() / ".ummaya" / "memdir" / "user" / "sessions"`. Add `--gc-empty-stubs` CLI flag.
...
|-------|----------|------------------------|
| 1a — pytest | `tests/test_session_store_path.py` | YES (covers Python `_DEFAULT_SESSION_DIR` repoint, `UMMAYA_SESSION_DIR` env override, `--gc-empty-stubs`). |
...
| 2 — stdio JSONL probe | `specs/spec-027-session-storage-migration/scripts/probe-write-path.sh` | YES (asserts a tool-call session writes to UMMAYA path, not CC path). |
...
**Bypass declaration**: This Epic touches `tui/src/**` extensively (sessionStorage.ts, sessionStoragePortable.ts, new ummayaPaths.ts, new migrateSessions command). NO bypass available — all verification layers required.
...
- `docs/requirements/ummaya-migration-tree.md § L1-A A5` — same invariant from the requirements tree.
...
- Spec 035 `~/.ummaya/memdir/user/consent/` — sibling dir under same root.
...
- **Q2 (resolved by spec.md Q2)**: Per-cwd subdirs at the UMMAYA path? **No** — flat layout per FR-014.
- **Q3 (resolved by R-3 decision in research.md)**: `~/.ummaya/sessions/` action? **Repoint + gc**.
- **Q4 (NEW, escalate if blocking)**: Should `getUmmayaSessionsDir()` live in a new `tui/src/utils/ummayaPaths.ts` (preferred — single helper file for all canonical UMMAYA paths) OR be inlined into `envUtils.ts` next to `getClaudeConfigHomeDir()` (simpler diff)? **Recommendation**: new file. `ummayaPaths.ts` becomes the home for any future `getUmmayaConsentDir()` / `getUmmayaPluginsDir()` factoring (DRY across Spec 035 / Spec 1636 helpers).

---

## Phase 6 — PR closure plan

- PR title: `feat(spec-027-session-storage-migration): route citizen sessions to ~/.ummaya/memdir/user/sessions/ (closes Epic 3)`
- PR body cites: Spec 027 + `ummaya-migration-tree.md § L1-A A5` + this plan.
