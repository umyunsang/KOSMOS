...
- **Structural metaphor roles** — `kosaxCore`, `orbitalRing`, `wordmark`, `subtitle`. These map to the visual affordances of the KOSAX (은하계) integration metaphor described in §1. A token named `kosaxCore` decorates the central orchestrator visual; `orbitalRing` decorates the tool-loop ring affordance that communicates active agent traversal to the citizen.
