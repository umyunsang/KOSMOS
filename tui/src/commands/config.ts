...
// Emits ummaya.ui.surface=config at command start (FR-037).
...
// UMMAYA non-secret settings exposed in the overlay.
// Values are read from env at command time (never hardcoded).
export const UMMAYA_CONFIG_CATALOG: Omit<ConfigEntry, 'value'>[] = [
  {
 ...
 *
 * Emits `ummaya.ui.surface=config` (FR-037) and returns the config entry
...
export function executeConfig(openSecretFor?: string): ConfigCommandResult {
  ...
  const entries: ConfigEntry[] = UMMAYA_CONFIG_CATALOG.map((template) => ({
    ...template,
