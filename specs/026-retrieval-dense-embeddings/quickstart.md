...
**Audience**: UMMAYA operators and developers who want to exercise the three
...
```bash
export UMMAYA_RETRIEVAL_BACKEND=dense
# UMMAYA_RETRIEVAL_MODEL_ID is optional; default is the e5-small above.
# Example override (NOT recommended unless you know why):
# export UMMAYA_RETRIEVAL_MODEL_ID="sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"
...
```bash
# Baseline (BM25)
unset UMMAYA_RETRIEVAL_BACKEND
uv run python -m ummaya.eval.retrieval \
...
|----------|--------|---------|-------|---------|
| `UMMAYA_RETRIEVAL_BACKEND` | `bm25` \| `dense` \| `hybrid` | `bm25` | #468 | Select ranking backend. Unknown values fail-closed at registry construction (FR-001). |
| `UMMAYA_RETRIEVAL_MODEL_ID` | HF model slug (Apache-2.0-compatible only) | `intfloat/multilingual-e5-small` | #468 | Dense model identifier. Licence is vetted during spec review (shortlist in `research.md` §Models); operators overriding to a non-vetted slug take responsibility for compatibility. No runtime licence enforcement is performed at load time. |
| `UMMAYA_RETRIEVAL_FUSION` | `rrf` | `rrf` | #468 | Fusion algorithm for hybrid mode. RSF is deferred (`NEEDS TRACKING`). |
| `UMMAYA_RETRIEVAL_FUSION_K` | integer ≥ 1 | `60` | #468 | RRF dampening constant. `k=60` per Cormack SIGIR 2009. |
| `UMMAYA_LOOKUP_TOPK` | 1..20 | `5` | #022 | Adaptive top-k clamp (FROZEN — preserved by FR-005). |
