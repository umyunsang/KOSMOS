...
```text
[UMMAYA][bootGuard] 도구 '<tool_id>' 등록 검증 실패. 누락 필드: <missingFields.join(', ')>.
...
```text
[UMMAYA][bootGuard] 어댑터 '<tool_id>'에 real_classification_url 또는 policy_authority가 비어 있습니다.
UMMAYA는 정책 인용을 발명하지 않습니다 — 기관이 공개한 URL이 필수입니다.
