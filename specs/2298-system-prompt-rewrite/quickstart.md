...
```bash
cd /Users/um-yunsang/KOSAX-w-2298
...
git worktree add ../KOSAX-w-2298 2298-system-prompt-rewrite
cd ../KOSAX-w-2298
uv sync --quiet
...
grep -E '"(simple_auth_module|modid|kec|geumyung_module|any_id_sso)' src/kosax/tools/registry.py | wc -l
# Expected: 5

# 11-arm AuthContext union present
grep -A12 'AuthContext = Annotated\[' src/kosax/primitives/verify.py | grep -cE 'Context\b'
# Expected: 11
...
```bash
uv run python -c "from pathlib import Path; from kosax.context.prompt_loader import PromptLoader; PromptLoader(manifest_path=Path('prompts/manifest.yaml')); print('OK')"
...
1. Keyframe 1 — must show `KOSAX` brand text.
...
| `<verify_chain_pattern>` chain example uses non-existing tool_id | LLM emits the wrong tool_id → IPC `unknown_tool_id` error | Use `mock_lookup_module_hometax_simplified` and `mock_submit_module_hometax_taxreturn` exactly (verify via `grep -r mock_lookup_module_hometax_simplified src/kosax/tools/`) |
