...
| # | 위치 | 증상 | Severity | Fix |
|---|---|---|---|---|
| 1 | `tui/src/components/onboarding/PreflightStep.tsx:78,83` | env var 이름 mismatch (`FRIENDLI_API_KEY` / `KOSAX_DATA_GO_KR_KEY` only — 실제 .env 는 `KOSAX_FRIENDLI_TOKEN` / `KOSAX_DATA_GO_KR_API_KEY`) | UX (onboarding 항상 ✗) | canonical+alias 둘 다 체크 (envGuard.ts / useApiKeyVerification.ts 와 동일) |
...
## 잔존 (별도 issue 추적)
- **#4-deferred — Onboarding `showDialog` Enter routing**: PreflightStep 의 `useInput` 가 tmux 자동화에서 Enter key 받지 못하는 듯 (또는 `useKoreanIME.isComposing` 영구 true). 우회: `~/.kosax/memdir/user/onboarding/state.json` 의 `current_step_index: 5` 로 수동 마킹 시 정상 동작. 근본 원인은 React state architecture (Ink useInput + showDialog wrapper) 정밀 진단 필요.
...
- Layer 5 boot post-fix: KOSAX branding + UFO mascot + REPL prompt 정상 렌더 (frames/04-onboarding-complete/ 박제)
