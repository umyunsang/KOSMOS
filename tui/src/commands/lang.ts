...
// updates the process.env['KOSAX_TUI_LOCALE'] key (visible to all getUiL2I18n
...
// tui/src/i18n/uiL2.ts evaluates KOSAX_TUI_LOCALE at module-load time via
 ...
 *
 * Side-effect: sets `process.env['KOSAX_TUI_LOCALE']` to the new locale so
...
export function parseLangCommand(argStr: string): LangCommandResult {
  ...
  process.env['KOSAX_TUI_LOCALE'] = locale
...
export function getCurrentLocale(): SupportedLocale {
  const env = process.env['KOSAX_TUI_LOCALE']
  return env === 'en' ? 'en' : 'ko'
}
