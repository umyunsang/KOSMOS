...
**Created**: 2026-04-29
**Status**: Draft
**Input**: User description: "Epic α — `.references/claude-code-sourcemap/restored-src/` 와의 UMMAYA TUI parity 를 read-only audit 로 입증하고, 212 modified 파일을 후속 Epic 입력으로 분류한다."
**Authority** (cite in every downstream artefact):
- `AGENTS.md § CORE THESIS` — UMMAYA = AX-infrastructure callable-channel client (3차 thesis canonical)
...
UMMAYA Lead (Opus) 와 후속 Epic 팀이 본 Epic 산출물을 입력으로 받아 Epic β (#2293, UMMAYA-original UI residue cleanup) 와 Epic δ (#2295, 백엔드 permissions 정리) 의 작업 범위를 결정해야 한다. 그러기 위해 `tui/src/` 하위 212 modified 파일 each 가 (a) UMMAYA-needed 정당한 변경인지, (b) 잔재 정리 필요 항목인지, (c) 추가 audit 가 필요한 의심 변경인지 단일 스프레드시트형 분류로 묶여 있어야 한다.
...
**Independent Test**: 사용자가 `cc-parity-audit.md § Modified Files` 표를 열었을 때 212행 모두가 `Legitimate / Cleanup-needed / Suspicious` 중 정확히 하나로 분류되어 있고, 각 행이 reference (CC source-of-truth 경로 또는 UMMAYA spec id) 를 인용하면 통과.
...
3. **Given** Suspicious 파일 1개, **When** 사용자가 행을 펼침, **Then** 변경 요약 + UMMAYA spec 인용 + "왜 의심인가" 사유 한 줄이 포함되어 있다.

---

### User Story 2 — Byte-Identical Parity Spot-Check (Priority: P2)

UMMAYA 의 핵심 invariant 는 "1,604 KEEP 파일이 진짜 CC 와 byte-identical 이다" 이다. 이 invariant 가 시간에 따라 drift 했을 가능성을 검증하기 위해, `tui/src/` 의 1,531 byte-identical 후보 파일 중 무작위 50 개 표본을 hash 비교로 검증한다. 표본 결과는 reproducibility script 와 함께 문서화되어, 향후 누구나 다른 시드로 재검증할 수 있어야 한다.
...
1. **Given** 1,531 byte-identical 후보, **When** audit 실행, **Then** 50 개 무작위 표본이 추출되고 각 파일에 대해 (UMMAYA path, CC source path, hash 일치 여부) 가 기록된다.
...
**Why this priority**: 73 파일은 UMMAYA-internal SDK 경로 (`@ummaya/...` vs `@anthropic/...` 등) 만 바뀐 것으로 추정되어 Epic β·γ 작업 우선순위가 가장 낮다. 본 검증으로 73 파일을 사실상 KEEP 군에 합산할 수 있게 된다.
...
- **CC 원본 경로가 `restored-src` 에 존재하지 않는 UMMAYA-only 파일이 modified 카테고리에 잘못 포함된 경우**: audit 실행 중 발견되면 UMMAYA-only ADDITIONS (274) 카테고리로 reclassify 하고 본 Epic 범위에서 제외한다.
...
- **FR-002**: Audit 산출물은 1,531 byte-identical 후보 중 무작위 50 개 이상의 표본을 추출하고, 각 표본에 대해 (UMMAYA path, CC source path, hash match 여부) 를 기록해야 한다.
...
- **FR-004**: Modified 파일 분류 표의 각 행은 (a) CC source-of-truth 경로 또는 (b) UMMAYA spec id (예: `Spec 1633`, `Spec 287`) 중 적어도 하나의 reference 를 인용해야 한다.
...
- **AuditEntry** (modified file 1 행): `ummaya_path`, `cc_source_path`, `classification` (Legitimate / Cleanup-needed / Suspicious), `change_summary` (한 줄), `reference_citation` (CC 경로 또는 UMMAYA spec id), `notes` (선택, suspicious 사유).
- **SpotCheckSample** (byte-identical 표본 1 행): `ummaya_path`, `cc_source_path`, `hash_match` (true/false), `sampling_seed_or_method`.
- **ImportDiffEntry** (SDK import 73 검증 1 행): `ummaya_path`, `cc_source_path`, `import_lines_changed` (요약), `body_diff_present` (true/false), `reclassified_to_modified` (true/false).
...
- modified 파일의 변경 사유 추적은 `git log` + `git blame` 과 UMMAYA spec history (`specs/`, `docs/`) 로 충분하다. 추적 불가 항목은 자동으로 Suspicious 분류.
...
- **소스 파일 수정**: Epic α 는 read-only audit. 어떤 `tui/src/`, `src/ummaya/` 파일도 수정하지 않는다. 수정은 Epic β·γ·δ·ε·ζ·η 에서 처리.
- **백엔드 Python (`src/ummaya/`) audit**: 본 Epic 은 `tui/src/` parity 만 다룬다. 백엔드 cleanup 은 Epic δ (#2295) 와 `cc-source-scope-audit.md § 2` 가 담당.
- **UMMAYA-only ADDITIONS (274 파일) audit**: 본 Epic 은 KEEP (1,604) + Modified (212) 만 다룬다. UMMAYA-only 파일 검토는 Epic β 의 6 검토 후보 (Monitor / ReviewArtifact / SuggestBackgroundPR / Tungsten / VerifyPlanExecution / Workflow) 등에 분산.
...
|------|---------------------|-------------------|----------------|
| Suspicious 파일 실제 수정 (Spec 1633 잔재 등) | 본 Epic 은 read-only audit; 수정은 분류 결과를 받은 후 후속 Epic 의 일 | Epic β (#2293) — UMMAYA-original UI residue cleanup | #2293 |
