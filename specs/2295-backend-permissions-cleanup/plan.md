...
Initiative #2290 의 네 번째 Epic (β/γ 와 병렬 가능). `src/kosax/permissions/` 의 ~20 KOSAX-invented Spec 033 잔재 파일을 모두 제거하고, Spec 035 영수증 ledger 7 파일은 보존. 동시에 `AdapterRealDomainPolicy` 라는 단일 표준 Pydantic v2 모델을 신설해 18 어댑터의 metadata 를 KOSAX-invented 권한 분류에서 "기관 published 정책 URL 의 cite" 형태로 옮긴다.

기술 접근:
- (a) `src/kosax/permissions/` 의 잔재 파일 ~20 개 importer 추적 (`grep -r "from kosax.permissions" src/ tests/`) → importer cleanup 후 잔재 파일 삭제 (memory `feedback_no_stubs_remove_or_migrate`)
...
- (c) `src/kosax/tools/models.py` 에 `AdapterRealDomainPolicy` Pydantic v2 모델 추가 — `frozen=True`, `extra="forbid"`, 4 필드 type-annotated, 모든 str 필드 non-empty validator
...
모든 작업은 `/Users/um-yunsang/KOSAX-w-2295/` worktree 에서 수행. main / Epic β worktree 와 file conflict 없음 — 본 Epic 은 Python `src/kosax/` 만 다룸.
...
**Storage**: 본 Epic 은 in-memory + filesystem-only — Spec 035 receipt ledger (`~/.kosax/memdir/user/consent/`) 등 기존 storage 구조 변경 없음.
...
**Project Type**: Backend Python cleanup + 신규 모델 1 + 18 metadata 마이그레이션. 신규 모듈 0 (모든 신규 코드는 `src/kosax/tools/models.py` 의 기존 파일에 추가).
...
| **V. Policy Alignment** | ✅ 간접 (cite) | `AdapterRealDomainPolicy` 가 Korea AI Action Plan Principle 8 (cross-ministry citizen services) + 9 (Open API + OpenMCP) 의 "기관이 정책 published 하면 KOSAX 가 cite" 흐름을 codify. PIPA 영향 없음 (citizen data flow 변경 0). |
...
```text
# 본 Epic 은 src/kosax/ 하위 변경 — TUI 변경 0.
src/kosax/
...
│   ├── models.py                      ← DELETE (KOSAX-invented enums)
│   ├── modes.py                       ← DELETE
│   ├── pipeline.py                    ← DELETE
...
**Structure Decision**: 본 Epic 은 "잔재 파일 deletion + 단일 모델 추가 + 18 metadata 마이그레이션" 패턴. 신규 디렉토리 0, 신규 모듈 0 (모델은 기존 `tools/models.py` 에 추가). plan-template 의 Option 1 (Single project) 에 가까움 — `src/kosax/permissions/` 는 receipt-only 디렉토리로 축소되고, `src/kosax/tools/models.py` 가 어댑터 정책 표현의 단일 진실원이 됨.
