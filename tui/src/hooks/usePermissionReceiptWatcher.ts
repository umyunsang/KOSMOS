...
//   1. User selection is captured in the UMMAYA modal (UmmayaPrimitivePermissionRequest).
...
//
// CC reference: no upstream analog (UMMAYA-original, Spec 033 + Spec 1635).
...
/**
 * Mounts a fire-and-forget listener on the UMMAYA IPC bridge that watches
...
export function usePermissionReceiptWatcher(addReceipt: AddReceiptFn): void {
  ...
  useEffect(() => {
    ...
    bridge.onFrame = (frame, direction, latencyMs) => {
      ...
      const sessionId = getUmmayaBridgeSessionId() ?? 'unknown'
      ...
      const computedLayer: 1 | 2 | 3 = primitiveKind
        ? (aalToLayer(
            primitiveKind as UmmayaPrimitive,
            isIrreversible,
          ) ?? 1)
        : 1
