...
### Spec 026 prompt entities (`src/ummaya/prompts/`)
...
- **`gen_ai.*`** — GenAI semconv v1.40 attributes already emitted by `src/ummaya/llm/client.py`.
- **`ummaya.prompt.hash`** — UMMAYA extension from Spec 026; already emitted Python-side.
...
The `UserInputFrame` payload is defined by Spec 032; this Epic does **not** extend its shape. System-prompt SHA-256 propagation happens via the envelope's `ummaya.prompt.hash` OTEL attribute, not a new payload field.
...
The new file `tui/src/ipc/llmTypes.ts` defines UMMAYA-scoped types **structurally compatible** with the existing Anthropic SDK type surface that `QueryEngine.ts` + `query.ts` consume as type-only imports. These types are not serialized to the wire — they exist only so the agentic-loop code keeps compiling after `@anthropic-ai/sdk` imports are removed.
...
export type UmmayaRawMessageStreamEvent =
  ...
  | { type: 'message_delta'; delta: { stop_reason?: string }; usage?: UmmayaUsage }
  | { type: 'message_stop' };
...
**Mapping from `AssistantChunkFrame` to `UmmayaRawMessageStreamEvent`**:
...
- **V4**: OTEL `gen_ai.client.invoke` span on the TUI side MUST attach `ummaya.prompt.hash` attribute with a 64-char hex SHA-256 value.
- **V5**: FriendliAI usage fields from backend MUST map to `UmmayaUsage.input_tokens` / `output_tokens` / `cache_read_input_tokens`. Ignore fields not in the UMMAYA schema (forward compatibility).
...
**None introduced by this Epic.** Sessions remain in `~/.ummaya/memdir/user/sessions/` (Spec 027). OTEL spans go to local Langfuse via Spec 028 OTLP collector. Consent records in Spec 035 memdir. No schema changes to any persistent store.
