...
| UMMAYA file | Procedure | Byte-copy SHA match | Swap commits | Unjustified hunks | Missing citations |
|---|---|---|---|---|---|
| tui/src/services/api/claude.ts | A | ✅ | 12 | 0 | n/a |
| tui/src/ipc/llmClient.ts | B | n/a | 4 | 0 | 0 |
| src/ummaya/llm/client.py | B | n/a | 8 | 0 | 0 |
| src/ummaya/ipc/stdio.py | B | n/a | 6 | 0 | 0 |

### Stream-event channel coverage (CC services/api/claude.ts:1980-2295)

| CC event | UMMAYA handler | Status |
|---|---|---|
...
| content_block_start server_tool_use (2003) | (skipped) | ⏭ UMMAYA-N/A: server-side tools not used |
...
| content_block_delta signature_delta (2127) | (skipped) | ⏭ UMMAYA-N/A: K-EXAONE does not emit signatures |
| ...
...
```json
{
  ...
  "per_file": [
    {
      "ummaya_path": "tui/src/services/api/claude.ts",
      "procedure": "A",
  ...
  "stream_channel_coverage": [
    {
      ...
      "ummaya_handler_path": "tui/src/services/api/claude.ts:2148",
      "ummaya_skip_reason": null,
      "byte_copied": true
    }
  ],
  "exit_code": 0
...
3. For each UMMAYATargetFile (parsed from `parity-matrix.md`):
   a. If `procedure=A`: compute `sha256sum <ummaya_path>` AT byte-copy commit (parsed from git log subject `byte-copy(2521): import CC <cc_source_path>`); compare with `sha256sum <cc_source_path>`. Mismatch → mark `byte_copy_sha_match=false`.
   ...
   c. Compute `git diff <byte_copy_sha>..HEAD -- <ummaya_path>`; for each hunk, find which SwapCommit owns it. Hunks owned by NO commit (e.g., merge commits without category) → add to `unjustified_hunks`.
...
4. Enumerate StreamEventChannel from `.references/claude-code-sourcemap/restored-src/src/services/api/claude.ts:1980-2295` using `awk` on `case '...'` blocks. For each channel, find UMMAYA handler (grep for the same `case '...'` token in target files) OR a `// SKIPPED — UMMAYA-N/A:` comment.
