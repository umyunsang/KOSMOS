...
**Given** `dispatchPrimitive(opts)` with `timeoutMs = 30_000` (default) or `KOSAX_TUI_PRIMITIVE_TIMEOUT_MS` env override.
...
- An OTEL span attribute `kosax.tui.primitive.timeout = true` is set on the active span.
