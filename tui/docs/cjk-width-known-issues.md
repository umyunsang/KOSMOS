...
This document catalogs the two upstream Ink issues that affect KOSAX TUI rendering, the mitigation applied in every primitive renderer, and the residual known defect deferred to a future release.
...
**Reproduction context in KOSAX TUI**
...
This defect is **accepted as-is for v1** per Spec 287 § Edge Cases R5 (impact: Medium; probability: Low). The practical frequency in KOSAX backend data is low because `data.go.kr` API responses do not systematically use multi-codepoint emoji in structured fields.
