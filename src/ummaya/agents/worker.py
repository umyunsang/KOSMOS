...
`_query_inner` from `src/ummaya/engine/query.py` verbatim (FR-009).
...
from ummaya.agents.errors import AgentConfigurationError, PermissionDeniedError
...
from ummaya.engine.query import _query_inner
...
if TYPE_CHECKING:
    from ummaya.agents.mailbox.base import Mailbox
...
class Worker:
    ...
    async def _run_inner(self, instruction: str, correlation_id: UUID) -> None:  # noqa: C901
        ...
        from ummaya.llm.models import ChatMessage  # local import to avoid cycle
        ...
        async for event in _query_inner(query_ctx):
            ...
            with _tracer.start_as_current_span(span_name) as span:
                span.set_attribute(UMMAYA_AGENT_ROLE, self._ctx.specialist_role)
                span.set_attribute(UMMAYA_AGENT_SESSION_ID, str(self._ctx.session_id))
        ...
        if lookup_output is not None:
            ...
            from ummaya.tools.models import LookupMeta  # local import
    ...
    def _extract_lookup_output(
        ...
        from ummaya.tools.models import LookupOutput  # local import
    ...
    async def _request_permission(self, tool_id: str, correlation_id: UUID) -> bool:
        ...
        async for response_msg in self._mailbox.receive(self._ctx.worker_id):
            if response_msg.msg_type == MessageType.permission_response:
                from ummaya.agents.mailbox.messages import PermissionResponsePayload
