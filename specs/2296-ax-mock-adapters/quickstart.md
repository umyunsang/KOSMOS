...
This document walks through the canonical Epic ε demonstration: a Korean citizen tells KOSAX "내 종합소득세 신고해줘", and the LLM autonomously chains `verify(modid)` → `lookup(hometax_simplified)` → `submit(hometax_taxreturn)` → returns 접수번호. Every step uses Mock adapters with the six transparency fields fully populated.
...
- KOSAX worktree at `/Users/um-yunsang/KOSAX-w-2296` checked out on branch `2296-ax-mock-adapters` (Epic ε post-merge)
- Python 3.12+ via `uv`
- Bun v1.2.x for the TUI
- `expect` for the Layer 2 PTY scenario
- `vhs` ≥ 0.11 for the Layer 4 visual scenario
- `KOSAX_FRIENDLI_API_KEY` set in `.env` (the LLM must be reachable for the full chain; for offline runs, see § 6 alternative)
...
uv sync
uv run python -m kosax.ipc.demo.mock_backend &  # starts the Mock backend listening on stdio
# (Note: the backend is normally launched by the TUI via KOSAX_BACKEND_CMD; this manual launch
...
```text
INFO  kosax.tools.register_all  Registered tool: resolve_location
INFO  kosax.tools.register_all  Registered tool: lookup
INFO  kosax.tools.register_all  Registered tool: koroad_accident_search
... (12 Live tools)
INFO  kosax.tools.mock          Registered mock adapter: mock_verify_mobile_id (verify)
... (5 retrofitted existing verify mocks)
INFO  kosax.tools.mock          Registered mock adapter: mock_verify_module_simple_auth (verify)
... (5 new verify mocks)
INFO  kosax.tools.mock          Registered mock adapter: mock_submit_module_hometax_taxreturn (submit)
... (3 new submit mocks)
INFO  kosax.tools.mock          Registered mock adapter: mock_lookup_module_hometax_simplified (lookup, GovAPITool)
... (2 new lookup mocks)
INFO  kosax.ipc.demo.mock_backend  All 20 mock surfaces registered. Emitting AdapterManifestSyncFrame ...
INFO  kosax.ipc.adapter_manifest_emitter  Manifest emitted: 16 main-registry entries + 18 sub-registry entries; SHA-256=8a7b6c5d...
INFO  kosax.ipc.demo.mock_backend  Listening on stdio.
```
...
The TUI shows the KOSAX branding boot screen → idle prompt.
...
- Appends a `delegation_issued` event to `~/.kosax/memdir/user/consent/<today>.jsonl`
...
```bash
tail -3 ~/.kosax/memdir/user/consent/$(date +%Y-%m-%d).jsonl | jq -c .
```
...
1. spawns `bun run tui` with `KOSAX_BACKEND_CMD` set to the Mock backend
2. waits for KOSAX branding to appear
...
- `Screenshot specs/2296-ax-mock-adapters/smoke-keyframe-1-boot.png` — boot + KOSAX branding visible
...
If `KOSAX_FRIENDLI_API_KEY` is unavailable, replace step 3's "natural language to LLM" path with a scripted tool-call sequence using the backend RPC harness:
...
uv run python -c "
from kosax.ipc.demo.mock_backend import latest_manifest_emit_log
