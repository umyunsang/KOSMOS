...
from kosax.security.audit import MAX_CLOCK_SKEW_SECONDS, ToolCallAuditRecord
...
def test_model_validate_under_5ms_p_avg():
    ...
    The test skips when KOSAX_SKIP_PERF=1 to allow CI on constrained runners
    to opt out without failing the suite.
    """
    if os.environ.get("KOSAX_SKIP_PERF") == "1":
        pytest.skip("KOSAX_SKIP_PERF=1 — skipping performance assertion")
