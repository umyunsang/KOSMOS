...
> KOSAX 플러그인을 처음 만드는 기여자를 위한 가이드입니다. `git clone` 부터 로컬 `pytest` 통과까지 **30분 이내** 가 목표입니다 (SC-001).
>
...
>
> 참고: [Migration tree § B8](../requirements/kosax-migration-tree.md), [Spec 022 BM25 retrieval](../../specs/022-mvp-main-tool/spec.md), [Constitution §IV — Live API 차단 규칙](../../.specify/memory/constitution.md).
...
- 선택: KOSAX TUI — `kosax plugin init` 인터랙티브 플로우에만 필요. 비-TUI 환경에서는 `uvx kosax-plugin-init <name>` 으로도 스캐폴딩 가능.
...
1. <https://github.com/kosax-plugin-store/kosax-plugin-template> 방문
2. "Use this template" → "Create a new repository" 클릭
3. 새 repo 이름은 `kosax-plugin-<your-plugin-name>` 형식 (예: `kosax-plugin-busan-bike`)
4. 로컬 클론:
   ```sh
...
### 옵션 B — `kosax plugin init` CLI (TUI 가 있을 때)

```sh
kosax plugin init busan_bike --non-interactive --tier live --layer 1 --no-pii
cd busan_bike
```
...
processes_pii: false
slsa_provenance_url: https://github.com/<your-org>/kosax-plugin-busan-bike/releases/download/v0.1.0/busan_bike.intoto.jsonl
otel_attributes:
...
규칙:
- API 키는 반드시 `KOSAX_*` env var 에서 읽기 (Q-NO-HARDCODED-KEY).
...
| 한국어 | English | 설명 |
|---|---|---|
| 어댑터 | adapter | `kosax.tools.models.GovAPITool` 을 export 하는 플러그인의 코어 모듈 (`adapter.py`). |
...
| 카탈로그 | catalog | `kosax-plugin-store/index/index.json` — 설치 가능한 플러그인 메타 인덱스. |
| 수탁자 | trustee | PIPA §26 기반 위탁자/수탁자 체인의 수탁 측. Live tier + PII 처리 시 필수. |
| 동의 영수증 | consent receipt | 플러그인 install/uninstall 시 `~/.kosax/memdir/user/consent/<id>.json` 에 append. Spec 035 ledger 확장. |
...
- **`kosax-plugin-store` 게시** — quickstart 범위 외; PR merge 후 자동 진행.
