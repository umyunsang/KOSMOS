...
When the KOSAX Python backend POSTs to `/v1/chat/completions`, the server
...
The Python LLM client (`src/kosax/llm/client.py`) accumulates fragments and
...
answer), the KOSAX Python backend's agentic loop will:
