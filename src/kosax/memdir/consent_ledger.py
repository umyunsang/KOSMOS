...
Storage layout (per Spec 035 + data-model.md § 6):
    ``~/.kosax/memdir/user/consent/<YYYY-MM-DD>.jsonl``
...
HMAC ledger at ``~/.kosax/consent_ledger.jsonl`` handles that).
...
_DEFAULT_LEDGER_ROOT: Path = Path.home() / ".kosax" / "memdir" / "user" / "consent"
...
def append_delegation_issued(
    ...
    *,
    ...
    Args:
        ...
        ledger_root: Override ledger directory (default: ``~/.kosax/memdir/user/consent``).
...
class FileLedgerReader:
    ...
    Used by ``validate_delegation`` in ``kosax.primitives.delegation``.
    """
