...
**Audience**: New UMMAYA contributor who wants to (a) get a working dev environment in one minute, (b) edit a prompt and see the shadow-eval workflow run against their change, (c) dry-run the release manifest locally before pushing a tag.

**Prerequisites**: Docker Desktop (or Colima / Podman), VS Code with the "Dev Containers" extension, a Git checkout of `umyunsang/UMMAYA`, and a GitHub account with read access to the repo.

## A. First-time contributor — one-shot devcontainer onboarding

```bash
# From a shell on your host machine:
git clone https://github.com/umyunsang/UMMAYA.git
cd UMMAYA
code .
```
...
4. The container opens a terminal inside `/workspaces/UMMAYA`.
...
uv run python -m ummaya.context.prompt_loader --regenerate-manifest
...
docker build -f docker/Dockerfile -t ummaya:local .
```
...
docker inspect --format='{{.Size}}' ummaya:local | awk '{ printf "%.2f GB\n", $1/1024/1024/1024 }'

# 2. Runtime user must be UID 1000 (non-root).
docker run --rm ummaya:local id -u    # expected: 1000
```
...
uv run python -m ummaya.context.prompt_loader --emit-hashes > /tmp/prompt-hashes.json

# Render a candidate manifest.
uv run python -m tools.release_manifest.render \
    --commit-sha "$COMMIT_SHA" \
    --uv-lock-hash "$UV_LOCK_HASH" \
    --docker-digest "sha256:$(docker inspect --format='{{index .RepoDigests 0}}' ummaya:local | awk -F@ '{print $2}' | sed 's/^sha256://')" \
    --prompt-hashes-file /tmp/prompt-hashes.json \
    --friendli-model-id "$UMMAYA_FRIENDLI_MODEL_ID" \
...
export UMMAYA_PROMPT_REGISTRY_LANGFUSE=true
export UMMAYA_LANGFUSE_HOST="https://cloud.langfuse.com"
export UMMAYA_LANGFUSE_PUBLIC_KEY="pk-lf-..."
export UMMAYA_LANGFUSE_SECRET_KEY="sk-lf-..."
...
uv run python -m ummaya.cli repl
```
...
|---|---|---|
| `PromptRegistryError: hash mismatch for system_v1` at startup | You edited `prompts/system_v1.md` but forgot to regenerate `manifest.yaml`. | `uv run python -m ummaya.context.prompt_loader --regenerate-manifest` |
...
- `docs/vision.md § Reference materials` — the canonical reference table for every architectural decision in UMMAYA.
