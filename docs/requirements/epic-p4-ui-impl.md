...
`docs/requirements/ummaya-migration-tree.md § UI L2 결정사항` 의 승인된 UI
결정들을 실제 TUI 컴포넌트에 반영. 와이어프레임 mjs 파일들의 시각적 결정을
포팅된 CC 컴포넌트 + UMMAYA hook에 와이어링.
...
- UMMAYA 홈 시그널 마스코트로 교체 완료 (지붕 + 출입구 + 호출 신호)
...
### Related decisions
`docs/requirements/ummaya-migration-tree.md § UI L2 결정사항`
