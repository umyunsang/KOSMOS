...
- Claude Code's upstream types are hand-written TS (no code-gen) because the upstream backend is Anthropic's API, which publishes an OpenAPI schema. UMMAYA inverts this — the backend is our own Python, so Pydantic is the source of truth.
...
- Claude Code's `commands/` directory has 50+ entries (observed via `ls`); UMMAYA's v1 command set is 4. The registry can hold either — upstream shape is preserved even with a smaller enabled set.
...
- **Full command-set lift (50+)**: Rejected for v1 — most Claude Code commands (`/debug-tool-call`, `/autofix-pr`, `/break-cache`) have no UMMAYA analog. Deferred items table is the correct place for extending the set.
...
- `#468` env registry (FR-041 `UMMAYA_TUI_THEME` registration)
...
### 2.9 Backend: `ummaya-backend --ipc stdio` entrypoint

**Decision**: Add one module `src/ummaya/ipc/stdio.py` implementing an `asyncio` reader+writer loop that dispatches JSONL IPC frames to existing Spec 031 primitive handlers + Spec 027 coordinator mailbox. Add one CLI flag `--ipc stdio` to `src/ummaya/cli/__main__.py`. No new Python runtime deps (AGENTS.md hard rule).
**Rationale**:

- Existing Phase 1 CLI (`src/ummaya/cli/`) is retained unchanged per spec Assumptions. The `--ipc stdio` mode is an additional dispatch target — it reuses the same primitive registry, session store, observability span emission.
...
- Observability: every IPC frame emits a `ummaya.ipc.frame` span child of the session span (Spec 021), so frame-level latency is visible in the same OTEL pipeline (#501).
...
- `src/ummaya/cli/__main__.py` (existing entrypoint)
- `src/ummaya/primitives/` (existing primitive registry)
...
| 9 | Python `src/ummaya/ipc/stdio.py` + `--ipc stdio` CLI flag; no new deps | AGENTS.md hard rule, existing CLI |
