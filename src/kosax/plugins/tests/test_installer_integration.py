...
"""End-to-end integration tests for ``kosax.plugins.installer.install_plugin``.
...
``~/.kosax`` filesystem (``KOSAX_PLUGIN_*`` env vars + monkeypatched
...
def _manifest_dict(*, plugin_id: str = "demo_plugin", tier: str = "live") -> dict[str, Any]:
    ...
    return {
        ...
        "otel_attributes": {"kosax.plugin.id": plugin_id},
