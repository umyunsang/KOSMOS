...
Register two new `GovAPITool` adapters in the KOSAX Tool System (Layer 2) under
...
**Target Platform**: Linux server (KOSAX backend). CLI TUI unaffected.
**Project Type**: library/backend — extends `src/kosax/tools/` with two new
...
(`src/kosax/tools/ssis/codes.py`), two `TOOL_MIN_AAL` additions, one DPA
...
| IV. Government API Compliance | PASS | `research.md §7` prescribes recorded fixtures only; `rate_limit_per_minute=10` on both; `KOSAX_DATA_GO_KR_API_KEY` reused, no hardcoded keys; happy-path + error-path tests required per spec §4. |
...
### Source Code (repository root — KOSAX single-project layout)
...
**Structure Decision**: KOSAX single-project Python backend with the existing
`src/kosax/tools/<provider>/` convention (same shape as `nmc/`, `hira/`,
`kma/`, `koroad/`). No new top-level layout. No new module naming pattern.
The shared SSIS codes module (`src/kosax/tools/ssis/codes.py`) anticipates the
