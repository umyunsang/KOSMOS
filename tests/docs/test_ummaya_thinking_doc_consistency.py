...
the implementation in ``src/ummaya/llm/client.py``).
...
def test_implementation_default_is_true() -> None:
    """src/ummaya/llm/client.py must hard-code default 'true' for the env var."""
    ...
    assert 'os.environ.get("UMMAYA_K_EXAONE_THINKING", "true")' in content, (
        f"client.py must use default='true' for UMMAYA_K_EXAONE_THINKING.\nSearched in: {client_py}"
    )
...
def test_doc_does_not_contain_stale_false_default(doc_path: Path) -> None:
    """No canonical doc should say 'UMMAYA_K_EXAONE_THINKING ... default false'."""
    ...
    assert not matches, (
        f"Found stale 'default false' phrasing for UMMAYA_K_EXAONE_THINKING in "
