...
1. FR-016 (byte-identity): ``ummaya.primitives.lookup`` is the *same coroutine*
   as ``ummaya.tools.lookup.lookup`` — not a wrapper that alters input/output
...
from ummaya.tools.lookup import lookup as _canonical_lookup
...
def test_primitives_lookup_is_same_object_as_canonical() -> None:
    ...
    assert _primitives_lookup is _canonical_lookup, (
        "ummaya.primitives.lookup must be the identical object to "
        "ummaya.tools.lookup.lookup — any wrapping would break byte-identity."
    )
...
def test_primitives_lookup_preserves_dunder_module() -> None:
    ...
    assert "ummaya.tools.lookup" in _primitives_lookup.__module__
