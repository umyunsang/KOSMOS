...
"""LiteLLM callback hooks for the KOSAX content-moderation layer (Layer B).
...
from kosax.safety._models import ModerationBlockedEvent, ModerationWarnedEvent
from kosax.safety._span import emit_safety_event
...
def _call_moderation_api(content: str) -> tuple[tuple[str, ...], bool]:
    ...
    from kosax.settings import settings  # noqa: PLC0415 (avoid circular import)
...
def _ensure_enabled() -> bool:
    ...
    from kosax.settings import settings  # noqa: PLC0415 (avoid circular at module level)
