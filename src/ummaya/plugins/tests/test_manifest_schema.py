...
"""Unit tests for ``ummaya.plugins.manifest_schema``.
...
* Acknowledgment hash module integration — verifies the canonical
  SHA-256 constant exposed by :mod:`ummaya.plugins.canonical_acknowledgment`
...
from ummaya.plugins import canonical_acknowledgment as _canon
...
def _make_pipa_ack(
    *,
    ...
    return PIPATrusteeAcknowledgment(
        trustee_org_name="UMMAYA Plugin Authors",
...
class TestCrossFieldValidators:
    ...
    def test_v_otel_attribute_value_must_match_plugin_id(self) -> None:
        with pytest.raises(ValidationError) as exc:
            PluginManifest(
                **_make_manifest_kwargs(
                    otel_attributes={"ummaya.plugin.id": "wrong_id"},
                )
            )
        assert 'otel_attributes["ummaya.plugin.id"] must equal plugin_id' in str(exc.value)
    ...
    def test_v_otel_attribute_required_key_present(self) -> None:
        ...
        assert "ummaya.plugin.id" in str(exc.value)
