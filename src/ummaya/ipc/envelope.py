...
"""NDJSON emit/parse helpers for the UMMAYA IPC frame protocol (Spec 032 T012).
...
def compute_schema_file_hash(schema_path: Path) -> str:
    ...
    Used at backend startup to emit ``ummaya.ipc.schema.hash`` as an OTEL
...
def attach_envelope_span_attributes(
    ...
    *,
    ...
    Sets ``ummaya.ipc.correlation_id`` (always) and, when populated,
    ``ummaya.ipc.transaction_id`` and ``ummaya.ipc.tx.cache_state``.  The
    ...
    Args:
        ...
        tx_cache_state: ``"miss"`` / ``"hit"`` / ``"stored"`` from the
            :class:`~ummaya.ipc.transaction_lru.TransactionLRU` path when an
            irreversible-tool frame is being emitted; ``None`` otherwise.
    """
    span = trace.get_current_span()
    if not span.is_recording():
        return
    span.set_attribute(UMMAYA_IPC_CORRELATION_ID, frame.correlation_id)
    tx_id = getattr(frame, "transaction_id", None)
    if tx_id is not None:
        span.set_attribute(UMMAYA_IPC_TRANSACTION_ID, tx_id)
    if tx_cache_state is not None:
        span.set_attribute(UMMAYA_IPC_TX_CACHE_STATE, tx_cache_state)
...
def emit_schema_hash_resource_attribute(schema_path: Path | None = None) -> str | None:
    """Emit ``ummaya.ipc.schema.hash`` on the current OTEL span at backend startup.
    ...
    if span.is_recording():
        span.set_attribute(UMMAYA_IPC_SCHEMA_HASH, digest)
    logger.info(
