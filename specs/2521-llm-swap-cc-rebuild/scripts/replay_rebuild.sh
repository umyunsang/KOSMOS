...
# Procedure-A files: (cc_source_relative, kosax_target)
# Only one file is Procedure-A per spec.md FR-001 table.
PROC_A_CC="src/services/api/claude.ts"
PROC_A_KOSAX="tui/src/services/api/claude.ts"
...
if [[ "$SELF_TEST" -eq 1 ]]; then
  run_self_test() {
    ...
    # 4. Verify the captured (current) PROC_A_KOSAX SHA equals the CC SHA
    ...
    local KOSAX_FILE_ABS="${REPO_ROOT}/${PROC_A_KOSAX}"
    if [[ ! -f "$KOSAX_FILE_ABS" ]]; then
      error "Self-test: KOSAX target file missing: $KOSAX_FILE_ABS"
      return 2
    fi
...
if false; then
  _placeholder_legacy_self_test() {
    ...
    ACTUAL_FINAL_SHA="$(sha256_file "$KOSAX_FILE_ABS")"
...
main_replay() {
  ...
  info "PROC-A    : $PROC_A_CC  →  $PROC_A_KOSAX"
  ...
  info "Step 3: Byte-copying $PROC_A_CC → $PROC_A_KOSAX"

  local KOSAX_FILE_ABS="${REPO_ROOT}/${PROC_A_KOSAX}"
  if [[ "$DRY_RUN" -eq 1 ]]; then
    info "[DRY-RUN] cp $CC_FILE_ABS $KOSAX_FILE_ABS"
  else
    cp "$CC_FILE_ABS" "$KOSAX_FILE_ABS"
  fi
  ...
  if [[ "$DRY_RUN" -ne 1 ]]; then
    local POST_COPY_SHA
    POST_COPY_SHA="$(sha256_file "$KOSAX_FILE_ABS")"
    if [[ "$POST_COPY_SHA" != "$ACTUAL_CC_SHA" ]]; then
      error "SHA-256 mismatch after byte-copy!"
      error "  CC source : $ACTUAL_CC_SHA"
      error "  KOSAX    : $POST_COPY_SHA"
      diff "$CC_FILE_ABS" "$KOSAX_FILE_ABS" | head -40 || true
      exit 1
    fi
  ...
  if [[ "$DRY_RUN" -eq 1 ]]; then
    ...
    info "Run: git diff ${REBUILD_BRANCH} -- ${PROC_A_KOSAX}"
