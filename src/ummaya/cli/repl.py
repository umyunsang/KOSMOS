...
"""REPL loop for the UMMAYA CLI — citizen conversation interface."""
...
class REPLLoop:
    """Async REPL loop for citizen conversation with the UMMAYA engine.
    ...
    - Session persistence via :class:`~ummaya.session.manager.SessionManager`
    - Interrupt handling:
    ...
    async def run(self) -> None:
        ...
        while True:
            try:
                user_input = await prompt_session.prompt_async("UMMAYA › ")
    ...
    async def _init_session(self) -> None:
        ...
        from ummaya.session.manager import SessionManager  # noqa: PLC0415
