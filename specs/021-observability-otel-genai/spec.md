...
**Why this priority**: KOSAX는 멀티턴·멀티도구 루프라 blackbox 로그만으로는 장애 원인을 찾을 수 없다. Phase 2 시나리오가 본격화되기 전 "최소 1개 trace로 전체 흐름을 볼 수 있는" 관측성이 선행 조건이다 (Infrastructure Initiative SC-7).
...
2. **Given** LLM 스트리밍 호출 중 FriendliAI가 `429` 응답을 반환해 재시도가 발생한다, **When** 재시도가 성공한다, **Then** 해당 `chat` span은 하나로 유지되고 재시도는 `kosax_llm_rate_limit_retries_total` counter에 1회 증가되며 span 상태는 `UNSET`으로 마감된다.
...
- OTel semantic conventions가 "Development" 안정성이라 향후 breaking rename 가능: 발생 시 KOSAX는 최신 rename을 반영하되, 이전 속성명은 유지하지 않는다(수동 span이라 매핑이 명확하므로).
...
- **FR-001**: 사용자의 한 질의가 에이전트 루프에 진입하면 시스템은 `invoke_agent kosax-query` 이름의 부모 span을 생성한다. 이 span은 `gen_ai.conversation.id` 속성으로 세션 식별자를 달고, 자식 span들이 모두 이 부모 아래에 묶인다.
...
- **Auto-instrumentors (`opentelemetry-instrumentation-*`, OpenLLMetry/Traceloop 등)**: KOSAX의 LLM 호출은 `httpx` 기반 수동 프로토콜이며 auto-instrumentor가 훅 걸 대상이 없다. AGENTS.md 외부 의존성 최소 원칙과도 상충.
