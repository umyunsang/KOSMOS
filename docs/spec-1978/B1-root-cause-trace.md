...
> "TUI 입력 미반응 (별개 버그): PTY 재현 결과 — TUI 부팅 OK + 텍스트 타이핑 OK, **Enter 후 25초 응답 0 byte**, IPC frame 송신 자체가 안 일어남. PromptInput.onSubmit 가드 swallow 위치 미확정 (UMMAYA_TUI_LOG_LEVEL=DEBUG stderr 분리 캡처 필요)."
...
These auto-stubs exist on disk in the main `UMMAYA/` checkout but are never committed because they are matched by `.gitignore`. A fresh worktree under `git worktree add` therefore lacks them, and `import env from './commands/env/index.js'` (`tui/src/commands.ts:169`) throws `Cannot find module`.

The same `.gitignore` rule hides **45 P0-stub files** total under `tui/src/`, `src/ummaya/`, and `scripts/`. Categories (sample):
...
These were copied from `UMMAYA/` into `UMMAYA-wiring/` mid-trace (411 files including Python `__pycache__` artefacts). The `Cannot find module` error went away.
...
- The actual TUI banner (`UMMAYA v…`) is never printed
