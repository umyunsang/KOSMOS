...
export class LLMClient {
   ...
   *  4. Translate each inbound frame to a KosaxRawMessageStreamEvent; yield.
   ...
   */
  stream(params: KosaxMessageStreamParams): AsyncGenerator<KosaxRawMessageStreamEvent, KosaxMessageFinal, void>;
   ...
   */
  complete(params: KosaxMessageStreamParams): Promise<KosaxMessageFinal>;
}
...
- **G6** The generator's return value (final `KosaxMessageFinal`) carries `{ stop_reason, usage: { input_tokens, output_tokens, cache_read_input_tokens? } }` populated from the Python backend's done-frame trailer.
...
export type KosaxRawMessageStreamEvent =
  ...
  | { type: 'message_delta'; delta: { stop_reason?: 'end_turn' | 'max_tokens' | 'tool_use' | 'stop_sequence' }; usage?: KosaxUsage }
  | { type: 'message_stop' };
...
| `gen_ai.request.max_tokens` | int | ✅ | from `KosaxMessageStreamParams.max_tokens` |
| `gen_ai.request.temperature` | float | ⬜ | if provided |
| `gen_ai.usage.input_tokens` | int | ✅ | from final `KosaxMessageFinal.usage.input_tokens` |
| `gen_ai.usage.output_tokens` | int | ✅ | from final `KosaxMessageFinal.usage.output_tokens` |
| `kosax.prompt.hash` | string | ✅ | SHA-256 hex, forwarded by backend in response metadata |
| `kosax.correlation_id` | string | ✅ | the envelope `correlation_id` used for this turn |
| `kosax.transaction_id` | string | ⬜ | envelope `transaction_id` if set |
| `kosax.session_id` | string | ✅ | active session |
