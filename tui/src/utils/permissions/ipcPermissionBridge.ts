...
// KOSAX-original — Epic FU-4 · IPC permission_request → toolUseConfirmQueue bridge.
//
...
//   docs/requirements/kosax-migration-tree.md § UI-C
...
// ---------------------------------------------------------------------------
// Audit-4 P0-5 fix — per-request decision stash so the KOSAX adapter can
...
/**
 * Adapter-only setter. The KOSAX PrimitivePermissionRequest adapter calls
...
function primitiveKindToTool(kind: PrimitiveKind): Tool {
  switch (kind) {
    ...
    default: {
      ...
      console.warn(`[kosax.ipc.permission] unknown primitive_kind=${String(_exhaustive)}, falling back to VerifyPrimitive`)
      return VerifyPrimitive
    }
  }
}
 ...
 * frame via the kosax bridge's write channel (process.stdout NDJSON).
 *
...
export function pushIpcPermissionRequest(frame: PermissionRequestFrame): void {
  ...
  if (setter === null) {
    console.warn(
      `[kosax.ipc.permission] no setter registered — cannot present permission modal for request_id=${frame.request_id}. Is REPL mounted?`,
    )
    return
  }
  ...
  const assistantMessage = createAssistantMessage({
    content: `[kosax permission gate] ${frame.description_en}`,
  })
  ...
  // module-level stash (set by KosaxPermissionRequestAdapter just before
  // it called us). Defaults to 'allow_once' for non-KOSAX adapters that
  ...
  const confirm: ToolUseConfirm = {
    ...
    input: {
      ...
      // finally to the primitive verb. The KOSAX adapter feeds this into
 ...
 * `process.stdout.write(encodeFrame(...))` directly. In the KOSAX TUI
...
function _sendPermissionResponse(
  ...
  const responseFrame: PermissionResponseFrame = {
    ...
    // `rcpt-<hex>` string written to ~/.kosax/memdir/user/consent/.
  ...
  if (!sent) {
    ...
    process.stderr.write(
      `[KOSAX permissionBridge WARN] permission_response drop ` +
