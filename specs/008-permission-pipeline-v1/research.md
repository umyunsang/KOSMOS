...
**Question**: FR-021 requires returning `ToolResult(error_type="permission_denied")`, but the existing Literal in `src/ummaya/tools/models.py` is:
```python
...
**Action in plan**: Phase 4 explicitly modifies `src/ummaya/tools/models.py`. This is the only touch to Layer 2 code.
