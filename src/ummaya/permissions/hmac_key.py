...
"""HMAC key loader / generator for the UMMAYA consent ledger (Spec 033 FR-D04).

Manages the 32-byte HMAC-SHA-256 secret stored at ``~/.ummaya/keys/ledger.key``
(configurable via ``UMMAYA_PERMISSION_KEY_PATH`` env var).
...
def load_or_generate_key(path: Path) -> bytes:
    ...
    Args:
        path: Filesystem path for the HMAC key file.  Typically
              ``Path.home() / ".ummaya" / "keys" / "ledger.key"``.
    ...
    _logger.info("ummaya.permissions.ledger - initialised at %s", path)
    return key
...
def bootstrap_hmac_key(
    ...
    Args:
        key_path: Absolute path to the HMAC key file (``~/.ummaya/keys/ledger.key``).
        key_registry_path: Absolute path to the key registry JSON
            (``~/.ummaya/keys/registry.json``).
