...
**Location**: `docs/api/<source>/<tool>.md`. The `<source>` segment matches `ummaya.tools.<source>` directory naming with normalization noted in research.md § R5 (e.g., `ssis` → `mohw`).
...
| 6. Worked example | `## Worked example` | At least one `lookup(mode="fetch")` (or `submit` / `verify`) invocation: input JSON, output JSON, UMMAYA conversation snippet |
...
| `$id` | `https://ummaya.example/api/schemas/<tool_id>.json` (the example domain is a placeholder; FR-007 idempotency needs only stability, not resolvability) |
| `title` | The tool_id |
...
**Inputs**: registry instance constructed by importing `ummaya.tools.register_all`.
...
The UMMAYA v0.1-alpha entry in `CHANGELOG.md`.
...
```markdown
## v0.1-alpha — UMMAYA Migration Completion
...
- UMMAYA now routes a Korean citizen's request through the migrated Claude Code harness:
...
- 5-tier plugin DX (P5) onboards external contributors via `ummaya plugin init`.
...
No state transitions; everything is materialized once at the close of P6 and persists as the canonical record of the UMMAYA migration.
