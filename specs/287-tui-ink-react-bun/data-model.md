...
```python
# Python source of truth — src/ummaya/ipc/frame_schema.py (new)
...
- If the backend process exits non-zero, the TUI surfaces `<CrashNotice />` within 5 s. All `UMMAYA_*` env var values in the trace are redacted (FR-004).
...
| `tui/src/commands/resume.ts` | (new, UMMAYA-original following registry shape) |
...
### 4.6 UMMAYA-original (no upstream)

| TUI path | Why original |
|---|---|
| `tui/src/ipc/bridge.ts` / `codec.ts` / `crash-detector.ts` | UMMAYA stdio protocol; Claude Code uses HTTP SSE |
| `tui/src/components/primitive/*` | UMMAYA 5-primitive renderers; no Claude Code analog |
| `tui/scripts/gen-ipc-types.ts` | Pydantic → TS code-gen; Claude Code uses hand-written TS types |
| `tui/scripts/diff-upstream.sh` | UMMAYA traceability tool |
| `src/ummaya/ipc/stdio.py` | Python multiplexer; Anthropic backend doesn't need this |
...
|---|---|---|---|
| `UMMAYA_TUI_THEME` | `default \| dark \| light` | `default` | Theme selection (FR-039, FR-041) |
| `UMMAYA_TUI_LOG_LEVEL` | `DEBUG \| INFO \| WARN \| ERROR` | `WARN` | IPC frame logging verbosity (FR-010) |
| `UMMAYA_TUI_SUBSCRIBE_TIMEOUT_S` | int (seconds) | `120` | `subscribe` stream timeout (Edge Case) |
| `UMMAYA_TUI_IME_STRATEGY` | `fork \| readline` | (set by ADR) | Korean IME strategy selector (FR-014) |
| `UMMAYA_TUI_SOAK_EVENTS_PER_SEC` | int | `100` | Soak test rate (dev only; FR-007) |

All `UMMAYA_TUI_*` values MUST be redacted from crash notices per #468 guard pattern (FR-004).
...
```
  OPEN --(stream_event*)--> OPEN --(stream_closed: exhausted|revoked|timeout)--> CLOSED
         |
         | no event for UMMAYA_TUI_SUBSCRIBE_TIMEOUT_S
         v
