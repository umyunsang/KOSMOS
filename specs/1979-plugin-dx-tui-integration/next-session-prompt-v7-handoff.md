...
- **Codex P1 deferred → #2395**: TS-side `context.options.tools` (14 KOSAX tools) ≠ backend Python registry (18 agency adapters). adapter manifest IPC sync 가 Epic ε piggyback.
...
cd ../KOSAX-w-2297
# /speckit-specify Epic ζ 시작 — End-to-end 시나리오 + docs/scenarios OPAQUE + KOSAX v0.1-beta tag
```
...
7. **PTY smoke 가 backend 로직 보장 안 함** — `KOSAX_BACKEND_CMD=sleep 60` mock 은 stale-import / dead-JSX-path 회귀만 보장. adapter dispatch 검증은 별도 (Epic ε 의 mock fixture 도입 후 가능).
