...
**Created**: 2026-04-29
**Status**: Draft
**Input**: User description: "Epic δ — `src/ummaya/permissions/` 에서 ~20 UMMAYA-invented Spec 033 파일 삭제 + Spec 035 영수증 ledger 7 파일 보존 + `AdapterRealDomainPolicy` Pydantic v2 모델 신설 + 18 어댑터 metadata 마이그레이션."
**Authority** (cite in every downstream artefact):
- `AGENTS.md § CORE THESIS` — UMMAYA = AX-infrastructure callable-channel client (3rd thesis canonical); UMMAYA does NOT invent permission policy
- `AGENTS.md § Hard rules` — Pydantic v2 strict typing for tool I/O; no `Any`; English source text; `UMMAYA_` env vars; never add deps outside spec
- `.specify/memory/constitution.md § II Fail-Closed Security (NON-NEGOTIABLE)` — UMMAYA-invented 권한 분류 (5-mode spectrum / pipa_class / auth_level / permission_tier / is_personal_data / is_irreversible / requires_auth / dpa_reference) MUST NOT be reintroduced
...
### User Story 1 — Spec 033 UMMAYA-invented permission residue 제거 (Priority: P1) 🎯 MVP

UMMAYA Lead 가 `src/ummaya/permissions/` 의 UMMAYA-invented Spec 033 잔재 파일을 모두 제거한다. Constitution II 가 NON-NEGOTIABLE 하게 금지한 5-mode spectrum / pipa_class / auth_level / permission_tier / is_personal_data / is_irreversible / requires_auth / dpa_reference 같은 분류가 코드 레벨에서 0 회 잔존해야 한다. 단 Spec 035 영수증 ledger 관련 파일 (ledger.py / action_digest.py / hmac_key.py / canonical_json.py / audit_coupling.py / ledger_verify.py / otel_emit.py / otel_integration.py) 은 보존해 receipt 발행 시스템이 계속 작동.

**Why this priority**: UMMAYA thesis (CC + 2 swaps) 의 boundary 가 코드에서 enforce 되려면 UMMAYA-invented 권한 정책이 0 이어야 한다. 백엔드 잔재가 남아 있으면 어댑터 metadata 마이그레이션 (US3) 이 "정리 안 된 위에 새 모델 얹기" 가 되어 의미가 흐려진다.

**Independent Test**: cleanup 후 (a) `src/ummaya/permissions/` 디렉토리에 14 잔재 source file + `steps/` 0 회 잔존 + Spec 035 receipt 8 파일 + `models.py` (SessionContext-only) + `credentials.py` + `__init__.py` 만 보존, (b) FR-006 narrowed grep gate 출력 0 행 (어댑터 metadata + `src/ummaya/permissions/` 영역만), (c) `uv run pytest` baseline 대비 NEW failure 0.

**Acceptance Scenarios**:

1. **Given** `src/ummaya/permissions/` 의 14 Spec 033 잔재 source file + `steps/` list, **When** Lead 가 각 파일의 importer 를 `grep -r` 로 추적해 importer 도 함께 cleanup 후 잔재 파일 삭제, **Then** `git ls-files src/ummaya/permissions/` 가 11 entry (Spec 035 receipt 8 + `models.py` SessionContext-only + `credentials.py` + `__init__.py`) 만 출력.
2. **Given** Constitution II 금지 토큰 list, **When** FR-006 narrowed grep gate 실행 (어댑터 metadata + `src/ummaya/permissions/` 영역), **Then** 0 행.
...
UMMAYA-invented 권한 분류를 대체할 단일 표준 모델 `AdapterRealDomainPolicy` 를 Pydantic v2 strict 로 신설한다. 본 모델은 어댑터가 (a) 한국 정부 기관의 published 정책 URL 을 인용하고, (b) 시민에게 노출할 gate 카테고리 (`read-only` / `login` / `action` / `sign` / `submit`) 를 선언하며, (c) 정책 인용의 마지막 검증 시점을 박제하는 단일 진실원으로 작동한다.

**Why this priority**: UMMAYA = AX-infrastructure callable-channel client thesis 의 핵심 — UMMAYA 는 권한 정책을 발명하지 않고 기관의 정책을 cite 한다. `AdapterRealDomainPolicy` 가 그 cite 의 codified 형태. US1 (잔재 제거) 후 US3 (어댑터 마이그레이션) 진입 전 본 모델이 박제되어 있어야 함.

**Independent Test**: 모델이 `src/ummaya/tools/models.py` 에 추가되고, frozen=True + `extra="forbid"` 로 strict 강제, 모든 필드 type-annotated, `uv run pytest` 가 모델의 schema 검증 케이스 통과.
...
기존 18 어댑터 (KOROAD ×2 + KMA ×6 + HIRA ×1 + NMC ×1 + NFA119 ×1 + MOHW ×1 + 6 mocks) 의 metadata 를 Constitution II 금지 필드에서 `AdapterRealDomainPolicy` 인스턴스로 옮긴다. 각 어댑터는 기관의 published 정책 URL 을 cite 하며, UMMAYA 가 권한을 invent 하지 않음을 코드로 증명한다.

**Why this priority**: 본 마이그레이션이 "Tool wrapping is the work" 의 1:1 적용. 한 어댑터당 한 metadata block 마이그레이션이 단위 작업. US1+US2 완료 후 진입.

**Independent Test**: 18 어댑터 모두가 (a) 금지 필드 0 회 잔존, (b) `policy: AdapterRealDomainPolicy` 인스턴스 보유, (c) 모든 인스턴스의 `real_classification_url` 이 non-empty + 기관 published 정책 URL 형식. `uv run python -c "from ummaya.tools.registry import ToolRegistry; r = ToolRegistry(); ..."` 로 18 어댑터 모두 import 통과.
...
- **금지 토큰 grep 이 docstring 또는 docs/ 의 historical reference 에서 매치**: 본 Epic 의 grep 검사는 `src/ummaya/` 하위 source code 만 대상. docs/ 의 historical reference (예: `cc-source-scope-audit.md` 자체) 는 대상 외.
...
- **FR-002a** (NEW): UMMAYA-needed 보조 파일 2개도 보존:
  - **(a) `credentials.py`** — `src/ummaya/recovery/auth_refresh.py` 가 `candidate_env_vars` / `resolve_credential` 을 import 해 credential resolution 에 사용.
  - **(b) `models.py`** — Spec 035 receipt schema 가 의존하는 클래스들 + harness session schema 만 잔존:
    - `SessionContext` — `src/ummaya/cli/{app,repl}.py` + `src/ummaya/engine/models.py` + observability tests 가 import (harness session state, UMMAYA-invented permission policy 가 아님 — 단순 session bookkeeping).
    ...
    - 삭제 대상 (UMMAYA-invented orchestration): `AccessTier`, `PermissionDecision`, `PermissionCheckRequest`, `PermissionStepResult`, `AuditLogEntry`, `PermissionRule`.
    - `PermissionMode` Literal — `modes.py` 가 삭제되므로 **inline 으로 `models.py` 에 통합** (Spec 035 receipt schema 가 `mode` 필드로 참조).
- **FR-003**: `AdapterRealDomainPolicy` Pydantic v2 모델이 `src/ummaya/tools/models.py` 에 추가되어야 한다. 필드 (4개, frozen=True, extra="forbid"):
...
- **FR-003a** (NEW · Path B 2026-04-29): Spec 024/025/1636 V1-V6 invariant 인프라가 `policy.citizen_facing_gate` 기반으로 재작성되어야 한다. 새 모듈 `src/ummaya/tools/policy_derivation.py` 가 canonical mapping 제공:
  ...
  - `GovAPITool` 의 V1-V6 model_validator 는 `self.policy` 가 set 일 때만 실행 (UMMAYA internal tool: `ministry='UMMAYA'` ⇒ policy=None 허용).
...
- **FR-006** (NARROWED 2026-04-29): UMMAYA 가 발명한 권한 분류 토큰이 **어댑터 metadata 영역**에서 0 회 잔존해야 한다. Grep gate 명시:
  - **검사 대상 (must be 0 lines)**: `grep -rE 'pipa_class|auth_level|permission_tier|is_personal_data|is_irreversible|requires_auth|dpa_reference' src/ummaya/tools/koroad/ src/ummaya/tools/kma/ src/ummaya/tools/hira/ src/ummaya/tools/nmc/ src/ummaya/tools/nfa119/ src/ummaya/tools/ssis/ src/ummaya/tools/mock/`
  - **명시 제외 (UMMAYA-needed legitimate uses, NOT permission policy invention)**:
    - `src/ummaya/permissions/` — Spec 035 receipt schema (`ConsentLedgerRecord` / `ConsentDecision` / `ToolPermissionContext` / `AdapterPermissionMetadata` 등) 가 **historical-record Optional 필드**로 이 토큰들을 담는다. 영수증은 결정 시점의 사실을 박제하는 schema 이지, UMMAYA 가 새 권한 정책을 invent 하는 곳이 아니다. (Spec 035 ledger v1.0.0 schema invariant 보존 — 변경 시 hash chain 검증 깨짐.)
    - `src/ummaya/security/audit.py` — Spec 024 `ToolCallAuditRecord` invariant (TOOL_MIN_AAL 매트릭스 enforce, 어댑터가 declare 한 field 의 consistency 만 강제 — 권한 정책 발명 X)
    - `src/ummaya/tools/permissions.py` — Spec 025 v6 `compute_permission_tier(auth_level, is_irreversible)` (UI-C layer derivation; 같은 invariant)
    - `src/ummaya/tools/register_all.py` — single comment referencing `compute_permission_tier`
    - `src/ummaya/plugins/checks/q3_security.py` — Spec 1636 plugin Q3-V2/V3/V4/V6 invariant gates
    - `src/ummaya/plugins/checks/q2_failclosed.py` — Spec 1636 plugin Q2 fail-closed gate
    - `src/ummaya/plugins/cli_init.py` — Spec 1636 plugin template (`auth_level="AAL1"` default)
    - `src/ummaya/plugins/tests/*` — plugin test fixtures
    - `src/ummaya/ipc/tx_cache.py` — Spec 032 T041 idempotency pin enforcement (`is_irreversible=True` ⇒ pinned cache)
    - `src/ummaya/ipc/demo/register_irreversible_fixture.py` — Spec 032 demo fixture
    - `src/ummaya/recovery/auth_refresh.py` — credential resolution (uses `ummaya.permissions.credentials`)
  - **Rationale**: UMMAYA thesis (CC + 2 swaps) 는 UMMAYA 가 어댑터 metadata 단계에서 새 권한 정책을 *발명*하지 않음을 의미한다. 위 제외 항목들은 (a) 어댑터가 **이미 declare 한** field 들의 *consistency invariant* 만 강제하거나 (Spec 024/025/1636 Q3), (b) infrastructure-level state (Spec 032 idempotency cache, Spec 035 receipt ledger schema) 를 다룬다 — 권한 정책 발명이 아니다. 어댑터 metadata 자체에서 UMMAYA-invented field 가 0 이면 thesis enforce 됨.
...
- **FR-010**: `__init__.py` 가 다음만 export — UMMAYA-invented 잔재 모듈 (bypass / killswitch / modes / pipeline / pipeline_v2 / rules / session_boot / synthesis_guard / mode_bypass / mode_default / aal_backstop / adapter_metadata / cli / prompt) export 0:
...
- **AdapterRealDomainPolicy**: 4 필드 Pydantic v2 frozen 모델. UMMAYA 의 단일 권한 표현 — 기관 published 정책 URL 인용 + 시민-facing gate 카테고리 + 검증 시점.
...
- **SC-001**: 14 Spec 033 잔재 source file + `steps/` 디렉토리 100% 삭제 (`git ls-files src/ummaya/permissions/` 의 11 row 만 잔존 — Spec 035 receipt 8 + `models.py` (trimmed to SessionContext + receipt-schema dependencies) + `credentials.py` + `__init__.py`).
...
- **SC-006**: Constitution II 금지 토큰 grep 결과 0 행 매치 (FR-006 narrowed scope: 어댑터 metadata 영역 + `src/ummaya/permissions/` 만).
...
- Spec 035 receipt ledger 7 파일은 UMMAYA 가 발행하는 영수증 (consent / action 기록) 의 정형화된 인프라이며, 본 Epic 의 cleanup 로 인해 receipt 발행 use case 가 깨지지 않아야 한다 — receipt 단위 테스트가 baseline 에서 통과 중이면 그대로 통과해야 한다.
...
- 본 Epic 작업은 `/Users/um-yunsang/UMMAYA-w-2295/` worktree 에서 진행되며 Epic β / main worktree 와 file conflict 없음.
...
| Spec 035 receipt ledger 의 UMMAYA 통합 시점 (현재는 backend-only, TUI 노출 미연결) | 본 Epic 은 receipt ledger 보존만 다룸; receipt 발행 → TUI 노출 (사용자에게 영수증 보여주기) 은 별도 작업 | Epic ζ (#2297) E2E smoke + 정책 매핑 doc | #2297 |
