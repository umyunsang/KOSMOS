...
"""Live observability pipeline verification tests for UMMAYA.
...
* ``UMMAYA_DATA_GO_KR_API_KEY`` — data.go.kr shared API key (KOROAD)
...
Event types (``src/ummaya/observability/events.py``):
...
Metric names (``src/ummaya/tools/executor.py``,
...
def _attach_capture_handler(logger_name: str = "ummaya.events") -> _InMemoryLogHandler:
...
def _detach_handler(handler: _InMemoryLogHandler, logger_name: str = "ummaya.events") -> None:
...
async def test_live_metrics_collector_under_live_llm_stream(
    ...
    The LLMClient implementation (src/ummaya/llm/client.py) does NOT record
...
async def test_live_event_logger_emits_tool_events(
    ...
    Captured via a custom logging.Handler attached to the 'ummaya.events' logger,
    ...
    assert len(tool_events) >= 1, (
        f"Expected ≥1 tool_call event in 'ummaya.events' log, "
...
async def test_live_event_logger_emits_llm_events(
    ...
    assert len(llm_events) >= 1, (
        f"Expected ≥1 llm_call event in 'ummaya.events' log, "
