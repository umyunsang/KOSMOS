...
"""CLI configuration loaded from environment variables with ``KOSAX_CLI_`` prefix."""
...
class CLIConfig(BaseSettings):
    """Runtime configuration for the KOSAX CLI.

    All fields are configurable via environment variables prefixed with
    ``KOSAX_CLI_``.  For example, ``KOSAX_CLI_SHOW_USAGE=false`` disables
    the per-turn usage summary.
    """

    model_config = SettingsConfigDict(env_prefix="KOSAX_CLI_")
