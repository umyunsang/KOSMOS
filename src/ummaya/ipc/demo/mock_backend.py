...
A fully-functional UMMAYA backend process that boots the complete mock
...
JSONL stdio loop (same pattern as :mod:`ummaya.ipc.mcp_server`).

This module is the canonical target for ``UMMAYA_BACKEND_CMD`` during
smoke-testing and offline demos:

    UMMAYA_BACKEND_CMD="uv run python -m ummaya.ipc.demo.mock_backend" bun run tui

Design decisions (research.md Decision 5):
- Reuses the stdio loop from ``mcp_server._run_loop`` (DRY principle).
- Imports ``ummaya.tools.mock`` EXPLICITLY after ``register_all_tools()`` so
...
Entry point::
    ...
    uv run python -m ummaya.ipc.demo.mock_backend

Module docstring notes ``UMMAYA_BACKEND_CMD`` invocation so any reader of the
        ...
        if method == "initialize":
            return {
                ...
                "result": {
                    ...
                    "serverInfo": {"name": "ummaya-mock-backend", "version": "0.1.0"},
                },
            }
...
async def main() -> None:
    ...
    4. Import ``ummaya.tools.mock`` — triggers __init__.py side-effects that register all
       verify and submit mock surfaces into their per-primitive sub-registries.
       Subscribe adapters are intentionally not imported until UMMAYA has an
    ...
    from ummaya.ipc.adapter_manifest_emitter import emit_manifest  # noqa: PLC0415
    ...
    # Step 4: Import ummaya.tools.mock to register all 20 mock surfaces.
    ...
    try:
        from ummaya.ipc.adapter_manifest_emitter import _build_entries  # noqa: PLC0415
