...
import { LLMClient, UMMAYA_DEFAULT_MODEL } from '../../src/ipc/llmClient.js'
...
describe('LLMClient.complete() — OTEL gen_ai.client.invoke span (T021, SC-008/SC-010)', () => {
  ...
  test('span gen_ai.request.model equals UMMAYA_DEFAULT_MODEL', async () => {
    const { bridge } = makeMockBridgeWithPromptHash(FAKE_PROMPT_HASH)
    const client = new LLMClient({
      bridge,
      model: UMMAYA_DEFAULT_MODEL,
    ...
    const span = _exporter.getFinishedSpans().find(s => s.name === 'gen_ai.client.invoke')
    expect(span).toBeDefined()
    expect(span!.attributes['gen_ai.request.model']).toBe(UMMAYA_DEFAULT_MODEL)
  })

  test('span has ummaya.session_id === sess-1', async () => {
    const { bridge } = makeMockBridgeWithPromptHash(FAKE_PROMPT_HASH)
    const client = new LLMClient({
      bridge,
      model: UMMAYA_DEFAULT_MODEL,
    ...
    const span = _exporter.getFinishedSpans().find(s => s.name === 'gen_ai.client.invoke')
    expect(span).toBeDefined()
    expect(span!.attributes['ummaya.session_id']).toBe(SESSION_ID)
  })

  test('span ummaya.correlation_id matches UUIDv7 pattern', async () => {
    const { bridge } = makeMockBridgeWithPromptHash(FAKE_PROMPT_HASH)
    const client = new LLMClient({
      bridge,
      model: UMMAYA_DEFAULT_MODEL,
    ...
    const span = _exporter.getFinishedSpans().find(s => s.name === 'gen_ai.client.invoke')
    expect(span).toBeDefined()
    const correlationId = span!.attributes['ummaya.correlation_id']
    expect(typeof correlationId).toBe('string')
    expect(correlationId as string).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-/)
  })

  test('span ummaya.prompt.hash equals the bridge systemPromptHash', async () => {
    const { bridge } = makeMockBridgeWithPromptHash(FAKE_PROMPT_HASH)
    const client = new LLMClient({
      bridge,
      model: UMMAYA_DEFAULT_MODEL,
    ...
    const span = _exporter.getFinishedSpans().find(s => s.name === 'gen_ai.client.invoke')
    expect(span).toBeDefined()
    expect(span!.attributes['ummaya.prompt.hash']).toBe(FAKE_PROMPT_HASH)
  })
