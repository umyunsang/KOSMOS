...
다른 LLM 들도 비슷한 multi-tool 케이스 가질 수 있음. KOSAX 의 `isSameAssistantToolStack` 패턴 재사용:
...
3. **Token budget worry 의 진짜 답은 prompt cache + dynamic suffix**. KOSAX 가 이미 `prompts/system_v1.md` 의 static prefix 를 cache 하고 dynamic suffix 만 매 turn 새로 생성 — suffix 길이 늘어도 재계산 비용은 BM25 lookup + render 만. cache 효과는 prefix 가 보존되므로 그대로.
...
5. **반대 케이스 — 진짜 token bloat 위험**: response body / 큰 enum (>20) / nested object 의 모든 필드 dump 는 LLM 이 안 보고 token 만 먹음. Top-level required + optional 만 + 80자 description 이 sweet spot. KOSAX 의 fix 가 정확히 이 균형.
...
- [ ] `src/kosax/llm/client.py` 의 `parallel_tool_calls = False` 보존 (LLM swap 시 재확인)
- [ ] `src/kosax/ipc/stdio.py` 의 multi-tool drop guard 보존
...
- [ ] `src/kosax/ipc/stdio.py:_build_available_adapters_suffix` 가 input_schema_json 의 properties 를 per-field signature 로 dump (`· name (type, 필수|선택 pattern=... enum=...) — desc`)
...
- 변경 파일들 (working tree, 미커밋):
  - `src/kosax/llm/client.py:989` — parallel_tool_calls=False
  - `src/kosax/ipc/stdio.py:1661` — multi-tool drop guard
