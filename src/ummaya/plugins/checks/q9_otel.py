...
``otel_attributes['ummaya.plugin.id']`` matching ``plugin_id`` ?
...
def check_otel_attr(ctx: CheckContext) -> CheckOutcome:
    """Q9-OTEL-ATTR — otel_attributes['ummaya.plugin.id'] equals plugin_id."""
    blocked = _ensure_manifest(ctx, "Q9-OTEL-ATTR")
    if blocked:
        return blocked
    assert ctx.manifest is not None
    expected = ctx.manifest.plugin_id
    actual = ctx.manifest.otel_attributes.get("ummaya.plugin.id")
    if actual != expected:
        return failed(
            ko=(
                f"otel_attributes['ummaya.plugin.id'] = {actual!r} 는 "
...
def _collect_install_span_attributes(plugin_id: str) -> dict[str, object] | None:
    ...
    ``ummaya.plugin.install``. Returns the attribute dict for the most
    ...
    with tracer.start_as_current_span("ummaya.plugin.install") as span:
...
def check_otel_emit(ctx: CheckContext) -> CheckOutcome:
    """Q9-OTEL-EMIT — install span actually carries ummaya.plugin.id at runtime.
    ...
    # (the manifest carries ummaya.plugin.id correctly); skipping the
    ...
    if attrs.get("ummaya.plugin.id") != ctx.manifest.plugin_id:
        return failed(
            ko=(
                f"ummaya.plugin.install span 에서 ummaya.plugin.id = "
                f"{attrs.get('ummaya.plugin.id')!r} 로 attach 되지 않음"
            ),
            en=(
                f"ummaya.plugin.install span did not carry ummaya.plugin.id="
                f"{ctx.manifest.plugin_id!r} (got {attrs.get('ummaya.plugin.id')!r})"
            ),
        )
    return passed()
