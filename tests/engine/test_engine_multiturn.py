...
async def test_preprocessing_triggered_with_small_context_window(
    ...
    A context_window=10000 with threshold=0.6 means preprocessing fires when
    the history exceeds ~6000 tokens. The system prompt is ~7200 tokens after
    the verify-trigger expansion (인증/본인확인/공동인증서/금융인증서/KEC/
    모바일ID/마이데이터 인증/Any-ID SSO mappings), pushing baseline past
    7000. Bumping to 10000 keeps the test invariant — system prompt + room
...
async def test_preprocessing_compresses_stale_tool_results(
    ...
    config = QueryEngineConfig(
        ...
        # 7000; bump to 10000 so the preprocessing threshold (0.05 * 10000
        # = 500) still fires on typical test message sizes while leaving
