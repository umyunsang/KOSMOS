...
"""Structured event logger for UMMAYA observability.

``ObservabilityEventLogger`` is the single emit point for all structured
observability events in UMMAYA.  It:
...
Usage::
    ...
    from ummaya.observability import ObservabilityEventLogger, ObservabilityEvent

    logger = ObservabilityEventLogger()
    logger.emit(ObservabilityEvent(event_type="llm_call", success=True))

The underlying Python logger name is ``ummaya.events``.  Consumers can
...
from ummaya.observability.events import EventType, ObservabilityEvent

_events_logger = logging.getLogger("ummaya.events")
...
class ObservabilityEventLogger:
    """Fail-safe structured event emitter for UMMAYA observability.

    Args:
        logger: Optional custom Python logger.  Defaults to ``ummaya.events``
