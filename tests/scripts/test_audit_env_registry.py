...
def test_ar01_clean_state(tmp_path: Path) -> None:
    ...
    _write_src_py(
        tmp_path,
        _make_py_source(["UMMAYA_SYNTHETIC_FOO", "UMMAYA_SYNTHETIC_BAR"]),
    )
    _write_registry(
        tmp_path,
        [
            "| `UMMAYA_SYNTHETIC_FOO` | yes (all envs) | — | string | `ummaya.test` | — |",
            "| `UMMAYA_SYNTHETIC_BAR` | yes (all envs) | — | string | `ummaya.test` | — |",
        ],
    )
...
def test_ar02_code_var_not_in_registry(tmp_path: Path) -> None:
    """T-AR02: UMMAYA_SYNTHETIC_NEW in code but not in registry → exit 1."""
    _write_src_py(
        tmp_path,
        _make_py_source(["UMMAYA_SYNTHETIC_KNOWN", "UMMAYA_SYNTHETIC_NEW"]),
    )
    _write_registry(
        tmp_path,
        [
            "| `UMMAYA_SYNTHETIC_KNOWN` | yes (all envs) | — | string | `ummaya.test` | — |",
        ],
    )
    ...
    assert "UMMAYA_SYNTHETIC_NEW" in missing, f"Missing not flagged: {missing}"
...
def test_ar03_registry_var_not_in_code(tmp_path: Path) -> None:
    """T-AR03: UMMAYA_SYNTHETIC_STALE in registry but not in code → exit 1."""
    _write_src_py(
        tmp_path,
        _make_py_source(["UMMAYA_SYNTHETIC_ACTIVE"]),
    )
    _write_registry(
        tmp_path,
        [
            "| `UMMAYA_SYNTHETIC_ACTIVE` | yes (all envs) | — | string | `ummaya.test` | — |",
            "| `UMMAYA_SYNTHETIC_STALE` | yes (all envs) | — | string | `ummaya.test` | — |",
        ],
    )
    ...
    assert "UMMAYA_SYNTHETIC_STALE" in stale, f"Stale not flagged: {stale}"
...
def test_ar04_malformed_registry_missing_header(tmp_path: Path) -> None:
    ...
    _write_src_py(tmp_path, _make_py_source(["UMMAYA_SYNTHETIC_OK"]))
...
def test_ar05_langfuse_allowlist_clean(tmp_path: Path) -> None:
    ...
    _write_src_py(
        tmp_path,
        _make_py_source(["UMMAYA_SYNTHETIC_BASE", "LANGFUSE_PUBLIC_KEY"]),
    )
    _write_registry(
        tmp_path,
        [
            "| `UMMAYA_SYNTHETIC_BASE` | yes (all envs) | — | string | `ummaya.test` | — |",
            "| `LANGFUSE_PUBLIC_KEY` | yes (prod only) | — | string "
            "| `ummaya.observability.langfuse` | — |",
        ],
    )
...
def test_ar06_override_family_clean(tmp_path: Path) -> None:
    """T-AR06: UMMAYA_KOROAD_ACCIDENT_SEARCH_API_KEY in code + family row → exit 0."""
    _write_src_py(
        tmp_path,
        _make_py_source(["UMMAYA_KOROAD_ACCIDENT_SEARCH_API_KEY", "UMMAYA_SYNTHETIC_BASE"]),
    )
    _write_registry(
        tmp_path,
        [
            "| `UMMAYA_SYNTHETIC_BASE` | yes (all envs) | — | string | `ummaya.test` | — |",
            "| `UMMAYA_{TOOL_ID}_API_KEY` | conditional (see note) | — | string "
            "| `ummaya.permissions.credentials` | — |",
        ],
    )
...
def test_ar07_performance(tmp_path: Path) -> None:
    ...
    src_dir = tmp_path / "src" / "ummaya" / "generated"
    src_dir.mkdir(parents=True)

    # Generate 1100 Python files each referencing one known var.
    var_name = "UMMAYA_SYNTHETIC_PERF"
    ...
    _write_registry(
        tmp_path,
        [
            f"| `{var_name}` | yes (all envs) | — | string | `ummaya.test` | — |",
        ],
    )
...
# ---------------------------------------------------------------------------
# T-AR08 (beyond contract): Prefix violation — non-UMMAYA_/non-LANGFUSE_ in .env.example
#
...
def test_ar08_prefix_violation(tmp_path: Path) -> None:
    """T-AR08: NON_UMMAYA_VAR in .env.example → exit 1, prefix_violations."""
    # Populate registry with one known var so the code scan won't drift.
    _write_src_py(tmp_path, _make_py_source(["UMMAYA_SYNTHETIC_OK"]))
    _write_registry(
        tmp_path,
        [
            "| `UMMAYA_SYNTHETIC_OK` | yes (all envs) | — | string | `ummaya.test` | — |",
        ],
    )

    # Write .env.example with a non-UMMAYA_, non-LANGFUSE_ assignment.
...
def test_ar09_override_family_unmatched(tmp_path: Path) -> None:
    """T-AR09: UMMAYA_KOROAD_ACCIDENT_SEARCH_API_KEY in code, NO family row → exit 1."""
    _write_src_py(
        tmp_path,
        _make_py_source(["UMMAYA_KOROAD_ACCIDENT_SEARCH_API_KEY", "UMMAYA_SYNTHETIC_BASE"]),
    )
    ...
    assert "UMMAYA_KOROAD_ACCIDENT_SEARCH_API_KEY" in unmatched, (
