...
## Phase 1 — Setup
- **T001**: Verify worktree state (`/Users/um-yunsang/KOSMOS-w-2641` on
...
- **T003 [P]**: Insert 4-section 박제 header at the top of
  ...
  `KOSMOS_ENABLE_DEAD_TEAM_MEM_SYNC`). (SC-005)
...
- **T004 [P]**: Create `tui/src/services/teamMemorySync/__tests__/dead-call-gate.test.ts`.
  ...
  expected message when the env override is unset. Use a `delete process.env.KOSMOS_ENABLE_DEAD_TEAM_MEM_SYNC`
...
- **T005 [P]**: Insert 4-section 박제 header at the top of
  ...
  Gate text per FR-005 (env override `KOSMOS_ENABLE_DEAD_SETTINGS_SYNC`).
  (SC-006)
...
- **T006 [P]**: Create `tui/src/services/settingsSync/__tests__/dead-call-gate.test.ts`.
  ...
  override is unset. Use `delete process.env.KOSMOS_ENABLE_DEAD_SETTINGS_SYNC`
