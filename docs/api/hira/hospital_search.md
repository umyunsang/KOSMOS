...
| Evidence | `/tmp/kosmos-evidence/medical-evidence.md § 1. HIRA` |

## Envelope

**Input model**: `HiraHospitalSearchInput` — `src/kosmos/tools/hira/hospital_search.py:40–88`.
...
**Output model**: `LookupCollection` dict — `src/kosmos/tools/hira/hospital_search.py:95–213`.
...
HIRA's parameter naming reversal (x=lon, y=lat) is the agency standard — not a KOSMOS convention.
...
KOSMOS: 강남구 반경 2km 이내 병원 48곳을 찾았습니다. 가장 가까운 곳은 '강북삼성병원' (상급종합) 으로 약 936m 거리, 전화 02-2001-2001 입니다.
```
...
- **JSON format quirk (v4 fix)**: The `_type=json` param (underscore prefix) is mandatory for
  ...
  evidence: `/tmp/kosmos-evidence/medical-evidence.md § HIRA Key Findings`.
