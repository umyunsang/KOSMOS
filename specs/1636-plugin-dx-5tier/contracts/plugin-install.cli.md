# Contract — `kosax plugin install <name>` CLI

**Surface**: `tui/src/commands/plugin-install.ts` (TUI front-end) → IPC → `src/kosax/plugins/installer.py` (backend executor).
**Trigger**: User types `/plugin install <name>` in TUI REPL OR `kosax plugin install <name>` shell.
...
| `--catalog <url>` | `$KOSAX_PLUGIN_CATALOG_URL` | Override catalog index URL. |
| `--vendor-slsa-from <path>` | `$KOSAX_PLUGIN_VENDOR_ROOT/slsa-verifier/<platform>/slsa-verifier` | Override slsa-verifier binary path. |
...
2. 📦 번들 다운로드 중…
   GET bundle_url → ~/.kosax/cache/plugin-bundles/<plugin_id>-<sha>.tar.gz
   verify SHA-256 == CatalogVersion.bundle_sha256
3. 🔐 SLSA 서명 검증 중…
   GET provenance_url → ~/.kosax/cache/plugin-bundles/<plugin_id>-<sha>.intoto.jsonl
   subprocess: slsa-verifier verify-artifact \
                  --provenance-path <prov> \
                  --source-uri github.com/kosax-plugin-store/kosax-plugin-<name> \
                  <bundle.tar.gz>
...
6. 🔄 등록 + BM25 색인 중…
   Move bundle contents → ~/.kosax/memdir/user/plugins/<plugin_id>/
   Backend: registry.register_plugin_adapter(manifest)
            → ToolRegistry.register(adapter)
            → BM25Index.add_or_update(adapter.tool_id, search_hints)
            → emit OTEL span kosax.plugin.install w/ kosax.plugin.id=<id>
7. 📜 동의 영수증 기록 중…
   PluginConsentReceipt(action_type="plugin_install", ...) → ~/.kosax/memdir/user/consent/<receipt_id>.json
...
`KOSAX_PLUGIN_SLSA_SKIP=1` (env) bypasses verification entirely (dev only). Doing so writes `slsa_verification: "skipped"` to the consent receipt, AND the citizen sees a red banner "⚠️  서명 검증 우회됨 (개발 모드)". CI tests assert the banner appears.
...
This becomes a 20th arm. The Spec 032 `frame.schema.json` must be updated; the SHA-256 hash emitted as the `kosax.ipc.schema.hash` OTEL attribute changes accordingly. Contract change tracked in this Epic; Spec 032 schema bump documented in the eventual PR description.
...
6. `--dry-run` succeeds but creates zero files; assert no `~/.kosax/memdir/user/plugins/<plugin_id>/` exists after.
7. `KOSAX_PLUGIN_SLSA_SKIP=1` → SLSA phase skipped; consent receipt records `slsa_verification: "skipped"`; red banner shown.
