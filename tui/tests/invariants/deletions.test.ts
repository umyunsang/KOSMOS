...
// KOSAX-original — Epic #1633 T041 US3 invariant test.
...
//   as KOSAX no-op stubs for print.ts byte-copy pipeline (R-3-cascade + R-4).
//   Both carry SWAP/anti-anthropic-1p(2637) headers; Anthropic OAuth/remote-settings
//   surface is null/no-op in KOSAX. Removed from banned list; added to stub-exists list.
//
...
describe('Epic #1633 T041 — US3 invariant: CC dead-code directories removed', () => {
  test.each([
    ...
    // KOSAX no-op stubs for the byte-copied services/api/claude.ts. They
    ...
    // companion "stub files restored as KOSAX no-ops" describe block
    // below, which is the canonical home for byte-copy bridge stubs.
    'utils/secureStorage',
    'remote',
    'services/policyLimits',
    // 'services/remoteManagedSettings' — RESTORED Epic #2637 as KOSAX no-op cascade stub
    ...
    // 'constants/oauth.ts' — RESTORED Epic #2637 as KOSAX no-op stub (R-4).
  ...
  test('tui/src/services/api/client.ts MUST exist (Epic #2077 — KOSAX no-op stub)', () => {
    expect(isFile('services/api/client.ts'), 'services/api/client.ts missing').toBe(true)
  })
})

describe('Epic #1633 T041 — US3 invariant: stub files restored as KOSAX no-ops', () => {
  // analytics/, auth.ts, oauth/ are stub-restored per FR-004 "strip or noop"
  // clause. They MUST exist but carry the KOSAX stub header.
  test.each([
    ...
    // restored services/api/claude.ts. Each is a KOSAX no-op that
