...
def test_load_returns_str_type(valid_prompt_tree):
    ...
    assert type(result) is str, f"Expected type(result) is str, got {type(result)!r}"
...
def test_no_public_cache_setter(valid_prompt_tree):
    ...
    loader = PromptLoader(manifest_path=valid_prompt_tree)
    assert not hasattr(loader, "set_cache"), "PromptLoader must not expose a 'set_cache' attribute"
