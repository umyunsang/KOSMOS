...
#
# Scope:
#   - Session JSONL canonical-path write (~/.ummaya/memdir/user/sessions/)
#   - /resume picker (UMMAYA + CC-legacy dual-path enumeration, dedup, sort)
...
send_enter_pane
wait_for_pane "export|PDF|pdf|ummaya-export|Downloads|영수증|generate" 30 || true
snapshot_pane 4-export-dialog
sleep 2
# Confirm with Enter (writes the PDF) — best-effort
send_keys_pane Enter
wait_for_pane "wrote|written|saved|complete|ummaya-export.*\.pdf|성공|완료|error|fail" 90 || true
