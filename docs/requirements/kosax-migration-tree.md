...
> CC가 **"개발자 중심 코딩 하네스"** 라면 KOSAX는 **"국민·국가·국가행정
...
| A5 Session | `~/.kosax/memdir/user/sessions/` JSONL · --continue/--resume/--fork/new | Spec 027 |
...
| B4 Permission | CC `<PermissionRequest>` byte-identical 사용 · 각 어댑터가 기관 자체 정책 citation. KOSAX는 권한 정책 발명 X. |
...
| B9 Caller 정체성 | KOSAX = 국가AX 인프라가 정형화할 LLM-accessible 보안 wrapping 통로의 **client-side reference implementation**. 기관 시스템 변경 요구 X. 상세 구현은 Initiative #2290 spec-driven 진행. |
...
Spec 작성 시 `docs/requirements/kosax-migration-tree.md § N` 형식으로 참조.
