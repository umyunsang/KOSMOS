...
This file resolves every NEEDS CLARIFICATION carried into Phase 0 (none — spec was clean), maps every plan decision to a canonical reference per Constitution §I, derives the 50-item review checklist from authoritative KOSAX sources (Assumption A8), and validates the Deferred Items table per Constitution §VI.

---

## R-0 · Reference mapping (Constitution §I obligation)

Every design decision in `plan.md` traces to one of the following sources from `docs/vision.md § Reference materials` or a prior KOSAX Spec.

| Plan decision | Reference source | Notes |
|---|---|---|
| Pydantic v2 manifest schema with discriminated nested PIPA block | **Pydantic AI** (`pydantic/pydantic-ai`, MIT) — schema-driven tool registry pattern | We adopt the *pattern* (declarative typed registration with discriminated unions). The actual manifest fields are KOSAX-specific (`processes_pii`, `pipa_trustee_acknowledgment`). |
| Auto-discovery loader scanning installed plugins at boot | **Claude Agent SDK** (`anthropics/claude-agent-sdk-python`, MIT) — agent-definition discovery + Claude Code `builtinPlugins.ts` (`registerBuiltinPlugin` at startup) | CC uses `Map<string, BuiltinPluginDefinition>` populated by an `initBuiltinPlugins()` call. KOSAX uses `importlib` walk over `~/.kosax/memdir/user/plugins/` because plugins are user-installed, not bundled. |
...
| `kosax.plugin.id` OTEL span attribute | **Spec 021** (`specs/021-observability-otel-genai`) — GenAI semantic-conventions extension via `kosax.*` prefix | New attribute under existing `kosax.*` namespace; emission is enforced at adapter invocation time by the registry wrapper, not by individual plugins. |
...
| Consent receipt for `kosax plugin install` action written to memdir | **Spec 035** (`specs/035-onboarding-brand-port`) consent ledger at `~/.kosax/memdir/user/consent/` | New action type `plugin_install`; existing append-only ledger format unchanged. |
...
| ID | Korean | English | Source | Mechanical check |
|---|---|---|---|---|
| Q9-OTEL-ATTR | otel_attributes 에 kosax.plugin.id 포함 | otel_attributes contains kosax.plugin.id | FR-021 + Spec 021 | Dict-key check |
| Q9-OTEL-EMIT | invocation 시 span 에 kosax.plugin.id 가 실제로 attach | Span actually carries kosax.plugin.id at runtime | FR-021 | Test-time fake-OTLP collector assertion |
...
**Decision**: Standalone repositories under the new `kosax-plugin-store` GitHub organization. One repo per example: `kosax-plugin-seoul-subway`, `kosax-plugin-post-office`, `kosax-plugin-nts-homtax` (Mock), `kosax-plugin-nhis-check` (Mock).

**Rationale**:
1. **Authentic dogfooding** — external contributors will publish their plugins as standalone repos under `kosax-plugin-store`. The 4 examples following the same convention is the highest-fidelity demonstration. A mono-repo of examples teaches contributors to do the wrong thing.
...
- *Sub-directories under a single `kosax-plugin-examples` repo*: simpler initial setup, but breaks SLSA-per-artifact and creates an awkward asymmetry between examples and contributor plugins. Rejected.
- *Sub-directories under the main KOSAX repo (`examples/plugins/`)*: keeps everything close, but would push generated example code into the main repo and conflict with the "external contribution path" the examples are demonstrating. Rejected.

**Cost of decision**: 4 new GitHub repos to create (one-time scripted via `gh repo create`), 4 separate CI configurations to maintain (mitigated by all four sharing the canonical `plugin-validation.yml` from this repo, fetched as a reusable workflow via `uses: umyunsang/KOSAX/.github/workflows/plugin-validation.yml@<sha>`).
...
4. **Cross-platform** — `slsa-verifier` ships pre-built binaries for darwin/amd64, darwin/arm64, linux/amd64, linux/arm64. The `installer.py` resolves the right binary from `~/.kosax/vendor/slsa-verifier/<platform>/slsa-verifier` (downloaded once at first install or pre-fetched at TUI bootstrap).

**Verification flow**:
```
kosax plugin install seoul-subway
  → installer.py:
      1. Fetch catalog index from kosax-plugin-store/index.json
      ...
      4. subprocess: slsa-verifier verify-artifact --provenance-path <prov> \
                       --source-uri github.com/kosax-plugin-store/kosax-plugin-seoul-subway \
                       <bundle.tar.gz>
      5. Verify exit code 0
      6. Validate manifest (manifest_schema.py)
      7. Stage to ~/.kosax/memdir/user/plugins/seoul-subway/
      8. Append consent receipt to ~/.kosax/memdir/user/consent/
...
class PluginManifest(BaseModel):
    ...
    # --- the embedded AdapterRegistration ---
    adapter: AdapterRegistration  # from kosax.tools.registry
    ...
    otel_attributes: dict[str, str]  # must contain "kosax.plugin.id"
...
**Join with `tools/registry.py` BM25 index**: at install time, `installer.py` constructs the `AdapterRegistration` from `manifest.adapter`, calls `ToolRegistry.register_plugin_adapter(adapter, source=manifest)`, which (a) validates against the existing Spec 022/024/025/031/033 invariant chain (free reuse — no new validators), (b) inserts the tool into the in-memory registry, (c) calls `BM25Index.add_or_update(adapter.tool_id, search_hint_ko + " " + search_hint_en)`, (d) emits the install OTEL span with `kosax.plugin.id=<plugin_id>`. The backend exports a single new function `register_plugin_adapter()` — everything else is reuse.
...
All NEEDS CLARIFICATION resolved (none in spec). All 6 special orders decided. 50-item checklist derived from authoritative KOSAX sources with full traceability. Constitution §VI deferred-items gate PASS. Phase 0 complete — proceed to Phase 1 (data-model.md, contracts/, quickstart.md).
