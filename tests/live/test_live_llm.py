...
"""Live validation tests for the KOSAX LLM client against the real FriendliAI K-EXAONE API.
...
Required environment variable:
    KOSAX_FRIENDLI_TOKEN — FriendliAI API token (validated by the ``friendli_token`` fixture)
"""
...
async def test_live_llm_stream_basic(friendli_token: str) -> None:
    ...
    The ``friendli_token`` fixture ensures ``KOSAX_FRIENDLI_TOKEN`` is set in
