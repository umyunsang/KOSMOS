...
After this swap:
- `/plugin install <name>` → `tui/src/commands/plugin.ts` (KOSAX singular) → emits `plugin_op_request:install`.
- `/plugin uninstall <name>` → same KOSAX singular → emits `plugin_op_request:uninstall`.
- `/plugin list` → same KOSAX singular → emits `plugin_op_request:list` (returns synchronously to its acknowledgment surface).
- `/plugin pipa-text` → same KOSAX singular → returns canonical SHA-256.
...
| Description | flex-grow | `description_ko` (or `_en` if `KOSAX_TUI_LOCALE=en`) |
...
- `KOSAX_TUI_LOCALE=en` environment variable suppresses Korean primary, English-only mode.
...
`tui/src/commands/plugins.ts:executePlugins` continues to emit `kosax.ui.surface=plugins` (existing Spec 1635 FR-037).

The new `awaitPluginOpComplete` round-trip wraps the existing IPC frame plumbing — no new TUI-side OTEL needed; backend's `kosax.plugin.list` span (R-1) covers it.
...
1. Type `/plugins` → expect Korean header "✻ KOSAX 플러그인" within 2 s.
...
- `tui/src/commands/plugin.ts:1-209` (KOSAX singular — current orphaned)
- `tui/src/commands.ts:133` (CC residue import — to be swapped)
- `docs/requirements/kosax-migration-tree.md § UI-E.3` (key binding mandate)
- `docs/requirements/kosax-migration-tree.md § UI-C.1` (layer color glyph mandate)
