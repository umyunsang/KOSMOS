...
"""Unit tests for ``kosax.plugins.manifest_schema``.
...
* Acknowledgment hash module integration — verifies the canonical
  SHA-256 constant exposed by :mod:`kosax.plugins.canonical_acknowledgment`
...
from kosax.plugins import canonical_acknowledgment as _canon
...
def _make_pipa_ack(
    *,
    ...
    return PIPATrusteeAcknowledgment(
        trustee_org_name="KOSAX Plugin Authors",
...
def _make_manifest_kwargs(**overrides: Any) -> dict[str, Any]:
    base: dict[str, Any] = {
        ...
        "slsa_provenance_url": (
            "https://github.com/kosax-plugin-store/kosax-plugin-demo/"
...
class TestPluginManifestPositive:
    def test_baseline_construction_round_trips(self) -> None:
        ...
        assert m.otel_attributes["kosax.plugin.id"] == m.plugin_id
...
class TestCrossFieldValidators:
    ...
    def test_v_otel_attribute_value_must_match_plugin_id(self) -> None:
        with pytest.raises(ValidationError) as exc:
            PluginManifest(
                **_make_manifest_kwargs(
                    otel_attributes={"kosax.plugin.id": "wrong_id"},
                )
            )
        assert 'otel_attributes["kosax.plugin.id"] must equal plugin_id' in str(exc.value)
    ...
    def test_v_otel_attribute_required_key_present(self) -> None:
        ...
        assert "kosax.plugin.id" in str(exc.value)
