...
**Verify**: `tail -20 ~/.ummaya/logs/audit.jsonl` shows one `user-interrupted` record with the current session ID and the interrupted tool-call ID.
...
**Expected**: indicator advances to "default" within 200 ms; no buffer content changes; an OTel span `ummaya.permission.mode=default` is emitted (visible in Langfuse local, `docker-compose up -d` of Spec 028 collector).
...
**Verify** (after exit):
```bash
jq '.event_type' ~/.ummaya/logs/audit.jsonl | tail -5
...
```bash
echo '{"ctrl+r": null}' > ~/.ummaya/keybindings.json
cd tui && bun run dev
...
**Reset**:
```bash
rm ~/.ummaya/keybindings.json
```

### 8. User override — remap history search (User Story 7)

```bash
echo '{"ctrl+f": "history-search", "ctrl+r": null}' > ~/.ummaya/keybindings.json
cd tui && bun run dev
...
```bash
echo '{"ctrl+c": "draft-cancel"}' > ~/.ummaya/keybindings.json
cd tui && bun run dev
