...
"""Tests for ummaya.config.guard — fail-fast startup guard.
...
from ummaya.config import guard

# ---------------------------------------------------------------------------
# Helpers
# ---------------------------------------------------------------------------

# Full-required set keeps dev/ci/prod all green. Guard inspects UMMAYA_ENV at
...
ALL_REQUIRED_SET: dict[str, str] = {
    ...
    "UMMAYA_OTEL_ENDPOINT": "http://localhost:4318",
}


@pytest.fixture(autouse=True)
def _isolate_env(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    """Strip every UMMAYA_*/LANGFUSE_* var plus UMMAYA_ENV before each test."""
...
def test_t_g01_empty_env_defaults_to_dev(monkeypatch: pytest.MonkeyPatch) -> None:
    """UMMAYA_ENV unset defaults to dev and has no Infisical-required user key."""
...
def test_t_g03_prod_requires_langfuse(monkeypatch: pytest.MonkeyPatch) -> None:
    """UMMAYA_ENV=prod with LANGFUSE_PUBLIC_KEY missing → diagnostic lists it."""
...
def test_t_g04_dev_does_not_require_langfuse(monkeypatch: pytest.MonkeyPatch) -> None:
    """Same state as T-G03 but UMMAYA_ENV=dev → LANGFUSE not in missing list."""
    _set_all(monkeypatch, overrides={"LANGFUSE_PUBLIC_KEY": None})
    monkeypatch.setenv("UMMAYA_ENV", "dev")
    diag = guard.check_required()
...
def test_t_g05_whitespace_value_is_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _set_all(monkeypatch, overrides={"UMMAYA_OTEL_ENDPOINT": "   \t "})
    monkeypatch.setenv("UMMAYA_ENV", "prod")
    diag = guard.check_required()
    assert diag is not None
    assert "UMMAYA_OTEL_ENDPOINT" in diag.missing


def test_t_g06_unknown_env_falls_through_to_dev(monkeypatch: pytest.MonkeyPatch) -> None:
    """UMMAYA_ENV=staging (unknown) → classified as dev, no prod-only vars flagged."""
    _set_all(monkeypatch, overrides={"LANGFUSE_PUBLIC_KEY": None})
    monkeypatch.setenv("UMMAYA_ENV", "staging")
    diag = guard.check_required()
...
def test_verify_startup_exits_78_on_missing(
    ...
    assert "\n" not in line
    assert line.startswith("UMMAYA config error [env=")
    assert "missing required variables:" in line
    assert "See https://github.com/umyunsang/UMMAYA/blob/main/docs/configuration.md" in line
...
def test_current_env_values(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("UMMAYA_ENV", raising=False)
    assert guard.current_env() == "dev"
    monkeypatch.setenv("UMMAYA_ENV", "ci")
    assert guard.current_env() == "ci"
    monkeypatch.setenv("UMMAYA_ENV", "prod")
    assert guard.current_env() == "prod"
    monkeypatch.setenv("UMMAYA_ENV", "")
    assert guard.current_env() == "dev"
    monkeypatch.setenv("UMMAYA_ENV", "production")
    assert guard.current_env() == "dev"
...
def test_guard_diagnostic_frozen() -> None:
    ...
    with pytest.raises((AttributeError, Exception)):
        diag.missing = ("UMMAYA_Y",)  # type: ignore[misc]
