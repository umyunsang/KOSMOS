...
// returned by the mocked getOrCreateKosaxBridge() as a live closure,
...
mock.module(join(TUI_ROOT, 'src/ipc/bridgeSingleton.js'), () => ({
  getOrCreateKosaxBridge: () => _currentBridge,
  getKosaxBridgeSessionId: () => 'test-session-handlers',
  closeKosaxBridge: async () => {},
}))
...
mock.module(join(TUI_ROOT, 'src/utils/messages.js'), () => ({
  SYNTHETIC_MODEL: 'kosax-test-model',
...
async function run(buildFrames: (corrId: string) => StagedFrame[]): Promise<unknown[]> {
  const previousPrimary = process.env.KOSAX_FRIENDLI_TOKEN
  const previousSession = process.env.KOSAX_FRIENDLI_SESSION_ACTIVE
  process.env.KOSAX_FRIENDLI_TOKEN = 'test-token-handlers'
  process.env.KOSAX_FRIENDLI_SESSION_ACTIVE = '1'
  ...
  try {
    ...
    if (previousPrimary === undefined) {
      ...
      process.env.KOSAX_FRIENDLI_TOKEN = previousPrimary
    }
    if (previousSession === undefined) {
      delete process.env.KOSAX_FRIENDLI_SESSION_ACTIVE
    } else {
      process.env.KOSAX_FRIENDLI_SESSION_ACTIVE = previousSession
    }
  }
}
...
describe('stream-event projection I6', () => {
  test('terminal AssistantMessage content array contains text + N tool_use blocks', async () => {
    ...
    // most one block-type; KOSAX' multi-turn-into-one-message assembly
