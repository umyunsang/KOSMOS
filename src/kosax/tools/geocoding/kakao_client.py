...
merge — see :func:`kosax.tools.resolve_location._kakao_coords_fanout`.
...
Korean wrapper for this API. KOSAX originally shipped only
...
Key source: ``KOSAX_KAKAO_API_KEY`` environment variable.
...
from kosax.tools.errors import ConfigurationError, _require_env
...
async def search_address(
    ...
    *,
    ...
    Raises:
        ConfigurationError: If ``KOSAX_KAKAO_API_KEY`` is not set.
        ConfigurationError: If ``KOSAX_KAKAO_API_KEY`` is not set.
    ...
    """
    api_key = _require_env("KOSAX_KAKAO_API_KEY")
...
class KakaoPlaceDocument(BaseModel):
    ...
    :func:`kosax.tools.resolve_location._kakao_coords_fanout`.
    """
...
async def coord_to_region_code(
    *,
    ...
    latitude for ``coord2regioncode``. KOSAX uses this as canonical
