...
git pull --ff-only
git worktree list                          # confirm KOSAX-w-2294 exists
cd ../KOSAX-w-2294                        # branch 2294-5-primitive-align
...
```bash
grep -E "KOSAX|의정부|응급실|nmc_emergency_search|real_classification_url" \
  specs/2294-5-primitive-align/smoke-emergency-lookup-pty.txt
```

Expected lines, in order:
1. `KOSAX` branding banner.
...
Expected: all snapshots match pre-refactor baseline. The only **allowed** new attribute is `kosax.adapter.real_classification_url` (introduced by Epic δ commit `c6747dd`). Any other drift is a regression.
...
| `verifyBootRegistry` fails on adapter | Check `src/kosax/tools/<adapter>/manifest.py` for missing `real_classification_url`. May indicate Epic δ deferred #2362 work surfaced — file as a sub-issue. |
| PTY smoke times out | Check FriendliAI Tier 1 quota + `KOSAX_LLM_API_KEY` env var. Memory `feedback_env_check_first` applies. |
