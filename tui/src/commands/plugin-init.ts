...
//
// Epic #1636 P5 — `ummaya plugin init <name>` scaffold command.
//
...
function manifestForOptions(opts: PluginInitOptions): Record<string, unknown> {
  return {
    ...
    slsa_provenance_url:
      ...
      `https://github.com/ummaya-plugin-store/ummaya-plugin-${opts.name}/releases/download/v0.1.0/${opts.name}.intoto.jsonl`,
    otel_attributes: { 'ummaya.plugin.id': opts.name },
...
name = "ummaya-plugin-${name}"
version = "0.1.0"
description = "UMMAYA plugin scaffolded by 'ummaya plugin init'"
...
# The UMMAYA host expects two module-level symbols:
#   - TOOL: a ummaya.tools.models.GovAPITool instance describing the adapter.
...
"""UMMAYA plugin adapter for ${name} (Mock tier).
...
UMMAYA does not have, or the integration is documented but not
...
const README_KO_TEMPLATE = (name: string, tier: string) => `# ummaya-plugin-${name}

UMMAYA 플러그인 — \`${name}\` 어댑터.

## 설치

\`\`\`bash
ummaya plugin install ${name}
\`\`\`
...
- [docs/plugins/quickstart.ko.md](https://github.com/umyunsang/UMMAYA/blob/main/docs/plugins/quickstart.ko.md)
- [docs/plugins/architecture.md](https://github.com/umyunsang/UMMAYA/blob/main/docs/plugins/architecture.md)
...
const README_EN_TEMPLATE = (name: string, tier: string) => `# ummaya-plugin-${name}

UMMAYA plugin — \`${name}\` adapter.
...
jobs:
  validate:
    uses: umyunsang/UMMAYA/.github/workflows/plugin-validation.yml@main
    secrets: inherit
`
