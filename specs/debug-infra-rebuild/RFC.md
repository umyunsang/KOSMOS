...
| OTEL GenAI semconv | 🟢 (`gen_ai.request.streaming` + chunk events) | 🟡 (`kosax.tui.*` 우리가 추가) |
...
**무엇**: [aimock.copilotkit.dev](https://aimock.copilotkit.dev/) 를 Docker 컨테이너로 CI 에서 띄우고 `KOSAX_FRIENDLI_BASE_URL=http://localhost:4010` 로 가리킴.
...
# Test env
env:
  KOSAX_FRIENDLI_BASE_URL: http://localhost:4010/v1
  KOSAX_FRIENDLI_TOKEN: aimock-fake-token
```
...
**왜**: teatest 의 `WaitFor` 패턴이 Bubble Tea 진영의 표준 ([Pattern Matched playbook](https://patternmatched.substack.com/p/testing-bubble-tea-interfaces)). KOSAX 의 `.expect` script 의 모든 `Sleep <wallclock>` 는 K-EXAONE latency 변동에 민감해서 무용지물.
...
```bash
tmux new-session -d -s kosax 'bun run tui'
tmux send-keys -t kosax '부산 날씨 알려줘' Enter
# polled wait until predicate
until tmux capture-pane -t kosax -p | grep -q "● lookup"; do sleep 0.3; done
tmux capture-pane -t kosax -p > frame_lookup.txt
```
...
### P4 — OTEL `kosax.llm.chunk` + `kosax.tui.frame_commit` span events

**무엇**: 두 새 OTEL span event:
- `kosax.llm.chunk` — backend 가 stream 받을 때마다 emit. attributes: `correlation_id`, `bytes`, `delta_ms_since_first_token`, `channel: content|reasoning|tool_call`
- `kosax.tui.frame_commit` — frontend Ink reconcile 마다 emit. attributes: `correlation_id`, `frame_hash`, `frame_seq`
...
### P5 — `KosaxCassette` (vcr-langchain 스타일, optional)
...
# 1. Detached session running KOSAX TUI
...
span.add_event("kosax.llm.chunk", attributes={
    "kosax.correlation_id": correlation_id,
    "kosax.llm.bytes": len(chunk_bytes),
    "kosax.llm.delta_ms": elapsed_ms_since_first_chunk,
    "kosax.llm.channel": channel_name,  # content|reasoning|tool_call
})
```
...
```typescript
// Each Ink reconcile
tracer.startSpan('kosax.tui.frame_commit', {
  attributes: {
    'kosax.correlation_id': correlationId,
    'kosax.tui.frame_hash': hashFrame(latestFrame),
    'kosax.tui.frame_seq': frameSeqCounter,
  },
}).end()
```
...
- [x] `kosax.tui.frame_commit` OTEL event 추가 (`tui/src/utils/frameCommitOtel.ts` — `useFrameCommitTracker` hook wired into `MessagesImpl`)

### Phase 5 — OTEL chunk events (Optional, 1 Epic)

- [ ] `kosax.llm.chunk` event 추가 (`src/kosax/llm/client.py`)
