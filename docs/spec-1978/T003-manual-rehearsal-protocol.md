...
```bash
cd ~/KOSAX/tui      # NOT ~/KOSAX — the `tui` script lives in tui/package.json,
...
#   tail -f /tmp/kosax-tui.log

# In the FIRST pane, launch with stderr split:
KOSAX_TUI_LOG_LEVEL=DEBUG bun run tui 2> /tmp/kosax-tui.log
```

Expected timeline:

1. Banner renders (`KOSAX v… · K-EXAONE 236B …`).
2. Type any message in Korean (e.g., `안녕`).
3. Press Enter.

Three possible outcomes:

| Stderr line in `/tmp/kosax-tui.log` | Diagnosis |
|---|---|
...
Paste the relevant `/tmp/kosax-tui.log` lines (the block immediately around the Enter keystroke — usually 5–15 lines) into a comment on Epic [#1978](https://github.com/umyunsang/KOSAX/issues/1978) or share with the implementing agent. We will land T004 (the actual patch) based on which guard fired.
