...
`_query_inner` from `src/kosax/engine/query.py` verbatim (FR-009).
...
# KOSAX canonical citizen-facing timezone (Asia/Seoul). Internal
...
from kosax.agents.errors import AgentConfigurationError, PermissionDeniedError
...
from kosax.engine.query import _query_inner
...
if TYPE_CHECKING:
    from kosax.agents.mailbox.base import Mailbox
...
class Worker:
    ...
    async def _run_inner(self, instruction: str, correlation_id: UUID) -> None:  # noqa: C901
        ...
        from kosax.llm.models import ChatMessage  # local import to avoid cycle
        ...
        async for event in _query_inner(query_ctx):
            ...
            with _tracer.start_as_current_span(span_name) as span:
                span.set_attribute(KOSAX_AGENT_ROLE, self._ctx.specialist_role)
                span.set_attribute(KOSAX_AGENT_SESSION_ID, str(self._ctx.session_id))
        ...
        if lookup_output is not None:
            ...
            from kosax.tools.models import LookupMeta  # local import
    ...
    def _extract_lookup_output(
        ...
        from kosax.tools.models import LookupOutput  # local import
    ...
    async def _request_permission(self, tool_id: str, correlation_id: UUID) -> bool:
        ...
        async for response_msg in self._mailbox.receive(self._ctx.worker_id):
            if response_msg.msg_type == MessageType.permission_response:
                from kosax.agents.mailbox.messages import PermissionResponsePayload
