...
"""System prompt augmentation helper for the KOSAX LLM harness.
...
``kosax.prompt.hash`` OTEL span attribute (which hashes only the base prompt),
...
from kosax.llm.models import ToolDefinition as LLMToolDefinition
...
def build_system_prompt_with_tools(
    ...
    logger.debug(
        "kosax.system_prompt.augmented_chars=%d kosax.system_prompt.tool_count=%d",
