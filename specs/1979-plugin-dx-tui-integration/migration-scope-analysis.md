...
The KOSAX TUI carried **three actively-mounted** permission UX components and
...
arrow+Enter pattern, preserving every KOSAX-original semantic that has no CC
...
- **FR-037 / T039**: `kosax.ui.surface=permission_gauntlet` OTEL emission on mount.
...
The `kosaxPendingConsent` state in REPL.tsx (line 860) is dead code —
...
classified by whether it ports a CC original 1:1 or carries pre-CC KOSAX
...
PermissionLayerHeader.tsx (KOSAX-only — defensive Layer 1/2/3 glyph; no CC analog needed)
...
### Class B — KOSAX-only, migrated this spec

| File | Old pattern | New pattern | Mounted in |
|---|---|---|---|
| `permissions/PermissionGauntletModal.tsx` | `Y/A/N` direct | Arrow+Enter | `REPL.tsx:5523` (via `kosaxPendingConsent` — currently dormant slot) |
| `coordinator/PermissionGauntletModal.tsx` | `y/n` direct | Arrow+Enter | `REPL.tsx:5541` via `KosaxActivePermissionGate` (production path) |
...
### Class C — KOSAX-only, dormant (recommended migration)
...
### Class D — KOSAX-only, no migration applicable

These components have no Y/N keystroke surface; they implement KOSAX-specific
...
permissions/StatusBar.tsx          ← KOSAX-original status badge
...
KOSAX = CC + 2 swaps (model: K-EXAONE, tools: citizen-centric primitives).
Spec 033 5-mode spectrum / PIPA §15(2) ConsentDecision 4-tuple / NIST AAL hint
/ Layer 1/2/3 reinforcement / dontAsk mode were KOSAX-original speculation
...
- **kosaxPendingConsent dead-state cleanup** in REPL.tsx — non-functional
...
- **PluginInstallFlow inline-mount migration** — the auxiliary dispatch
  ...
  stdin raw mode hot.  No need to mount inline like `kosaxPendingConsent`
