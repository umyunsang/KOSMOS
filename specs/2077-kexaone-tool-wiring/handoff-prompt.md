...
K-EXAONE 이 `<tool_call>{"name":"Read",...}</tool_call>` 같은 **CC 학습 데이터 도구를 hallucinate 하는 문제**를 해결한다. 진짜 원인은 **TUI 가 `ChatRequestFrame.tools` 를 비워 보내고 backend 도 fallback inject 가 없어서** K-EXAONE 이 `tools=None` 으로 호출되는 것. 그 결과 모델은 KOSAX 의 5 primitive (`lookup`/`resolve_location`/`submit`/`subscribe`/`verify`) 를 모르고 자기 학습 데이터의 CC tool 들 (Read, Glob, Bash 등) 을 응답에 박는다. 본 epic 은 CC 소스맵의 tool wiring + agentic loop 패턴을 KOSAX 로 마이그레이션해서 K-EXAONE 이 KOSAX-등록 도구만 호출하고, 호출 결과가 `tool_use` content block 으로 transcript 에 paint 되고, follow-up turn 까지 진행되도록 만든다.

## 2. Canonical references

- `AGENTS.md` — KOSAX 룰 (Conventional Commits, English source, no Co-Authored-By, no new runtime dep)
- `docs/vision.md` — 6-layer 아키텍처
- `docs/requirements/kosax-migration-tree.md § L1-A.A3` — Tool protocol = K-EXAONE native function calling
- `docs/requirements/kosax-migration-tree.md § L1-B.B6` — composite tool 제거 (LLM primitive chain)
- `docs/requirements/kosax-migration-tree.md § L1-C.C7` — `plugin.<id>.<verb>` 네임스페이스 · 4 root 예약
- `src/kosax/llm/_cc_reference/claude.ts:1900-2304` — fdfd3e9 commit 으로 cp 된 CC streaming/agentic baseline
- 메모리 파일들:
...
### 3.1 Backend 누락 (`src/kosax/ipc/stdio.py`)
...
| Whitelist (line 627-679 / 890-939) | 하드코딩된 `{lookup, resolve_location, submit, subscribe, verify}` | primitives 카탈로그 (`src/kosax/primitives/__init__.py`) 에서 single source of truth 로 끌어오기 |
...
| `src/utils/permissions/permissions.ts` | 1486 | permission gauntlet 본체 (Spec 033 와 매핑) | `src/kosax/llm/_cc_reference/permissions.ts` |
...
위 § 4 의 9 개 파일 cp. `src/kosax/llm/_cc_reference/README.md` 작성: 파일별 1-line description + KOSAX 매핑.
...
- `registry.export_core_tools_openai()` (현재 `src/kosax/tools/registry.py:373`) 가 primitive 5 종 + MVP 보조를 OpenAI function shape 로 반환
...
CC reference: `_cc_reference/permissions.ts` (1486 lines, KOSAX Spec 033 와 매핑 검토 필요).
...
fdfd3e9 feat(tui): paint chain unblocked + K-EXAONE thinking via CC reference (KOSAX-1633 P3) (#2075)
