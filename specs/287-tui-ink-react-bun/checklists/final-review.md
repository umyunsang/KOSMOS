...
Every ported file carries a `// Source: .references/claude-code-sourcemap/restored-src/...` attribution header (FR-011). Every KOSAX-original file carries a `// SPDX-License-Identifier: Apache-2.0\n// KOSAX-original: ...` header.

- [X] 64 files under `tui/src/` bear `// Source:` headers — lifted from Claude Code 2.1.88 restored-src, research-use only
- [X] 37 files under `tui/src/` bear `KOSAX-original` headers — plain TS skeletons with no upstream analog
...
TUI layer is a front-end to the Python backend; fail-closed defaults for tool adapters live in `src/kosax/tools/` (Spec 024/025 territory, unchanged). What the TUI must uphold:
...
- [X] **Every IPC frame is a Pydantic v2 `BaseModel`** — `src/kosax/ipc/frame_schema.py` defines `_BaseFrame`, `UserInputFrame`, `AssistantChunkFrame`, `ToolCallFrame`, `ToolResultFrame`, `ToolResultEnvelope`, `CoordinatorPhaseFrame`, `WorkerStatusFrame`, `PermissionRequestFrame`, `PermissionResponseFrame`, `SessionEventFrame`, `ErrorFrame` — all using `Literal["..."]` discriminators.
- [X] **No `Any` in IPC schemas** — `grep -n ": Any\b" src/kosax/ipc/frame_schema.py` returns no matches in field declarations (Pydantic `extra="forbid"` via `ConfigDict`).
...
**Evidence**: `src/kosax/ipc/frame_schema.py:23-148`, `tui/src/ipc/frames.generated.ts` (auto-generated).
...
- [X] **No hardcoded keys anywhere in the TUI layer**: `grep -rn "data.go.kr\|serviceKey=" tui/src/` returns zero matches. All credentials remain behind `KOSAX_*` env vars resolved by the Python backend.
...
|-----------|--------|--------------|
| I. Reference-Driven Development | PASS | 64 `// Source:` + 37 `KOSAX-original` headers, `attribution-audit-phase10.md` |
